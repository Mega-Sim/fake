...
int param_extract(char *ifname, uint16 slave)
{
        ...
        /*
         * When this function is triggered directly from the GUI the
         * parameter table might not have been populated yet.  To make the
         * extraction self contained we load the parameter description from
         * a csv file if needed so that Param_Raw contains valid
         * Name/ParamNo/Index/SubIndex/DataType information.
         */
        ...
        for(int i = 0; i < Param_Raw.iCount; i++)
        {
                Param_Raw.Parsed[i].i_SetValue = 0;
        }
                ...
                if ( ec_config_init(FALSE) > 0 )
                {
                        ...
                        if(ec_statecheck(slave, EC_STATE_PRE_OP, EC_TIMEOUTSTATE * 4) == EC_STATE_PRE_OP)
                        {
                                for(int i = 0; i < Param_Raw.iCount; i++)
                                {
                                        ...
                                        case TYPE_USINT:
                                                fwLength = sizeof(u_8_int);
                                                ec_SDOread(slave, Param_Raw.Parsed[i].ui16_Index,
                                                           Param_Raw.Parsed[i].ui8_iSubIndex,
