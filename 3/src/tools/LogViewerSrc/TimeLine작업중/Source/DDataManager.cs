...
namespace LogViewer_DataManager
{
    ...
    {
        None, Ping, Read_OHT_Version, File_Upload, Log_Download, File_Download, Monitor_Download, Check_Monitor_Log, D_Drv_CHK, CountOf
    }
        ...
        private void Execute()
        {
			...
			switch (m_CurTask)
			{
				...
				case eDataManTask.D_Drv_CHK:
				{
					...
					string remotePath = @"\\" + OHT.IP + @"Log"; // @"\물류자동화그룹\OHT8.0\OHT.exe";
					...
					string tmp = String.Format("[{0}/{1}] D Drive Check: {2} ({3}), Version: {4} , Elapse Time: {5}sec", m_nCurOhtIdx + 1, m_nTotalOhtCnt, OHT.Name, OHT.IP, msg, TimeChk.Get_sec(1));
					m_Parent.m_LogQ.EnQ(bRet ? "> OK: " + tmp : "> NG: " + tmp);
