...
namespace LogViewer
{
    public partial class UAmcRecvTable : Form
    {
		...
		List<UTimeLine> m_TimeLineList = new List<UTimeLine>();
        ...
        public bool OpenFile(string path)
        {
            ...
            ShowTable(tmpTable, new DateTime(), path);
            return true;
        }
        ...
        public void DestroySelf()
        {
			try
			{
				foreach(UTimeLine dlg in m_TimeLineList)
				{
					dlg.Close();
				}
        ...
        private void InitForm()
        {
            ...
            // Columns Setting 
            dataGridView1.ColumnCount = (int)eAmcRcvIdx.COUNTOF;

            for (eAmcRcvIdx i = 0; i < eAmcRcvIdx.COUNTOF; i++)
            {
                string str = DAmcRcvRowItem.GetHeaderName(i, asMultiLine: false);
                str = str.Replace('\n', '.');
                dataGridView1.Columns[(int)i].Name = str;
                dataGridView1.Columns[(int)i].HeaderText = str;
				
				//Trace.WriteLine(str);

				//if(i > eColumn_AmcRcv.In1_Hand_Inpos)
				//    this.dataGridView1.Columns[(int)i].Visible = false;
				dataGridView1.Columns[(int)i].DefaultCellStyle.Alignment = i == eAmcRcvIdx.Time_Date ? DataGridViewContentAlignment.MiddleCenter : DataGridViewContentAlignment.MiddleRight;
        ...
        public void InitGrid()
        {
			...
			//dataGridView1.ClipboardCopyMode = DataGridViewClipboardCopyMode.EnableAlwaysIncludeHeaderText;
			string[] rowData = new string[(int)eAmcRcvIdx.COUNTOF];
            ...
            for (eAmcRcvIdx i = 0; i < eAmcRcvIdx.COUNTOF; i++)
            {
                string strColName = DAmcRcvRowItem.GetHeaderName(i, asMultiLine: true);
                rowData[(int)i] = strColName;
            }
            ...
            {
                if (i < (int)eAmcRcvIdx.In1_Hand_Inpos)
                    this.dataGridView1.Rows[0].Cells[i].Style.BackColor = Color.LightGray;
                else if (i < (int)eAmcRcvIdx.Out1_Hand_Close)
        ...
        public void ShowTable(DataTable table, DateTime selectedTime, string filePath)
        {
            ...
            this.dataGridView1.Columns[(int)eAmcRcvIdx.Mode_Stat].Frozen = true;    // stat 에서 열고정
			//dataGridView1.ClipboardCopyMode = DataGridViewClipboardCopyMode.EnableAlwaysIncludeHeaderText;
        ...
        public void RefreshCellColor()
        {
            ...
            for (int r = 1; r < nCnt; r++)
            {
                try
                {
                    ...
                    for (int i = 0; i < (int)eAmcRcvIdx.COUNTOF; i++)
                    {
						if (DAmcRcv.CellColor[i].conditionList.Count == 0)
						{
        ...
        public void toolStripBtn_TimeLine_Click(object sender, EventArgs e)
        {
			...
			List<int> selectedCols = MyTools.GridHelper.GetSelectedColumns(dataGridView1);
			List<int> selectedRows = MyTools.GridHelper.GetSelectedRows(dataGridView1);
			selectedRows.Sort();
			selectedCols.Sort();
			if (selectedRows.Count == 0)
			{
				MessageBox.Show("Row를 선택해 주세요.");
				return;
			}

            UTimeLine dlg = new UTimeLine();
			m_TimeLineList.Add(dlg);
			dlg.CreateAmcTimeLine(m_Parent.AmcRcv, selectedRows[0], selectedRows[selectedRows.Count-1], selectedCols);
        ...
        public void toolStripBtn_Header_Click(object sender, EventArgs e)
        {
