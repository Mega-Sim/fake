...
namespace LogViewer
{
    public partial class UAmcRecvCols : Form
    {
        ...
        {
            public string name = "";
            public List<eAmcRcvIdx> Items = new List<eAmcRcvIdx>();
            public void Add(eAmcRcvIdx n)
            {
                Items.Add(n);
            }
        }
        ...
        public List<DPreSet> presetItem = new List<DPreSet>();
        public DShowItem []listViewItems = new DShowItem[(int)eAmcRcvIdx.COUNTOF];
		...
		public UAmcRecvCols()
        {
            ...
            {
                string col = DAmcRcvRowItem.GetHeaderName(i, false);
                col = col.Replace('\n', '.');
                listViewItems[(int)i] = new DShowItem();
                listViewItems[(int)i].text = col;
                if (i == eAmcRcvIdx.Time_Date)
                    listViewItems[(int)i].isChecked = false;
                else if(i >= eAmcRcvIdx.AMC_Input1 && i <= eAmcRcvIdx.State1)
                    listViewItems[(int)i].isChecked = false;
                else
                    listViewItems[(int)i].isChecked = true;
                //listViewItems[(int)i].isChecked = i < eAmcRcvIdx.PauseStatus ? true : false;
                listViewItems[(int)i].tag = i;
                listViewItems[(int)i].visible = true;
            }
        ...
        void LoadINI(string path)
        {
            try
            {
                ...
                for (int i = 0; i < nCnt; i++)
                {
                    ...
                    {
                        int nVal = 0;
                        if (!Int32.TryParse(col, out nVal))
                            continue;
                        item.Items.Add((eAmcRcvIdx)nVal);
        ...
        void SaveINI(string path)
        {
            try
            {
                ...
                for (int i = 0; i < presetItem.Count; i++)
                {
                    ...
                    string strItems = "";
                    foreach (eAmcRcvIdx col in presetItem[i].Items)
                        strItems += Convert.ToString((int)col) + ",";
        ...
        private bool AddCurPreSet(string name)
        {
            ...
            {
                if(tmp.name == oneset.name)
                {
                    MessageBox.Show("같은 이름의 Preset이 있습니다.", "Preset 생성 실패", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
            }
        ...
        private void SetPreset(string str)
        {
            ...
            foreach(eAmcRcvIdx col in presetItem.ElementAt(nIdx).Items)
            {
                listViewItems[(int)col].isChecked = true;
            }
            RefreshListView();
        }
        ...
        {
            if (AddCurPreSet(comboBox_Presets.Text))
            {
                RefreshPresetComboBox();
