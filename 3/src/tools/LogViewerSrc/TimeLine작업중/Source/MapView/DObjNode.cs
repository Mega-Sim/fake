...
namespace ObjectDrawig
{
    ...
    public class DObjNode : DObjBase
    {
        ...
        public static Pen Pen_Normal = new Pen(Color.FromArgb(0x97, 0xD0, 0xA3), 2);          // 노리끼리한 색
        ...
        public override void Draw(Graphics g, DScale scale)
        {
            ...
            if (scale.dScale < VISIBLE_SCALE_NODE_TEXT)
            {
                ...
                if(bDrawNodeDetail) //detail view
                    g.DrawString(GetNodeDetailText(), applyFont, Brushes.White, xdc, ydc);
                else // normal view
                    g.DrawString(GetID(), applyFont, Brushes.White, xdc, ydc);
            }
