...
namespace LogViewer
{
    ...
    public class DUserLogEventChangeRowItem
    {
        ...
        public void SetData(DUserLogRowItem val, int nLineNo)
        {
           ...
           subItems[(int)eEventChangeLogIdx.Node]                 =   val.subItems[ (int)eUserLogIdx.Current_Node]       ;
			subItems[(int)eEventChangeLogIdx.Steer]			= val.subItems[(int)eUserLogIdx.Steetring];
    ...
    public class DUserLog
    {
		...
		public bool ParseLine(string filePath, string line, DUserLogRowItem row, bool bFirstLine)
        {
            ...
            for (int i = 0; i < arrSplit.Length; i++) //7점대 호환을 위해 (int)eUserLogSplittedIdx.COUNTOF -> arrSplit.Length 로 변경
            {
            ...
            else
            {
                ...
                if (row.subItems[(int)eUserLogIdx.Mode] != strPrevStat || row.subItems[(int)eUserLogIdx.Current_Node] != nPrevNode || row.subItems[(int)eUserLogIdx.Current_Station] != nPrevStation)// || row.subItems[(int)eUserLogIdx.Steetring] != cPrevSteer)
                {
                    DUserLogEventChangeRowItem eventRow = new DUserLogEventChangeRowItem();
                    eventRow.SetData(row, table.Rows.Count);
                    EventChangeList.Add(eventRow);
			...
			cPrevSteer = row.subItems[(int)eUserLogIdx.Steetring];
