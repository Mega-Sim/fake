...
// ---------------------------------------------------------------------------
// Default define 값 정의
#define MAX_PACKET_SIZE	1024		// 패킷 최대크기. 다이후쿠는 1024.
...
#if(OHT_NAME == OHT_NAME_STD_PATROL)
	#define MACHINETYPE_VHL 		MACHINETYPE_VHL_PATROL
#else
...
namespace TagID
{
	...
	// 작업지시 관련 명령등
		...
		JOB_PROGRESS_REPORT						= 1103, // 진척보고 (CLW -> MCP)
...
typedef struct
{
	...
	unsigned char			TeachingData_Reserved[8];    // Target 이적재 Tag Type
...
} PROTOCOL_BODY_JOB_PROGRESS_REPORT_CLW_TO_MCP_ID_103; 	// CLW -> MCP 진척보고 (ID:103)
...
typedef struct
{
	...
	PROTOCOL_TAG												TAG;
	PROTOCOL_STATUS_OHT											STATUS;
	PROTOCOL_BODY_JOB_PROGRESS_REPORT_CLW_TO_MCP_ID_103			BODY;
} PKT_JOB_PROGRESS_REPORT_ID_103;				// CLW -> MCP 진척보고 (ID:103)

void Conv_PKT_JOB_PROGRESS_REPORT_ID_103(PKT_JOB_PROGRESS_REPORT_ID_103& Pkt)
{
	ConvertEndian(&Pkt.HEADER.MachineType);
	ConvertEndian(&Pkt.TAG.Value);	
