...
//	m_StatusCheckThread->m_pHWCommon = GetHWCommon();
	...
	//Jig Check Box 실행
	m_JigBox = new TUnitCheckBox(this);
	m_JigBox->Show();
...
//	m_HWSet->m_pLamp->LampOff();//Main 종료시 LED OFF
	...
	m_JigThread->qiutRealThreadProc();
...
int TOHTMainForm::Jig_ErrReturn()
{
	int err = (int)m_ControlTask->GetOHTError();
	return err;
}

JigThread* TOHTMainForm::GetJigCtrl()
{
    if(this->m_JigThread == NULL)
        return NULL;
    else
		return this->m_JigThread;
