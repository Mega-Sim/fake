	...
	for(int i=0 ; i < NodeNum_Map ; i++ )
		{
			...
			//리프터 제외
			if(m_NodeArray_Map[i].LeftNodeID != 0 && m_NodeArray_Map[i].NodeType != LIFTIN_NTYPE_LEFT && m_NodeArray_Map[i].NodeType !=LIFTIN_NTYPE_RIGHT)
			{
			...
			if(m_NodeArray_Map[i].RightNodeID != 0 && m_NodeArray_Map[i].NodeType != LIFTIN_NTYPE_LEFT && m_NodeArray_Map[i].NodeType !=LIFTIN_NTYPE_RIGHT)
			{
...
//			pMapdata->m_NodeArray[i].CollisionInfo = 0;
 ...
 ADD_LOG_MAPMAKE_STD("LoadDataFromSTDMap Step2");
		...
		for(int i=0 ; i < NodeNum_Map ; i++ )
		{
			...
			if(pMapdata->m_PathArray[i].Speed < 500 || pMapdata->m_PathArray[i].Speed > 7000)			{
		...
		for(int i=0 ; i < NodeNum_Map ; i++ )
		{
			if( m_NodeArray_Map[i].RightNodeID != 0 ) // 오른쪽 분기 노드 존재 유무 확인
			{
				...
				if(pMapdata->m_PathArray[NodeNum_Map+tmp].Speed<500 || pMapdata->m_PathArray[NodeNum_Map+tmp].Speed > 7000)
				{
...
ADD_LOG_MAPMAKE_STD("LoadDataFromSTDMap Step4");
 ...
 ADD_LOG_MAPMAKE_STD("LoadDataFromSTDMap Step5");
		...
		for(int i=0 ; i < NodeNum_Map ; i++ )
		{
			...
			//pMapdata->m_PathArray[tmp].PathDirection = m_NodeArray_Collision[i].LeftUBGRegion;
//			if(m_NodeArray_Collision[i].LeftUBGRegion == m_NodeArray_Collision[i].LeftObstacleRegion)
//			{
				//pMapdata->m_PathArray[tmp].PathDirection = m_NodeArray_Collision[i].LeftUBGRegion;
...
//			}
			...
			if( m_NodeArray_Collision[i].RightNodeID != 0 ) // 노드가 존재시.
			{
				...
				if( pMapdata->m_PathArray[NodeNum_Map+tmp].EndNID == m_NodeArray_Collision[i].RightNodeID )
				{
					//pMapdata->m_PathArray[NodeNum_Map+tmp].PathDirection = m_NodeArray_Collision[i].RightUBGRegion;
//					if(m_NodeArray_Collision[i].RightUBGRegion == m_NodeArray_Collision[i].RightObstacleRegion)
//					{
						//pMapdata->m_PathArray[NodeNum_Map+tmp].PathDirection = m_NodeArray_Collision[i].RightUBGRegion;
