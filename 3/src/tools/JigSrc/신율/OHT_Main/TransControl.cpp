...
EXECUTE_ACK TransControl::SetGeneralCmd(EXECUTE_TRANS_INFO sTransInfo)
{
	...
	m_dHoistHomePos =	sTransInfo.TransCmd.Tposition_HomePos / 10.0;
// 2016-12-05, Add by Labeler, ROHT의 경우 Rotate 관련 Position 정보 할당 추가
	ADD_LOG("Teaching pos : %f - %f", sTransInfo.TransCmd.Tposition_Hoist, m_dHoist_TeachingPos);
...
//			nError = CheckNMoveTeachingPoint();
		...
		if(nError == NO_ERR)
        {
			...
			m_dwHandDelay_Time = timeGetTime();
...
//					m_pOHTMainStatus->StatusCommon.CurNodeOffset);
		...
		#if((OHT_NAME == OHT_NAME_STD_V85) || (OHT_NAME == OHT_NAME_STD_V86))
			if(OHTUTILITY::PreventOverflowGetTimeMs(m_dwHandDelay_Time) > 1000)  //V8.5 신률테스트 핸드 흔들림 때문에 딜레이 추가
			{
				m_dwHandDelay_Time = 0;
				m_nTransStep++;
			}
...
int TransControl::Move_EUV_Rotate(int nMoveCase) // Rotate 로직 안씀
{
//	int nError = 0;
//
//	switch(nMoveCase)
...
int TransControl::Check_EUV_Rotate()         //Rotate 안씀
{
//	bool bIsError = true;
//	DWORD dwStartTime = timeGetTime();
//	int nError = NO_ERR;
...
//	ADD_HOIST_POS_LOG("%f", dStartPosition_mm);
	...
	if(m_nTransType == TRANSTYPE_LOAD)
	{
		...
		case HOIST_UP_TP2TO:
			...
			{
				dDestinationPosition_mm = m_dHoistHomePos + TransDefine->nHoistMoveUp_TeachingOffset_Unload;
#if((OHT_NAME == OHT_NAME_STD_V85) || (OHT_NAME == OHT_NAME_STD_V86))
		...
		case HOIST_UP_TO2SDP:
			dDestinationPosition_mm = m_dHoistHomePos + TransDefine->nHoistMoveUp_HomeOffset_Unload;
			if(m_dHoist_TeachingPos < TransDefine->dFast_Move_Limit){
#if((OHT_NAME == OHT_NAME_STD_V85) || (OHT_NAME == OHT_NAME_STD_V86))
				dVel_mmpermsec = HOIST_MAX_SPEED * 0.45;
				dAccel_mmpermsec2 = 0.0035;
		...
		case HOIST_UP_SDP2HO:
			dDestinationPosition_mm = m_dHoistHomePos;
#if((OHT_NAME == OHT_NAME_STD_V85) || (OHT_NAME == OHT_NAME_STD_V86))
			if(m_dHoist_TeachingPos < TransDefine->dFast_Move_Limit){
				dVel_mmpermsec = HOIST_MAX_SPEED * 0.03;
				dAccel_mmpermsec2 = 0.0035;
...
//			else nCount = 0;
		...
		case HOIST_UP_HO2H:
			...
			if(bRunning == true)
				return nError;
			ADD_LOG("HOIST_UP_HO2H end. Hoist Position: %f", dCurPos);
		break;
...
int TransControl::CheckNMoveTeachingPoint()
{
	...
	if(m_dHoist_TeachingPos<= m_pHWSet->m_pHoistAxis->GetCurrentPosition())
	{
		m_dCmdFinal_HoistPos = m_pHWSet->m_pHoistAxis->GetCurrentPosition();
		m_pHWSet->m_pHoistAxis->MoveStop(HOIST_ESTOP_DECEL);
	...
	{
		nCount = 0;
		while(m_dHoist_TeachingPos > m_pHWSet->m_pHoistAxis->GetCurrentPosition())
		{
			nCount++;
		   if(nCount > 5) // 250msec
		   {
	...
	{
		case TRANSTYPE_LOAD:
		if((nError == NO_ERR) && (m_dHoist_TeachingPos > m_pHWSet->m_pHoistAxis->GetCurrentPosition())  )
		{
			   ...
			   while((OHTUTILITY::PreventOverflowGetTimeMs(dwSTime) < 5000))
			   {
					...
					if((m_dHoist_TeachingPos<= m_pHWSet->m_pHoistAxis->GetCurrentPosition()) &&(m_pHWSet->m_pHoistAxis->IsRunning() == false))
					{
						 // 센서가 100mec 동안 ON 유지
						if(nCheckCount >= 10)
						{
...
//		pParamSet->TransDefine.nDnState_UP_SpeedDown_Percent_SpecialEQ = 100;
		...
		pParamSet->TransDefine.nHoistMoveDown_TeachingOffset_Load = 100;  ///< Hoist Down ?à??¸ :?ӱ??? Offset  //timy 저속구간속도
