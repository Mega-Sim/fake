...
public:
	...
	void SetFileHeader(MAPMAKERFILE_HEADER_STD *pFileHeader);
...
//	NODE_INFO_STD_MAPMAEKRFILE* Find(BYTE MapMakerType, UINT StationID);
	...
	NODE_INFO_STD_MAPMAEKRFILE* FindUpdate(BYTE MapMakerType);
	...
	bool MapMakerFileFindUpdate(BYTE MapMakerType, UINT StationID);
	...
	void InitMapMakeFileMemory(); //Loading된 Map maker File 초기화. 파일 전송완료할때
