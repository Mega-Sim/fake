...
//	m_pRightEQPIO			= new EQPIO(m_pHWCommon, RIGHT_SIDE);
...
#if (((OHT_NAME == OHT_NAME_STD_V80) || (OHT_NAME == OHT_NAME_STD_V81) ||(OHT_NAME == OHT_NAME_STD_V81S)) && (SAMSUNG_COMMON_SPEC==ON))
...
//	m_pRailSupport			= new RailSupport(m_pHWCommon);
	...
	m_pShiftAxis			= new Shift(m_pHWCommon, 3);   // 기본 기어비
...
#if (((OHT_NAME == OHT_NAME_STD_V80)  || (OHT_NAME == OHT_NAME_STD_V81) ||(OHT_NAME == OHT_NAME_STD_V81S)||(OHT_NAME == OHT_NAME_STD_V85) || (OHT_NAME == OHT_NAME_STD_V86) || (OHT_NAME == OHT_NAME_STD_V80_RETICLE)) && (SAMSUNG_COMMON_SPEC==ON))
...
void HWSet::Sound_On(int nKindOfSound)
{
   ...
   switch (nKindOfSound)
   {
	  case SETSOUND_ERROR :
		 ...
		 sndPlaySound(strTmpFileName.c_str(), SND_ASYNC|SND_LOOP);
