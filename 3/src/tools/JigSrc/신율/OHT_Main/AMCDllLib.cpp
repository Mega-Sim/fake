...
//  **8.1하고 8.2하고 둘다 같은 OHT_NAME_STD_V81 를 사용해서 주석으로 구분  timy.
//  IO부분이 동일해서 8.2를 따로 def_OHT에 추가하지 않음.
...
// 8.1 or 8.1 SOM6869
	#ifdef  UNICODE
		#if USE_SOM6869
			LPCTSTR strTmpFileName = L"DataFiles\\amc6869.dll";       //TCHAR
		#else
	...
	#else
		#if USE_SOM6869
			LPCTSTR strTmpFileName = "DataFiles\\amc6869.dll";       //TCHAR
		#else
...
#if USE_SOM6869
		else if(!DeviceSerialLoadSetting || !DeviceSerialOpen || !DeviceSerialClose)
		{
			FreeLibrary(hAMCLib);
...
int AMCLibrary::MoveDistance(int nAxisNum, DWORD dwTime_Up, DWORD dwTime_Down, double dVel, double dDistance)
{
	...
	if(m_bAMCReady == false)
	{
		AMC_ADD_LOG("[TEST] AMC READY Fail!!!");
		return ERR_AMC_NOTOPEN;
	}
	...
	if(nTmpError==NO_ERR)
	{
		...
		m_AMCDllLibQueue.putData(QueuingAmcCmd , &nTmpError);
		 ////timy 불필요한 LOG 삭제
		AMC_ADD_LOG("[TEST] AMC MoveDST 1st:Error:%d,Axis~[%d:%8.1lf:%8.1lf:%8.1lf:%8.1lf]", nTmpError,nAxisNum,dAccel_Time_sec,dDecel_Time_sec,dVel_CtsPerSec,dDistance_Cts);
...
m_Input3Mask_R01.POD_EUV_2				= 0;
m_Input3Mask_R01.Reserved_1				= 0;
m_Input3Mask_R01.Reserved_2				= 0;
m_Input3Mask_R01.Reserved_3				= 0;
m_Input3Mask_R01.SPARE_12				= 0;
m_Input3Mask_R01.SPARE_13				= 0;
m_Input3Mask_R01.SPARE_14 				= 0;
