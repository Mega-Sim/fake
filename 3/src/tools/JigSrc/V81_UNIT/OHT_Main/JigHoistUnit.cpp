...
void __fastcall THoistUnit::FormClose(TObject *Sender, TCloseAction &Action)
{
	...
	//** Form 관련해서 Close 할때 다 초기화 시킬지 고민 필요 0420

	//Hoist 축 세팅
//	gSeq->m_pHWCommon->Set_Jig(2, false);// Hoist 축
//	gSeq->bHoistJig = false;    // Hoist Jig 관련 시퀀스 / 이적재 작업할때 쓰임
//	OHTMainForm->GetTransControl()->bHoistJig = false;
//	OHTMainForm->GetTransControl()->nHoistTeaching = 0;
...
//	pMaskOfInput->bUse  = false;
//
//	gSeq->m_pHWSet->m_pShutter->bHoistJig = false;
...
//		int CountUpdate = JigCtrl->m_JigParam.nHoist_CurrentCnt;
//		OHTMainForm->GetTransControl()->SetCycleCount(CountUpdate);
		SaveParam();
	}
	// 메모리 Param -> UI로 업데이트
	else
...
//	gSeq->m_pHWSet->m_pHoistAxis->SetAMPEnable(false);
...
//
//		}
//		gSeq->m_pHWSet->m_pHoistAxis->MoveStop(HOIST_ESTOP_DECEL);
//		gSeq->m_pHWSet->m_pHoistAxis->Reset();
//		JigCtrl->m_StepJig_Hoist.bSignal = false;
//		JigCtrl->m_StepJig_Hoist.Step = 0;
//		JigCtrl->m_StepJig_Hoist.StepComment = "Thread Stop";
//	}
