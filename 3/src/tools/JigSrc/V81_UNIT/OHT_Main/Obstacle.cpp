...
Obstacle::Obstacle(HWCommon *pHWCommon):VirtualDetect()
{
    ...
    m_nComPortNum = OBS_ComPort;                // Com Port 9
...
int Obstacle::ReceiveData(char *ReadBuff)
{
    int nError = NO_ERR;
	DWORD dwLen = 0;
    ...
    else
    {
	    ...
	    if(dwLen>0)
	    {
            ...
            OBS_LOG("OBS Data Receive : %d", dwLen);
		    nError = dwLen;
    	}
	    else
	    {
		    nError = 0;
            OBS_LOG("OBS Data Receive Fail");
	    }
...
int Obstacle::CheckObsSensorData(void)
{
    ...
    if(nReceiveDataSize == OBS_ReceiveDataSize)
    {
        if((Receive_Data[0] == OBS_Start_Flag_1) && (Receive_Data[1] == OBS_Start_Flag_2))
        {
            ...
            RapidLog(OBS_AREA_LOG, Receive_Data);
