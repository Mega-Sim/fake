...
#pragma resource "*.dfm"
TProbeOnlineCMDForm *ProbeOnlineCMDForm;
TCHKOHT CmdProbeOHTCheck;

// by zzang9un 2013.04.24 : Timer Interval Setting
#define TIMER_INTV_TRANSLONGRUN     50  ///< 이적재 롱런 타이머 Interval

// ---------------------------------------------------------------------------
__fastcall TProbeOnlineCMDForm::TProbeOnlineCMDForm(TComponent *Owner) : TForm(Owner)
{
...
// ---------------------------------------------------------------------------
void __fastcall TProbeOnlineCMDForm::WMEXITSIZEMOVE(TMessage &msg)
{
    int nTmpLeft, nTmpTop, nTmpRight;

    nTmpLeft  = ProbeOnlineCMDForm->Left;
    nTmpTop   = ProbeOnlineCMDForm->Top;
    nTmpRight = nTmpLeft + ProbeOnlineCMDForm->Width;
...
// ---------------------------------------------------------------------------
TTextAttributes *__fastcall TProbeOnlineCMDForm::CurrText(void)
{
...
// ---------------------------------------------------------------------------
/**
 @brief   ProbeOnlineCMDForm의 Show 발생시 호출되는 이벤트 함수
 @author  puting
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::FormShow(TObject *Sender)
{
...
// ---------------------------------------------------------------------------
void __fastcall TProbeOnlineCMDForm::FormClose(TObject *Sender,
    TCloseAction &Action)
{
    timerAllDisable();
}
...
// ---------------------------------------------------------------------------
int TProbeOnlineCMDForm::Check_Invalid_Command(String strTmpCMD)
{
    ...
    if((strTmpCMD.Pos("[") != 0) || (strTmpCMD.Pos("]") != 0))
    {
        ...
        {
			if(CmdProbeOHTCheck.Is_Executable
...
void __fastcall TProbeOnlineCMDForm::mniMemoClearClick(TObject *Sender)
{
    PMACMemo->Clear();
}

// ---------------------------------------------------------------------------
void TProbeOnlineCMDForm::PMACDataDisplay(TColor TmpFontColor, String strDisplayData)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnHandCloseClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnHandOpenClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        if(gm_OHTStatus->nProgram_Mode == MODE_SETUP)// 개발자
        {
            ...
            if(gm_OHTStatus->IOInput->uInput.InputProbe.HAND_Right_Grip == OFF &&
                gm_OHTStatus->IOInput->uInput.InputProbe.HAND_Left_Grip == OFF)
            {
 ...
 */
void __fastcall TProbeOnlineCMDForm::btnGoClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
...
/**
 ...
 @param   EStop
 */
void __fastcall TProbeOnlineCMDForm::btnStopClick(TObject *Sender)
{
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnTransMarkForwardClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
	{
        String strTmpCMD;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnTransMarkBackwardClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        String strTmpCMD;
        ...
        {
			if((gm_OHTStatus->IOInput->uInput.InputProbe.STEER_F_Left == gm_OHTStatus->IOInput->uInput.InputProbe.STEER_R_Left) &&
				(gm_OHTStatus->IOInput->uInput.InputProbe.STEER_F_Right == gm_OHTStatus->IOInput->uInput.InputProbe.STEER_R_Right))
            {
                TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TProbeOnlineCMDForm::memoJogKeyControlExit(TObject *Sender)
{
...
void __fastcall TProbeOnlineCMDForm::memoJogKeyControlClick(TObject *Sender)
{
...
@date    2013.02.19
*/
void __fastcall TProbeOnlineCMDForm::memoJogKeyControlKeyDown(TObject *Sender,
...
@date    2013.02.19
*/
void __fastcall TProbeOnlineCMDForm::memoJogKeyControlKeyUp(TObject *Sender,
    WORD &Key, TShiftState Shift)
{    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
	...
	TPCmd.CmdTP_Jog_DIST.Accel    = StrToFloat(edtAcceleration->Text) * 1000;
	TPCmd.CmdTP_Jog_DIST.Decel    = StrToFloat(edtDeceleration->Text) * 1000;
	TPCmd.CmdTP_Jog_DIST.Velocity = StrToFloat(panMPerSec->Caption) * 1000;
...
*/
void __fastcall TProbeOnlineCMDForm::timerDrivingJogTimer(TObject *Sender)
{
	bool bError = NO_ERR;
	...
	if(memoJogKeyControl->Focused())
	{
		...
		{
			Warnning_Display(COLOR_RED, "Driving Axis Error.........");
			timerDrivingJog->Enabled = false;
			return;
		}		

		if( (gm_OHTStatus->m_OHTStatus.Data.ErrorCode) &&
			((gm_OHTStatus->IOInput->uInput.InputProbe.STEER_F_Left != gm_OHTStatus->IOInput->uInput.InputProbe.STEER_R_Left) ||
			(gm_OHTStatus->IOInput->uInput.InputProbe.STEER_F_Right != gm_OHTStatus->IOInput->uInput.InputProbe.STEER_R_Right)))
		{
        ...
        if(CmdProbeOHTCheck.Is_Executable(EXCLUDE_DRIVING_RUNNING) == NO_ERR)
        {
		...
		if(gm_OHTStatus->IOInput->uInput.InputProbe.CARD_Detect_F == true || gm_OHTStatus->IOInput->uInput.InputProbe.CARD_Detect_R == true)
		{
			/* 2015-06-19, TU.Park PCard 테스트 위함
			if (gm_OHTStatus->IOInput->uInput.InputProbe.LEFT_HAND_Close == false ||
				gm_OHTStatus->IOInput->uInput.InputProbe.LEFT_HAND_Open == true ||
				gm_OHTStatus->IOInput->uInput.InputProbe.RIGHT_HAND_Close == false ||
				gm_OHTStatus->IOInput->uInput.InputProbe.RIGHT_HAND_Open == true ||
				gm_OHTStatus->IOInput->uInput.InputProbe.HAND_Right_Grip == false ||
				gm_OHTStatus->IOInput->uInput.InputProbe.HAND_Left_Grip == false)
			{
			...
			if (gm_OHTStatus->IOInput->uInput.InputProbe.SHUTTER_F_Close == false ||
				gm_OHTStatus->IOInput->uInput.InputProbe.SHUTTER_R_Close == false)
			{
		...
		{
			/* 2015-06-19, TU.Park PCard 테스트 위함
			if (gm_OHTStatus->IOInput->uInput.InputProbe.LEFT_HAND_Close == true ||
				gm_OHTStatus->IOInput->uInput.InputProbe.LEFT_HAND_Open == false ||
				gm_OHTStatus->IOInput->uInput.InputProbe.RIGHT_HAND_Close == true ||
				gm_OHTStatus->IOInput->uInput.InputProbe.RIGHT_HAND_Open == false ||
				gm_OHTStatus->IOInput->uInput.InputProbe.HAND_Right_Grip == true ||
				gm_OHTStatus->IOInput->uInput.InputProbe.HAND_Left_Grip == true)
			{
			...
			*/
			...
			if (gm_OHTStatus->IOInput->uInput.InputProbe.SHUTTER_F_Open == false ||
				gm_OHTStatus->IOInput->uInput.InputProbe.SHUTTER_R_Open == false)
			{
				Warnning_Display(COLOR_RED, "ERROR : ERR_SHUTTER_NOTORIGIN...");
				timerDrivingJog->Enabled = false;
				return;
			}
		}
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnHandOriginClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
	{
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnHandResetClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnHandFreeClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TProbeOnlineCMDForm::btnSteeringLeftClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable(EXCLUDE_DRIVING_RUNNING) == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
// ---------------------------------------------------------------------------
void __fastcall TProbeOnlineCMDForm::btnSteeringRightClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable(EXCLUDE_DRIVING_RUNNING) == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
@date    2012.12.14
*/
void __fastcall TProbeOnlineCMDForm::btnShutterOpenClick(TObject *Sender)
{
	// by zzang9un 2013.02.28 : 변수명이 길어서 지역변수로 대입한 후 사용
    bool Grip_R     = gm_OHTStatus->IOInput->uInput.InputProbe.HAND_Right_Grip;
	bool Grip_L     = gm_OHTStatus->IOInput->uInput.InputProbe.HAND_Left_Grip;
	bool CardDetectF = gm_OHTStatus->IOInput->uInput.InputProbe.CARD_Detect_F;
	bool CardDetectR = gm_OHTStatus->IOInput->uInput.InputProbe.CARD_Detect_R;

	if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
	{
		if((gm_OHTStatus->IOInput->uInput.InputProbe.LEFT_HAND_Open == ON) && (gm_OHTStatus->IOInput->uInput.InputProbe.RIGHT_HAND_Open == ON))
		{
			if((Grip_L == OFF) && (Grip_R == OFF) && (CardDetectF == OFF) && (CardDetectR == OFF))
            {
...
@date    2012.12.14
*/
void __fastcall TProbeOnlineCMDForm::btnShutterCloseClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
@date    2012.12.14
*/
void __fastcall TProbeOnlineCMDForm::btnShutterOriginClick(TObject *Sender)
{
...
@date    2012.12.14
*/
void __fastcall TProbeOnlineCMDForm::btnShutterFreeClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
// ---------------------------------------------------------------------------
int TProbeOnlineCMDForm::Set_UBGColor(int nOutput)
{
    int nError = 0;
...
/**
 @brief   ProbeOnlineCMDForm UI를 주기적으로 갱신하는 Timer 이벤트 함수
 @author  zzang9un
 @date    2013.03.29
 */
void __fastcall TProbeOnlineCMDForm::timerUpdateDisplayTimer(TObject *Sender)
{
    String strVelData;
    ...
    shpF_DrivingSen->Brush->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.STOP_Driving_Front == ON) ? COLOR_LIME : COLOR_WHITE;
    shpBCRDriving->Brush->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.BCR_Driving_OK == ON) ? COLOR_LIME : COLOR_WHITE;
    shpBCRDriving->Brush->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.BCR_Driving_NG == ON) ? COLOR_RED :
        shpBCRDriving->Brush->Color;

    shpF_TransSen->Brush->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.STOP_Trans_Front == ON) ? COLOR_LIME : COLOR_WHITE;
    shpR_TransSen->Brush->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.STOP_Trans_Rear == ON) ? COLOR_LIME : COLOR_WHITE;
    shpBCRTransLt->Brush->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.BCR_Trans_OK == ON) ? COLOR_LIME : COLOR_WHITE;
    shpBCRTransLt->Brush->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.BCR_Trans_NG == ON) ? COLOR_RED :
    ...
    // ** Hand ***
	// input
	LHandOpenLt->Brush->Color    = (gm_OHTStatus->IOInput->uInput.InputProbe.LEFT_HAND_Open == ON) ? COLOR_LIME : COLOR_WHITE;
	RHandOpenLt->Brush->Color    = (gm_OHTStatus->IOInput->uInput.InputProbe.RIGHT_HAND_Open == ON) ? COLOR_LIME : COLOR_WHITE;
	LHandInPosLt->Brush->Color   = (gm_OHTStatus->IOInput->uInput.InputProbe.LEFT_HAND_Inposition == ON) ? COLOR_LIME : COLOR_WHITE;
	RHandInPosLt->Brush->Color   = (gm_OHTStatus->IOInput->uInput.InputProbe.RIGHT_HAND_Inposition == ON) ? COLOR_LIME : COLOR_WHITE;
	LHandCloseLt->Brush->Color   = (gm_OHTStatus->IOInput->uInput.InputProbe.LEFT_HAND_Close == ON) ? COLOR_LIME : COLOR_WHITE;
	RHandCloseLt->Brush->Color   = (gm_OHTStatus->IOInput->uInput.InputProbe.RIGHT_HAND_Close == ON) ? COLOR_LIME : COLOR_WHITE;
	LHandPushLt->Brush->Color 	 = (gm_OHTStatus->IOInput->uInput.InputProbe.LEFT_HAND_Push == ON) ? COLOR_LIME : COLOR_WHITE;
	RHandPushLt->Brush->Color 	 = (gm_OHTStatus->IOInput->uInput.InputProbe.RIGHT_HAND_Push == ON) ? COLOR_LIME : COLOR_WHITE;
	LHandAlarmLt->Brush->Color   = (gm_OHTStatus->IOInput->uInput.InputProbe.LEFT_HAND_Alarm == ON) ? COLOR_RED : COLOR_WHITE;
	RHandAlarmLt->Brush->Color   = (gm_OHTStatus->IOInput->uInput.InputProbe.RIGHT_HAND_Alarm == ON) ? COLOR_RED : COLOR_WHITE;
	LHandGripLt->Brush->Color    = (gm_OHTStatus->IOInput->uInput.InputProbe.HAND_Left_Grip == ON) ? COLOR_LIME : COLOR_WHITE;
	RHandGripLt->Brush->Color    = (gm_OHTStatus->IOInput->uInput.InputProbe.HAND_Right_Grip == ON) ? COLOR_LIME : COLOR_WHITE;
	panLHandPosition->Color		= (gm_OHTStatus->IOInput->uInput.InputProbe.LEFT_HAND_Pin_Position == ON) ? COLOR_LIME : COLOR_WHITE;
	panRHandPosition->Color		= (gm_OHTStatus->IOInput->uInput.InputProbe.RIGHT_HAND_Pin_Position == ON) ? COLOR_LIME : COLOR_WHITE;
	HandOriginLt->Color         = (gm_OHTStatus->bIs_Hand_Origin == true) ? COLOR_LIME : COLOR_FUCHSIA;
	HandGoLt->Color             = (gm_OHTStatus->IOInput->uInput.InputProbe.HAND_PIO_Go == 1) ? COLOR_LIME : COLOR_FUCHSIA;
	// output
	grpHand->Color       = ((gm_OHTStatus->IOOutput->uOutput.OutputProbe.LEFT_HAND_Free == ON)&&(gm_OHTStatus->IOOutput->uOutput.OutputProbe.RIGHT_HAND_Free == ON)) ? COLOR_RED : (TColor)0x00FFDFDF;
	btnHandFree->Caption = ((gm_OHTStatus->IOOutput->uOutput.OutputProbe.LEFT_HAND_Free == ON)&&(gm_OHTStatus->IOOutput->uOutput.OutputProbe.RIGHT_HAND_Free == ON)) ? "[Hand Enable]" : "[Hand Free]";

	// ** Steering **
	// input
	FStrLeftLt->Brush->Color  = (gm_OHTStatus->IOInput->uInput.InputProbe.STEER_F_Left == ON) ? COLOR_LIME : COLOR_WHITE;
	FStrRightLt->Brush->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.STEER_F_Right == ON) ? COLOR_LIME : COLOR_WHITE;
    RStrLeftLt->Brush->Color  = (gm_OHTStatus->IOInput->uInput.InputProbe.STEER_R_Left == ON) ? COLOR_LIME : COLOR_WHITE;
    RStrRightLt->Brush->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.STEER_R_Right == ON) ? COLOR_LIME : COLOR_WHITE;
    // output
	strStrTorque.sprintf(L"%d %d", gm_OHTStatus->IOOutput->uOutput.OutputProbe.STEER_F_TRQ_0,
		gm_OHTStatus->IOOutput->uOutput.OutputProbe.STEER_F_TRQ_1);
    panFStrTorque->Caption = strStrTorque;
	strStrTorque.sprintf(L"%d %d", gm_OHTStatus->IOOutput->uOutput.OutputProbe.STEER_R_TRQ_0,
		gm_OHTStatus->IOOutput->uOutput.OutputProbe.STEER_R_TRQ_1);
    panRStrTorque->Caption = strStrTorque;

    // ** Shutter **
	shpShutterOpen_Front->Brush->Color  = (gm_OHTStatus->IOInput->uInput.InputProbe.SHUTTER_F_Open == ON) ? COLOR_LIME : COLOR_WHITE;
    shpShutterOpen_Rear->Brush->Color  = (gm_OHTStatus->IOInput->uInput.InputProbe.SHUTTER_R_Open == ON) ? COLOR_LIME : COLOR_WHITE;
    shpShutterClose_Front->Brush->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.SHUTTER_F_Close == ON) ? COLOR_LIME : COLOR_WHITE;
    shpShutterClose_Rear->Brush->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.SHUTTER_R_Close == ON) ? COLOR_LIME : COLOR_WHITE;
    shpShutterAlarm_Front->Brush->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.SHUTTER_F_Alarm == ON) ? COLOR_RED : COLOR_WHITE;
    shpShutterAlarm_Rear->Brush->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.SHUTTER_R_Alarm == ON) ? COLOR_RED : COLOR_WHITE;
    shpShutterInposition_Front->Brush->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.SHUTTER_F_Inposition == ON) ? COLOR_LIME : COLOR_WHITE;
    shpShutterInposition_Rear->Brush->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.SHUTTER_R_Inposition == ON) ? COLOR_LIME : COLOR_WHITE;
    panShtrOrigin->Color      = (gm_OHTStatus->bIs_Shutter_Origin == true) ? COLOR_LIME : COLOR_FUCHSIA;

    grpShutter->Color = ((gm_OHTStatus->IOOutput->uOutput.OutputProbe.SHUTTER_F_Free == ON) ||
        (gm_OHTStatus->IOOutput->uOutput.OutputProbe.SHUTTER_R_Free == ON)) ? COLOR_RED : (TColor)0x00FFDFDF;
    btnShutterFree->Caption = ((gm_OHTStatus->IOOutput->uOutput.OutputProbe.SHUTTER_F_Free == ON) ||
        (gm_OHTStatus->IOOutput->uOutput.OutputProbe.SHUTTER_R_Free == ON)) ? "[Enable]" : "[Free]";
    ...
    // ** ETC **
    panHoist_Home->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.HOIST_Home == ON) ? COLOR_LIME : COLOR_WHITE;
    panOscillation->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.OSCILLATION == ON) ? COLOR_LIME : COLOR_WHITE;
    // if((gm_OHTStatus->strOHTVer!=OHTVER_V10LP)&&(gm_OHTStatus->strOHTVer!=OHTVER_V20_MZ))
    // //수정 puting 2012.11.27
    // {
    panLookdown->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.LOOKDOWN_Detect == ON) ? COLOR_LIME : COLOR_WHITE;
    panLookdown->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.LOOKDOWN_Error == ON) ? COLOR_RED : panLookdown->Color;

    // }
    panOBSLong->Color   = (gm_OHTStatus->IOInput->uInput.InputProbe.OBSTACLE_Long == ON) ? COLOR_LIME : COLOR_WHITE;
    panOBSMiddle->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.OBSTACLE_Middle == ON) ? COLOR_LIME : COLOR_WHITE;
    panOBSShort->Color  = (gm_OHTStatus->IOInput->uInput.InputProbe.OBSTACLE_Short == ON) ? COLOR_LIME : COLOR_WHITE;
    panOBSFail->Color   = (gm_OHTStatus->IOInput->uInput.InputProbe.OBSTACLE_Fail == ON) ? COLOR_RED : COLOR_WHITE;

    // 추가 puting 2012.11.27
	panMC_Status->Color   = (gm_OHTStatus->IOInput->uInput.InputProbe.MC_Status == ON) ? COLOR_LIME : COLOR_WHITE;
    ...
    panUBGLong->Color   = (gm_OHTStatus->IOInput->uInput.InputProbe.OHTDETECT_Long == ON) ? COLOR_LIME : COLOR_WHITE;
    panUBGMiddle->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.OHTDETECT_Middle == ON) ? COLOR_LIME : COLOR_WHITE;
    panUBGShort->Color  = (gm_OHTStatus->IOInput->uInput.InputProbe.OHTDETECT_Short == ON) ? COLOR_LIME : COLOR_WHITE;
    panUBGFail->Color   = (gm_OHTStatus->IOInput->uInput.InputProbe.OHTDETECT_Fail == ON) ? COLOR_RED : COLOR_WHITE;

	if(panCardDetectF->Visible == false)
		panCardDetectF->Visible = true;
	panCardDetectF->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.CARD_Detect_F == ON) ? COLOR_LIME : COLOR_WHITE;
	if(panCardDetectR->Visible == false)
		panCardDetectR->Visible = true;
	panCardDetectR->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.CARD_Detect_R == ON) ? COLOR_LIME : COLOR_WHITE;

    if(panShift_Home->Visible == false)
        panShift_Home->Visible = true;
    panShift_Home->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.SHIFT_Home == ON) ? COLOR_LIME : COLOR_WHITE;

    if(panUTB->Visible == false)
        panUTB->Visible = true;
	panUTB->Color = (gm_OHTStatus->IOInput->uInput.InputProbe.UTB_Detect == ON) ? COLOR_LIME : COLOR_WHITE;
	...
	// ** Power ON/OFF
    int j                 = 0;
    PWGrid->Cells[1][j++] = (gm_OHTStatus->IOOutput->uOutput.OutputProbe.HAND_Power_Reset == ON) ? "OFF" : "ON";
    // *** Hand ***
    if(gm_OHTStatus->IOOutput->uOutput.OutputProbe.HAND_Power_Reset == 1)
    {
    ...
    int nTr1, nTr2;
	nTr1 = gm_OHTStatus->IOOutput->uOutput.OutputProbe.STEER_F_TRQ_0;
	nTr2 = gm_OHTStatus->IOOutput->uOutput.OutputProbe.STEER_F_TRQ_1;
    if((nTr1 == 1) && (nTr2 == 1))
    {
	...
	nTr1 = gm_OHTStatus->IOOutput->uOutput.OutputProbe.STEER_R_TRQ_0;
	nTr2 = gm_OHTStatus->IOOutput->uOutput.OutputProbe.STEER_R_TRQ_1;
    if((nTr1 == 1) && (nTr2 == 1))
    {
    ...
    strLR = (gm_OHTStatus->IOInput->uInput.InputProbe.EQPIO_Load_REQ == ON) ? "L" : "0";
    strUR = (gm_OHTStatus->IOInput->uInput.InputProbe.EQPIO_Unload_REQ == ON) ? "U" : "0";
    strRE = (gm_OHTStatus->IOInput->uInput.InputProbe.EQPIO_Ready == ON) ? "R" : "0";
    if(gm_OHTStatus->nIsRFPIOType == PIOTYPE_E84)
    {
        strHO = (gm_OHTStatus->IOInput->uInput.InputProbe.EQPIO_HO_AVBL == ON) ? "H" : "0";
        strES = (gm_OHTStatus->IOInput->uInput.InputProbe.EQPIO_ES == ON) ? "E" : "0";
    ...
    strCS = (gm_OHTStatus->IOOutput->uOutput.OutputProbe.EQPIO_CS_0 == ON) ? "C" : "0";
    strVA = (gm_OHTStatus->IOOutput->uOutput.OutputProbe.EQPIO_Valid == ON) ? "V" : "0";
	strTR = (gm_OHTStatus->IOOutput->uOutput.OutputProbe.EQPIO_TR_Req == ON) ? "T" : "0";
    strBU = (gm_OHTStatus->IOOutput->uOutput.OutputProbe.EQPIO_Busy == ON) ? "B" : "0";
    strCO = (gm_OHTStatus->IOOutput->uOutput.OutputProbe.EQPIO_Complete == ON) ? "C" : "0";
    if(gm_OHTStatus->nIsRFPIOType == PIOTYPE_E23)
    {
		strCS1 = (gm_OHTStatus->IOOutput->uOutput.OutputProbe.EQPIO_CS_1 == ON) ? "1" : "0";
    ...
    if(gm_OHTStatus->IOInput->uInput.InputProbe.FAN_Fail)
    {
...
@date    2012.12.14
*/
void __fastcall TProbeOnlineCMDForm::btnStopTagBCRClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
@date    2012.12.14
*/
void __fastcall TProbeOnlineCMDForm::btnMotor2HomeClick(TObject *Sender)
{
    CmdProbeOHTCheck.Axis_Hoist_HOME();
}
// ---------------------------------------------------------------------------
...
i226 = (M262/3072) * 16
*/
void __fastcall TProbeOnlineCMDForm::mniHoistOriginSetClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TProbeOnlineCMDForm::mniShiftOriginSetClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
@date    2012.12.14
*/
void __fastcall TProbeOnlineCMDForm::edtSpeedChange(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnMoveStopClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable(EXCLUDE_HOIST_RUNNING EXCLUDE_SHIFT_RUNNING EXCLUDE_ROTATE_RUNNING) == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TProbeOnlineCMDForm::FormCreate(TObject *Sender)
{
    int j                         = 0;
    ProbeOnlineCMDForm->Top            = MainForm->Top;
    ProbeOnlineCMDForm->Left           = MainForm->Left + MainForm->Width + StatusForm->Width;
...
// ---------------------------------------------------------------------------
void __fastcall TProbeOnlineCMDForm::pgcMainControlDrawTab(
...
// ---------------------------------------------------------------------------
void TProbeOnlineCMDForm::Warnning_Display(TColor tWColor, String strData)
{
...
void __fastcall TProbeOnlineCMDForm::PWGridDrawCell(TObject *Sender, int ACol,
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnHaONClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnHaOFFClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        String strTmpMessage;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnHaRstClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        String strTmpMessage;
...
/**
 ...
 @param   Sender : TObject
 */
void __fastcall TProbeOnlineCMDForm::timerTransLongRunTimer(TObject *Sender)
{
...
void __fastcall TProbeOnlineCMDForm::chkLongRunClick(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnShutterAlarmResetClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @param   EStop
 */
void __fastcall TProbeOnlineCMDForm::btnMotor34HomeClick(TObject *Sender)
{
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
    int nTmpRotateAccel, nTmpRotateSpeed;
    String strTmpData;

    if((gm_OHTStatus->IOInput->uInput.InputProbe.HOIST_Home == ON) &&
        (fabs(StrToFloat(gm_OHTStatus->Motor_Hoist->dPosition_mm)) < HOIST_ORIGIN_LIMIT))
    {
        if((gm_OHTStatus->IOInput->uInput.InputProbe.SHUTTER_F_Open == ON) &&
            (gm_OHTStatus->IOInput->uInput.InputProbe.SHUTTER_R_Open == ON))
        {
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnLoadMouseDown(TObject *Sender,
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnUnloadMouseDown(TObject *Sender,
    TMouseButton Button, TShiftState Shift, int X, int Y)
{
    TPoint P, Q;
    if(Button == mbLeft)
...
void __fastcall TProbeOnlineCMDForm::pmDrivingFServoOnOffPopup(TObject *Sender)
{
...
void __fastcall TProbeOnlineCMDForm::pmDrivingRServoOnOffPopup(TObject *Sender)
{
...
void __fastcall TProbeOnlineCMDForm::pmHoistServoOnOffPopup(TObject *Sender)
{
...
void __fastcall TProbeOnlineCMDForm::pmShiftServoOnOffPopup(TObject *Sender)
{
...
void __fastcall TProbeOnlineCMDForm::pmRotatePopup(TObject *Sender)
{
...
void __fastcall TProbeOnlineCMDForm::mniDrvFServoOnClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TProbeOnlineCMDForm::mniDrvFServoOffClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TProbeOnlineCMDForm::mniDrivingRServoOnClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TProbeOnlineCMDForm::mniDrivingRServoOffClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TProbeOnlineCMDForm::mniHoistServoOnClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TProbeOnlineCMDForm::mniHoistServoOffClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
	{
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TProbeOnlineCMDForm::mniShiftServoOnClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TProbeOnlineCMDForm::mniShiftServoOffClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TProbeOnlineCMDForm::mniRotateServoOnClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TProbeOnlineCMDForm::mniRotateServoOffClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TProbeOnlineCMDForm::mniRotateOriginSetClick(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnRotate_OrignClick(TObject *Sender)
{
    bool bExec = false;

    // if((CmdProbeOHTCheck.Is_Executable()==NO_ERR)&&(CmdProbeOHTCheck.Is_MoveAxis_Rotate()==NO_ERR))
    if((gm_OHTStatus->IOInput->uInput.InputProbe.HOIST_Home == ON) &&
        (fabs(StrToFloat(gm_OHTStatus->Motor_Hoist->dPosition_mm)) < HOIST_ORIGIN_LIMIT))
    {
        if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
        {
            if((gm_OHTStatus->IOInput->uInput.InputProbe.HAND_Left_Grip == OFF) &&
                (gm_OHTStatus->IOInput->uInput.InputProbe.HAND_Right_Grip == OFF))
                bExec = true;
            else
            {
                if((gm_OHTStatus->IOInput->uInput.InputProbe.SHUTTER_F_Open == ON) &&
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnRotate_EnableClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnRotate_DisableClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnRotate_StopClick(TObject *Sender)
{
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnFSteerONClick(TObject *Sender)
{
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnFSteerOFFClick(TObject *Sender)
{
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnRSteerONClick(TObject *Sender)
{
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnRSteerOFFClick(TObject *Sender)
{
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TProbeOnlineCMDForm::chkDistClick(TObject *Sender)
{
...
// ---------------------------------------------------------------------------
int TProbeOnlineCMDForm::DistChang_mm_2(void)
{
    int nError = 0;
...
// ---------------------------------------------------------------------------
int TProbeOnlineCMDForm::DistChang_cts_2(void)
{
    int nError = 0;
    float dDist_mm, dDist_M, dDist_cts;
    String strdd;
...
// ---------------------------------------------------------------------------
void __fastcall TProbeOnlineCMDForm::edtDistmmChange(TObject *Sender)
{
    DistChang_mm_2();
}
// ---------------------------------------------------------------------------

void __fastcall TProbeOnlineCMDForm::edtDistctsChange(TObject *Sender)
{
    DistChang_cts_2();
}
// ---------------------------------------------------------------------------

void __fastcall TProbeOnlineCMDForm::btnMoveClick(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnScanUTBClick(TObject *Sender)
{
    String strTmpMessage;
	...
	if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        ...
        else
        {
            ...
            if(Application->MessageBox(strTmpMessage.w_str(), L"Confirm", MB_OKCANCEL) == IDOK)
            {
                ...
                if(ProbeOnlineCMDForm->chkLongRun->Checked == true)
...
void __fastcall TProbeOnlineCMDForm::btnTractionLeftClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable(EXCLUDE_DRIVING_RUNNING) == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TProbeOnlineCMDForm::btnTractionRightClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable(EXCLUDE_DRIVING_RUNNING) == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TProbeOnlineCMDForm::chkTractionModeClick(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnRotateOriginClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnRotateResetClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnRotate0Click(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnRotate90Click(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnRotate180Click(TObject *Sender)
{
	if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnRotate270Click(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnRotateFreeClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnMCControlOnClick(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnMCControlOffClick(TObject *Sender)
{
    ///추가 puting 2012.11.28
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnSTB_R_WrokOnClick(TObject *Sender)
{
    ///추가 puting 2012.11.28
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnSTB_R_WorkOffClick(TObject *Sender)
{
    ///추가 puting 2012.11.28
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnSTB_L_WrokOnClick(TObject *Sender)
{
    ///추가 puting 2012.11.28
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnSTB_L_WrokOffClick(TObject *Sender)
{
    ///추가 puting 2012.11.28
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnDrivingBCRClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnDrivingMarkBackwardClick(
    TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TProbeOnlineCMDForm::btnDrivingMarkForwardClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        String strTmpCMD;
...
@return  변환된 값
*/
BYTE TProbeOnlineCMDForm::GetTransNodeType(int index)
{
    BYTE rtnVal = 0x00;
...
@param   Sender : TObject
*/
void __fastcall TProbeOnlineCMDForm::chkGoStationClick(TObject *Sender)
{
    if(chkGoStation->Checked)
    {
...
void __fastcall TProbeOnlineCMDForm::mniLoadEQClick(TObject *Sender)
{
...
void __fastcall TProbeOnlineCMDForm::mniLoadEQ_XClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
	{
...
void __fastcall TProbeOnlineCMDForm::mniLoadSTOCKERClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TProbeOnlineCMDForm::mniLoadSTB_LClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TProbeOnlineCMDForm::mniLoadSTB_RClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TProbeOnlineCMDForm::mniLoadUTBClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TProbeOnlineCMDForm::mniLoadLOADERClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
	if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TProbeOnlineCMDForm::mniUnloadEQClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TProbeOnlineCMDForm::mniUnloadEQ_XClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TProbeOnlineCMDForm::mniUnloadSTOCKERClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TProbeOnlineCMDForm::mniUnloadLOADERClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TProbeOnlineCMDForm::mniUnloadSTB_LClick(TObject *Sender)
{
	// by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TProbeOnlineCMDForm::mniUnloadSTB_RClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TProbeOnlineCMDForm::mniUnloadUTBClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
...
/**
 ...
 @date    2013.03.26
 */
void __fastcall TProbeOnlineCMDForm::btnQRTagLeftMarkClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
	{
        if((gm_OHTStatus->Motor_Driving_F->nError == 1) ||
...
/**
 ...
 @date    2013.03.26
 */
void __fastcall TProbeOnlineCMDForm::btnQRTagRightMarkClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        if((gm_OHTStatus->Motor_Driving_F->nError == 1) ||
...
// ---------------------------------------------------------------------------
void __fastcall TProbeOnlineCMDForm::panTeachingSaveClick(TObject *Sender)
{
    TeachingForm->Show();
...
/**
@brief   ProbeOnlineCMDForm의 Timer를 Enable하는 함수
@author  zzang9un
@date    2013.11.20
*/
void TProbeOnlineCMDForm::timerAllDisable()
{
    timerUpdateDisplay->Enabled = false;
    timerDrivingJog->Enabled = false;
    timerTransLongRun->Enabled = false;
...
void __fastcall TProbeOnlineCMDForm::btnMoveTeachingPointMouseDown(TObject *Sender, TMouseButton Button,
...
void __fastcall TProbeOnlineCMDForm::mniMoveTPEQClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TProbeOnlineCMDForm::mniMoveTPEQ_XClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TProbeOnlineCMDForm::mniMoveTPSTOCKERClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TProbeOnlineCMDForm::mniMoveTPLOADERClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TProbeOnlineCMDForm::mniMoveTPSTB_LClick(TObject *Sender)
{
	if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TProbeOnlineCMDForm::mniMoveTPSTB_RClick(TObject *Sender)
{
    if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TProbeOnlineCMDForm::mniMoveTPUTBClick(TObject *Sender)
{
	if(CmdProbeOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
