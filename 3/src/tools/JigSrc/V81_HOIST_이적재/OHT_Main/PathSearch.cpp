...
bool PathSearch::GetIsNBrenchMapMake(MAP *pMap, UINT nNextNode)
{
	...
	nNodeIndex = pTmpMap->FindNodeIndex(nNextNode);   ///< 출발 노드 Index
	pNODE_INFO = &(pTmpMap->m_NodeArray[nNodeIndex]);  ///< 출발 노드 정보
...
@return  Error Code
*/
int PathSearch::StandardSearchPassPermit(MAP *pMap, STD_FlowControl *pFlowMap, PARSING_GO_INFO* CmdGoData, EXECUTE_DRIVING_ADD_INFO AdditionalInfo, LList *DRIVING_INFO_List, UINT &uCheckExtraNum,bool bIsFoup,double PathExtra,bool bIgnorOverrun)
{
	// 0. 값 초기화 ------------------------------------------------------------
	int nTmpError				= NO_ERR;					// 현재 에러 초기화
...
//				 pPATH_INFO->PathDirection  = PATH_DIRECTION_SPECIAL_R;
			...
			}else if((bIsFoup ==false) && (m_pParamSet->nIsCurveSpeedUp == true))
			{
#if(USE_CHANGENODE == 1)
...
int PathSearch::StandardSearchOnlyOnePassPermit(MAP *pMap, STD_FlowControl *pFlowMap, PARSING_GO_INFO* CmdGoData, EXECUTE_DRIVING_ADD_INFO AdditionalInfo, LList *DRIVING_INFO_List, bool bcheckTarget, PKT_CYCLECMDDATA *FinalCycleData, UINT &uCheckExtraNum, double &dTempDis, UINT FinalSeqNum,PASSPERMIT_COMMON TempPassPermitData,bool bIsFoup, bool bIgnorOverrun)
{
	// 0. 값 초기화 ------------------------------------------------------------
	int nTmpError				= NO_ERR;				// 현재 에러 초기화
...
//			pEXECUTE_DRIVING_INFO->BasicInfo.DeviceType     = pNODE_INFO->InOutInfo;
			...
			pEXECUTE_DRIVING_INFO->BasicInfo.StartNType		= aNodeType[0];
			pEXECUTE_DRIVING_INFO->BasicInfo.EndNType		= aNodeType[1];
			pEXECUTE_DRIVING_INFO->BasicInfo.dSpeed			= (double)(pPATH_INFO->Speed/1000.0)*0.4;			// mm/s -> mm/msec
			pEXECUTE_DRIVING_INFO->BasicInfo.dLength		= (double)(pPATH_INFO->Distance);		// UINT --> double
			pEXECUTE_DRIVING_INFO->BasicInfo.Steering		= (PATH_STEERING_INFO)(pPATH_INFO->Steering);
#if(USE_CHANGENODE == 1)
			pEXECUTE_DRIVING_INFO->BasicInfo.UBGRegion		= (PATH_DIRECTION_INFO)(pPATH_INFO->UBGRegion);
...
bool PathSearch::loadParameterRecovery(char* szFilePath, PATHSEARCH_PARAM_SET* pParamSet)
{
	...
	if(bSuccess ==false)
	{
		bCheckParam = loadParameter("DataFiles\\PathSearch.bgk", m_pParamSet);
		if(bCheckParam)
			saveParameter("DataFiles\\PathSearch.ini", m_pParamSet);
		else
	...
	{
		saveParameter("DataFiles\\PathSearch.bgk", m_pParamSet);
...
bool PathSearch::loadParameter(char* szFilePath, PATHSEARCH_PARAM_SET* pParamSet)
{
	...
	if(pIniFile != NULL)
	{
		...
		pIniFile->ReadIntegerMinMaxCheck("PASSPERMIT","PassPermit_Count ", 7, 1, 10, &nTmpValue);
		pParamSet->nPassPermitCount = nTmpValue;
		...
		pIniFile->ReadIntegerMinMaxCheck("PASSPERMIT","PassPermitRollbackFinshTime", 400, 350, 800, &nTmpValue);
		pParamSet->nPassPermitRollbackFinshTime = nTmpValue;
	...
	else
	{
		...
		pParamSet->nPassPermitPeriod = 100;
		pParamSet->nPassPermitCount  = 5;
...
//			}
	...
	while(bArrived==false && nTmpError == NO_ERR)
	{
		...
		{
		  if(GetIsNBrenchMapMake(pMap, uTmpToNode))
		  {
