/**
@file    Remocon.cpp
@version OHT 7.0
@brief   Remocon Class CPP File
*/

#include "Remocon.h"
#include "OHTMain.h"

/**
@brief   Remocon Class 생성자
@author  ji7412.lee
@date    2013.04.02
@param   pHWCommon : HWCommon 클래스 포인터
*/
Remocon::Remocon(HWCommon *pHWCommon)
{
	m_nComPortNum = COM1;
	m_nBaudRate = HANDTP_BAUDRATE;
	m_pCommPort = NULL;
	bConnect = false;
}

/**
@brief   Remocon Class 소멸자
@author  ji7412.lee
@date    2013.04.02
*/
Remocon::~Remocon()
{
	CloseCom();
}


/**
@brief   Remocon 접속 유무를 체크하는 함수
@author  ji7412.lee
@date    2012.10.22
@return  Remocon이 접속인 경우 True, 비접속인 경우 False를 Return
*/
bool Remocon::IsConnect()
{
	return bConnect;
}

 /**
@brief   Remocon 에서 받은 Data를 분석하는 함수
@author  ji7412.lee
@date    2013.04.02
@return
*/
int Remocon::AnalysisData()
{
}

/**
@brief   Remocon Comm Port를 Open하는 함수
@author  ji7412.lee
@date    2013.04.02
@return  Open 하는데 문제 없으면 0, 문제가 발생하면 ERR_OPEN 리턴
*/
int Remocon::OpenCom()
{
	int nError = 0;

	if(m_pCommPort==NULL)
	{
		m_pCommPort = new Comm_Port(m_nComPortNum, m_nBaudRate);
		if(m_pCommPort==NULL)
		{
			nError=ERR_BCR_OPEN;
		}
		else
		{
			bConnect = true;
		}
	}
	else if(m_pCommPort!=NULL)
	{
		nError = ERR_BCR_OPEN;
		bConnect = false;
	}
	return nError;
}

/**
@brief   Remocon Comm Port를 Close하는 함수
@author  ji7412.lee
@date    2013.04.02
*/
int Remocon::CloseCom()
{
	int nError = 0;

	if(m_pCommPort!=NULL)
	{
		delete m_pCommPort;
		m_pCommPort = NULL;
	}
	bConnect = false;
	return nError;
}

/**
@brief   Remocon Comm Port를 ReOpen하는 함수
@author  ji7412.lee
@date    2013.04.02
*/
int Remocon::ReOpenCom()
{
	if(m_pCommPort!=NULL)
	{
		CloseCom();	// 통신이 연결 시 종료하고 재연결
	}
	return OpenCom();
}

/**
@brief   Remocon Read 명령을 주고 그에 대한 응답을 받는 함수
@author  ji7412.lee
@date    2013.04.02
@param   nID : 읽은 결과 값을 저장하는 변수, 실패 시 0 대입
@return  Read하는데 문제 없으면 0, 문제 발생 시 해당 에러 코드 리턴
*/
int Remocon::GetID(unsigned char* pReadBuff)
{
	int Error = NO_ERR;
	DWORD dwLen = 0;
	int nReadBuffLength = 0;
	unsigned char szReadBuff[1024] = {0,};

	if(m_pCommPort==NULL)
	{
		Error = ReOpenCom();
	}

	if(m_pCommPort==NULL)
	{
		return ERR_BCR_OPEN;
	}

	dwLen = m_pCommPort->ReadLinkerUart(szReadBuff, END_FLAG);
	if(dwLen>0)
	{
		//memcpy(pReadBuff, szReadBuff, sizeof(szReadBuff));
    	//checksum 및 END_FLAG 검사
    	if(szReadBuff[0] == REMOTE_TYPE_IR)
    	{
    		if(szReadBuff[3] != END_FLAG)//
    			return ERR_HANDTP_READ_LENGTH;
    		// IR은 CHECK SUM 확인 안함.
    	}
    	else if(szReadBuff[0] == REMOTE_TYPE_RF)
    	{
			if(szReadBuff[16] != END_FLAG)
    			return ERR_HANDTP_READ_LENGTH;
    		unsigned char checksum_tmp=0;
    		for(int i = 0 ; i<15 ; i++)
    		{
    			checksum_tmp += szReadBuff[i];
    		}
    		if( checksum_tmp != szReadBuff[15])
    			return ERR_HANDTP_CHECKSUM;
    	}
    	else
    	{
    		return ERR_HANDTP_REMOTE_TYPE;
    	}


		//memcpy(pReadBuff, szReadBuff, sizeof(szReadBuff));
		memcpy(pReadBuff, szReadBuff, 16);
	}
	else
	{
		Error = ERR_HANDTP_NO_DATA;
	}
	return Error;
}

/**
@brief   Remocon Read 명령을 Write하는 함수
@author  ji7412.lee
@date    2013.04.02
@return  명령 Write에 문제 없으면 0, 문제 발생시 ERR_BCR_WRITE 리턴
*/
int Remocon::SendTriger(unsigned char* ackData, int Length)
{
	unsigned char strBCR[40] = {0,};
	int Error=0;

	if(m_pCommPort==NULL)
	{
		Error = ReOpenCom();
	}

	if(m_pCommPort==NULL)
	{
		return ERR_BCR_OPEN;
	}

	if(Error==NO_ERR && m_pCommPort!=NULL)
	{
		memcpy(strBCR+1, ackData, Length);
		//Check sum 생성

		for(int i=0 ; i<15 ; i++)
		{
			strBCR[16]+=ackData[i];//checksum
		}
		strBCR[0]='(';//Linker 수신 시작 바이트
		strBCR[17]=')';//Linker 수신 종료 바이트
	}

	//strlen으로 전송시 데이터 0을 전송 못함
	//Linker로 데이터 전송은 고정된 크기의 데이터를 전송 함. 18Byte
	//if(strlen(strBCR) != m_pCommPort->WriteCommBlock(strBCR, strlen(strBCR)))
	if(OHT_TO_LINKER_DATA_LENTH != m_pCommPort->WriteCommBlock(strBCR, OHT_TO_LINKER_DATA_LENTH))
	{
		Error=ERR_BCR_WRITE;
	}
	return Error;
}

//Crc Check 테이블[256]
static const WORD __CRC_TABLE[256] = {
	0x0000, 0x8005, 0x800F, 0x000A, 0x801B, 0x001E, 0x0014,
	0x8011, 0x8033, 0x0036, 0x003C, 0x8039, 0x0028, 0x802D,
	0x8027, 0x0022, 0x8063, 0x0066, 0x006C, 0x8069, 0x0078,
	0x807D, 0x8077, 0x0072, 0x0050, 0x8055, 0x805F, 0x005A,
	0x804B, 0x004E, 0x0044, 0x8041, 0x80C3, 0x00C6, 0x00CC,
	0x80C9, 0x00D8, 0x80DD, 0x80D7, 0x00D2, 0x00F0, 0x80F5,
	0x80FF, 0x00FA, 0x80EB, 0x00EE, 0x00E4, 0x80E1, 0x00A0,
	0x80A5, 0x80AF, 0x00AA, 0x80BB, 0x00BE, 0x00B4, 0x80B1,
	0x8093, 0x0096, 0x009C, 0x8099, 0x0088, 0x808D, 0x8087,
	0x0082, 0x8183, 0x0186, 0x018C, 0x8189, 0x0198, 0x819D,
	0x8197, 0x0192, 0x01B0, 0x81B5, 0x81BF, 0x01BA, 0x81AB,
	0x01AE, 0x01A4, 0x81A1, 0x01E0, 0x81E5, 0x81EF, 0x01EA,
	0x81FB, 0x01FE, 0x01F4, 0x81F1, 0x81D3, 0x01D6, 0x01DC,
	0x81D9, 0x01C8, 0x81CD, 0x81C7, 0x01C2, 0x0140, 0x8145,
	0x814F, 0x014A, 0x815B, 0x015E, 0x0154, 0x8151, 0x8173,
	0x0176, 0x017C, 0x8179, 0x0168, 0x816D, 0x8167, 0x0162,
	0x8123, 0x0126, 0x012C, 0x8129, 0x0138, 0x813D, 0x8137,
	0x0132, 0x0110, 0x8115, 0x811F, 0x011A, 0x810B, 0x010E,
	0x0104, 0x8101, 0x8303, 0x0306, 0x030C, 0x8309, 0x0318,
	0x831D, 0x8317, 0x0312, 0x0330, 0x8335, 0x833F, 0x033A,
	0x832B, 0x032E, 0x0324, 0x8321, 0x0360, 0x8365, 0x836F,
	0x036A, 0x837B, 0x037E, 0x0374, 0x8371, 0x8353, 0x0356,
	0x035C, 0x8359, 0x0348, 0x834D, 0x8347, 0x0342, 0x03C0,
	0x83C5, 0x83CF, 0x03CA, 0x83DB, 0x03DE, 0x03D4, 0x83D1,
	0x83F3, 0x03F6, 0x03FC, 0x83F9, 0x03E8, 0x83ED, 0x83E7,
	0x03E2, 0x83A3, 0x03A6, 0x03AC, 0x83A9, 0x03B8, 0x83BD,
	0x83B7, 0x03B2, 0x0390, 0x8395, 0x839F, 0x039A, 0x838B,
	0x038E, 0x0384, 0x8381, 0x0280, 0x8285, 0x828F, 0x028A,
	0x829B, 0x029E, 0x0294, 0x8291, 0x82B3, 0x02B6, 0x02BC,
	0x82B9, 0x02A8, 0x82AD, 0x82A7, 0x02A2, 0x82E3, 0x02E6,
	0x02EC, 0x82E9, 0x02F8, 0x82FD, 0x82F7, 0x02F2, 0x02D0,
	0x82D5, 0x82DF, 0x02DA, 0x82CB, 0x02CE, 0x02C4, 0x82C1,
	0x8243, 0x0246, 0x024C, 0x8249, 0x0258, 0x825D, 0x8257,
	0x0252, 0x0270, 0x8275, 0x827F, 0x027A, 0x826B, 0x026E,
	0x0264, 0x8261, 0x0220, 0x8225, 0x822F, 0x022A, 0x823B,
	0x023E, 0x0234, 0x8231, 0x8213, 0x0216, 0x021C, 0x8219,
	0x0208, 0x820D, 0x8207, 0x0202
};

//이전에 계산한 CRC 값을 추가된 데이터에 맞춰 갱신함
//이전:      ___________
//	  8005 | XX 00 00
//				yy yy ---> CRC
//
//갱신:      ______________
//	  8005 | XX ZZ 00 00
//				yy yy 00
//				   ww ww ---> CRC'
//
WORD Remocon::crc16(WORD crc, BYTE *ptr,WORD size)
{
	WORD i, j;

	for(j=0; j<size; j++)
	{
		/* 추가된 데이터 ZZ는 이전 계산시 나머지항의
		  상위 바이트와 자리수가 맞으므로 그 둘을 합하고
		  다시 그 자리까지의 나머지를 구함 */
		i = ((WORD)(crc >> 8) ^ *ptr++) & 0xff;
		crc = (crc << 8) ^ __CRC_TABLE[i];
		/* 나머지항의 하위 바이트는 뒤에 0x00이 추가되어
		   자리수가 올라가고 앞자리의 나머지와 더해짐 */
	}
	return crc;
}


