...
//	m_HWSet->m_pLamp->LampOff();//Main 종료시 LED OFF
	Sleep(100);
    SaveJigParameter("DataFiles\\OHTPARAM.ini");
...
static UINT nLongRunCnt = 0;
void __fastcall TOHTMainForm::tmUIupdateTimer(TObject *Sender)
{
	Update_UI();

	if(nLongRunCnt != m_TransControl->GetCycleCount())
	{
		nLongRunCnt = m_TransControl->GetCycleCount();
		HoistCnt->Text = nLongRunCnt;
	}
	BYTE Status = gm_OHTMainStatus->StatusCommon.MSCmdID.Info.Status;

	if((int)m_ControlTask->GetOHTError() != 0)
	{
		ADD_EXCEPT_LOG("[HOIST JIG]Error : %d",(int)m_ControlTask->GetOHTError());
		LongrunChk->Checked == false;
		bStartFlag = false;
		return;
	}
	...
	//롱런
	if(LongrunChk->Checked == true && bHoistJigSetting == true)
	{
		//처음시작일 때
		if(!bStartFlag)
		{
			switch(Status)
			{
			   case STS_INIT:
					//Open일때만 시작
					if(m_HWCommon->m_Input.HOIST_Home == ON &&
						 m_HWCommon->m_Input.HAND_Open== ON &&
						 m_HWCommon->m_Input.HAND_Close == OFF &&
						 m_HWCommon->m_Input.HAND_Right_Grip == OFF &&
						 m_HWCommon->m_Input.HAND_Left_Grip == OFF)
						 {
							bStartFlag = true;
							ADD_EXCEPT_LOG("[HOIST JIG]START");
						 }
					else
							LongrunChk->Checked = false;
			   break;
			...
			switch(Status)
			{
				...
				case STS_INIT:
					if(m_HWCommon->m_Input.HOIST_Home == ON &&
						 ...
						 m_HWCommon->m_Input.HAND_Left_Grip == OFF)
						 {
							ADD_EXCEPT_LOG("[HOIST JIG]Unload Start");
							m_ControlTask->HoistLongRun(2, edtTeaching->Text.ToIntDef(1));
						 }
				...
				case STS_LOADED:
					if(m_HWCommon->m_Input.HOIST_Home == ON &&
						 ...
						 m_HWCommon->m_Input.HAND_Left_Grip == OFF)
						 {
							ADD_EXCEPT_LOG("[HOIST JIG]unload");
							m_ControlTask->HoistLongRun(2, edtTeaching->Text.ToIntDef(1));
                         }
				break;
				//Load
				case STS_UNLOADED:
					if(m_HWCommon->m_Input.HOIST_Home == ON &&
						 m_HWCommon->m_Input.HAND_Open == OFF &&
						 m_HWCommon->m_Input.HAND_Close == ON &&
						 m_HWCommon->m_Input.HAND_Right_Grip == ON &&
						 m_HWCommon->m_Input.HAND_Left_Grip == ON)
						 {
							ADD_EXCEPT_LOG("[HOIST JIG]load");
							m_ControlTask->HoistLongRun(1, edtTeaching->Text.ToIntDef(1));
						 }
				break;

				case STS_ERROR:
					ADD_EXCEPT_LOG("[HOIST JIG]Error");
					LongrunChk->Checked = false;
					bStartFlag = false;
				break;
...
void TOHTMainForm::SaveJigParameter(char* szFilePath)
{
	...
	{
		m_lWriteIniFile->WriteInteger("HOIST", "nJigCount", nLongRunCnt);
		delete m_lWriteIniFile;
	}
}
//---------------------------------------------------------------------------
bool TOHTMainForm::LoadParameter(char* szFilePath, DEFAULT_PARAM_SET* pParamSet)
{
	...
	if(pIniFile != NULL)
	{
		...
		pIniFile->ReadInteger("HOIST", "nJigCount", 0.0, &nTmpValue);
		nLongRunCount = nTmpValue;
...
void __fastcall TOHTMainForm::HoistClick(TObject *Sender)
{

	//에러 무시처리 I/O Mask : 44, 46, 47, 48, 64
	MASK_INPUT  *pMaskOfInput   = NULL;

	pMaskOfInput = m_MaskOfHW->getMaskOfInput(44-1);
	pMaskOfInput->Value = false;
	pMaskOfInput->bUse  = true;

	pMaskOfInput = m_MaskOfHW->getMaskOfInput(46-1);
	pMaskOfInput->Value = false;
	pMaskOfInput->bUse  = true;

	pMaskOfInput = m_MaskOfHW->getMaskOfInput(47-1);
	pMaskOfInput->Value = false;
	pMaskOfInput->bUse  = true;

	pMaskOfInput = m_MaskOfHW->getMaskOfInput(48-1);
	pMaskOfInput->Value = false;
	pMaskOfInput->bUse  = true;

	pMaskOfInput = m_MaskOfHW->getMaskOfInput(64-1);
	pMaskOfInput->Value = false;
	pMaskOfInput->bUse  = true;

	//셔터 input 무시처리 : 21~28
	//셔터 오리진 무시처리 : m_HWSet->m_pShutter->bHoistJig = true;
	pMaskOfInput = m_MaskOfHW->getMaskOfInput(21-1);
	pMaskOfInput->Value = false;
	pMaskOfInput->bUse  = true;

	pMaskOfInput = m_MaskOfHW->getMaskOfInput(22-1);
	pMaskOfInput->Value = false;
	pMaskOfInput->bUse  = true;

	pMaskOfInput = m_MaskOfHW->getMaskOfInput(23-1);
	pMaskOfInput->Value = true;
	pMaskOfInput->bUse  = true;

	pMaskOfInput = m_MaskOfHW->getMaskOfInput(24-1);
	pMaskOfInput->Value = true;
	pMaskOfInput->bUse  = true;

	pMaskOfInput = m_MaskOfHW->getMaskOfInput(25-1);
	pMaskOfInput->Value = false;
	pMaskOfInput->bUse  = true;

	pMaskOfInput = m_MaskOfHW->getMaskOfInput(26-1);
	pMaskOfInput->Value = false;
	pMaskOfInput->bUse  = true;

	pMaskOfInput = m_MaskOfHW->getMaskOfInput(27-1);
	pMaskOfInput->Value = true;
	pMaskOfInput->bUse  = true;

	pMaskOfInput = m_MaskOfHW->getMaskOfInput(28-1);
	pMaskOfInput->Value = true;
	pMaskOfInput->bUse  = true;

	m_HWSet->m_pShutter->bHoistJig = true;

	//Hoist 2축 이외 0,1,3 축 무시 처리
	m_HWCommon->Set_Jig(2);
	//Task 무시 처리 변수
	m_ControlTask->bHoistJig = true;

	//Trans 무시처리, Hoist Teaching Setting
	m_TransControl->bHoistJig = true;
	m_TransControl->nHoistTeaching = edtTeaching->Text.ToInt();

	bHoistJigSetting = true;


}
//---------------------------------------------------------------------------

void __fastcall TOHTMainForm::LongrunChkClick(TObject *Sender)
{
	if(bHoistJigSetting == false)
		LongrunChk->Checked = false;
