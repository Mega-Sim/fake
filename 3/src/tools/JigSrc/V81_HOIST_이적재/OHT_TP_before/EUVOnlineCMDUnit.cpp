...
//			else if(gm_OHTStatus->IOInput->uInputCommon.POD_EUV_1 != gm_OHTStatus->IOInput->uInputCommon.POD_EUV_2)
//			{
//				Warnning_Display(COLOR_RED, "ERROR : ERR_EUV_POD_SENSOR_DIFF_STATE");
//				timerDrivingJog->Enabled = false;
//				return;
//			}
			else
...
//	if((gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_EUV) && (gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Ver == OHT_VERSION_71))
//	    panMC_Status->Color   = (gm_OHTStatus->IOInput->uInputCommon.MC_Status == ON) ? COLOR_LIME : COLOR_WHITE;
//	else
...
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    // shkim  임시 주의!!!!!  if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
