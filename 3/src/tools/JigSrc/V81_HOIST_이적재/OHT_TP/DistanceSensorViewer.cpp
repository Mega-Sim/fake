	...
	: TForm(Owner)
{
	...
	m_distanceSensorData.pDistancePoint = new TPoint[tmpMaxSize+1];
...
void TDistanceSensorViewrForm::ConvertDistanceToXY(double radius, double scale)
{
	...
	for(int i = 0; i <= m_distanceSensorData.nDistanceDataSize; i++)
	{
		...
		if(m_distanceSensorData.nSensorType == UBG_OHTDETECT_SENSOR)
		{
			// 상하 반전
			m_distanceSensorData.pDistancePoint[i].Y = height - m_distanceSensorData.pDistancePoint[i].Y;		}
		else if(m_distanceSensorData.nSensorType == UBG_OBSTACLE_SENSOR)
		{
			// 상하 좌우 반전
...
void TDistanceSensorViewrForm::DrawDetectArea(double radius, double scale)
{
    ...
    */
    ...
    double degree;
	txtMinDist->Caption = "거리: " + IntToStr(m_distanceSensorData.pDistanceData[m_distanceSensorData.indexClosestDistance]) + "mm";
...
void TDistanceSensorViewrForm::DrawBackGround(double radius, double scale)
{
	...
	for(int i=1, n=9; i<n; i++)
	{
		...
		if(m_distanceSensorData.nSensorType == UBG_OHTDETECT_SENSOR || m_distanceSensorData.nSensorType == UBG_OBSTACLE_SENSOR)
		{
			// 상하 반전
			tempY = height-tempY;
...
void TDistanceSensorViewrForm::SetCanvas(int CanvasType)
{
	...
	case DISTANCE_SENSOR_DETECT_AREA:
		paintBoxDistanceSensor->Canvas->Brush->Style = bsClear;
	...
	case DISTANCE_SENSOR_PATTERN_AREA:
