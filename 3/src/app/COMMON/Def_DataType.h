...
typedef struct TRANS_CMD_
{
	...
	BYTE SpecialEQLevel;        ///< SpecailEQ (0~9)
	...
	BYTE MaterialType;          ///< MaterialType 0x00 : none, 0x01 : RSP-150 , 0x02 : RSP-200, 0x03 : EUV-POD
...
typedef struct TEACHING_INFO_ETC_
{
	...
	UINT SpecialEQLevel : 4; ///< SpecialEQ Level
	UINT Reserved2      : 1; ///<
	UINT Reserved1      : 1; ///<
...
typedef struct USE_INFO_
{
	...
	UINT OhtDetectSound         : 1; ///< OHT 대차 센서 감지 Sound 사용 여부
	...
	UINT TransEQSound           : 1; ///< EQ PORT 이적재 Sound 사용 여부
	UINT TransSTKSound          : 1; ///< STK PORT 이적재 Sound 사용 여부
	UINT TransSTBSound          : 1; ///< STB 이적재 Sound 사용 여부
} USE_INFO;
...
@date    2013.07.22
*/
typedef struct FLAG_STATE_INFO_
{
	BYTE IsWarningState  		: 3;
	BYTE IsblackboxState       	: 1;
	BYTE EmergencyNodeMTL 		: 1;
	BYTE IsAutoTeachingVision   : 1;
	BYTE IswarningLogThreadCycleTime	: 1;
	BYTE Reserved4  			: 1;
} FLAG_STATE_INFO;
...
typedef struct MSG_STATUS_OHT2TP_
{
	...
	BYTE IsErrorIgnolState;            ///< FBWF 유무  AOHT-0001_FOUPCOVER
    ...
    BYTE IsTPPause;
    BYTE IsCIDPause;
	BYTE IsAutoTeachingFail;
	int VolatageMonitor[2];
	uDISTANCE_SENSOR_DATA DistanceSensorData;
	double dHoist_pulse;
	bool bCheckCompletePodEuv;     //OHT -> TP에 현재 POD EUV인지 아닌지 판별

	char cHID_ChargeState;
} MSG_STATUS_OHT2TP;

/**
@brief   Status(OHT->VMS) 구조체
@author  puting
@date    2012.11.19
*/
typedef struct MSG_STATUS_OHT2VMS_
{
	BYTE	 CurMode;
	BYTE 	 CurStatus;
	UINT     CurNodeID;
...
/**
@brief   Status(OHT->BLACKBOX) 구조체
@author  puting
@date    2012.11.19
*/
typedef struct MSG_STATUS_OHT2BLACKBOX_
{
	BYTE	 CurMode;
	BYTE 	 CurStatus;
	UINT     CurNodeID;
	...
	char	 Iout_sign;
	int  	 Iout;
	float    IoT_Torque_Driving_F;
	float    IoT_Torque_Driving_R;
	int      ErrorCode;
	BYTE     OHTDetectLevel;
	BYTE     OBSDetectLevel;
	//DWORD    StopWaitingTime;   //?? 무슨 의미 일까?
} MSG_STATUS_OHT2BLACKBOX;

typedef struct MSG_EVENT_BLACKBOX2OHT_
{
	BYTE 	 EventType;
    BYTE 	 EventResult;
} MSG_EVENT_BLACKBOX2OHT;
...
//	int WarningCode;        ///< warning 코드
	...
	// BMS 통합 파워 모듈용 추가 정보 sg.lee 25.06.26
	char InputStatus;				// 레귤레이터 입력 보호회로 동작 상태
	char BMSStatus;					// 통합파워모듈 상태 정보
	char BatteryStatus;				// 배터리 상태 정보
	char ExCharge;					// 외부 충전 상태
	int BMS_Edc;					// 통합파워모듈 Output Voltage (CP 후단)
	int BMS_Iout;					// 통합파워모듈 Output Current 부호
	int BMS_Iout_Sign;				// 통합파워모듈 Output Current
	int BVoltage;					// Battery Vlotage (측정)
	char BIout_Sign;				// Battery Current 부호 (측정)
	int BCurrent;					// Battery Current (측정)
	int ConverterEfficiency;		// Converter 효율
	int HeatsinkTemperature;		// Heatsink 온도
	int CTransTemperature;			// Converter Trans 온도
	int CMosFETPriTemperature;		// Converter MosFET Pri 온도 (300V 쪽, 레귤레이터)
	int CMOsFETSecTemperature;		// Converter MosFET Sec 온도 (25.3V 쪽, 배터리)
	int Soc_per;					// Battery SOC & 환산
	int BMS_Voltage;				// 배터리 전압 (BMS)
	int BMS_Current;				// 배터리 Ref 전류
	int PCBTemperature;				// BMS PCB 온도
	int StartDate;					// 배터리 사용 시작일
	char TotalTime[9];				// 배터리 사용 시간 (BMS Counter data)
	int TotalIac;					// 배터리 누적 충전 전류량
	int TotalIout;					// 배터리 누적 방전 전류량

	int CellVoltage[11];			// 배터리 셀 전압 (BMS)
...
typedef enum DISTANCE_SENSOR_TYPE_
{
	...
	UBG_OBSTACLE_SENSOR,
	UBG_OHTDETECT_SOSLAB_SENSOR,
	UBG_OBSTACLE_SOSLAB_SENSOR
...
//	//Trans Sensor Information
//	BYTE HandPushSen			:1;
//	BYTE InputHandOpen			:1;
//	BYTE InputHandClose			:1;
//	BYTE InputHandGripLeft		:1;
//	BYTE InputHandGripRight		:1;
//	BYTE InputHandInposition	:1;
//	BYTE HoistHomeSen			:1;
//	BYTE TransReserved			:1;
//
//	//Trans Type, Step, Pos
//	int TranStepDetail;
//	double TransHoistPos;
//	int TransTypeDetail;
...
typedef struct // puting
{
	...
	unsigned char 		BI;
	unsigned char 		CAT[2];
	unsigned int 		DTn[1000];   //(2Byte Distance, 2Byte pulse width) *1000
	unsigned char 		InputArea;
	unsigned char 		OutputArea;
