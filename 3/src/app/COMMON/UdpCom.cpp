...
UdpServer  			*pUdpTpServerForMapView; // mapview-tp 통신 전용
...
int UdpCom::UnicastSend(AnsiString strIpAddr, int iPort, char* buf, int len)
{
    ...
    static DWORD dwLastTimeOfUnicastSend = timeGetTime();
...
//#endif
	...
	if(PreventOverflowGetTimeMs(dwLastTimeOfUnicastSend) > 10)
	{
        ...
        dwLastTimeOfUnicastSend = timeGetTime();
	}
#if 0
...
//---------------------------------------------------------------------------
int UdpCom::UnicastSend_toMapView(AnsiString strMessage)
{

#ifdef IS_TP
	pUdpUniCastSender->UnicastSendMessage("127.0.0.1", PORT_MAPVIEW, strMessage);
#endif
...
#ifdef IS_TP
int UdpCom::TP_MAPVIEW_Bind()
{
	int iRetVal = -1;

	if (pUdpTpServerForMapView == NULL)
		pUdpTpServerForMapView = new UdpServer();



	iRetVal = pUdpTpServerForMapView->Bind(TP_FIX_PORT_NUM_FOR_MAPVIEW);


	if(iRetVal == 0)
	{
		DisplayLog(L"[TP - MAPVIEW Server 포트] Bind 동작 성공");
	}
	else if(iRetVal == -3)
	{
		DisplayLog(L"[TP - MAPVIEW Server 포트] 이미 Bind 되어 있습니다");
	}
	else
	{
		DisplayLog(L"[TP - MAPVIEW Server 포트] Bind 동작 실패");
	}
...
//	unsigned char oneChar;
	...
	if (iReturn > 0)	// 가져온 정보가 있는지 확인하자
	{
		...
		switch(Pkt.TAG.Value)	// 명령어 종류에 따른 분기
		{
			...
			{
				PKT_JOB_PROGRESS_REPORT_ID_13	CmdPkt;		// 작업지시 패킷으로 재 복사한다
				iPacketSize = sizeof(PKT_JOB_PROGRESS_REPORT_ID_13);

				// 패킷 크기 검증
				if(iReturn != iPacketSize)
				{
					strTmp.sprintf("[OHT->OCS 제어] CLW가 OCS로 진척보고 (ID 13) - 패킷의 크기가 너무 작습니다. 최소 size : %d 수신size : %d", iPacketSize, iReturn);
					DisplayLog(strTmp);
					return UNICAST_LISTEN_RESULT_PACKET_SIZE_INVALID;
				}
				...
				// 문제 없는 패킷으로 확인되면 로그에 중요 내용을 표시
				memcpy(&CmdPkt, &buf, iPacketSize);
				Conv_PKT_JOB_PROGRESS_REPORT_ID_13(CmdPkt);
				...
				strTmp.sprintf("CLW가 OCS로 진척보고   (ID 13) [HEADER] MachineType : %d MachineID : %s Tag : %d [OHT STATUS] Working : %s Running : %s Carriage : %s",
								CmdPkt.HEADER.MachineType, strChk, CmdPkt.TAG.Value,
								Char2BinString(cVar1), Char2BinString(cVar2), Char2BinString(cVar3));
				DisplayLog(strTmp);
            ...
            case TagID::RESPONSE_PASSPERMIT_CANCEL_OHT2OCS:	// OHT -> OCS 통과 취소 상태 보고 (ID:271)
			{
				PKT_RESPONSE_PASSPERMIT_CANCEL_OHT2OCS_ID_271	CmdPkt;		// 통과 허가 상태 보고 패킷으로 재 복사한다
				iPacketSize = sizeof(PKT_RESPONSE_PASSPERMIT_CANCEL_OHT2OCS_ID_271);

				// 패킷 크기 검증
				if(iReturn != iPacketSize)
				{
					strTmp.sprintf("[OHT->OCS 제어] 통과 취소 상태 보고(ID:271) - 패킷의 크기가 너무 작습니다. 최소 size : %d 수신size : %d", iPacketSize, iReturn);
					DisplayLog(strTmp);
					return UNICAST_LISTEN_RESULT_PACKET_SIZE_INVALID;
				}
				...
				// 문제 없는 패킷으로 확인되면 로그에 중요 내용을 표시
				memcpy(&CmdPkt, &buf, iPacketSize);
				Conv_PKT_RESPONSE_PASSPERMIT_CANCEL_OHT2OCS_ID_271(CmdPkt);
				...
				strTmp.sprintf("CLW가 OCS로 통과 취소 상태 보고 (ID:271) [HEADER] MachineType : %d MachineID : %s Tag : %d ",
								CmdPkt.HEADER.MachineType, strChk, CmdPkt.TAG.Value);
				DisplayLog(strTmp);
...
//		DisplayLog(strMsg);
		...
		// if(Pkt.HEADER.MachineType == MACHINETYPE_VHL)
		{
			switch(Pkt.TAG.Value)	// 명령어 종류에 따른 분기
			{
				...
				case TagID::PASSPERMIT_CANCEL_OCS2OHT:	// 통과 허가 요구 응답 (ID:270)
				{
					PKT_PASSPERMIT_CANCEL_OCS2OHT_ID_270	CmdPkt;		// 작업지시 패킷으로 재 복사한다
					iPacketSize = sizeof(PKT_PASSPERMIT_CANCEL_OCS2OHT_ID_270);

					// 패킷 크기 검증
					if(iReturn > iPacketSize)
					{
						strTmp.sprintf("[OCS->OHT 제어] 통과취소 (ID:270) - 패킷의 크기가 너무 작습니다. 최소 size : %d 수신size : %d", iPacketSize, iReturn);
						DisplayLog(strTmp);
						return UNICAST_LISTEN_RESULT_PACKET_SIZE_INVALID;
					}
					...
					// 문제 없는 패킷으로 확인되면 로그에 중요 내용을 표시
					memcpy(&CmdPkt, &buf, iPacketSize);
					Conv_PKT_PASSPERMIT_CANCEL_OCS2OHT_ID_270(CmdPkt);
					...
					strTmp.sprintf("통과 취소 (ID:270) [HEADER] MachineType : %d MachineID : %s Tag : %d [OCS STATUS] StatusOrder : %s StopOrder : %s StopReason : %d MaintananceOrder : %s DataTransOrder : %s",
									CmdPkt.HEADER.MachineType, strConv, CmdPkt.TAG.Value,
									Char2BinString(cVar1), Char2BinString(cVar2), CmdPkt.STATUS.StopReason, Char2BinString(cVar3),Char2BinString(cVar4));
					DisplayLog(strTmp);
				...
				case TagID::RESPONSE_CARRIERINFO_OCS_OHT:
                {
                    PKT_CMD_RESPONSE_CARRIERINFO_OCS_OHT_ID_1302     CmdPkt;
                    iPacketSize = sizeof(PKT_CMD_RESPONSE_CARRIERINFO_OCS_OHT_ID_1302);
					...
					// 문제 없는 패킷으로 확인되면 로그에 중요 내용을 표시
					memcpy(&CmdPkt, &buf, iPacketSize);
                    Conv_PKT_CMD_RESPONSE_CARRIERINFO_OCS_OHT_ID_1302(CmdPkt);
					...
					strTmp.sprintf("CarrierReq 응답 (ID:1302) [HEADER] MachineType : %d MachineID : %s Tag : %d [OCS STATUS] StatusOrder : %s StopOrder : %s StopReason : %d MaintananceOrder : %s DataTransOrder : %s",
									CmdPkt.HEADER.MachineType, strConv, CmdPkt.TAG.Value,
									Char2BinString(cVar1), Char2BinString(cVar2), CmdPkt.STATUS.StopReason, Char2BinString(cVar3),Char2BinString(cVar4));
					DisplayLog(strTmp);
				...
				{
					PKT_REQUEST_AUTOCHANGE_OCS_OHT_ID_251	CmdPkt;
					iPacketSize = sizeof(PKT_REQUEST_AUTOCHANGE_OCS_OHT_ID_251);
										// 패킷 크기 검증
					if(iReturn != iPacketSize)
					{
						strTmp.sprintf("[OCS->OHT 제어] 정전복구 지시 - 패킷의 크기가 너무 작습니다. 최소 size : %d 수신size : %d", iPacketSize, iReturn);
						DisplayLog(strTmp);
						return UNICAST_LISTEN_RESULT_PACKET_SIZE_INVALID;
					}
					memcpy(&CmdPkt, &buf, iPacketSize);
					Conv_PKT_REQUEST_AUTOCHANGE_OCS_OHT_ID_251(CmdPkt);
					...
					strTmp.sprintf("AutoChange 응답 (ID:1251) [HEADER] MachineType : %d MachineID : %s Tag : %d [OCS STATUS] StatusOrder : %s StopOrder : %s StopReason : %d MaintananceOrder : %s DataTransOrder : %s",
									CmdPkt.HEADER.MachineType, strConv, CmdPkt.TAG.Value,
									Char2BinString(cVar1), Char2BinString(cVar2), CmdPkt.STATUS.StopReason, Char2BinString(cVar3),Char2BinString(cVar4));
					DisplayLog(strTmp);
	...
	if (iReturn > 0)	// 가져온 정보가 있는지 확인하자
	{
		...
		if(Pkt.HEADER.MachineType == MACHINETYPE_TP)
		{
			switch(Pkt.TAG.Value)	// 명령어 종류에 따른 분기
			{
				...
				case TagID::CMD_TP_DUAL_STAGE_STB_TRANS:
				{
					strTpAddr = strClientAddr;
					strTmp.sprintf("TP Send TP_DUAL_STAGE_STB_TRANS Address : %s ",strTpAddr);
					DisplayLog(strTmp);
...
//					{ iReturn = 0; }
				...
				case TagID::CMD_WINDOW_OS_SHUTDOWN:
				{
					strTpAddr = strClientAddr;
					strTmp.sprintf("TP Send TP에서 Window OS 종료 명령을 OHT로 전송한다 Address : %s ",strTpAddr);
					DisplayLog(strTmp);
					iPacketSize = sizeof(PKT_OCS2OHT);
					memcpy(&OcsCmdPkt, &buf, iPacketSize);
					return iReturn;
				...
				case TagID::CMD_CID_SETTING_SEND:
				{
					strTpAddr = strClientAddr;
					strTmp.sprintf("TP에서 CID Setting으로 읽어달라고 요청  Address : %s ",strTpAddr);
					DisplayLog(strTmp);
					iPacketSize = sizeof(PKT_OCS2OHT);
					memcpy(&OcsCmdPkt, &buf, iPacketSize);
					return iReturn;
				...
				case TagID::CMD_TP_ERRORIGNOL_ONOFF:   // //AOHT-0001_FOUPCOVER
				{
					strTpAddr = strClientAddr;
					strTmp.sprintf("TP에서 ErrorIgnol On/OFF 요청  Address : %s ",strTpAddr);
					DisplayLog(strTmp);
					iPacketSize = sizeof(PKT_OCS2OHT);
					memcpy(&OcsCmdPkt, &buf, iPacketSize);
					return iReturn;
				...
				case TagID::CMD_TP_AUTOTEACHING_VISION:
				{
					strTpAddr = strClientAddr;
				...
				case TagID::CMD_TP_VIB_DATA_ALIVE:
				{
					strTpAddr = strClientAddr;
					strTmp.sprintf("TP Send CMD_TP_VIB_DATA_ALIVE Address : %s ",strTpAddr);
					DisplayLog(strTmp);
				...
				case TagID::CMD_TP_CHECK_MATERIAL:
				{
					strTpAddr = strClientAddr;
					strTmp.sprintf("TP에서Check Material Type Command Send  Address : %s ",strTpAddr);
					DisplayLog(strTmp);
					iPacketSize = sizeof(PKT_OCS2OHT);
					memcpy(&OcsCmdPkt, &buf, iPacketSize);
					return iReturn;
				...
				case TagID::CMD_AUTO_DRIVING_TEST_ROUTE_REQ_TP2OHT:
				{
					strTpAddr = strClientAddr;
					strTmp.sprintf("TP Send AutoMoveTest Route Request Address : %s ",strTpAddr);
					DisplayLog(strTmp);
				...
				case TagID::CMD_AUTO_DRIVING_TEST_MOVE_REQ_TP2OHT:
				{
					strTpAddr = strClientAddr;
					strTmp.sprintf("TP Send AutoMoveTest Move Request Address : %s ",strTpAddr);
					DisplayLog(strTmp);
...
#ifdef IS_TP
int UdpCom::MAPVIEW_TP_ThreadCall()
{
     AnsiString strTmp, strAnsiAddr;
	...
	iReturn = pUdpTpServerForMapView->Listen(buf, iLen, strClientAddr, iClientPort); // 이 정보가 다 필요한 것은 아니지만, 넘겨주니까..
	strAnsiAddr = strClientAddr;
	...
	if (iReturn > 0)	// 가져온 정보가 있는지 확인하자
	{
		...
		try {
			...
			//1. Header, MapView에서 온 신호인지 체크
			if(strcmp(message[0],"MapView"))
			{
            	return -1;
			}

			//2. TagID에 따라 다르게 파싱
			TagID = atoi(message[1]);
			switch(TagID)
			{
				//MAP VIEW -> TP 상태요청응답
				case TagID::CMD_MAPVIEW_TO_TP_STATUS_RESPONSE:
					{
						int nMapViewStatus = atoi(message[2]);
						SocketForm->UdpSocket_MapViewStatusRead(nMapViewStatus);
					}
					break;
				//MAP VIEW -> TP Map Version 요청응답
				case TagID::CMD_MAPVIEW_TO_TP_MAP_VERSION_RESPONSE:
					{
						SocketForm->UdpSocket_MapViewMapVersionRead(message[2]);
					}
					break;
				//MAP VIEW -> TP UI 요청응답
				case TagID::CMD_MAPVIEW_TO_TP_UI_UPDATE_RESPONSE:
					{
						int nResult = atoi(message[2]);
						SocketForm->UdpSocket_MapViewUpdateResultRead(2, nResult);
					}
					break;
...
//						PKT_OHT2TP_RESPONSE CmdPkt; 	// normal 패킷으로 재 복사한다
				...
				case TagID::CMD_CID_SETTING_SEND_RETURN:
				{
					iPacketSize = sizeof(PKT_TP2OHT_SIMPLE_COMMAND);
					...
					DisplayLog("TP로CMD_CID_SETTING_SEND_RETURN 전송");

					PKT_TP2OHT_SIMPLE_COMMAND OhtCmdPkt; 				// CLW 상태보고 패킷으로 재 복사한다
					memcpy(&OhtCmdPkt, &buf, iPacketSize);

//					SocketForm->UdpSocket_Hoist_Param_Result_Read(OhtCmdPkt);    TP에는 추후에 구현
					return iReturn;
				}
				...
				case TagID::RESPONSE_AUTO_DRIVING_TEST_ROUTE_OHT2TP:	// link fail or link disconnection
				{
					iPacketSize = sizeof(PKT_OHT2TP_AUTO_DRIVING_TEST_ROUTE_RESPONSE);
					// 패킷 크기 검증
					if(iReturn != iPacketSize)
					{
						strTmp.sprintf("패킷의 크기가 너무 작습니다. 최소 size : %d 수신size : %d", iPacketSize, iReturn);
					...
					PKT_OHT2TP_AUTO_DRIVING_TEST_ROUTE_RESPONSE OhtCmdPkt; 				// CLW 상태보고 패킷으로 재 복사한다
					memcpy(&OhtCmdPkt, &buf, iPacketSize);

					SocketForm->UdpSocket_AutoDriving_Route_Response(OhtCmdPkt);
					DisplayLog("Response AutoDrivingTest Route Response from OHT");

					return iReturn;
				}

				case TagID::RESPONSE_AUTO_DRIVING_TEST_MOVE_OHT2TP:	// link fail or link disconnection
				{
					iPacketSize = sizeof(PKT_OHT2TP_AUTO_DRIVING_TEST_MOVE_RESPONSE);
					// 패킷 크기 검증
					if(iReturn != iPacketSize)
					{
						strTmp.sprintf("패킷의 크기가 너무 작습니다. 최소 size : %d 수신size : %d", iPacketSize, iReturn);
					...
					PKT_OHT2TP_AUTO_DRIVING_TEST_MOVE_RESPONSE OhtCmdPkt; 				// CLW 상태보고 패킷으로 재 복사한다
					memcpy(&OhtCmdPkt, &buf, iPacketSize);

					SocketForm->UdpSocket_AutoDriving_Drive_Response(OhtCmdPkt);
					DisplayLog("Response AutoDrivingTest Drive Response from OHT");
