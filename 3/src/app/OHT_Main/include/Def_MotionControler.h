...
typedef struct _CMD_ANALYSIS
{
	...
	unsigned int SetLowVibration 			: 1;///< 저진동 적용 AOHC-285
	unsigned int SetLowVibrationRelease		: 1;///< 저진동 적용 AOHC-285

 #if((OHT_NAME == OHT_NAME_STD_V82)||(OHT_NAME == OHT_NAME_STD_V85S)|| (OHT_NAME == OHT_NAME_STD_V86) || (OHT_NAME == OHT_NAME_P4_MASK))
	unsigned int SetClearFault              : 1;//////<네트워크 제어기 에러 Reset 위하여 적용
 #else
...
typedef enum _MOTION_CONTROLLER_CMD_TYPE
{
	...
	CMD_TYPE_SET_VHLDETECTIOSET    				,
	CMD_TYPE_SET_LOWVIBRATION         			,
	CMD_TYPE_SET_LOWVIBRATION_RELEASE          	,
