...
#define MAX_LIST_COUNT_FOR_PRESTEER				7777
...
typedef struct NodeDrivingTestSt_
{
	int index;
	int indexNode;
	int VisitLinkCnt;
...
//	double GetSpeedDownDist(double dAccel, double dSpeed);
	...
	bool bCheckPathParam;  //AOHC-358 파라미터 적용 오류 시, 에러 발생
...
//    void* ReferFirstPath();           //미사용
	...
	bool GetIsNBrenchMapMake(MAP *pMap, UINT nPreNode, UINT nNextNode);  //puting
//	int	GetPreSteering(MAP *pMap, UINT StopNode, UINT PreSteeringNode, PATH_STEERING_INFO *pPreSteeringInfo);      //미사용
...
//	int SearchDrivingPath(MAP *pMap, UINT FromNode, UINT ToNode, EXECUTE_DRIVING_ADD_INFO AdditionalInfo, LList *DRIVING_INFO_List);	///< PathSearch 결과 Return       //미사용
	int StandardSearchPassPermit(MAP *pMap,STD_FlowControl *pFlowMap, PARSING_GO_INFO* CmdGoData, EXECUTE_DRIVING_ADD_INFO AdditionalInfo, LList *DRIVING_INFO_List, UINT &uCheckExtraNum, bool bIsFoup,double PathExtra =0.0, bool bIgnorOverrun = false,bool bNoonechek = false);	///< PathSearch 결과 Return
    int StandardSearchOnlyOnePassPermit(MAP *pMap, STD_FlowControl *pFlowMap, PARSING_GO_INFO* CmdGoData, EXECUTE_DRIVING_ADD_INFO AdditionalInfo, LList *DRIVING_INFO_List, bool bcheckTarget, PKT_CYCLECMDDATA *FinalCycleData, UINT &uCheckExtraNum,double &dTempDis, UINT FinalSeqNum, PASSPERMIT_COMMON TempPassPermitData,bool bIsFoup, bool bIgnorOverrun = false, bool bturnoff=false);	///<//AOHC-195 오버런 개선 작업(유형2) ( PathSearch 결과 Return)
	...
	bool CheckEmergencyPathInfo(MAP *pMap,UINT nCurNode);  //puting
	...
	int SearchExceptPathForV80_AutoSearch(MAP * pMap,   PROTOCOL_BODY_AUTO_DRIVING_TEST_MOVE_REQ PACKET_BODY,EXCEPT_PATH_LIST_V80 *pPath);
...
public:
	...
	LList LaneCutList;
	bool LaneCutLoad();
	bool LaneCutLoad(MAP *pMap, PROTOCOL_BODY_AUTO_DRIVING_TEST_ROUTE_REQ PACKET_BODY);

	//주행경로자동탐색 (진동측정)
	bool FindTheWay(MAP *pMap, UINT FromNode, UINT ToNode,
					EXECUTE_DRIVING_ADD_INFO AdditionalInfo, LList *DRIVING_INFO_List);
	void Fn_VisitCheck(RouteSt* pRouteSt);


	//주행테스트자동화 (노드)
//	bool m_bNodeDrivingTest;
	DWORD dwPathSearchTime;
	bool NodeDrivingTest(MAP *pMap, UINT FromNode, int nLinkSetting);                      //LaneCutList.ini 사용시
	bool NodeDrivingTest(MAP *pMap, PROTOCOL_BODY_AUTO_DRIVING_TEST_ROUTE_REQ PACKET_BODY);//TP에서 보낸 레인컷 리스트 사용시
	bool VisitNodeCheck(DrivingTestSt* pDrivingTestSt, int nLinkSetting);
	bool VisitLinkCheck(DrivingTestSt* pDrivingTestSt, int VisitLinkCnt);

	bool RouteCheckNode(MAP *pMap, PROTOCOL_BODY_AUTO_DRIVING_TEST_MOVE_REQ PACKET_BODY, EXECUTE_DRIVING_ADD_INFO AdditionalInfo, LList *DRIVING_INFO_List);
	bool bUseAVS;
