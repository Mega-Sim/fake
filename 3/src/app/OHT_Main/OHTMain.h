...
//혼용 PIO Device Type(HW 종류)
typedef enum EQPIO_SET_INFO_
{
	TYPE_PIO_IR   = 0,
	TYPE_PIO_RF   = 1,
	TYPE_PIO_HYBRID = 2
...
//Map상 PIOType 사용 정보(EQPIO Set이 Hybrid 일 경우만 사용 가능)
...
__published:	// IDE-managed Components
  //  TTimer *tmMainTask;
	...
	TButton *btnUBGData;
	TLabel *lbAMCDLLVer;
	TCheckBox *chkNodeEstop;
	TEdit *NodeEstop;
	TButton *Button1;
	TLabel *lbSOSLAB;
	TGroupBox *grpBlackBox;
	TButton *btnBlackBoxTime;
	TButton *btnSendStatus;
	TButton *btnEventSave;
	TButton *btnEventStreaming;
	...
	void __fastcall btnUBGDataClick(TObject *Sender);
	void __fastcall Button1Click(TObject *Sender);
	void __fastcall btnBlackBoxTimeClick(TObject *Sender);
	void __fastcall btnEventSaveClick(TObject *Sender);
	void __fastcall btnEventStreamingClick(TObject *Sender);
...
private:	// User declarations
	...
	LogUnit *pLogFirmware;	   /// Firmware Version Log 저장을 위한 변수 선언
...
//	SOCKET ClientInfo;
	...
	DEFAULT_LINE_PARAM_SET  *m_pLineParamSet;   ///<라인 파라미터 AOHC-285
	OHTMainStatus *m_pOHTMainStatus;
	OHTDetect		*m_pOHTDetect;
	AvsControl *m_AvsControl;
	CarrierID   *m_CarrierID;
...
//	MonitorControlThread *m_MonitorControlThread;	///< MonitorControlThread Class
	...
	void DestroyClass();
	void CaptureScreen();
	int CheckUBGInputStatus();
	int CheckUBGIOFinalStatus();
	...
	bool    LoadLineParameter(char* szFilePath, DEFAULT_LINE_PARAM_SET* pParamSet);    ///< 파라미터 불러오는 함수  //AOHC-285
	bool    LoadAutoPatchParameter (char* szFilePath, DEFAULT_AUTOPATCH_PARAM_SET* pParamSet); ///< AutoPatch Error 상위 보고
...
//	STD_MapMakerFile* GetMapMakerFile_STD();
	...
	DEFAULT_LINE_PARAM_SET* GetLineParam();

	DEFAULT_AUTOPATCH_PARAM_SET  *m_pAutoPatchParamSet; ///<AutoPatch 완료 후 Auoto 전환 중 Error 발생 시 OCS 보고 위하여
	DEFAULT_AUTOPATCH_PARAM_SET* GetAutoPatchParam();
	...
	BlackBoxThread *m_BlackBoxThread;

	VMS* GetVMS();
	VMSThread *m_VMSThread;

	bool saveAutoPatchParameter(char* szFilePath, DEFAULT_AUTOPATCH_PARAM_SET* pParamSet);
	bool saveAutoPatchParameterRecovery(char* szFilePath, DEFAULT_AUTOPATCH_PARAM_SET* pParamSet);
	int ChangeAutoPatchParam(int UseIndex, bool YesOrNo);
	...
	unsigned int m_CheckParamResult;	//AOHC-358 파라미터 적용 오류 시, 에러 발생
	unsigned int m_CheckUBGIOFinalResult;
	//AOHC-285
   	bool m_bErrHoistParamMatching;
	double m_dFinalHoistParam_Pulse;
	double m_dFinalHoistParam_User;

	bool m_bErrDrivingVibrationMatching; //AOHC-285
	bool m_checkLowvibration; //AOHC-285
	...
	//2022.12.30 sseung 버전 관리 시스템용 함수 및 인자. Main 실행시에만 한 번 동작
	void GetFirmwareVersion(); 				 //Firmware의 버전정보를 받아서 출력하는 함수
	void WriteVersionFile(int nSW, int nFW); //버전정보 DB 형식으로 출력하는 함수
	void LoadVersionFile(char* szFileName);  //DB 파일 읽어오는 함수
	void FindFileName(char* path);           //폴더에서 DB파일 개수, 이름 찾는 함수
	AnsiString strVersion_Date;
	...
	void SetUBGCheckComplete(bool set);
	bool GetUBGCheckComplete();
	bool m_bUBGIOCheckComplete;
