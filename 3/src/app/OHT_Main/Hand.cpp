...
void Hand::Init()
{
	...
	m_pHWCommon->SetBit(OUT_HAND_PIO_SEL, ON);      //V9.0 Hand PIO SEL 초기화 시 ON
#endif
...
int Hand::IO_Init()
{
	...
	nError = m_pHWCommon->SetBit(OUT_HAND_RESERVED6, OFF);
#endif
	if (nError == NO_ERR)
...
#if(OHT_NAME != OHT_NAME_P4_MASK)
		if (nError == NO_ERR)
			nError = m_pHWCommon->SetBit(OUT_HAND_RESERVED6, ON);
#endif
		if (nError)
...
// limit sensor check
	...
	{
		if(IsClose() == false)
		{    ADD_MD_LOG("SQA : ERR_HAND_LIMIT_SEN_ALL_OFF");
			 return ERR_HAND_LIMIT_SEN_ALL_OFF;
		}
	}
#endif
...
int Hand::Origin()
{
	...
	if (nError == NO_ERR)
	{
		if (IsLeftGripOn()!=IsRightGripOn())
		{
			...
			if  (IsLeftGripOn()!=IsRightGripOn())
			{
				ADD_LOG("[Hand Origin] ERR_GRIP_SEN_MISMATCH, %d(ms) wait time excess",(int)HAND_GRIPON_WAIT_TIME);
				nError = ERR_GRIP_SEN_MISMATCH;
			}
		}
	}
