...
#define  ConnectionRequest                      0x0001
#define  ConnectionReply                      	0x0002
...
typedef struct FDC_PARAM_
{
	...
	int		nFDC_Server_Connect_Try_Time;
...
private:
	...
	bool				m_bFDCUse;
	char				m_bFDCConnectIP[16];
	bool				m_bFDCServerConnect;
    DWORD				m_dwServerConnectTryTime;
...
public:
	...
	void FDC_Manager_Off();

	FDC_PARAM_SET *m_pParamSet;
	FDC_PARAM_SET *m_pTmpParamSet;

	void FDC_Server_Connection_Snd(short cmd);
	bool FDC_Server_Connection_Rcv(char *Msg);
