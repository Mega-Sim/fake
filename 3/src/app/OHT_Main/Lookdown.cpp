...
int Lookdown::OpenCom()
{
    ...
    m_nStatus = STATUS_PBS_NOT_CONNTECTED;
	if(m_pCommPort==NULL)
	{                                // (m_nComPortNum, 115200, 8 ,NOPARITY, 1);
#if((OHT_NAME == OHT_NAME_STD_V80) ||(OHT_NAME == OHT_NAME_STD_V81) || (OHT_NAME == OHT_NAME_STD_V81S)  ||(OHT_NAME == OHT_NAME_STD_V85)|| (OHT_NAME == OHT_NAME_STD_V80_RETICLE))
		m_pCommPort = new Comm_Port(m_nComPortNum, m_nBaudRate);
#else
		m_pCommPort = new Comm_Port(m_nComPortNum, 57600, 7, NOPARITY, 0);
#endif
		if(m_pCommPort==NULL)
		{
...
{
	//AOHC-288
	if ((m_nStatus == STATUS_PBS_WAIT_CODE && (OHTUTILITY::PreventOverflowGetTimeMs(dwStartTime)>=PBS_WAIT_CODE_TIME))
		|| (m_nStatus == STATUS_PBS_WAIT_LINK && (OHTUTILITY::PreventOverflowGetTimeMs(dwStartTime)>=PBS_WAIT_LINK_TIME))
		|| (m_nStatus == STATUS_PBS_WAIT_DATA && (OHTUTILITY::PreventOverflowGetTimeMs(dwStartTime)>=PBS_WAIT_DATA_TIME)))
	{
		Obs_OnCommunication();
		dwStartTime = timeGetTime();
	}
...
void Lookdown::Obs_OnCommunication()
{
	...
	ADD_LOOKDOWN_LOG("num_data: %d:", num_data);
...
void Lookdown::PBSDetect_ReadStop(void)
{
    ...
    ADD_LOOKDOWN_LOG("PBSDetect_ReadStop, CloseCom");    //AOHC-288
    CloseCom();
}
...
bool Lookdown::OnGetData(void)
{
	...
	if(NumDecodedData !=246 )
	{
		...
		strLog.sprintf("PBS : Error[Data[%d] != 246][Count=%d]",NumDecodedData, nError);
...
int Lookdown::UpdataDate(int *Buff)
{
	int nError = NO_ERR;
