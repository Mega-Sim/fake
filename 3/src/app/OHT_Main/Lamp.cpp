...
int Lamp::LampOn(int nLampStatus)
{
	...
	case SETLAMP_R_FLICKER:
		...
		nError = m_pHWCommon->SetBit(OUT_LAMP_RED_RIGHT, ON);
		if (nError == NO_ERR)
			nError = m_pHWCommon->SetBit(OUT_LAMP_RED_LEFT, ON);
#else
...
#if(OHT_NAME == OHT_NAME_P4_MASK)
		nError = m_pHWCommon->SetBit(OUT_LAMP_GREEN_RIGHT, ON);
		if (nError == NO_ERR)
			nError = m_pHWCommon->SetBit(OUT_LAMP_GREEN_LEFT, ON);
#else
...
#if(OHT_NAME == OHT_NAME_P4_MASK)
	nError = m_pHWCommon->SetBit(OUT_LAMP_RED_RIGHT, OFF);
	if (nError == NO_ERR)
		nError = m_pHWCommon->SetBit(OUT_LAMP_RED_LEFT, OFF);
	if (nError == NO_ERR)
		nError = m_pHWCommon->SetBit(OUT_LAMP_YELLOW_RIGHT, OFF);
	if (nError == NO_ERR)
		nError = m_pHWCommon->SetBit(OUT_LAMP_YELLOW_LEFT, OFF);
	if (nError == NO_ERR)
		nError = m_pHWCommon->SetBit(OUT_LAMP_GREEN_RIGHT, OFF);
	if (nError == NO_ERR)
		nError = m_pHWCommon->SetBit(OUT_LAMP_GREEN_LEFT, OFF);
#else
...
bool Lamp::IsLampOn(int nColor)
{
...
#else
	case DEF_RED_RIGHT:
		if (m_pHWCommon->m_Output.LAMP_Red_Right == ON)
			bResult = true;
		else
			bResult = false;
		break;
	case DEF_YELLOW_RIGHT:
		if (m_pHWCommon->m_Output.LAMP_Yellow_Right == ON)
			...
			bResult = false;
		break;
	case DEF_GREEN_RIGHT:
		if (m_pHWCommon->m_Output.LAMP_Green_Right == ON)
			bResult = true;
		else
			bResult = false;
		break;
	case DEF_RED_LEFT:
		if (m_pHWCommon->m_Output.LAMP_Red_Left == ON)
			bResult = true;
		else
			bResult = false;
		break;
	case DEF_YELLOW_LEFT:
		...
		break;
	case DEF_GREEN_LEFT:
		if (m_pHWCommon->m_Output.LAMP_Green_Left == ON)
			bResult = true;
		else
			bResult = false;
		break;
