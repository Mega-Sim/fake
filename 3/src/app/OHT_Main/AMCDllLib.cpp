...
#define EXP_LED_INT_WD_PWR_STS			0x01 // RO
#define EXP_DSP_STCNT					0x02 // RO
#define EXP_FPGA_STCNT					0x03 // RO
#define EXP_PC2DSP_INTCNT				0x04 // RO
#define EXP_DSP2PC_INTCNT				0x05 // RO
#define EXP_200US_INTCNT				0x06 // RO
#define EXP_1MS_INTCNT					0x07 // RO
...
#define ADD_MD_LOG(...)    WRITE_LOG_ON_THIS_FILE("D:\\log\\MonitoringDetail.log", __VA_ARGS__) //test 후 삭제 필요
...
wchar_t *strOHTMODEL[] = {L"FOUP", L"MAC", L"PROBECARD", L"PROBECARD_JIG", L"FOUP_REV01",L"UNKNOW",L"UNKNOW",L"UNKNOW",L"UNKNOW", L"UNKNOW",
						  ...
						  L"OHT_STD_V80",L"OHT_STD_V81",L"OHT_STD_V81S",L"UNKNOW",L"OHT_STD_V85",L"OHT_STD_V85S", L"OHT_STD_V82",L"OHT_STD_V86",L"OHT_P4_MASK",};
...
//AMC dll, ATLAS dll 구분
#if((OHT_NAME == OHT_NAME_STD_V82)|| (OHT_NAME == OHT_NAME_STD_V85S)||(OHT_NAME == OHT_NAME_STD_V86) || (OHT_NAME == OHT_NAME_P4_MASK))
//ATLAS dll 로딩
#ifdef  UNICODE
	LPCTSTR strTmpFileName = L"DataFiles\\atlas.dll";       //TCHAR
	_wfullpath(strTmpFullPathFileName, strTmpFileName, MAX_LENGTH_OF_FULL_PATH);
#else
	LPCTSTR strTmpFileName = "DataFiles\\atlas.dll";       //TCHAR
	_fullpath(strTmpFullPathFileName, strTmpFileName, MAX_LENGTH_OF_FULL_PATH);
#endif

#else
//AMC dll 로딩
	LPCTSTR strTmpFileName;
#ifdef  UNICODE
	if(m_defualtparam->bSOM6869 == true)
	{
		strTmpFileName = L"DataFiles\\amc6869.dll";       //TCHAR
	}
	else
	{
		strTmpFileName = L"DataFiles\\amc.dll";       //TCHAR
	}
	_wfullpath(strTmpFullPathFileName, strTmpFileName, MAX_LENGTH_OF_FULL_PATH);
#else
	if(m_defualtparam->bSOM6869 == true)
	{
		strTmpFileName = "DataFiles\\amc6869.dll";       //TCHAR
	}
	else
	{
		strTmpFileName = "DataFiles\\amc.dll";       //TCHAR
	}
	_fullpath(strTmpFullPathFileName, strTmpFileName, MAX_LENGTH_OF_FULL_PATH);
#endif
	...
	else
	{
		...
		DeviceAMCVerChk_PC	= (AMC_VERSION_CHK_PC)	GetProcAddress(hAMCLib, "version_chk_pc");
#if ((OHT_NAME == OHT_NAME_STD_V82) || (OHT_NAME == OHT_NAME_STD_V86) || (OHT_NAME == OHT_NAME_P4_MASK))
		DeviceAMCGetFWVersion = (AMC_GET_FW_VERSION) GetProcAddress(hAMCLib, "get_fw_version"); // AMOHTV80F-1703
#endif
	//==========================================================================
	...
	//==========================================================================
		...
		DeviceAMCAutoPatch = (AMC_FS_AUTOPATCH) GetProcAddress(hAMCLib, "fs_autopatch");//AOHC-154
		...
		if(m_defualtparam->bSOM6869 == true)
		{
			DeviceSerialOpen        = (OPENSERIALDEVICE)             GetProcAddress(hAMCLib, "amcms_initialize");
			DeviceSerialClose       = (CLOSESERIALDEVICE)            GetProcAddress(hAMCLib, "amcms_release");
			DeviceSerialLoadSetting = (LOADSETTINGSERIALDEVICE)      GetProcAddress(hAMCLib, "amcms_load_setting");
		}
#endif
		...
		if (!DeviceAMCOpen || !DeviceAMCClose || !DeviceAMCInit || !DeviceAMCSetPosition || !DeviceAMCGetPosition ||
			...
			!DeviceAMCVerChk || !DeviceAMCVerChk_PC || !DeviceAMCPMove || !DeviceAMCNMove || !DeviceAMCSMove || !DeviceAMCDSMove || !DeviceAMCSetEncoderOffset ||
...
#if ((OHT_NAME == OHT_NAME_STD_V82) || (OHT_NAME == OHT_NAME_STD_V86) || (OHT_NAME == OHT_NAME_P4_MASK))
			!DeviceAMCGetFWVersion || // AMOHTV80F-1703
#endif
			!DeviceAMCGetDatas || !DeviceAMCGatheringTraceData || !DeviceAMCGatheringTraceData_1 || !DeviceAMCGetPhysicalAddr)
		{
			FreeLibrary(hAMCLib);
			OHTMainForm->memoMain->Lines->Add("Device AMC False");
			LeaveCriticalSection(&m_CS);
			return NULL;
		}
#if((OHT_NAME == OHT_NAME_STD_V80) ||(OHT_NAME == OHT_NAME_STD_V81) || (OHT_NAME == OHT_NAME_STD_V81S)  ||(OHT_NAME == OHT_NAME_STD_V85)|| (OHT_NAME == OHT_NAME_STD_V80_RETICLE))
		else if((m_defualtparam->bSOM6869 == true)&&(!DeviceSerialLoadSetting || !DeviceSerialOpen || !DeviceSerialClose))
		{
			FreeLibrary(hAMCLib);
			OHTMainForm->memoMain->Lines->Add("Device Serial False");
			LeaveCriticalSection(&m_CS);
			return NULL;
		}
#endif
...
AMCLibrary::AMCLibrary()
{
	...
	{
		m_adPositiveLimit[nIndex]=(1U<<(sizeof(UINT)*8-1))-1;	// 엔코더 Max 값 설정 : AMC에서 실질적으로 int로 사용됨
...
#if((OHT_NAME == OHT_NAME_STD_V80) ||(OHT_NAME == OHT_NAME_STD_V81) || (OHT_NAME == OHT_NAME_STD_V81S)  ||(OHT_NAME == OHT_NAME_STD_V85)|| (OHT_NAME == OHT_NAME_STD_V80_RETICLE))
	if(m_defualtparam->bSOM6869 == true)
		nError = MultiSerialInit();
#endif

	// 3. 에러 확인
	if (!nError)
	{
		DeviceAMCVerChk(0, nAMCVer);
		nAMDDllVer = DeviceAMCVerChk_PC();
	...
	#if ((OHT_NAME == OHT_NAME_STD_V82) || (OHT_NAME == OHT_NAME_STD_V86) || (OHT_NAME == OHT_NAME_P4_MASK))
	// 4. RSA, Fine Slave 펌웨어 버전 확인 AMOHTV80F-1703
	int nRetValue = 0;
	int versions[1] = {0};
	char errorStr[50];
	char str[20];
	...
	{
		if (!nError){
		for (int fwNum = 0; fwNum < 6; fwNum++) // 0: RSA Driving Rear 1: RSA Driving Front 2: RSA Hoist 3: RSA Slide 4: Fine Slave Rear 5: Fine Slave Front
		{
			nRetValue = DeviceAMCGetFWVersion(fwNum, versions);
			nError = ConvertAMCError(nRetValue);
			nRSAnSlaveFWVer[fwNum] = versions[0];
			sprintf(errorStr, "Error code: %d", nError);
			AMC_ADD_LOG(errorStr);

			sprintf(str, "%d", versions[0]);
		}
		}
	}
	catch (const std::exception &e)
	{
		// 예외 처리
		AMC_ADD_LOG(e.what());
		AMC_ADD_LOG("Exception caught during GetFWVersions");
	}
	catch (...)
	{
		// 알 수 없는 예외 처리
		AMC_ADD_LOG("Unknown exception caught\n");
		AMC_ADD_LOG("Unknown exception caught during GetFWVersions");
	}
#endif
...
#else  //mgchoi. 2022.08-23. P4 MaskOHT용 InputMask Init
	memset(&m_P4MASKInput1Mask, 0x00, sizeof(m_P4MASKInput1Mask));
	memset(&m_P4MASKInput2Mask, 0x00, sizeof(m_P4MASKInput2Mask));
	memset(&m_P4MASKInput3Mask, 0x00, sizeof(m_P4MASKInput3Mask));
#endif
	InputMaskInit();
...
#if((OHT_NAME == OHT_NAME_STD_V80) ||(OHT_NAME == OHT_NAME_STD_V81) || (OHT_NAME == OHT_NAME_STD_V81S)  ||(OHT_NAME == OHT_NAME_STD_V85)|| (OHT_NAME == OHT_NAME_STD_V80_RETICLE))
	//1.5 SerialConfig File Load
	if((m_defualtparam->bSOM6869 == true)&&(nError == NO_ERR))
	{
...
//---------------------------------------------------------------------------
#if((OHT_NAME == OHT_NAME_STD_V80) ||(OHT_NAME == OHT_NAME_STD_V81) || (OHT_NAME == OHT_NAME_STD_V81S)  ||(OHT_NAME == OHT_NAME_STD_V85)|| (OHT_NAME == OHT_NAME_STD_V80_RETICLE))
int AMCLibrary::MultiSerialInit()
{
	int nError = 0;
	...
	if((nError == NO_ERR) && (!g_bSerialDriverOpen))
	{
		if(nError == NO_ERR)
		{
			...
			for(int i=0; i < 5; i++)
			{
				nRetValue = DeviceSerialOpen(5);     // Serial 에 사용할 PC interrupt No : 5.. 변경 가능...
				if(nRetValue == 0)
				{
				...
				OHTMainForm->memoMain->Lines->Add("AMC Serial DLL Open Retry...");
...
// by zzang9un 2012.12.03 : 원인을 알 수 없음
	...
	if(nError == NO_ERR)
	{
		OHTMainForm->memoMain->Lines->Add("AMC Serial Dll Load... OK!..\r\n");
	}
	else
	{
		OHTMainForm->memoMain->Lines->Add("AMC Serial Dll Load... Fail!..\r\n");
	}

   return nError;
}
//---------------------------------------------------------------------------
int AMCLibrary::SerialSettingFileLoad(void)
{
   int nError=0;
...
#if((OHT_NAME == OHT_NAME_STD_V80) ||(OHT_NAME == OHT_NAME_STD_V81) || (OHT_NAME == OHT_NAME_STD_V81S)  ||(OHT_NAME == OHT_NAME_STD_V85)|| (OHT_NAME == OHT_NAME_STD_V80_RETICLE))
   //MultiSeiral Close 구문 추가
   if((m_defualtparam->bSOM6869 == true)&&(g_bSerialDriverOpen == true))
   {
	   DeviceSerialClose();
	   g_bSerialDriverOpen = false;
   }
...
*/
int AMCLibrary::SetBit(int nIndex, bool bOnOff, bool bOBSControl, bool bUBGControl)
{
	...
	if(bOBSControl == true && bUBGControl == false) //버전별 UBG IO 고려 필요......
	{
#if((OHT_NAME == OHT_NAME_STD_V85S)||(OHT_NAME == OHT_NAME_STD_V86)||(OHT_NAME == OHT_NAME_STD_V82)||(OHT_NAME == OHT_NAME_STD_V81S)||(OHT_NAME == OHT_NAME_STD_V85) || (OHT_NAME == OHT_NAME_P4_MASK))
	   nMaskingUBG = m_anOutput[nArrayNum]&(0x01<<(nIndex%32 + 6)) | m_anOutput[nArrayNum]&(0x01<<(nIndex%32 + 7)) | m_anOutput[nArrayNum]&(0x01<<(nIndex%32 + 8)) | m_anOutput[nArrayNum]&(0x01<<(nIndex%32 + 9));
#else
	   nMaskingUBG = m_anOutput[nArrayNum]&(0x01<<(nIndex%32 + 5)) | m_anOutput[nArrayNum]&(0x01<<(nIndex%32 + 6)) | m_anOutput[nArrayNum]&(0x01<<(nIndex%32 + 7)) | m_anOutput[nArrayNum]&(0x01<<(nIndex%32 + 8));
#endif
	   m_anOutput_CMD[nArrayNum] = m_anOutput_CMD[nArrayNum]|nMaskingUBG;
	   AMC_ADD_LOG("bOBSControl : %d, bUBGControl : %d", bOBSControl, bUBGControl);
	}
...
//	AMC_ADD_LOG("AMC SetBit Error:%d %d %d// %d", bOnOff,nIndex,bOBSControl,m_anOutput_CMD[nArrayNum]);
...
int AMCLibrary::SetAMPEnable(int nAxisNum, bool bValue)
{
	...
	AMC_ADD_LOG("DeviceAMCSetAmpEnable_Check : %d",bValue);
...
@return  Error Code
*/
int AMCLibrary::SetForceAMPEnable(int nAxisNum, bool bValue)
{
	int nTmpError = 0;

	nTmpError = ConvertAMCError(DeviceAMCSetAmpEnable(nAxisNum, bValue));

	AMC_ADD_LOG("DeviceAMCSetAmpEnable_Check_Force : %d",bValue);

	return nTmpError;
}


int AMCLibrary::GetMoveDSAxisNum()
{
	AMC_ADD_LOG("GetMoveDsAxisNum : %d",m_nAxisNum);
	return m_nAxisNum;
}
...
int AMCLibrary::MoveDistance(int nAxisNum, double dAccel_Up, double dAccel_Down, double dVel_CtsPerMsec, double dDistance_Cts, TRIGGER_CTRL_INFO TriggerCtrlInfo)
{
	...
	static double dCheckPulse2 = 0.0;
	static bool bCheckPulse2 = false;
	...
	if(nTmpError==NO_ERR)
	{
		...
		m_nAxisNum = nAxisNum; // 마지막 축 명령 정보 갱신
		...
		//제어기 동일 명령에 대한 이슈 방어 코드
		if(nAxisNum == 0)
		{
			if(dCheckPulse2 == dDistance_Cts)
			{
				if(bCheckPulse2 == true)
				{
					QueuingAmcCmd.DataUnion.CmdAxis.Distance = dDistance_Cts+1;
...
int AMCLibrary::SetSoundCommand(int nSound)
{
	int nTmpError = 0;

	if(m_bAMCReady == false) return ERR_AMC_NOTOPEN;

	if(m_nSoundCmd!= nSound)
	 AMC_ADD_LOG("SetSoundCommand m_SndCmd.m_nSound %d, m_nSound :%d",m_nSoundCmd, nSound );
...
#else  //mgchoi. 2022.08.23. P4 MaskOHT용 InputMask Init
	memcpy(&Mask[0], &m_P4MASKInput1Mask, sizeof(m_P4MASKInput1Mask));
	memcpy(&Mask[1], &m_P4MASKInput2Mask, sizeof(m_P4MASKInput2Mask));
	memcpy(&Mask[2], &m_P4MASKInput3Mask, sizeof(m_P4MASKInput3Mask));
#endif
...
//      m_abAmpEnabled[nAxis]               =   (bool)RcvData.Source[nAxis]&0x0800; 			///< AMP Enable 상태인지
		...
		if(m_abIsRunning[nAxis] == true || fabs(m_adCurrentVelocity[nAxis]) >= 1)
		{
			//이동 누적값을 구함
...
//		 ||(PreOutput[n] != RcvData.Output[n]))  //로그 용량 확보로 임시 적용
		 {
			bCheckAMCLog = true;
		 }
	...
	nAMCLowVibraionStatus= RcvData.TargetPos[2];  //AOHC-285
	...
	// RcvData 는 해당 메소드 내부에서만 갱신되므로, 변경 비교하는 부분이 CriticalSection 밖에 있어도 상관 X
	CompareNodeCheckData(STRINGIFY(m_NodeCheckData), m_NodeCheckData, RcvData.NodeCheckData);
	...
	if(memcmp(&PreRcvData, &RcvData, sizeof(AMC_DATA))!=0)  // 이전 값과 다른 경우만 로그를 남긴다.
	{
		...
		if((OHTUTILITY::PreventOverflowGetTimeMs(dwAMCTrigerTime) > 100) || (bCheckAMCLog ==true))
		{
			...
			sprintf(orgLog, "/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d"
				, AmpEnable
				...
				, RcvData.PauseState
				, RcvData.Source[0], RcvData.Source[1], RcvData.Source[2], RcvData.Source[3], RcvData.State[0],RcvData.State[1], RcvData.State[2],RcvData.State[3]
				, RcvData.AxisFlag1[1]
			);
			// 로그 91진법으로 변환
			NumericSystem::EncodingLog(orgLog, encLog, sizeof(encLog));
...
/**
@brief   저진동 설정 요청하는 함수 //AOHC-285
@author  ehoto
@date    2012-11-28
*/
void AMCLibrary::SetLowVibration(void)
{
	int nTmpError = 0;
	QUEUEING_AMC_CMD QueuingAmcCmd;
	memset(&QueuingAmcCmd, 0, sizeof(QUEUEING_AMC_CMD));
	QueuingAmcCmd.CmdAnalysisUnion.CmdAnalysis.SetLowVibration = true;
//	AMC_ADD_LOG("SetLowVibration");
	m_AMCDllLibQueue.putData(QueuingAmcCmd , &nTmpError);
}
/**
@brief   저진동 해제  요청하는 함수   //AOHC-285
@author  ehoto
@date    2012-11-28
*/
void AMCLibrary::SetLowVibrationRelease(void)
{

	int nTmpError = 0;
	QUEUEING_AMC_CMD QueuingAmcCmd;
	memset(&QueuingAmcCmd, 0, sizeof(QUEUEING_AMC_CMD));
	QueuingAmcCmd.CmdAnalysisUnion.CmdAnalysis.SetLowVibrationRelease = true;
//	AMC_ADD_LOG("SetLowVibrationRelease");
	m_AMCDllLibQueue.putData(QueuingAmcCmd , &nTmpError);
}
...
*/
#if((OHT_NAME == OHT_NAME_STD_V82)|| (OHT_NAME == OHT_NAME_STD_V85S)||(OHT_NAME == OHT_NAME_STD_V86)||(OHT_NAME == OHT_NAME_P4_MASK))
void AMCLibrary::SetClearFault(bool bClear)
{
	int nTmpError = 0;
	QUEUEING_AMC_CMD QueuingAmcCmd;
	memset(&QueuingAmcCmd, 0, sizeof(QUEUEING_AMC_CMD));
	QueuingAmcCmd.CmdAnalysisUnion.CmdAnalysis.SetClearFault = bClear;
	AMC_ADD_LOG("SetClearFault");
	m_AMCDllLibQueue.putData(QueuingAmcCmd , &nTmpError);
}
#endif
/**
...
int AMCLibrary::GetNodeCountInfo(NODE_CHECK_DATA &NodeCheckData)
{
	...
	// m_NodeCheckData 는 다른 위치에서 갱신되므로, 변경 비교하는 부분이 CriticalSection 안에 있어야 함
	CompareNodeCheckData(STRINGIFY(NodeCheckData), NodeCheckData, m_NodeCheckData);
...
@return  State
*/
int AMCLibrary::GetLowVibraionStatusByAMC()
{
	if(m_bAMCReady == false) return ERR_AMC_NOTOPEN;

	 return nAMCLowVibraionStatus;
}
...
void AMCLibrary::InputMaskInit(void)
{
	...
	m_Input2Mask_R01.HOIST_Belt_Detect2		 = 1;    //AOHC-286
 
	m_Input2Mask_R01.TP_Enable    			 = 0;    //AOHC-286
	...
	m_Input2Mask_R01.HOIST_Belt_Detect1      = 1;    //AOHC-286
	m_Input2Mask_R01.PIO_Input3        		 = 0;    //AOHC-286
	m_Input2Mask_R01.PIO_Input5        		 = 0;    //AOHC-286
	m_Input2Mask_R01.PIO_Input6              = 0;    //AOHC-286
	...
	m_Input3Mask_R01.CID_NoRegistration      = 0;
	m_Input3Mask_R01.CID_Reserved_IN2        = 0;
	m_Input3Mask_R01.Rotate_0_Sensor   		 = 0;     	//AOHC-286
	m_Input3Mask_R01.Rotate_90_Sensor    	 = 0;     	//AOHC-286
	m_Input3Mask_R01.Rotate_180_Sensor    	 = 0;      //AOHC-286
	m_Input3Mask_R01.Rotate_270_Sensor   	 = 0; 		//AOHC-286

	m_Input3Mask_R01.Reverved_74    		= 0;        //AOHC-286
	m_Input3Mask_R01.Reverved_75    		= 0;        //AOHC-286
	m_Input3Mask_R01.Reverved_76          	= 0;        //AOHC-286
	m_Input3Mask_R01.Reverved_77          	= 0;        //AOHC-286
	m_Input3Mask_R01.Reverved_78       		= 0;        //AOHC-286
	m_Input3Mask_R01.Reverved_79            = 0;        //AOHC-286
	...
	m_Input3Mask_R01.BlackBox_Error    		= 1;        //AOHC-286
	m_Input3Mask_R01.Reverved_75    		= 0;        //AOHC-286
	m_Input3Mask_R01.Reverved_76          	= 0;        //AOHC-286
	m_Input3Mask_R01.Reverved_77          	= 0;        //AOHC-286
	m_Input3Mask_R01.Reverved_78       		= 0;        //AOHC-286
	...
	m_Input2Mask_R01.Reserved_40         	= 1;    //AOHC-286

	m_Input2Mask_R01.Reserved_41        	= 1;    //AOHC-286
	m_Input2Mask_R01.Reserved_42              = 1;    //AOHC-286
	m_Input2Mask_R01.FOUP_Detect             = 1;
	m_Input2Mask_R01.OSCILLATION             = 0;
	m_Input2Mask_R01.EMO_Press               = 1;
	m_Input2Mask_R01.BUMPER_Press            = 0;
	...
	m_Input2Mask_R01.Reserved_49    		 = 0;    //AOHC-286
	m_Input2Mask_R01.HANDTP_Connect          = 0;
	m_Input2Mask_R01.Reserved_51			 = 1;    //AOHC-286
	m_Input2Mask_R01.STOP_Driving_Front      = 1;
	m_Input2Mask_R01.STOP_Driving_PreCheck   = 1;
	m_Input2Mask_R01.STOP_Trans_Front        = 1;
	m_Input2Mask_R01.STOP_Trans_Rear         = 1;
	...
	m_Input3Mask_R01.Reserved_75    		= 0;        //AOHC-286
	m_Input3Mask_R01.Reserved_76    		= 0;        //AOHC-286
	m_Input3Mask_R01.Reserved_77          	= 0;        //AOHC-286
	m_Input3Mask_R01.Reserved_78          	= 0;        //AOHC-286
	m_Input3Mask_R01.Reserved_79       		= 0;        //AOHC-286
	...
	m_Input1Mask_R01.HAND_Open				= 0;
	m_Input1Mask_R01.HAND_Push				= 0;
	m_Input1Mask_R01.Reserved_7  			= 1;    //AOHC-286
	...
	m_Input2Mask_R01.BUMPER_Press			= 0;
	m_Input2Mask_R01.FAN_Fail				= 1;
	m_Input2Mask_R01.Reserved_48 			= 1;  //AOHC-286
	...
	m_Input3Mask_R01.POD_EUV_2				= 1;    //AOHC-286
	m_Input3Mask_R01.Reserved_79			= 0;    //AOHC-286
...
m_Input1Mask_R01.HAND_Open				= 0;
m_Input1Mask_R01.HAND_Push				= 0;
m_Input1Mask_R01.HNAD_Reserved_1		= 0;
m_Input1Mask_R01.HNAD_ES  				= 0;
...
m_Input1Mask_R01.OBSTACLE_Input_Extend	= 0;
m_Input1Mask_R01.OBSTACLE_Long			= 0;
m_Input1Mask_R01.OBSTACLE_Middle 		= 0;

// ========== Input 2 설정 ==========
// 0 - 15
m_Input2Mask_R01.OBSTACLE_Short			= 0;
m_Input2Mask_R01.OBSTACLE_Fail			= 0;
m_Input2Mask_R01.OHTDETECT_Input_Extend	= 0;
m_Input2Mask_R01.OHTDETECT_Long			= 0;
m_Input2Mask_R01.OHTDETECT_Middle		= 0;
m_Input2Mask_R01.OHTDETECT_Short 		= 0;
m_Input2Mask_R01.OHTDETECT_Fail			= 0;

m_Input2Mask_R01.LOOKDOWN_Detect 		= 0;
m_Input2Mask_R01.LOOKDOWN_Error			= 0;
m_Input2Mask_R01.SHIFT_Home             = 0;
m_Input2Mask_R01.STB_Detect_Left 		= 1;
m_Input2Mask_R01.STB_Detect_Right		= 1;
m_Input2Mask_R01.UTB_Detect				= 1;
m_Input2Mask_R01.FOUP_Detect 			= 0;
m_Input2Mask_R01.OSCILLATION 			= 1;

m_Input2Mask_R01.EMO_Press				= 0;
m_Input2Mask_R01.BUMPER_Press			= 0;
m_Input2Mask_R01.FAN_Fail				= 1;
m_Input2Mask_R01.HOIST_Belt_Detect2 	= 0;

m_Input2Mask_R01.Reserved_51 			= 0;
m_Input2Mask_R01.HANDTP_Connect			= 0;
m_Input2Mask_R01.POD_EUV_1				= 1;
m_Input2Mask_R01.STOP_Driving_Front		= 1;
...
m_Input2Mask_R01.BCR_Trans_OK			= 0;
m_Input2Mask_R01.BCR_Trans_NG			= 0;
m_Input2Mask_R01.HOIST_Belt_Detect1		= 0;
m_Input2Mask_R01.PIO_Input3				= 0;

// ========= Input 3 설정 ==========
// 0 - 15

m_Input3Mask_R01.PIO_Input5				= 0;
m_Input3Mask_R01.PIO_Input6				= 0;
m_Input3Mask_R01.SMPS_Short             = 0;
m_Input3Mask_R01.CID_Link_Compt_IN		= 0;
m_Input3Mask_R01.CID_Interlock_IN		= 0;
m_Input3Mask_R01.CID_Completion_ACK		= 0;
m_Input3Mask_R01.CID_Status_IN			= 0;
...
m_Input3Mask_R01.CID_Reserved_IN2		= 0;
m_Input3Mask_R01.HOIST_Home             = 1;
m_Input3Mask_R01.Blackbox_DI2           = 0;
m_Input3Mask_R01.Blackbox_DI1           = 0;
m_Input3Mask_R01.Blackbox_DI0           = 0;
m_Input3Mask_R01.Rotate_0_Sensor		= 0;
m_Input3Mask_R01.Rotate_90_Sensor		= 0;
...
m_Input3Mask_R01.Blackbox_DI1           = 0;
m_Input3Mask_R01.Blackbox_DI0           = 0;
m_Input3Mask_R01.SPARE_1				= 0;
m_Input3Mask_R01.SPARE_2				= 0;
m_Input3Mask_R01.SPARE_3				= 0;

m_Input3Mask_R01.SPARE_4				= 0;
m_Input3Mask_R01.SPARE_5				= 0;
m_Input3Mask_R01.SPARE_6				= 0;
m_Input3Mask_R01.SPARE_7				= 0;
m_Input3Mask_R01.SPARE_8				= 0;
...
m_Input2Mask_R01.UTB_Detect				= 1;
#else
m_Input2Mask_R01.UTB_Detect				= 0;  //MAC에서는 UTB 센서 미사용
#endif
m_Input2Mask_R01.FOUP_Detect 			= 0;
m_Input2Mask_R01.OSCILLATION 			= 1;
...
m_Input2Mask_R01.FOUP_Cover_Open_Detect	= 1;
#else
m_Input2Mask_R01.FOUP_Cover_Open_Detect	= 0;  //MAC에서는 Foup Cover 센서 미사용
#endif
m_Input2Mask_R01.STOP_Driving_Front		= 1;
...
#elif(OHT_NAME == OHT_NAME_P4_MASK)	// mgchoi. 2022.08.23. P4 MaskOHT용 InputMask Init

	/// Input1 설정
	m_P4MASKInput1Mask.HAND_Flange_Check		= 0;
	m_P4MASKInput1Mask.HAND_Alarm 				= 0;
	m_P4MASKInput1Mask.HAND_Right_Grip 			= 1;
	m_P4MASKInput1Mask.HAND_Left_Grip 			= 1;
	m_P4MASKInput1Mask.HAND_Work 				= 1;
	m_P4MASKInput1Mask.HAND_Home 				= 1;
	m_P4MASKInput1Mask.HAND_LoadPosition 		= 1;
	m_P4MASKInput1Mask.HAND_UnloadPosition 		= 1;
	m_P4MASKInput1Mask.HAND_ES 					= 0;
	m_P4MASKInput1Mask.HAND_PIO_Go				= 0;           // 10

	m_P4MASKInput1Mask.EQPIO_Load_REQ           = 0;
	m_P4MASKInput1Mask.EQPIO_Unload_REQ 		= 0;
	m_P4MASKInput1Mask.EQPIO_Ready 				= 0;
	m_P4MASKInput1Mask.EQPIO_HO_AVBL 			= 0;
	m_P4MASKInput1Mask.EQPIO_ES 				= 0;
	m_P4MASKInput1Mask.EQPIO_Right_Go			= 0;
	m_P4MASKInput1Mask.EQPIO_Left_Go 			= 0;

	m_P4MASKInput1Mask.STEER_F_Left 			= 0;
	m_P4MASKInput1Mask.STEER_R_Left 			= 0;
	m_P4MASKInput1Mask.STEER_F_Right 			= 0;           // 20
	m_P4MASKInput1Mask.STEER_R_Right 			= 0;

	m_P4MASKInput1Mask.SHUTTER_F_Close 			= 0;
	m_P4MASKInput1Mask.ANTIDROP_POD_Placement1 	= 0;
	m_P4MASKInput1Mask.SHUTTER_F_Open 			= 0;
	m_P4MASKInput1Mask.ANTIDROP_POD_Placement2 	= 0;
	m_P4MASKInput1Mask.SHUTTER_F_Alarm 			= 0;
	m_P4MASKInput1Mask.Reserved_27 				= 0;
	m_P4MASKInput1Mask.SHUTTER_F_Inposition 	= 0;
	m_P4MASKInput1Mask.Reserved_29 				= 0;
	m_P4MASKInput1Mask.Reserved_30 				= 0;           // 30

	m_P4MASKInput1Mask.OBSTACLE_Long 			= 0;
	m_P4MASKInput1Mask.OBSTACLE_Middle 			= 0;

	// Input2 설정
	m_P4MASKInput2Mask.OBSTACLE_Short 			= 0;
	m_P4MASKInput2Mask.OBSTACLE_Fail 			= 0;
	m_P4MASKInput2Mask.Reserved_35				= 0;
	m_P4MASKInput2Mask.OHTDETECT_Long 			= 0;
	m_P4MASKInput2Mask.OHTDETECT_Middle 		= 0;
	m_P4MASKInput2Mask.OHTDETECT_Short 			= 0;
	m_P4MASKInput2Mask.OHTDETECT_Fail 			= 0;
	m_P4MASKInput2Mask.LOOKDOWN_Detect 			= 0;           // 40
	m_P4MASKInput2Mask.LOOKDOWN_Error 			= 0;

	m_P4MASKInput2Mask.FFU_Fail                 = 0;
	m_P4MASKInput2Mask.Reserved_43  			= 0;
	m_P4MASKInput2Mask.Reserved_44  			= 0;
	m_P4MASKInput2Mask.UTB_Detect               = 1;
	m_P4MASKInput2Mask.EUV_POD_Detect           = 0;
	m_P4MASKInput2Mask.OSCILLATION              = 1;
	m_P4MASKInput2Mask.EMO_Press                = 0;

	m_P4MASKInput2Mask.BUMPER_Press             = 0;
	m_P4MASKInput2Mask.FFU_Fail_2               = 0;           // 50
	m_P4MASKInput2Mask.SLIDE_LIMIT_NEG          = 1;
	m_P4MASKInput2Mask.Reserved_52              = 0;
	m_P4MASKInput2Mask.Reserved_53              = 0;
	m_P4MASKInput2Mask.RSP200_Detect            = 1;
	m_P4MASKInput2Mask.STOP_Driving_Front       = 1;
	m_P4MASKInput2Mask.STOP_Driving_PreCheck    = 1;

	m_P4MASKInput2Mask.STOP_Trans_Front         = 1;
	m_P4MASKInput2Mask.STOP_Trans_Rear          = 1;
	m_P4MASKInput2Mask.BCR_Driving_OK           = 0;
	m_P4MASKInput2Mask.BCR_Driving_NG           = 0;           // 60
	m_P4MASKInput2Mask.BCR_Trans_OK             = 0;
	m_P4MASKInput2Mask.BCR_Trans_NG             = 0;
	m_P4MASKInput2Mask.SLIDE_LIMIT_POS          = 1;
	m_P4MASKInput2Mask.Reserved_64              = 0;

	// Input3 설정
	m_P4MASKInput3Mask.Reserved_65              = 0;
	m_P4MASKInput3Mask.Reserved_66              = 0;
	m_P4MASKInput3Mask.SMPS_Short               = 0;
	m_P4MASKInput3Mask.HOIST_Home               = 1;
	m_P4MASKInput3Mask.HOIST_EUV_POD_Home       = 1;
	m_P4MASKInput3Mask.HOIST_RSP200_Home        = 1;           // 70
	m_P4MASKInput3Mask.HOIST_RSP150_Home        = 1;
	m_P4MASKInput3Mask.Reserved_72              = 0;

	m_P4MASKInput3Mask.Reserved_73              = 0;
	m_P4MASKInput3Mask.SHIFT_Home               = 0;
	m_P4MASKInput3Mask.Reserved_75              = 0;
	m_P4MASKInput3Mask.Reserved_76              = 0;
	m_P4MASKInput3Mask.Reserved_77              = 0;
	m_P4MASKInput3Mask.POD_Detect               = 1;
	m_P4MASKInput3Mask.RSP150_Detect            = 1;
	m_P4MASKInput3Mask.ANTIDROP_POD_Placement3  = 0;
	m_P4MASKInput3Mask.ANTIDROP_POD_Placement4  = 0;

	m_P4MASKInput3Mask.CID_Link_Compt_IN		= 0;
	m_P4MASKInput3Mask.CID_Interlock_IN		= 0;
	m_P4MASKInput3Mask.CID_Completion_ACK		= 0;
	m_P4MASKInput3Mask.CID_Status_IN			= 0;
	m_P4MASKInput3Mask.CID_NoRegistration		= 0;
	m_P4MASKInput3Mask.CID_Reserved_IN2		= 0;
...
/**
@brief   AMC의 DLL버전을 얻어오는 함수
@author  zzang9un
@date    2013.10.07
@return  AMC의 버전 정보(int)를 리턴
*/
int AMCLibrary::GetAMCDLLVersion()
{
	return nAMDDllVer;
}
//---------------------------------------------------------------------------
...
int AMCLibrary::GetRSAnSlaveVersion(int num)
{
	if(num == 0)
		return nRSAnSlaveFWVer[RSADrivingRear];
	else if(num == 1)	
		return nRSAnSlaveFWVer[RSADrivingFront];
	else if(num == 2)	
		return nRSAnSlaveFWVer[RSAHoist];
	else if(num == 3)	
		return nRSAnSlaveFWVer[RSASlide];
	else if(num == 4)	
		return nRSAnSlaveFWVer[FineSlaveRear];
	else if(num == 5)	
		return nRSAnSlaveFWVer[FineSlaveFront];
	else
		return 0;
}
...
/**
//AOHC-154
@brief   AMC의 File AutoPatch 함수 
@author  kds
@date    2020.07.02
*/

int AMCLibrary::AMCFileAutoUpdate(char *pszBootLoader,  HWND hWnd)
{
	int nTmpError = 0, nAMCError = 0;
	if(m_bAMCReady == false) return ERR_AMC_NOTOPEN;
...
#if((OHT_NAME == OHT_NAME_STD_V80) ||(OHT_NAME == OHT_NAME_STD_V81) || (OHT_NAME == OHT_NAME_STD_V81S)  ||(OHT_NAME == OHT_NAME_STD_V85)|| (OHT_NAME == OHT_NAME_STD_V80_RETICLE))
//	char *pszpath = NULL;
//	nAMCError = DeviceAMCAutoPatch(pszpath, pszBootLoader, hWnd);

	nAMCError = DeviceAMCAutoPatch(pszBootLoader, hWnd);
#else
	char *pszpath = NULL;
	nAMCError = DeviceAMCAutoPatch(pszpath, pszBootLoader, hWnd);
#endif
	...
	AMC_ADD_LOG("nAMCError:%d", nAMCError);
	return nTmpError;
}
//---------------------------------------------------------------------------
...
/**
 ...
 @see     AMOHTV70S-236
*/
void AMCLibrary::CompareNodeCheckData(AnsiString name, NODE_CHECK_DATA &data1, NODE_CHECK_DATA &data2)
{
	AnsiString strTmp = "";
	OHTUTILITY::CompareVariables(strTmp.sprintf("%s (NodeCntPreChk)", name), data1.NodeCountOfPreCheck, data2.NodeCountOfPreCheck);
	OHTUTILITY::CompareVariables(strTmp.sprintf("%s (NodeCnt)", name), data1.NodeCount, data2.NodeCount);
	OHTUTILITY::CompareVariables(strTmp.sprintf("%s (PosDrvTagPreChk)", name), data1.PositionOfDrivingTagPreCheck, data2.PositionOfDrivingTagPreCheck);
	OHTUTILITY::CompareVariables(strTmp.sprintf("%s (PosDrvTag)", name), data1.PositionOfDrivingTag, data2.PositionOfDrivingTag);
	OHTUTILITY::CompareVariables(strTmp.sprintf("%s (PosTransTagFirst)", name), data1.PositionOfTransTagFirst, data2.PositionOfTransTagFirst);
	OHTUTILITY::CompareVariables(strTmp.sprintf("%s (PosTransTagFinal)", name), data1.PositionOfTransTagFinal, data2.PositionOfTransTagFinal);
}
...
bool AMCLibrary ::InterruptCntCheck()
{
	WORD w200usCnt = 0x0;

	if(FPGARegRead(EXP_200US_INTCNT, READ_RETRY_CNT, w200usCnt))
	{
		if(((int) w200usCnt >= 0) && ((int) w200usCnt <= 65535))
		{
			if(wpre_200usCnt == w200usCnt)
			{
				return true;
			}
			wpre_200usCnt = w200usCnt;
		}
	}
