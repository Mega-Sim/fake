...
{
	int Error = NO_ERR;
	if(PortType == PORT_TYPE_N2STB_L)
	{
		PortType = PORT_TYPE_STB_L;
		ADD_LOG("Save port Type Change : N2STB_L -> STB_L");
	}
	else if(PortType == PORT_TYPE_N2STB_R)
	{
		PortType = PORT_TYPE_STB_R;
		ADD_LOG("Save port Type Change : N2STB_R -> STB_R");
	}
...
// Update할 Teaching Data를 Find한다.
	...
	if(Error == NO_ERR)
	{
		...
		else
		{
			...
			if(PortType == PORT_TYPE_STB_L|| PortType == PORT_TYPE_N2STB_L)
			{
			...
			else if(PortType == PORT_TYPE_STB_R|| PortType == PORT_TYPE_N2STB_R)
			{
...
int AutoTeaching::ATSave_Current_Pos(UINT CurStationID, BYTE PortType, double dDTeaching, double dHTeacing, double dSTeacing)
{
	...
	if(Error == NO_ERR)
	{
		if (teachingData == NULL)
		{
				...
				teachingData->SpecialEQLevel = 0; //Default  //AOHC-294
				...
				if(Error == NO_ERR)
				{
					if (((double)(teachingData->Shift / 10) > SHIFT_TEACHINGLIMIT_P) ||
						((double)(teachingData->Shift / 10) < SHIFT_TEACHINGLIMIT_N))
					{
						Error = ERR_TDATA_DUPLICATION;
						ADD_LOG("[Data][Teaching][ERROR] StationID:%06d, PortType:%02X, Shift:%d()", teachingData->StationID, teachingData->PortType, teachingData->Shift);
					}
				}
...
bool AutoTeaching::QRDataCheck(BYTE PortType, int nTargetID)
{
	...
	if(PortType == PORT_TYPE_STB_L || PortType == PORT_TYPE_N2STB_L)
	{
		...
		ADD_LOG("%d %d %d",PortType,m_pHWSet->m_pLeftQRR->GetStaticQRID(),nTargetID);
	...
	else if(PortType == PORT_TYPE_STB_R|| PortType == PORT_TYPE_N2STB_R)
	{
		...
		ADD_LOG("%d %d %d",PortType,m_pHWSet->m_pRightQRR->GetStaticQRID(),nTargetID);
...
int AutoTeaching::CheckSTBShiftLimitError(int nPortType,double Shift)
{
	...
	if(nPortType == PORT_TYPE_STB_R || nPortType == PORT_TYPE_N2STB_R)
	{
		if(Shift > SHIFT_TEACHINGLIMIT_P)
		{
	...
	else if(nPortType == PORT_TYPE_STB_L|| nPortType == PORT_TYPE_N2STB_L)
	{
		if(Shift< SHIFT_TEACHINGLIMIT_N)
		{
...
int AutoTeaching::CheckSTBHoistLimitError(int nPortType, double Hoist)
{
	...
	if((nPortType == PORT_TYPE_STB_R) || (nPortType == PORT_TYPE_STB_L)||
		(nPortType == PORT_TYPE_N2STB_L) ||(nPortType == PORT_TYPE_N2STB_R))
	{
...
int AutoTeaching::Save_STBTeachingData(UINT CurStationID, BYTE PortType, double dDTeaching, double dSTeaching, double dHTeaching)
{
	int nError = NO_ERR;
	...
	if(m_defualtparam->bUseVisionTeaching == true)
	{
		...
		if(nError == NO_ERR)
		{
			...
			if (teachingData == NULL)
			{
				...
				teachingData->Hoist = (dHTeaching + TempHoistOffset) * 10 + (int)m_pExecuteInfo->TransDifference.dHoist;  //현재 위치
				teachingData->Shift = (dSTeaching + TempShiftOffset) * 10 +	(int)m_pExecuteInfo->TransDifference.dShift;  //현재 위치
				teachingData->Rotate = 0;
				...
				if(nError == NO_ERR)
				{
					nError = CheckSTBShiftLimitError(PortType,(double)(teachingData->Shift/10));
				}
				if(nError == NO_ERR)
				{
					nError = CheckSTBHoistLimitError(PortType,(double)(teachingData->Hoist/10));
				}
				...
				if (nError == NO_ERR)
				{
					...
					// by zzang9un 2013.07.23 : 로그 생성
					ADD_LOG("[Data][Teaching][Insert] StationID:%06d, PortType:%02X, Hoist:%d, Shift:%d, Rotate:%d, STB Offset:%d",
							teachingData->StationID,
							teachingData->PortType,
							teachingData->Hoist,
							teachingData->Shift,
							teachingData->Rotate,
							teachingData->STBOffSet);

					nError = OHTMainForm->GetTeachingDataManager()->Insert(teachingData);
				}
			...
			else
			{
				...
				{
					// by zzang9un 2013.07.23 : 로그 생성
					ADD_LOG("[Data][Teaching][Update] StationID:%06d, PortType:%02X, Hoist:%d, Shift:%d, Rotate:%d, STB Offset:%d(+%f)",
							teachingData->StationID,
							teachingData->PortType,
							teachingData->Hoist,
							teachingData->Shift,
					...
					nError = OHTMainForm->GetTeachingDataManager()->TeachingUpdate(teachingData);
				}
	...
	{
		nError = ERR_AUTOTEACHING_TEACHING_FAIL;
    }
