...
typedef struct Flow_Info_         //timy
{
	UINT NodeID;                 ///< 주행 Node Id
	BYTE NodeType;               ///< Node의 종류	//   0x1: Lift In, 0x2: Lift out 0x3: MTL In, 0x4 : MTL Middle, 0x5:MTL out
	UINT LeftNodeID;             ///< (왼쪽) 연결된 Node Id
	UINT RightNodeID;            ///< (오른쪽) 연결된 Node Id
	bool Flowflag;               ///< FlowControl Node Flag
} FLOW_INFO;
...
public:
	...
	FLOW_INFO *m_FlowCheckArray; ///< timy
...
//	int FindStationIndexFromTeachingMap(int NodeID);                 //미사용
//	int FindStationIndexFromLookDownMap(int NodeID);                 //미사용
...
//	int FindIndex_RailMerge_StopReset(unsigned long long NodeID);         //미사용
	...
	UINT calReOptRatio(double ChangeOptRatio);
	int FireDoorCheck(UINT NodeID, NODE_INFO_STD_FLOWCONTROL * m_FlowArray, UINT DataNum);
};
//---------------------------------------------------------------------------
...
//	bool GetMapLoaded();                  //미사용
//	unsigned int GetMapVersion();         //미사용
