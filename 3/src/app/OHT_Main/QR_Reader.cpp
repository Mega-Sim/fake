...
//	ADD_LOG_QRTEST("QR Test %d", dwLen);
	if((dwLen != 0) &&((m_MakerType==QR_MAKER_OMRON) ||(m_MakerType==QR_MAKER_DM50) || (m_MakerType==QR_MAKER_MS) || (m_MakerType==QR_MAKER_DM50_DECODETIME)))
	{
...
//			}
			...
			nTmpError = ParseData(szResult ,&nID, &dOffset, &nDecodeTime, &nSetupVersion);
			ADD_LOG_QRTEST("QR ParseData %d, %s, %d, %f, %d, %d", nTmpError, szResult, nID, dOffset, nDecodeTime, nSetupVersion);

			if(nTmpError == NO_ERR)
				nTmpError = CheckDecodeTime(nDecodeTime);
...
@return
*/
int QR_Reader::ParseData(char* szData, int *nID, double *dOffset, int *nDecodeTime, int *nSetupVersion)
{
	//12345B;;+0182,+0098;
	...
	for(int n=0; n<nLen; n++)
	{
		...
		else
		{
			...
			case ':':
				...
				else if((m_MakerType==QR_MAKER_DM50) || (m_MakerType==QR_MAKER_DM50_DECODETIME))
				{
					nDirection	= 1;
	...
	//------------------------------------------------------------------------------
	if(m_MakerType==QR_MAKER_DM50_DECODETIME)
	{
		...
		if((nIndexOfStart != -1) && (nIndexOfEnd != -1))
		{
			for(int n=nIndexOfStart+1; n<nIndexOfEnd; n++)
			{
				if(szData[n]>='0' && szData[n]<='9')
				{
					szTemp[n-nIndexOfStart-1] = szData[n];
				}
			}
			bMeetNoneZero = false;
			nMeetNoneZero = 0;
		...
		if(nTmpError == NO_ERR)
		{
			*nSetupVersion = atoi(szTemp);	// int 처리
		}
	}


	//2.2 DecodeTime을 받아 온다
	if(m_MakerType==QR_MAKER_DM50_DECODETIME)
	{
		memset(&szTemp, 0, sizeof(szTemp));

		// 2.2.1 DecodeTime의 마지막 값을 받아준다.
		for(int n=nLen-1; n>=0; n--)
		{
			if(szData[n]=='#')
			{
		...
		if(nTmpError == NO_ERR)
		{
			*nDecodeTime = atoi(szTemp);	// int 처리
		}
	}
	//------------------------------------------------------------------------------
	//2	3. Offset을 받아 온다
...
int QR_Reader::CheckDecodeTime(int nDecodeTime)
{
	 nCntDecodeTime++;
   if(nDecodeTime > nMaxDecodeTime)
	nMaxDecodeTime = nDecodeTime;
   if(nDecodeTime < nMinDecodeTime)
	nMinDecodeTime = nDecodeTime;

	nSumDecodeTime += nDecodeTime;
	dAvgDecodeTime = nSumDecodeTime/nCntDecodeTime;

	return 0;
}
int QR_Reader::GetMaxDecodeTime()
{
	//ADD_LOG_QRTEST("GetMaxDecodeTime : %d",nMaxDecodeTime);
	return nMaxDecodeTime;
}
int QR_Reader::GetMinDecodeTime()
{
	//ADD_LOG_QRTEST("GetMinDecodeTime : %d",nMinDecodeTime);
	return nMinDecodeTime;
}
double QR_Reader::GetAvgDecodeTime()
{
	//ADD_LOG_QRTEST("GetAvgDecodeTime : %d",dAvgDecodeTime);
	return dAvgDecodeTime;
}
int QR_Reader::GetSetupVersion()
{
	return nSetupVersion;
}
int QR_Reader::ResetDecodeTime()
{
	nCntDecodeTime = 0;
	nMinDecodeTime = 100;
	nMaxDecodeTime = -1;
...
int QR_Reader::GetTriggerCount(bool bInit)
{
