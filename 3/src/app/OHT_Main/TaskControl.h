...
typedef struct 
{
	bool bMainVersion;      // TRUE, FALSE. OHT Main Vsersion을 확인해 Patch 여부를 설정한다
...
#define AMC_AUTOPATCH_PATH			"C:\\User\\OHT\\AMC\\Update\\" //AOHC-154
#define AMC_BINARY_PATH             "C:\\User\\OHT\\DataFiles\\" //AOHC-154
#define WIRELESS_PARAM_FILE_PATH	"C:\\User\\OHT\\DataFiles\\"

#define INSTALL_PROGRAMS_PATH  "C:\\User\\Installprogram\\"
#define INSTALL_PROGRAMS_AUTOUPDATER_PATH  "C:\\User\\Installprogram\\AutoUpdater\\"

#define OHT_MAIN_FILE_NAME 		L"OHT.exe"
#define ASSISTANCE_FILE_NAME    L"OHTAssistant.exe"
#define FILE_MANAGER_FILE_NAME  L"FM.exe"
#define AMC_DLL_FILE_NAME       L"amc.dll"     //AOHC-154
#define AMC_LIB_FILE_NAME       L"amc.lib"     //AOHC-154
#define OHT_AGENT_FILE_NAME     L"Agent.exe" // 23.02.02 JUKim added
#define AMC6869_DLL_FILE_NAME       L"amc6869.dll"     //AOHC-154

#define AMC_BOOT_FILE_NAME		L"coffboot.bin"
//#define AMC_MAIN_FILE_NAME		L"amc.out"
#define AMC_MAIN_FILE_NAME		"*.out" //AOHC-154
...
private:
	...
	LogUnit *pLogUBGAbnormalChk; ///< UBGAbnormalCheck 관련 로그 변수
	LogUnit *pLogMCC;
...
//	DEFAULT_AUTOPATCH_PARAM_SET  *m_pAutoPatchParamSet;//AOHC-154
	...
	DWORD m_dwUBGBummperTrigerTime; //AOHC-237 UBG 좌/우 틀어짐 검출 기능 추가
	DWORD m_LogTime;  //AOHC-237 UBG 좌/우 틀어짐 검출 기능 추가

	int m_MoniUBGPat;

	bool m_bAMCDLLPatch;
   	bool m_bE140AutoRecovery;
	bool m_bAutoRecoveryCheck;
	bool bMapAdjust;
	bool bMapModify;
	AnsiString m_strCurMapVer;
	int m_nReUpdateCnt;
public:
	// 20161216 sh17.jo : Commfail 체크 기능 추가
	...
	int m_adapterResetCount; // AMOHTV80F-1683
	int m_nComOKCount;
    bool m_bCheckPassOpenListCount;
//    PATHSEARCH_UBG_CHECK_PARAM_SET  *m_pUBGParamSet;     //soming UBG 감지 단계 측정
	double m_dMinAccel;

	int m_nUseManualError;//AOHC-154
	bool m_bOCSCheckDataTransfer;   // AOHC-411
	DWORD dwCheckOCSData;
	bool LoadPatternData_Fine(char *path); //화인 센서 패턴 체크
	char CheckPattern[1024];    // 화인 센서 패턴

	short int m_nFailClass_Status;
	DWORD dwJogInterlockResetTime;
	void JogInterlockReset();
	bool m_bAutoPatch_Fail;
...
//	int GetPathData(NODE_INFO_STD_PATHDATA *pPathdata, UINT NodeID);
	//Line 정보 AOHC-285
	LINE_INFO GetLineInfo();                                     //AOHC-285
    LINE_CHECK_INFO CheckLineAndModelInfo(LINE_INFO slineinfo);  //AOHC-285
	// Station 관련 함수
//	int LoadStationData();
...
//	int MakeAckDataRemocon(char* szReadBuff, int Error);      //미사용
...
//	bool m_bJobChangeCheckFlag;   //작업변경 가능한 조건을 확인 처리하기 위한 변수
	...
	int MakeGoCycleList(PARSING_GO_INFO *pCmdData,int StartNode=0, int StationNode =0, int noffset =0, bool bTakeout =false);   ///<수동 작업지시도 여기서 해줌.
...
//	bool CheckCmdAutoTeachingResponse(PKT_OCS2OHT* pRcvData, int QuickCmd);    ///< file Teaching TP 명령에 대한 응답
	...
	int MakeAutoTeachingInfo(int TargetStationID, bool bMove, BYTE PortType,bool bTeachingNullType = false, bool bVisionType = false);     // 확인후 진행.
...
//	int DeleteMapMakerFile(); //완료되었을떄 파일 삭제 및 초기화
	...
	void Send_MapMakeResult(AnsiString strLog, int nResult =0);
...
private:
    ...
    bool CheckCmdPassPermitCancel(PKT_OCS2OHT* pRcvData, PKT_ACK_OHT2OCS* pAckPkt,int QuickCmd);
...
//	int FirmwareUpdate_WIRELESS(FIRMWARE_INFO& FIRMWARE_UPDATE_INFO);          //미사용
//	int FirmwareUpdate_AMC(FIRMWARE_INFO& FIRMWARE_UPDATE_INFO);               //미사용
...
//	void LoadWirelessModuleInfo(AnsiString& IpAddress, AnsiString& UserName, AnsiString& Password);        //미사용
	void FirmwareUpdateMessageToTp(AnsiString strMsg);
	bool FirmwareUpdate_ModeChange();
	int Change2AutoMode(void);
	int Change2SemiAutoMode(void);
...
//	void IpStrToByteAddr(AnsiString strAddr, int& iHi, int& iM1, int& iM2, int& iLow);       //미사용

	int ManualMoving_AutoSearch(PROTOCOL_BODY_AUTO_DRIVING_TEST_MOVE_REQ PACKET_BODY);
	int ManualMoving(int iStartNode, int iStopNode, bool iIsStation, int iPortType, int iCycle);

	bool ManualMovePathSearch_AutoSearch(PROTOCOL_BODY_AUTO_DRIVING_TEST_MOVE_REQ PACKET_BODY, int& iDirection, int& iCount, unsigned int* iNodeNumList);
	bool ManualMovePathSearch(int iStartNode, int iStopNode, int& iDirection, int& iCount, unsigned int* iNodeNumList, int iCycle=0x00);
	...
	*/
	...
	bool QRCheckForSemiAuto();
	int m_nQRCheckStep;       //정전복구 QR 체크
	DWORD m_dwQRTriggerTime;  //정전복구 QR Trigger Time
...
public:
	...
	void ExecuteSteps();
	void ExecuteSteps_SOS();
	void ExecuteResetSteps();                             //미사용
	void StandardOCSReadDataUpdate();
	void StandardPCReadDataUpdate();
    // 22.05.06 JUKim added
	void StandardAgentStatusUpdate();
	void StandardAGENTReadDataUpdate();
...
//	void Send_AutoTeachResult(int iResult);         //미사용
	...
	//AOHC-154
	int AutoPatch_AMCFileCopy();             //som
	bool AutoPatch_FindFile();               //som
//	bool m_bAMCFileNameCheck;

	void testmanualerror();
	//메모리 리셋 관련 부분 //----------------
	bool m_bResetworkingFlag;
	DWORD m_dwResetFlagTime;
	void CheckResetWorkingFlag();

	//CarrierID 관련 변수, 함수
	void CarrierIDTimer();
	int m_nPreCarrierIDStatus;
	int m_nPreOHTStatus;
	int m_nPreOHTMode;
	int m_nCarrierReqCount;
	void CheckDataTransferOK();

	void AutoPatch_AMCSetup_Public();
	void ShutterAutoRecoveryPause();
...
//	DWORD m_dwARPLastClearTime;
	...
	//AOHC-154
	bool m_bAMCFileCheck;                           //som
	int m_nAMCAutoPatch;
	int AutoPatch_AMCSetup();
	AnsiString sAMCFileName;
        AnsiString cmphash;
	bool m_bDoingAutoPatch;
	void JobChangeTotalDistCheck();
};
