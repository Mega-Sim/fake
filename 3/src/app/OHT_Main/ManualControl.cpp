...
#define ADD_MD_LOG(...)    WRITE_LOG_ON_THIS_FILE("D:\\log\\MonitoringDetail.log", __VA_ARGS__)
//---------------------------------------------------------------------------
...
#if((OHT_NAME == OHT_NAME_STD_V81S) ||(OHT_NAME == OHT_NAME_STD_V85) || (OHT_NAME == OHT_NAME_STD_V81)|| (OHT_NAME == OHT_NAME_STD_V80) || (OHT_NAME == OHT_NAME_STD_V80_RETICLE)||(OHT_NAME == OHT_NAME_STD_V85S)||(OHT_NAME == OHT_NAME_STD_V82)||(OHT_NAME == OHT_NAME_STD_V86))
...
#elif(OHT_NAME == OHT_NAME_P4_MASK)	//mgchoi. 2022.08.23. P4 MaskOHT
#define HOIST_MAX_SPEED			1.366
#else
...
#define MANUAL_DRIVING_ESTOP_SPEED  0.0040  // ms/msec2
#else
...
#elif(OHT_NAME == OHT_NAME_P4_MASK)	//mgchoi. 2022.08.23. P4 MaskOHT
#define MANUAL_HOIST_ESTOP_SPEED	   (HOIST_MAX_SPEED / 500.0)    // msec^2
#else
...
////////////// mgchoi. OHT_NAME_P4_MASK용 매크로 추가 디파인 //////////////
#define HOIST_ON_ANTIDROP_POSITION  36.0
#define HOIST_ON_ANTIDROP_POSITION_MAX_RSP150  65.0
#define HOIST_ON_ANTIDROP_POSITION_MAX_RSP200  40.0
#define HOIST_ON_ANTIDROP_POSITION_MAX_EUV     30.0

// Hoist Small Add를 위한 TriggerIO 설
#define HOIST_TRIGGER_IO_LOAD   			6
#define HOIST_TRIGGER_IO_UNLOAD   			7
#define HOIST_TRIGGER_IO_HOISTHOME          47
#define HOIST_TRIGGER_IO_LOWERHOME     		50
#define HOIST_TRIGGER_IO_EUVHOME     		68
#define HOIST_TRIGGER_IO_RSP200HOME     	69
#define HOIST_TRIGGER_IO_RSP150HOME     	70

#define HOIST_SMALLADD_3MM          		3.0
#define HOIST_SMALLADD_4MM          		4.0

#define SMALLADD_DIST_LOAD_UNLOAD  			1
#define SMALLADD_DIST_ANTIDROP     			2
#define SMALLADD_DIST_ANTIDROP_RSP150     	3
#define SMALLADD_DIST_ANTIDROP_RSP200     	4
#define SMALLADD_DIST_ANTIDROP_EUV	     	5
#endif
...
#if(USE_MAC == 1)
#define LOOKDOWN_DETECT_RANGE    700.0   // Lookdown 감지 범위(mm)
#else
#define LOOKDOWN_DETECT_RANGE    350.0
#endif
...
ManualControl::ManualControl()
{
    ...
    m_Lineparam = OHTMainForm->GetLineParam();
	...
	m_bUseOnOff = false;
	m_bNew = false;
	m_bFanFailFlag = false;

	if(m_Lineparam->bUseFoupCoverOepnwaitingInterlock ==true)
		m_bIgnorFoupcoveropenFalg = false;  //AOHT-0001 : FOUP COVER
	else
        m_bIgnorFoupcoveropenFalg = 0x02;  //AOHT-0001 : FOUP COVER   //0x00 : 항시 에러, 0x01 : 무시설정, 0x02 : 비활성화

	m_DiagManager = DiagManager::getInstance();

	m_dShift_PosLimit = 0.0;  //라인별 Shift Limit 값 구분으로 추가
	m_dShift_NegLimit = 0.0;  //라인별 Shift Limit 값 구분으로 추가
...
EXECUTE_ACK ManualControl::Stop(void)
{
	...
	m_pHWSet->m_pShiftAxis->MoveStop(MANUAL_SHIFT_ESTOP_SPEED);
...
#if (((OHT_NAME == OHT_NAME_STD_V81S) ||(OHT_NAME == OHT_NAME_STD_V85)|| (OHT_NAME == OHT_NAME_STD_V81) || (OHT_NAME == OHT_NAME_STD_V80) || (OHT_NAME == OHT_NAME_STD_V80_RETICLE) ||(OHT_NAME == OHT_NAME_STD_V85S)||(OHT_NAME == OHT_NAME_STD_V82)||(OHT_NAME == OHT_NAME_STD_V86)||(OHT_NAME == OHT_NAME_P4_MASK)) && (SAMSUNG_COMMON_SPEC==ON))
...
int ManualControl::JogCommand()
{
	...
	DWORD dwRotateMovetime = timeGetTime();

	if((strcmp(OHTMainForm->GetMapDataManager_STD()->m_MapFileHeader.VersionString, LINETYPE_P1_1F) == 0) || (strcmp(OHTMainForm->GetMapDataManager_STD()->m_MapFileHeader.VersionString, LINETYPE_P1_3F) == 0) ||(strcmp(OHTMainForm->GetMapDataManager_STD()->m_MapFileHeader.VersionString, LINETYPE_P1_4F) == 0) ||(strcmp(OHTMainForm->GetMapDataManager_STD()->m_MapFileHeader.VersionString, LINETYPE_P1_7F) == 0) )
	{
#if((OHT_NAME == OHT_NAME_STD_V81) ||(OHT_NAME == OHT_NAME_STD_V81S) ||(OHT_NAME == OHT_NAME_STD_V80)||(OHT_NAME == OHT_NAME_STD_V85S)||(OHT_NAME == OHT_NAME_STD_V82) ||(OHT_NAME == OHT_NAME_P4_MASK))
		m_dShift_PosLimit =  SHIFT_MOVELIMIT_P_TEMP;
		m_dShift_NegLimit =  SHIFT_MOVELIMIT_N_TEMP;
#else
		m_dShift_PosLimit =  SHIFT_MOVELIMIT_P;
		m_dShift_NegLimit =  SHIFT_MOVELIMIT_N;
#endif
	}
	else
	{
...
#if(OHT_NAME == OHT_NAME_P4_MASK)   // 부드러운 이동을 위해
		dPos_mm = m_pHWSet->m_pHoistAxis->GetCurrentPosition() - (double)40.0;
#else
...
#if(OHT_NAME == OHT_NAME_P4_MASK)     // 부드러운 이동을 위해
		dPos_mm = m_pHWSet->m_pHoistAxis->GetCurrentPosition() + (double)40.0;
		if( (m_pHWSet->m_pShutter->IsClose(FRONT) == true) && (m_pHWSet->m_pHoistAxis->GetCurrentPosition() > HOIST_ON_ANTIDROP_POSITION_MAX_EUV))
	...
	case TPCMD_JOG_HOIST_HOME:
		m_pOHTMainStatus->bIsHoistHomeCommand = true;        //SQA 요청
#if(OHT_NAME == OHT_NAME_P4_MASK)
		if((m_pHWSet->m_pHand->IsWork() == true) || (m_pHWSet->m_pHand->IsHome() == false))
			nError = ERR_HOIST_NOTHOME_AT_HANDWORK;
		if((m_pHWSet->m_pShutter->IsPodPlacement3On() == false) && (m_pHWSet->m_pShutter->IsPodPlacement4On() == false) && (m_pHWSet->m_pHand->IsLeftGripOn() != m_pHWSet->m_pHand->IsRightGripOn()))
		...
		{
#if(OHT_NAME == OHT_NAME_P4_MASK)
			dVel_mmpermsec = MANUAL_HOIST_MAX_SPEED * MANUAL_MULTIPLY_2_6P * 2.0;
#else
...
*/
#if(OHT_NAME == OHT_NAME_P4_MASK)
		if((m_pHWSet->m_pShutter->IsOpen(FRONT) == false) || (m_pHWSet->m_pShutter->IsClose(FRONT) == true))
		{
			nError = ERR_SLIDE_NOTMOVE_SHUTTER_CLOSE;
		}
#else
		if(m_pHWSet->m_pShiftAxis->GetCurrentPosition() < m_dShift_NegLimit)
		{
		...
		if (nError == NO_ERR)
		{
#if(OHT_NAME == OHT_NAME_STD_V80_RETICLE) //AOHC-288
			dPos_mm = m_pHWSet->m_pShiftAxis->GetCurrentPosition() - (double)3.0;
#elif(OHT_NAME == OHT_NAME_P4_MASK)
			dPos_mm = m_pHWSet->m_pShiftAxis->GetCurrentPosition() - (double)20.0;
#else
			dPos_mm = m_pHWSet->m_pShiftAxis->GetCurrentPosition() - (double)15.0;
#endif

#if(OHT_NAME != OHT_NAME_P4_MASK)
			if(dPos_mm < m_dShift_NegLimit)
				dPos_mm = m_dShift_NegLimit - 0.5;
#endif
        }
...
*/
...
#else
		if(m_pHWSet->m_pShiftAxis->GetCurrentPosition() > m_dShift_PosLimit)
		{
...
#if(OHT_NAME != OHT_NAME_P4_MASK)
			if(dPos_mm >  m_dShift_PosLimit)
				dPos_mm = m_dShift_PosLimit + 0.5;
#endif
		}
...
#endif
		dVel_mmpermsec = MANUAL_SHIFT_MAX_SPEED * MANUAL_MULTIPLY_34_10P;
		nError = m_pHWSet->m_pShiftAxis->MoveDistance((unsigned long)MANUAL_SHIFT_ACCTIME, (unsigned long)MANUAL_SHIFT_ACCTIME,	dVel_mmpermsec, 0.0);
    ...
    case TPCMD_JOG_SHIFT_ABS:
		...
		else if(dPos_mm < m_dShift_NegLimit)
		{
		   nError = ERR_SHIFT_OUT_OF_N_LIMIT;
		}
#endif
...
#if(OHT_NAME == OHT_NAME_P4_MASK)
	case TPCMD_HOIST_DOWN_UNLOADPOSITION:
		if(m_pHWSet->m_pHand->IsUnloadPosition() == true)
			break;
		if(m_pHWSet->m_pHand->IsLeftGripOn() != m_pHWSet->m_pHand->IsRightGripOn())
			nError = ERR_GRIP_SEN_MISMATCH;
		else if((m_pHWSet->m_pShutter->IsOpen(FRONT) == false) || (m_pHWSet->m_pShutter->IsClose(FRONT) == true))
			nError = ERR_HOIST_NOTMOVE_SHUTTER_CLOSE;
		if(nError == NO_ERR)
		{
			dPos_mm = m_pHWSet->m_pHoistAxis->GetCurrentPosition() + 300.0;  // 300mm이동하면 정지
			dVel_mmpermsec = MANUAL_HOIST_MAX_SPEED * MANUAL_MULTIPLY_2_3P;
			dAcceleration_mmpermsec2 = dVel_mmpermsec / MANUAL_HOIST_ACCTIME_150;
			nError = JogMove_Hoist_SmallAdd(dAcceleration_mmpermsec2, dAcceleration_mmpermsec2, dVel_mmpermsec, dPos_mm, HOIST_TRIGGER_IO_UNLOAD,
						m_pTransControl->getTransSmallAddDistForLH(SMALLADD_DIST_LOAD_UNLOAD));   // only Unload
			if(nError != NO_ERR)
				nError = ERR_NOTFOUND_UNLOADPOSITION;
		}
		break;
	case TPCMD_HOIST_DOWN_LOADPOSITION:
		if(m_pHWSet->m_pHand->IsLoadPosition() == true)
			break;
		if(m_pHWSet->m_pHand->IsLeftGripOn() != m_pHWSet->m_pHand->IsRightGripOn())
			nError = ERR_GRIP_SEN_MISMATCH;
		else if((m_pHWSet->m_pShutter->IsOpen(FRONT) == false) || (m_pHWSet->m_pShutter->IsClose(FRONT) == true))
		...
		if(nError == NO_ERR)
		{
			...
			dVel_mmpermsec = MANUAL_HOIST_MAX_SPEED * MANUAL_MULTIPLY_2_3P;
			dAcceleration_mmpermsec2 = dVel_mmpermsec / MANUAL_HOIST_ACCTIME_150;
			nError = JogMove_Hoist_SmallAdd(dAcceleration_mmpermsec2, dAcceleration_mmpermsec2, dVel_mmpermsec, dPos_mm, HOIST_TRIGGER_IO_LOAD,
						m_pTransControl->getTransSmallAddDistForLH(SMALLADD_DIST_LOAD_UNLOAD));   // only Unload
			if(nError != NO_ERR)
				nError = ERR_NOTFOUND_LOADPOSITION;
		}
		break;
	case TPCMD_HOIST_UP_LOADPOSITION:
		if(m_pHWSet->m_pHand->IsLoadPosition() == true)
			break;
		...
		if(nError == NO_ERR)
		{
			dPos_mm = m_pHWSet->m_pHoistAxis->GetCurrentPosition() - 100.0;  // 최대 100mm 상승하면 정지
			if(dPos_mm < 0.0) dPos_mm = 0.0;
			dVel_mmpermsec = MANUAL_HOIST_MAX_SPEED * MANUAL_MULTIPLY_2_3P;
			dAcceleration_mmpermsec2 = dVel_mmpermsec / MANUAL_HOIST_ACCTIME_150;
			nError = JogMove_Hoist_SmallAdd(dAcceleration_mmpermsec2, dAcceleration_mmpermsec2, dVel_mmpermsec, dPos_mm, HOIST_TRIGGER_IO_LOAD,
						m_pTransControl->getTransSmallAddDistForLH(SMALLADD_DIST_LOAD_UNLOAD));   // only Unload
		...
		break;
	case TPCMD_HOIST_DOWN_ANTIDROP:
			if(m_pHWSet->m_pHoistAxis->IsHoistEUVHome() == true)
				break;
			...
			else if(m_pHWSet->m_pHoistAxis->GetCurrentPosition() > HOIST_ON_ANTIDROP_POSITION_MAX_EUV)
			...
			if(nError == NO_ERR)
			{
				...
				dPos_mm = m_pHWSet->m_pHoistAxis->GetCurrentPosition() + HOIST_ON_ANTIDROP_POSITION_MAX_EUV;  // 35mm이동하면 정지, 실제 거리는 30mm
				dVel_mmpermsec = MANUAL_HOIST_MAX_SPEED * MANUAL_MULTIPLY_2_3P;
				dAcceleration_mmpermsec2 = dVel_mmpermsec / MANUAL_HOIST_ACCTIME_150;

				int nHoistLowerHomeInput = HOIST_TRIGGER_IO_EUVHOME;

				nError = JogMove_Hoist_SmallAdd(dAcceleration_mmpermsec2, dAcceleration_mmpermsec2, dVel_mmpermsec, dPos_mm, nHoistLowerHomeInput,
											m_pTransControl->getTransSmallAddDistForLH(SMALLADD_DIST_ANTIDROP));

				ADD_LOG("TPCMD_HOIST_DOWN_ANTIDROP DIST %.2f", m_pTransControl->getTransSmallAddDistForLH(SMALLADD_DIST_ANTIDROP));

				if(nError != NO_ERR) nError = ERR_NOTFOUND_LOWERHOISTHOME;
			}
		break;
	case TPCMD_HOIST_UP_HOISTHOME:
		...
		if(nError == NO_ERR)
		{
			dPos_mm = m_pHWSet->m_pHoistAxis->GetCurrentPosition() - HOIST_ON_ANTIDROP_POSITION_MAX_RSP150;
			...
			dVel_mmpermsec = MANUAL_HOIST_MAX_SPEED * MANUAL_MULTIPLY_2_3P;
			dAcceleration_mmpermsec2 = dVel_mmpermsec / MANUAL_HOIST_ACCTIME_150;
			nError = JogMove_Hoist_SmallAdd(dAcceleration_mmpermsec2, dAcceleration_mmpermsec2, dVel_mmpermsec, dPos_mm, HOIST_TRIGGER_IO_HOISTHOME,
						HOIST_SMALLADD_4MM);   // only Unload

			if(nError != NO_ERR)
				nError = ERR_NOTFOUND_HOISTHOME;
		}
		break;
	case TPCMD_HOIST_DOWN_ANTIDROP_EUV:
		if(m_pHWSet->m_pHoistAxis->IsHoistEUVHome() == true)
			break;
		...
		if(nError == NO_ERR)
		{
			...
			nError = JogMove_Hoist_SmallAdd(dAcceleration_mmpermsec2, dAcceleration_mmpermsec2, dVel_mmpermsec, dPos_mm, nHoistLowerHomeInput,
				m_pTransControl->getTransSmallAddDistForLH(SMALLADD_DIST_ANTIDROP_EUV));
			if(nError != NO_ERR)
				nError = ERR_NOTFOUND_LOWERHOISTHOME;
		}
		 break;
	case TPCMD_HOIST_DOWN_ANTIDROP_RSP200:
		if(m_pHWSet->m_pHoistAxis->IsHoistRSP200Home() == true)
			break;

		if(m_pHWSet->m_pHand->IsLeftGripOn() != m_pHWSet->m_pHand->IsRightGripOn())
			nError = ERR_GRIP_SEN_MISMATCH;
		else if(m_pHWSet->m_pHoistAxis->GetCurrentPosition() > HOIST_ON_ANTIDROP_POSITION_MAX_RSP200)
			nError = ERR_HOIST_NOTORIGIN;
		...
		if(nError == NO_ERR)
		{
			...
			dPos_mm = m_pHWSet->m_pHoistAxis->GetCurrentPosition() + HOIST_ON_ANTIDROP_POSITION_MAX_RSP200;  // 35mm이동하면 정지, 실제 거리는 30mm
			dVel_mmpermsec = MANUAL_HOIST_MAX_SPEED * MANUAL_MULTIPLY_2_3P;
			dAcceleration_mmpermsec2 = dVel_mmpermsec / MANUAL_HOIST_ACCTIME_150;

			int nHoistLowerHomeInput = HOIST_TRIGGER_IO_RSP200HOME;

			nError = JogMove_Hoist_SmallAdd(dAcceleration_mmpermsec2, dAcceleration_mmpermsec2, dVel_mmpermsec, dPos_mm, nHoistLowerHomeInput,
				m_pTransControl->getTransSmallAddDistForLH(SMALLADD_DIST_ANTIDROP_RSP200));
			if(nError != NO_ERR)
				nError = ERR_NOTFOUND_LOWERHOISTHOME;
		}
		break;
	case TPCMD_HOIST_DOWN_ANTIDROP_RSP150:
		if(m_pHWSet->m_pHoistAxis->IsHoistRSP150Home() == true)
			break;

		if(m_pHWSet->m_pHand->IsLeftGripOn() != m_pHWSet->m_pHand->IsRightGripOn())
			nError = ERR_GRIP_SEN_MISMATCH;
		else if(m_pHWSet->m_pHoistAxis->GetCurrentPosition() > HOIST_ON_ANTIDROP_POSITION_MAX_RSP150)
			nError = ERR_HOIST_NOTORIGIN;
		...
		if(nError == NO_ERR)
		{
			...
			dPos_mm = m_pHWSet->m_pHoistAxis->GetCurrentPosition() + HOIST_ON_ANTIDROP_POSITION_MAX_RSP150;  // 35mm이동하면 정지, 실제 거리는 30mm
			dVel_mmpermsec = MANUAL_HOIST_MAX_SPEED * MANUAL_MULTIPLY_2_3P;
			dAcceleration_mmpermsec2 = dVel_mmpermsec / MANUAL_HOIST_ACCTIME_150;

			int nHoistLowerHomeInput = HOIST_TRIGGER_IO_RSP150HOME;

			nError = JogMove_Hoist_SmallAdd(dAcceleration_mmpermsec2, dAcceleration_mmpermsec2, dVel_mmpermsec, dPos_mm, nHoistLowerHomeInput,
				m_pTransControl->getTransSmallAddDistForLH(SMALLADD_DIST_ANTIDROP_RSP150));
        ...
        else
        {
            switch(m_ManualCmd.uCmdTag.Info.CmdData)
            {
				 ...
				 case TPCMD_JOG_ROTATE_ABS :
					...
					ADD_LOG("TPCMD_JOG_ROTATE_ABS");
					OHTMainForm->GetTransControl()->Set_CmdEuvRotateMove(m_dJogDistance_mm);
					dwRotateMovetime = timeGetTime();
					while(1)
					{
						nError = OHTMainForm->GetTransControl()->Seq_EuvRotate_Move();

						if(nError != NO_ERR)
							break;

					   if(OHTMainForm->GetTransControl()->Get_StepEuvRotateMov() == STEP_EUV_ROT_NO_WORK)
							break;

					   if(OHTUTILITY::PreventOverflowGetTimeMs(dwRotateMovetime)>30000)
					   {
...
//	int nPIOChannel = Load_RFPIOParameter_CH();
	...
	int nPIOType = m_ManualCmd.uTPCmd.TPCmd.Data3;  // 신규 PIO 시리얼 설정
	int nPIOCS = 0;
	...
	while(OHTUTILITY::PreventOverflowGetTimeMs(dwCheckTime) < 5000)
	{
		  if(OHTMainForm->Get_EQPIO_Type() > TYPE_PIO_IR)
		  {
				if(OHTMainForm->Get_EQPIO_Type() == TYPE_PIO_HYBRID)
				{
                    ...
                    //puting MTL EQPIO 설정 방식 보완 IR Type    AOHC-367
					if((nPIOID == 0) && (nPIOChannel ==0))
						nError = m_pHWSet->m_pLeftEQPIO->Set_RFPIO_Data_RS232_Hybrid(EQ_PIO_IR, 0, 0, 0, m_defualtparam->VHLName);
					else
						nError = m_pHWSet->m_pLeftEQPIO->Set_RFPIO_Data_RS232_Hybrid(nPIOType, nPIOID, nPIOChannel, nPIOCS, m_defualtparam->VHLName);
...
int ManualControl::SetOriginAndLimit()
{
	...
	DWORD dwRotateOrigintime = timeGetTime();
...
// 2016-11-28, Add by Labeler, ROHT의 경우, Rotate 추가로 인한 구현
	...
	case TPCMD_ROTATE_GO_ORIGIN :
        ...
        {
	#if (((OHT_NAME == OHT_NAME_STD_V80_RETICLE) ||(OHT_NAME == OHT_NAME_STD_V85)||(OHT_NAME == OHT_NAME_STD_V86)) && (OHT_RETICLE_EXTEND_MODEL == RETICLE_EUV_1))
			ADD_LOG("TPCMD_ROTATE_GO_ORIGIN");
			OHTMainForm->GetTransControl()->Set_CmdEuvRotateOrigin();
			dwRotateOrigintime = timeGetTime();
			while(1)
			{
				nError = OHTMainForm->GetTransControl()->Seq_EuvRotate_Origin();

				if(nError != NO_ERR)
					break;

			   if(OHTMainForm->GetTransControl()->Get_StepEuvRotateOrg() == STEP_ROTATE_NO_WORK)
					break;

			   if(OHTUTILITY::PreventOverflowGetTimeMs(dwRotateOrigintime)>30000)
			   {
				   nError = ERR_ROTATE_MOVE_TIME_OUT;
				   break;
			   }
...
//			OHTMainForm->GetTransControl()->Set_CmdEuvRotateOrigin();
	...
	case TPCMD_HOIST_SETLIMIT_NEG:
		  nError = m_pHWCommon->SetNegativeSWLimit(2, m_dJogDistance_mm);  //pulse단위
		  if(nError==NO_ERR)
			nError = m_pHWSet->m_pHoistAxis->SetNegativeLimit4PulseMoving(m_dJogDistance_mm);
		break;
	case TPCMD_HOIST_SETLIMIT_POSI:
		  nError = m_pHWCommon->SetPositiveSWLimit(2, m_dJogDistance_mm);   //pulse단위
		break;
	case TPCMD_SHIFT_SETLIMIT_NEG:
		nError = 	m_pHWCommon->SetNegativeSWLimit(3, m_dJogDistance_mm); //pulse단위
		break;
	case TPCMD_SHIFT_SETLIMIT_POSI:
		  nError = m_pHWCommon->SetPositiveSWLimit(3, m_dJogDistance_mm);   //pulse단위
		break;
    }
    m_StatusSet.bIsOnProcess = false;
...
int ManualControl::ControlHW()
{
    ...
    case TPCMD_STEER_REAR_RIGHT:
		...
		break;
#if(OHT_NAME==OHT_NAME_P4_MASK)
	case TPCMD_HAND_OPEN:        //sseung 2022.11.09 CMD Name 동일하게 유지
	case TPCMD_HAND_WORK_IL:     // only for MASK OHT
		if((m_pHWSet->m_pShutter->IsPodPlacement3On() == true) || (m_pHWSet->m_pShutter->IsPodPlacement4On() == true))
		{
			nExec = 0;
			nError = ERR_HANDRMR_NOTMOVE_PLACEMENTSENON;
		}
		else
			nExec = 1;
		if (nError == NO_ERR && nExec == 1)
		{
			if(m_ManualCmd.uCmdTag.Info.CmdData == TPCMD_HAND_WORK_IL)
				nError = m_pHWSet->m_pHand->ManualWork(1);
			else
				nError = m_pHWSet->m_pHand->ManualWork(0);
		}
		break;
	case TPCMD_HAND_CLOSE:      //sseung 2022.11.09 CMD Name 동일하게 유지
	case TPCMD_HAND_OPEN_LIMIT: // 근무자 Mode시
		nError = m_pHWSet->m_pHand->ManualHome(0);
		break;
	case TPCMD_HAND_HOME_IL:     // HAND_OPEN
		nError = m_pHWSet->m_pHand->ManualHome(1);
		break;
	...
	case TPCMD_HAND_OPEN_LIMIT: // 근무자 Mode시
#if((OHT_NAME == OHT_NAME_STD_V80_RETICLE)||(OHT_NAME == OHT_NAME_STD_V85) || (OHT_NAME == OHT_NAME_STD_V86))
		m_pOHTMainStatus->StatusTP.bCheckCompletePodEuv = false; // ROHT Teaching 시 Hand Open 경우 재하 변경될 수 있으므로 Teaching Save 불가능하게 하기 위함
#endif
	...
	case TPCMD_HAND_HOMING:
#if((OHT_NAME == OHT_NAME_STD_V86) || (OHT_NAME == OHT_NAME_STD_V80_RETICLE)||(OHT_NAME == OHT_NAME_STD_V85))
		m_pOHTMainStatus->StatusTP.bCheckCompletePodEuv = false; // ROHT Teaching 시 Hand Open 경우 재하 변경될 수 있으므로 Teaching Save 불가능하게 하기 위함
#endif
		...
		{
			ADD_LOG("SQA : Hand Grip Sensor Mismatch");
			nError = ERR_GRIP_SEN_MISMATCH;
		}
		else
		{
...
// 2016-12-05, Add by Labeler, ROHT의 경우, Rotate 추가로 인해 Shutter Close 시 Interlock 추가
        ...
        {
#if((OHT_RETICLE_EXTEND_MODEL == RETICLE_EUV_1)  ||(OHT_NAME == OHT_NAME_STD_V85||(OHT_NAME == OHT_NAME_STD_V86)))
            if(((m_pHWSet->m_pHand->IsRightGripOn() == true) || (m_pHWSet->m_pHand->IsLeftGripOn() == true))
...
#elif(OHT_NAME == OHT_NAME_P4_MASK)
		if( ((m_pHWSet->m_pHand->IsRightGripOn() == true) || (m_pHWSet->m_pHand->IsLeftGripOn() == true)) &&
		   ((fabs(m_pHWSet->m_pHoistAxis->GetCurrentPosition()) > (DIST_HOIST_HOME_MARGIN + 1.0)) || (m_pHWSet->m_pHoistAxis->IsHoistHome() == false)) )
			nError = ERR_HOIST_NOTORIGIN;
		...
		else if( ((m_pHWSet->m_pShutter->IsPodPlacement3On() == true) || (m_pHWSet->m_pShutter->IsPodPlacement4On() == true)) )
			nError = ERR_SHR_NOTMOVE_PODPLACEMENT_ON;
		else if(m_pHWSet->m_pHand->IsRightGripOn() != m_pHWSet->m_pHand->IsLeftGripOn())
			nError = ERR_GRIP_SEN_MISMATCH;
		else if(m_pHWSet->m_pShiftAxis->IsShiftHome() == false)
...
// 셔터를 mid로 닫는 동작이므로 EUV에게만 의미가 있다
	...
	case TPCMD_SHUTTER_OPEN:
#if(OHT_NAME == OHT_NAME_P4_MASK)
		nExec = 1;   // initial
		if(nError == NO_ERR)    // Slide Clamp Hole 적용으로 Hoist Home에서만 이동하도록 변경
		{
			if( ((m_pHWSet->m_pHand->IsRightGripOn() == true) || (m_pHWSet->m_pHand->IsLeftGripOn() == true)) &&
			   ((fabs(m_pHWSet->m_pHoistAxis->GetCurrentPosition()) > (DIST_HOIST_HOME_MARGIN + 1.0)) || (m_pHWSet->m_pHoistAxis->IsHoistHome() == false)) )
			{
		...
		if ((nError == NO_ERR) && ((m_pHWSet->m_pHand->IsRightGripOn() == true) || (m_pHWSet->m_pHand->IsLeftGripOn() == true)) &&
			(fabs(m_pHWSet->m_pHoistAxis->GetCurrentPosition()) > (HOIST_ON_ANTIDROP_POSITION-3.0)) &&
			(m_pHWSet->m_pShiftAxis->IsShiftHome() == true) && (m_pHWSet->m_pInnerFoup->IsPODDetect() == false))
		{
			nExec = 0;
			nError = ERR_INNER_CARRIER_NOT_DETECT;
		}
		if ((nError == NO_ERR) && (m_pHWSet->m_pHand->IsRightGripOn() == false) && (m_pHWSet->m_pHand->IsLeftGripOn() == false) &&
			(m_pHWSet->m_pShiftAxis->IsShiftHome() == true) && (m_pHWSet->m_pInnerFoup->IsPODDetect() == true))
		{
			nExec = 0;
			nError = ERR_INNER_CARRIER_DETECT;
		}
#else
		...
		// FOUP이 존재할 경우만 확인, Foup Cover Open Sensor가 감지가 되면 ==> 에러 발생 AOHT-0001 : FOUP COVER
		if ((nError == NO_ERR) && (m_Lineparam->bUseFoupCoverOepnwaitingInterlock ==true)&& (m_pHWSet->m_pFoupCoverOpenDetect->IsDetect() == true) &&  ((m_pHWSet->m_pHand->IsRightGripOn() == true) || (m_pHWSet->m_pHand->IsLeftGripOn() == true)))
		{
			 nExec = 0;
	...
	case TPCMD_SHUTTER_ORIGIN:
		...
		{
			ADD_LOG("SQA_Shutter : ERR_SHIFT_HOMESEN_NOTON");
			nExec = 0;
...
// 2016-11-30, Labeler
    ...
    case TPCMD_OBSTACLE_AREA15:
        ...
        break;
	case TPCMD_LOOKDOWN_AREA1:
	case TPCMD_LOOKDOWN_AREA2:
	case TPCMD_LOOKDOWN_AREA3:
	case TPCMD_LOOKDOWN_AREA4:
	case TPCMD_LOOKDOWN_AREA5:
	case TPCMD_LOOKDOWN_AREA6:
	case TPCMD_LOOKDOWN_AREA7:
	case TPCMD_LOOKDOWN_AREA8:
	case TPCMD_LOOKDOWN_AREA9:
	case TPCMD_LOOKDOWN_AREA10:
	case TPCMD_LOOKDOWN_AREA11:
	case TPCMD_LOOKDOWN_AREA12:
	case TPCMD_LOOKDOWN_AREA13:
	case TPCMD_LOOKDOWN_AREA14:
	case TPCMD_LOOKDOWN_AREA15:
        nError = nError = m_pHWSet->m_pLookdown->AreaSelect(m_ManualCmd.uCmdTag.Info.CmdData - TPCMD_LOOKDOWN_AREA1 + 1);
        break;
...
//#if((OHT_NAME == OHT_NAME_STD_V85) || (OHT_NAME == OHT_NAME_STD_V81S)|| (OHT_NAME == OHT_NAME_STD_V81) || (OHT_NAME == OHT_NAME_STD_V80) || (OHT_NAME == OHT_NAME_STD_V80_RETICLE) ||(OHT_NAME == OHT_NAME_STD_V85S)||(OHT_NAME == OHT_NAME_STD_V82)||(OHT_NAME == OHT_NAME_STD_V86))
//		if (nError == NO_ERR)
//			nError = m_pHWSet->m_pHoistAxis->HoistExternalBrakeReleaseOnOFF(true);   //AOHC-290
//#endif
		break;
...
//	case TPCMD_UBG_FINE_SENSOR_REQUEST_START : // 화인 센서 데이터 리드
	case TPCMD_UBG_SOSLAB_SENSOR_REQUEST_START : // SOSLAB 대차
	case TPCMD_OBS_SOSLAB_SENSOR_REQUEST_START : // SOSLAB 장애물
	...
	case TPCMD_LSTB_TEACHING_MEASURE:
		if(m_defualtparam->bUseVisionTeaching == true)
			nError = m_pHWSet->m_pLeftSTBQRR->ReadData();
		break;

	case TPCMD_RSTB_TEACHING_MEASURE:
		if(m_defualtparam->bUseVisionTeaching == true)
			nError = m_pHWSet->m_pRightSTBQRR->ReadData();
		break;

	case TPCMD_GET_BLUETOOTH_INFO:
		if(m_defualtparam->bUseVMS == true)
			OHTMainForm->GetVMS()->GetBTID();
		else
			ADD_LOG("Not Use VMS!!");
		break;

	case TPCMD_BLUETOOTH_SET_WAIT:
		if(m_defualtparam->bUseVMS == true)
			OHTMainForm->GetVMS()->SetWait();
		else
			ADD_LOG("Not Use VMS!!");
		break;

	case TPCMD_BLUETOOTH_SW_RESET:
		if(m_defualtparam->bUseVMS == true)
			OHTMainForm->GetVMS()->SetSWReset();
		else
			ADD_LOG("Not Use VMS!!");
		break;
	case TPCMD_BLUETOOTH_HW_RESET:
		if(m_defualtparam->bUseVMS == true)
			OHTMainForm->GetVMS()->SetHWReset();
		else
			ADD_LOG("Not Use VMS!!");
		break;
	case TPCMD_VMS_CONNECT:
		if(m_defualtparam->bUseVMS == true)
			m_pOHTMainStatus->nStatusVMS = BT_CONNECT;
		else
			ADD_LOG("Not Use VMS!!");
		break;
	case TPCMD_VMS_DISCONNECT:
		if(m_defualtparam->bUseVMS == true)
			m_pOHTMainStatus->nStatusVMS = DISCONNECT;
		else
			ADD_LOG("Not Use VMS!!");
		break;
    }
...
int ManualControl::ControlDriving()
{
	...
	{
		if (m_pHWSet->m_pShutter->IsClose(FRONT) == false)
		{
...
#if(OHT_NAME == OHT_NAME_P4_MASK)
		if( (m_pHWSet->m_pHoistAxis->IsHoistHome() == false) && (m_pHWSet->m_pHoistAxis->IsHoistEUVHome() == false) &&
			(m_pHWSet->m_pHoistAxis->IsHoistRSP150Home() == false) && (m_pHWSet->m_pHoistAxis->IsHoistRSP200Home() == false))
		{
			nError = ERR_HOIST_NOTORIGIN;
		}
//		if( ((m_pHWSet->m_pHand->IsLeftGripOn() == true) || (m_pHWSet->m_pHand->IsRightGripOn() == true))
//			&& ((m_pHWSet->m_pShutter->IsPodPlacement1On() == false) || (m_pHWSet->m_pShutter->IsPodPlacement2On() == false)
//			 || (m_pHWSet->m_pHoistAxis->IsHoistLowerHome()== false)) )
//		{
//			nError = ERR_HOIST_NOTORIGIN;
//		}
		if( ((m_pHWSet->m_pShutter->IsPodPlacement3On() == true) || (m_pHWSet->m_pShutter->IsPodPlacement4On() == true))
			&& (m_pHWSet->m_pInnerFoup->IsPODDetect() == false))
		{
			nError = ERR_INNER_CARRIER_NOT_DETECT;
		}
		else if( ((m_pHWSet->m_pShutter->IsPodPlacement3On() == false) && (m_pHWSet->m_pShutter->IsPodPlacement4On() == false))
			&& (m_pHWSet->m_pInnerFoup->IsPODDetect() == true))
		{
			nError = ERR_INNER_CARRIER_DETECT;
		}
...
EXECUTE_ACK ManualControl::InsertGeneralCmd(void *sManualControlInfo)
{
	...
	EXECUTE_MANUALCONTROL_INFO *pTmpTransInfo = (EXECUTE_MANUALCONTROL_INFO*)sManualControlInfo;
...
#if(OHT_NAME == OHT_NAME_P4_MASK)
			   || ((m_ManualCmd.uCmdTag.Info.CmdData >= TPCMD_HOIST_DOWN_UNLOADPOSITION) && (m_ManualCmd.uCmdTag.Info.CmdData <= TPCMD_HOIST_DOWN_ANTIDROP))
			   || ((m_ManualCmd.uCmdTag.Info.CmdData >= TPCMD_HOIST_DOWN_ANTIDROP_EUV) && (m_ManualCmd.uCmdTag.Info.CmdData <= TPCMD_HOIST_DOWN_ANTIDROP_RSP150))
#endif
			)
...
UINT ManualControl::CheckAlways(void)
{
	...
	static int nlookdown_cnt = 0;
	static int nOscillation_cnt = 0;

	static int nHoistHomecount = 0;
	static int nHoistOriginCount = 0;
	static int nlookdownCount = 0;
	static int nOscillationCount = 0;
	static int nShiftHomecount = 0;

	static DWORD dwHoistHomeTime = timeGetTime();

#if(OHT_NAME == OHT_NAME_P4_MASK)
	static int nFfuFailHoistCount = 0;
	static int nFfuFailFrameCount = 0;
	bool bHoistHome = m_pHWSet->m_pHoistAxis->IsHoistHome();
	bool bHoistEuvHome = m_pHWSet->m_pHoistAxis->IsHoistEUVHome();
	bool bHoistRsp200Home = m_pHWSet->m_pHoistAxis->IsHoistRSP200Home();
	bool bHoistRsp150Home = m_pHWSet->m_pHoistAxis->IsHoistRSP150Home();
	bool bHoistRunning = m_pHWSet->m_pHoistAxis->IsRunning();
	double dHoistPosition = m_pHWSet->m_pHoistAxis->GetCurrentPosition();
#endif

#if ((OHT_NAME == OHT_NAME_STD_V85) || (OHT_NAME == OHT_NAME_STD_V81S) || (OHT_NAME == OHT_NAME_STD_V81)||(OHT_NAME == OHT_NAME_STD_V80) || (OHT_NAME == OHT_NAME_STD_V80_RETICLE) ||(OHT_NAME == OHT_NAME_STD_V85S)||(OHT_NAME == OHT_NAME_STD_V82)||(OHT_NAME == OHT_NAME_STD_V86)||(OHT_NAME == OHT_NAME_P4_MASK))
	static int nHoistBeltDetectCount = 0;
#endif

//AOHT-0001 : FOUP COVER
...
//   ERR_FOUP_COVER_OPEN_DETECT 에러관련 주행시만 감시하도록 변경됨. (센서 위치 변경으로)
	if(m_Lineparam->bUseFoupCoverOepnwaitingInterlock ==true)
	{
		...
		bool bFoupCoverOpenDetect = m_pHWSet->m_pFoupCoverOpenDetect->IsDetect();
		static bool bPreFoupCoverOpenDetect = m_pHWSet->m_pFoupCoverOpenDetect->IsDetect();
		static PATH_DIRECTION_INFO TempPreFoupCoverDectect = PATH_DIRECTION_NO_USE;


		if(TempPreFoupCoverDectect != m_pHWSet->m_pOHTDetect->GetUbgAreaStatus())
		{
			TempPreFoupCoverDectect =  m_pHWSet->m_pOHTDetect->GetUbgAreaStatus();
			dwLastTimeOfFoupCoverOpenDetect = timeGetTime();
		}

	//	ADD_LOG("[FoupCover byStatus] %d",m_pExecuteInfo->StatusCmdId.Info.Status);     //20191104 KDS

		//메뉴얼 무시 플래그 처리 필요.
		if((bFoupCoverOpenDetect == true) &&
		  ((m_bIgnorFoupcoveropenFalg == false) || (m_pExecuteInfo->StatusCmdId.Info.Status==STS_GOING))&&
		  ((m_pExecuteInfo->StatusCmdId.Info.Status==STS_INIT)|| (m_pExecuteInfo->StatusCmdId.Info.Status==STS_GOING) || (m_pExecuteInfo->StatusCmdId.Info.Status==STS_ARRIVED)|| (m_pExecuteInfo->StatusCmdId.Info.Status==STS_UNLOADED)|| (m_pExecuteInfo->StatusCmdId.Info.Status==STS_LOADED)|| (m_pExecuteInfo->StatusCmdId.Info.Status==STS_RUNNING)|| (m_pExecuteInfo->StatusCmdId.Info.Status==STS_COMPLETE)|| (m_pExecuteInfo->StatusCmdId.Info.Status==STS_AUTORECOVERY)|| (m_pExecuteInfo->StatusCmdId.Info.Status==STS_AUTORECOVERY_BLANKTRANS)|| (m_pExecuteInfo->StatusCmdId.Info.Status==STS_AUTORECOVERY_DOUBLETRANS)|| (m_pExecuteInfo->StatusCmdId.Info.Status==STS_AUTORECOVERY_OVERRUN)|| (m_pExecuteInfo->StatusCmdId.Info.Status==STS_TURN_OFF)|| (m_pExecuteInfo->StatusCmdId.Info.Status==STS_AUTO_TAUGHT)|| (m_pExecuteInfo->StatusCmdId.Info.Status==STS_AUTO_COMPLETE)))
		{
			...
			if((m_pHWSet->m_pOHTDetect->GetOutputSensorStright() == true) ||(m_pHWSet->m_pOHTDetect->CheckSensorOff() == true))
			{
				dwFoupCoverOpenDetectCheckTime = 1000;  //임시(파라미터값 하드코딩)
				ADD_LOG("[FoupCover] %d %d %d",dwFoupCoverOpenDetectCheckTime,OHTUTILITY::PreventOverflowGetTimeMs( dwLastTimeOfFoupCoverOpenDetect),m_pExecuteInfo->StatusCmdId.Info.Status);     //20191104 KDS
			}
			else
			{

				dwFoupCoverOpenDetectCheckTime = 3000; //임시(파라미터값 하드코딩)
				ADD_LOG("[FoupCover] %d %d %d",dwFoupCoverOpenDetectCheckTime,OHTUTILITY::PreventOverflowGetTimeMs( dwLastTimeOfFoupCoverOpenDetect),m_pExecuteInfo->StatusCmdId.Info.Status);   //20191104 KDS
			}
	...
	if(m_pOHTMainStatus->bIsHoistHomeCommand == true)
	{
		//Home 위치 도착 시 Flag 해제
		// 1. Hoist Running OFF
		// 2. Hoist Position in 1.0mm
		// 3. Only Hoist Home Sensor ON  (EUV Home OFF, RSP150 Home OFF, RSP200 Home OFF)
#if(OHT_NAME == OHT_NAME_P4_MASK)

		if((bHoistRunning == false) &&
		   (dHoistPosition < 1.0) &&
		   ((bHoistHome == true)&&(bHoistEuvHome == false)&&(bHoistRsp150Home == false)&&(bHoistRsp200Home == false)))
		{
			m_pOHTMainStatus->bIsHoistHomeCommand = false;
		}
#else
		if((m_pHWSet->m_pHoistAxis->IsRunning() == false) && (m_pHWSet->m_pHoistAxis->IsHoistHome() == true))
		{
			m_pOHTMainStatus->bIsHoistHomeCommand = false;
		}
#endif
		if((m_pHWSet->m_pLookdown->IsDetect() == true) &&((fabs(m_pHWSet->m_pHoistAxis->GetCurrentPosition()) > LOOKDOWN_DETECT_RANGE)))
		{
			if(nlookdown_cnt > 5)
			{
				m_pOHTMainStatus->bIsHoistHomeCommand = false;
				nlookdown_cnt = 0;
				return ERR_LOOKDOWN_DETECT;
			}
			nlookdown_cnt++;
		}
		else if((m_pHWSet->m_pOcsillation->IsDetect() == true)&&((fabs(m_pHWSet->m_pHoistAxis->GetCurrentPosition()) > OSCILLATION_IGNORE_DIST)))
		{
			if(nOscillation_cnt > 5)
			{
				m_pOHTMainStatus->bIsHoistHomeCommand = false;
				nOscillation_cnt = 0;
...
#if(OHT_NAME != OHT_NAME_P4_MASK)
	//1 2. FAN Fail 확인
	//AOHC-303 : Fan Fail 조건 수정
	if(m_pHWSet->m_pFan->IsFanFail() == true)
	{
		nFanFailCount++;
		ADD_MD_LOG("FAN_FAIL Count : %d", nFanFailCount);
...
#else
	if(m_pHWSet->m_pFfu->IsFfuHoistFail() == true)
	{
		nFfuFailHoistCount++;
		if(nFfuFailHoistCount > 10)
		{
			ADD_MD_LOG("[ERR_FFU_FAIL] IsFfuHoist Fail : %d, nFfuHoist FailCount : %d", m_pHWSet->m_pFfu->IsFfuHoistFail(), nFfuFailHoistCount);
			nFfuFailHoistCount = 0;
			return ERR_FFU_FAIL;
        }
	}
	else
	...
	if(m_pHWSet->m_pFfu->IsFfuFrameFail() == true)
	{
		nFfuFailFrameCount++;
		if(nFfuFailFrameCount > 10)
		{
			ADD_MD_LOG("[ERR_FFU_FAIL] IsFfuFrame Fail : %d, nFfuFrame FailCount : %d", m_pHWSet->m_pFfu->IsFfuFrameFail(), nFfuFailFrameCount);
			nFfuFailFrameCount = 0;
			return ERR_FFU_FAIL;
        }
	}
	else
...
#if(((OHT_NAME == OHT_NAME_STD_V85) || (OHT_NAME == OHT_NAME_STD_V81S)|| (OHT_NAME == OHT_NAME_STD_V81) || (OHT_NAME == OHT_NAME_STD_V80) || (OHT_NAME == OHT_NAME_STD_V80_RETICLE)||(OHT_NAME == OHT_NAME_STD_V85S)||(OHT_NAME == OHT_NAME_STD_V82)||(OHT_NAME == OHT_NAME_STD_V86)||(OHT_NAME == OHT_NAME_P4_MASK)) && (SAMSUNG_COMMON_SPEC==ON))
	// SMPS Fault 확인
...
//	if(nError==NO_ERR)   nError = OHTMainForm->GetTransControl()->Seq_EuvRotate_Move();
	...
	if((m_pHWSet->m_pShiftAxis->IsShiftHome() == false)&&(fabs(m_pHWSet->m_pShiftAxis->GetCurrentPosition()) < INPOS_LIMIT))
	{
		nShiftHomecount++;
		if(nShiftHomecount > 3)
		{
			ADD_LOG("SQA_Shift : ERR_SHIFT_HOMESEN_NOTON,nShiftHomecount : %d",nShiftHomecount);
            nShiftHomecount =0;
			nError = ERR_SHIFT_HOMESEN_NOTON;
		}
	}
	...
	if((m_pHWSet->m_pHoistAxis->IsHoistHome() == true) && ((m_pHWSet->m_pHoistAxis->GetCurrentPosition()>13) ||(m_pHWSet->m_pHoistAxis->GetCurrentPosition()<-7)))
	{

		if( (nError == NO_ERR) && (PreventOverflowGetTimeMs(dwHoistHomeTime) > 200) ) // 200ms 변경
		{
			ADD_LOG("SQA_Hoist : ERR_HOIST_ABNORMAL_MOVING,nHoistHomecount : %d, %.6f",
					PreventOverflowGetTimeMs(dwHoistHomeTime), m_pHWSet->m_pHoistAxis->GetCurrentPosition());
			nError = ERR_HOIST_ABNORMAL_MOVING;
		}
	}
	else
	{
		dwHoistHomeTime = timeGetTime();
	}

// 8.1, 8.2 동작 달라 SQA 문의 필요
	// if((m_pHWSet->m_pHoistAxis->IsHoistHome() == true) && (m_pHWSet->m_pHoistAxis->IsRunning() == true))
	// {
	// 	nHoistOriginCount ++;
	// 	if( (nHoistOriginCount > 10)) // 8.1 호기 채터링 안됨
	// 	{
	// 		ADD_LOG("SQA_Hoist : ERR_HOIST_NOTORIGIN, Hoist Dist : %.3f", m_pHWSet->m_pHoistAxis->GetCurrentPosition());
	// 		nHoistOriginCount = 0;
	// 		nError = ERR_HOIST_NOTORIGIN;
	// 	}
	// 	else
	// 	{
	// 		ADD_LOG("SQA_Hoist : Hoist Home On, Hoist Dist : %.3f", m_pHWSet->m_pHoistAxis->GetCurrentPosition()); // Test Log
	// 	}
	// }
	// else
	// {
	...
	if(m_pOHTMainStatus->bIsHoistHomeCommand == true)
	{
		...
		if((m_pHWSet->m_pLookdown->IsDetect() == true) && ((fabs(m_pHWSet->m_pHoistAxis->GetCurrentPosition()) > LOOKDOWN_DETECT_RANGE)))
		{
			if(nlookdownCount > 5)
			{
				ADD_LOG("SQA_Lookdown : ERR_LOOKDOWN_DETECT, nlookdownCount:%d", nlookdownCount);
				m_pOHTMainStatus->bIsHoistHomeCommand = false;
				nlookdownCount = 0;
				return ERR_LOOKDOWN_DETECT;
			}
		...
		{
			if(nOscillationCount > 5)
			{
				ADD_LOG("SQA_Oscillation : ERR_OSCILLATION_ERROR, nOscillationCount:%d", nOscillationCount);
				m_pOHTMainStatus->bIsHoistHomeCommand = false;
				nOscillationCount = 0;
				return ERR_OSCILLATION_ERROR;
			}
...
bool ManualControl::SendCidSetting(void)
{
	bool bReturn = 0;
	bReturn = m_pHWSet->m_pCID->Set_CID_Name_Setting_Cmd(m_defualtparam->VHLName);
...
//AOHC-303 : Fan Fail 조건 수정
void ManualControl::setFanFailFlag(bool Flag)
{
	m_bFanFailFlag = Flag;
}
//AOHC-343 : ErrorIgnol  조건 수정 AOHT-0001 : FOUP COVER
void ManualControl::setFoupCoverFlag(bool Flag)
{
	if(m_Lineparam->bUseFoupCoverOepnwaitingInterlock ==true)
		m_bIgnorFoupcoveropenFalg = Flag;
	else
		m_bIgnorFoupcoveropenFalg = 0x02;
}

int ManualControl::GetFoupCoverFlag()
{
  	return m_bIgnorFoupcoveropenFalg;
}

#if(OHT_NAME == OHT_NAME_P4_MASK)
/**
@brief   Hoist Small Add 기능
@author  mgchoi...Mask용 함수 가져옴
@date    2022.09.24
@note    v6.5 기능함수를 통합(two Function->one Function)
*/
int ManualControl::JogMove_Hoist_SmallAdd(double dAcc_mmpermsec2, double dDec_mmpermsec2, double dV_mmpermsec, double dCmdHoistPos, int dTriggerIONum, double dSmallDist)
{
	int nError = NO_ERR;
	...
	AdditionalInfo.bIsTriggerOnRisingEdge    = false;
...
void ManualControl::CheckCarrierType()
{
	if(m_pHWSet->m_pInnerFoup->IsEUVPodDetect() == true)
		m_pTransControl->setCarrierType(CARRIER_EUV);       // EUV
	else if(m_pHWSet->m_pInnerFoup->IsRSP200Detect() == true)
		m_pTransControl->setCarrierType(CARRIER_RSP200);       // RSP200
	else if(m_pHWSet->m_pInnerFoup->IsRSP150Detect() == true)
		m_pTransControl->setCarrierType(CARRIER_POD);       // RSP150
	else
		m_pTransControl->setCarrierType(CARRIER_NOTDEFINED);       // None
}
#endif
