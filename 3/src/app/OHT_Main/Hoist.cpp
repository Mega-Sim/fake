...
bool Hoist::IsHoistBeltDetect()
{
	...
	{
		//HoistExternalBrakeReleaseOnOFF(false);  //AOHC-290
		bResult = true;
	}
	else
		bResult = false;
#elif((OHT_NAME == OHT_NAME_STD_V80)||(OHT_NAME == OHT_NAME_STD_V81) || (OHT_NAME == OHT_NAME_STD_V81S)|| (OHT_NAME == OHT_NAME_STD_V85S)||(OHT_NAME == OHT_NAME_STD_V82))
...
#elif((OHT_NAME == OHT_NAME_STD_V85)||(OHT_NAME == OHT_NAME_STD_V86))
	if((m_pHWCommon->m_Input.HOIST_Belt_Detect1 == ON) || (m_pHWCommon->m_Input.HOIST_Belt_Detect2 == ON))
	{
		// HoistExternalBrakeReleaseOnOFF(false);   //AOHC-290
		bResult = true;
...
#if((OHT_NAME == OHT_NAME_STD_V85) || (OHT_NAME == OHT_NAME_STD_V85S)||(OHT_NAME == OHT_NAME_STD_V82)||(OHT_NAME == OHT_NAME_STD_V86)||(OHT_NAME == OHT_NAME_P4_MASK))
...
double Hoist::GetHoistPulseValue()
{
	return m_pHWCommon->GetCurrentPosition(m_nAxisNum);
}
int Hoist::SetNegativeLimit4PulseMoving(double dMinCtsValue)
{
	return m_pHoistDistanceConverter->SetNegativeLimit4PulseMoving(dMinCtsValue);
}

#if(OHT_NAME == OHT_NAME_P4_MASK)
/**
@brief   Lower Hoist Home Sensor Check용 함수
@author  mgchoi
@date    2022.08.19
@return  Lower Hoist Home Sensor가 ON이면 true, OFF이면 false를 리턴
*/
bool Hoist::IsHoistLowerHome()
{
	bool bResult = false;

	if (m_pHWCommon->m_Input.HOIST_LowerHome == ON)
		bResult = true;
	else
		bResult = false;
...
/**
@brief   EUV POD Home Sensor Check용 함수
@author  mgchoi
@date    2022.08.19
@return  EUV POD Home Sensor가 ON이면 true, OFF이면 false를 리턴
*/
bool Hoist::IsHoistEUVHome()
{
	bool bResult = false;

	if (m_pHWCommon->m_Input.EUVPod_Home == ON)
		bResult = true;
	else
		bResult = false;
...
/**
@brief   RSP150 Home Sensor Check용 함수
@author  mgchoi
@date    2022.08.19
@return  RSP150 Home Sensor가 ON이면 true, OFF이면 false를 리턴
*/
bool Hoist::IsHoistRSP150Home()
{
	bool bResult = false;

	if (m_pHWCommon->m_Input.RSP150_Home == ON)
		bResult = true;
	else
		bResult = false;
...
/**
@brief   RSP200 Home Sensor Check용 함수
@author  mgchoi
@date    2022.08.19
@return  RSP200 Home Sensor가 ON이면 true, OFF이면 false를 리턴
*/
bool Hoist::IsHoistRSP200Home()
{
	bool bResult = false;

	if (m_pHWCommon->m_Input.RSP200_Home == ON)
		bResult = true;
	else
		bResult = false;
