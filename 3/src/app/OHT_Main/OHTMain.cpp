...
#define FIRMWARE_VERSION_LOG(...)    WRITE_LOG_ON_THIS_FILE("D:\\log\\FirmwareVersion.log", __VA_ARGS__)
#define MAIN_EXIT_LOG(...)    WRITE_LOG_ON_THIS_FILE("D:\\log\\OHTTerminate.log", __VA_ARGS__)
#define ADD_UBG_CHK_LOG(...)    WRITE_LOG_ON_THIS_FILE("D:\\log\\UBGPatternComp.log", __VA_ARGS__)
	...
	: TForm(Owner)
{
	...
	m_dFinalHoistParam_Pulse = 0.0;
	m_dFinalHoistParam_User = 0.0;
...
//	m_MonitorControlThread = NULL;
	m_pParamSet = NULL;
	m_pLineParamSet = NULL;
	m_pAutoPatchParamSet = NULL;  //AOHC-154
	...
	strVersion_Date = FormatDateTime("yyyy-mm-dd", tdt); //2022.11.30 sseung 버전관리 시스템 날짜 갱신용

	checkLogDrive = false;
	checkLogFolder = false;
	m_checkLowvibration = false;

	m_bFpgaVersion = false;

	m_MapUpdateResult = NO_ERR;
	m_ParamUpdateResult = NO_ERR;
	m_CheckParamResult = NO_ERR;
	...
	m_bErrHoistParamMatching = true;
	m_bErrDrivingVibrationMatching = false;
	InitializeCriticalSection(&m_CS);

	Version_Info.nGetVer = false; //2022.12.30 sseung 버전관리 시스템 파일 업데이트 횟수 관리용 -> Main On 시에 진행 후 true 처리
	m_bUBGIOCheckComplete = false;

	m_bErrMACType = false;      //2023.12.13 sseung MAC Type 설정 에러 여부 확인용 변수
}
//---------------------------------------------------------------------------
/*
...
void __fastcall TOHTMainForm::FormDestroy(TObject *Sender)
{
	...
	delete CS_CmdList;
	delete CS_TaskList; ///< CmdList를 위한 Critical Section 변수
	delete CS_CycleList; ///< CmdList를 위한 Critical Section 변수
	delete CS_ResponceList; ///< CmdList를 위한 Critical Section 변수
	delete CS_PassOpenList;
	delete CS_OHTMainStatus;
	delete CS_ThreadCycleLog;
}

//---------------------------------------------------------------------------
/*
@Modify  AOHC-246 OHT Main P.G UI 개선 Cpu, Memory Display 추가  #Jrespect.im 2019.08.05
*/
void TOHTMainForm::Update_UI()
{
	static bool bCToggle = false;
	static bool bMToggle = false;
	static DWORD dwBlinkTick = 0;
	static AnsiString strSOSLABver = "";
	...
	if((int)gm_OHTMainStatus->m_cpMonitorData.pValue.nCPU >= 0 && (int)gm_OHTMainStatus->m_cpMonitorData.pValue.nCPU <= 40)
	{
	   pnCpu->Caption = gm_OHTMainStatus->m_cpMonitorData.pValue.nCPU;
	   pnCpu->Color = clGreen;
	   pnCpu->Visible = true;
	}
	else if((int)gm_OHTMainStatus->m_cpMonitorData.pValue.nCPU > 40 && (int)gm_OHTMainStatus->m_cpMonitorData.pValue.nCPU <= 75)
	{
	   pnCpu->Caption = gm_OHTMainStatus->m_cpMonitorData.pValue.nCPU;
	   pnCpu->Color = clYellow;
	   pnCpu->Visible = true;
	}
	else if((int)gm_OHTMainStatus->m_cpMonitorData.pValue.nCPU > 75 )
	{
	   pnCpu->Caption = gm_OHTMainStatus->m_cpMonitorData.pValue.nCPU;
	   pnCpu->Color = clRed;
       ...
       pnCpu->Visible = bCToggle;
	}

	if((int)gm_OHTMainStatus->m_cpMonitorData.pValue.lMem >= 0 && (int)gm_OHTMainStatus->m_cpMonitorData.pValue.lMem <= 40)
	{
	   pnMem->Caption = gm_OHTMainStatus->m_cpMonitorData.pValue.lMem;
	   pnMem->Color = clGreen;
	   pnMem->Visible  = true;
	}
	else if((int)gm_OHTMainStatus->m_cpMonitorData.pValue.lMem > 40 && (int)gm_OHTMainStatus->m_cpMonitorData.pValue.lMem <= 75)
	{
	   pnMem->Caption = gm_OHTMainStatus->m_cpMonitorData.pValue.lMem;
	   pnMem->Color = clYellow;
	   pnMem->Visible  = true;
	}
	else if((int)gm_OHTMainStatus->m_cpMonitorData.pValue.lMem >75)
	{
	   pnMem->Caption = gm_OHTMainStatus->m_cpMonitorData.pValue.lMem;
	   pnMem->Color = clRed;
       ...
       pnMem->Visible = bMToggle;
	}
#if((OHT_NAME == OHT_NAME_STD_V82)||(OHT_NAME == OHT_NAME_STD_V86)||(OHT_NAME == OHT_NAME_P4_MASK))
	if(m_pParamSet->OhtDetectSensorType == OHTDETECT_SOSLAB && GetUBGCheckComplete() == true)
	{
		if(m_pOHTDetect->GetVersion2Monitoring() == false)
		{
			strSOSLABver =m_pOHTDetect->SOS_FW_VersionCheck();
			lbSOSLAB->Caption =strSOSLABver;
			lbSOSLAB->Visible = true;
		}
	}
#endif
	// IoT Gateway 에러 일 경우 프로그램을 종료한다.
	if(m_bErrIotGateway ||m_bErrOHTTypeMatching || m_bErrHoistParamMatching || m_bErrDrivingVibrationMatching)
	{
        this->Close();
    }
}
...
// 2016-12-03, Add by Labeler
	...
	OHTInfo.OHTInfo.OHT_Ver = OHT_VERSION_86;
	OHTInfo.OHTInfo.OHT_Type = OHT_TYPE_EUV;
	...
	OHTInfo.OHTInfo.OHT_Ver = OHT_VERSION_82;
	if(m_pParamSet->nCarrierType == MAC_OHT)
		OHTInfo.OHTInfo.OHT_Type = OHT_TYPE_MAC;   //2023.11.16 sseung MacType 파라미터 존재시 재하타입 MAC으로 설정
	else
		OHTInfo.OHTInfo.OHT_Type = OHT_TYPE_FOUP;
#elif (OHT_NAME == OHT_NAME_P4_MASK)
	//sseung 2022-11-02 P4 Mask용
	OHTInfo.OHTInfo.OHT_Ver = OHT_VERSION_10;
	OHTInfo.OHTInfo.OHT_Type = OHT_TYPE_MASK;
  ...
  //	tmMainTask->Enabled = true;
	...
	String strOHTVer = "", strAMCVer = "", strAMCDLLVer = "", strFPGAVer="";
	int nAMCVer = 0;
	int nAMCDLLVer = 0;
	int nFPGAVer = 0;
//	AnsiString strSOSLABver = "";
	...
	else if(OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_MASK)     //sseung 2022-11-02 P4 Mask용
	{
	    strOHTVer.sprintf(L"OHT_MASK[Ver %u.%u.%u.%u]_%d",
	        (BYTE)OHTMainForm->VerInfo.Major,
	        (BYTE)OHTMainForm->VerInfo.Minor,
	        (BYTE)OHTMainForm->VerInfo.Release,
	        (BYTE)OHTMainForm->VerInfo.Build,
	...
	//2022.11.30 sseung 버전 관리 시스템용 Main 버전 입력
	Version_Info.Main_ver.sprintf("%u.%u.%u.%u",
						(BYTE)OHTMainForm->VerInfo.Major,
						(BYTE)OHTMainForm->VerInfo.Minor,
						(BYTE)OHTMainForm->VerInfo.Release,
						(BYTE)OHTMainForm->VerInfo.Build);
	Version_Info.Main_Date.sprintf(strVersion_Date.c_str());
	...
	//2022.11.30 sseung 버전 관리 시스템용 AMC 버전 입력
	if(!(nAMCVerMajor==0 && nAMCVerMinor==0 && nAMCVerRelease==0))
	{
		Version_Info.AMC_ver.sprintf("%d.%d.%02d", nAMCVerMajor, nAMCVerMinor, nAMCVerRelease);
		Version_Info.AMC_Date.sprintf(strVersion_Date.c_str());
	}

	nAMCDLLVer = m_HWSet->GetAMCDLLVersion();

	int nAMCDLLVerMajor = nAMCDLLVer / 10000;
	int nAMCDLLVerMinor = (nAMCDLLVer - (nAMCDLLVerMajor * 10000)) / 100;
	int nAMCDLLVerRelease = (nAMCDLLVer - (nAMCDLLVerMajor * 10000)) % 100;

	strAMCDLLVer.sprintf(L"AMCDLL[Ver %d.%d.%02d]", nAMCDLLVerMajor, nAMCDLLVerMinor, nAMCDLLVerRelease);

    //2022.11.30 sseung 버전 관리 시스템용 AMC DLL 버전 입력
	if(!(nAMCDLLVerMajor==0 && nAMCDLLVerMinor==0 && nAMCDLLVerRelease==0))
	{
		Version_Info.AMC_Dll_ver.sprintf("%d.%d.%02d", nAMCDLLVerMajor, nAMCDLLVerMinor, nAMCDLLVerRelease);
		Version_Info.AMC_Dll_Date.sprintf(strVersion_Date.c_str());
	}
	...
	// AMOHTV80F-1703
	char rsaRear[20];
	sprintf(rsaRear, "%x", m_HWSet->GetRSAnSlaveVersion(0)); // RSA Driving Rear
	nRSARearVer = atoi(rsaRear);
	// RSA Rear Version 정보 추출 및 문자열 포맷팅
	int nRSARearMajor = (nRSARearVer / 1000000) % 100;
	int nRSARearMinor = (nRSARearVer / 10000) % 100;
	int nRSARearRelease = (nRSARearVer / 100) % 100;
	int nRSARearBuild = nRSARearVer % 100;

	Version_Info.RSA_Driving_Rear_ver.sprintf("%d.%02d.%02d.%02d", nRSARearMajor, nRSARearMinor, nRSARearRelease, nRSARearBuild);
	Version_Info.RSA_Driving_Rear_Date.sprintf(strVersion_Date.c_str());

	char rsaFront[20];
	sprintf(rsaFront, "%x", m_HWSet->GetRSAnSlaveVersion(1)); // RSA Driving Front
	nRSAFrontVer = atoi(rsaFront);
	// RSA Front Version 정보 추출 및 문자열 포맷팅
	int nRSAFrontMajor = (nRSAFrontVer / 1000000) % 100;
	int nRSAFrontMinor = (nRSAFrontVer / 10000) % 100;
	int nRSAFrontRelease = (nRSAFrontVer / 100) % 100;
	int nRSAFrontBuild = nRSAFrontVer % 100;

	Version_Info.RSA_Driving_Front_ver.sprintf("%d.%02d.%02d.%02d", nRSAFrontMajor, nRSAFrontMinor, nRSAFrontRelease, nRSAFrontBuild);
	Version_Info.RSA_Driving_Front_Date.sprintf(strVersion_Date.c_str());

	char rsaHoist[20];
	sprintf(rsaHoist, "%x", m_HWSet->GetRSAnSlaveVersion(2)); // RSA Hoist
	nRSAHoistVer = atoi(rsaHoist);
	// RSA Hoist Version 정보 추출 및 문자열 포맷팅
	int nRSAHoistMajor = (nRSAHoistVer / 1000000) % 100;
	int nRSAHoistMinor = (nRSAHoistVer / 10000) % 100;
	int nRSAHoistRelease = (nRSAHoistVer / 100) % 100;
	int nRSAHoistBuild = nRSAHoistVer % 100;

	Version_Info.RSA_Hoist_ver.sprintf("%d.%02d.%02d.%02d", nRSAHoistMajor, nRSAHoistMinor, nRSAHoistRelease, nRSAHoistBuild);
	Version_Info.RSA_Hoist_Date.sprintf(strVersion_Date.c_str());

	char rsaSlide[20];
	sprintf(rsaSlide, "%x", m_HWSet->GetRSAnSlaveVersion(3)); // RSA Slide
	nRSASlideVer = atoi(rsaSlide);

	// RSA Slide Version 정보 추출 및 문자열 포맷팅
	int nRSASlideMajor = (nRSASlideVer / 1000000) % 100;
	int nRSASlideMinor = (nRSASlideVer / 10000) % 100;
	int nRSASlideRelease = (nRSASlideVer / 100) % 100;
	int nRSASlideBuild = nRSASlideVer % 100;

	Version_Info.RSA_Slide_ver.sprintf("%d.%02d.%02d.%02d", nRSASlideMajor, nRSASlideMinor, nRSASlideRelease, nRSASlideBuild);
	Version_Info.RSA_Slide_Date.sprintf(strVersion_Date.c_str());

	char fineRear[20];
	sprintf(fineRear, "%x", m_HWSet->GetRSAnSlaveVersion(4)); // Fine Rear
	nFineRearVer = atoi(fineRear);

	Version_Info.Fine_Slave_Rear_ver.sprintf("%d", nFineRearVer);
	Version_Info.Fine_Slave_Rear_Date.sprintf(strVersion_Date.c_str());

	char fineFront[20];
	sprintf(fineFront, "%x", m_HWSet->GetRSAnSlaveVersion(5)); // Fine Front
	nFineFrontVer = atoi(fineFront);

	Version_Info.Fine_Slave_Front_ver.sprintf("%d", nFineFrontVer);
	Version_Info.Fine_Slave_Front_Date.sprintf(strVersion_Date.c_str());
	...
	strFPGAVer.sprintf(L"FPGA[Ver %d.%d.%02d]", nFPGAVerMajor + m_bFpgaVersion, nFPGAVerMinor, nFPGAVerRelease);
	...
	lbAMCDLLVer->Caption = strAMCDLLVer;
	lblFPGA->Caption =strFPGAVer;
	lbSOSLAB->Visible = false;

	//Main 실행시간 Check를 위하여 추가
	SYSTEMTIME cst;
	GetLocalTime(&cst);
	String s;
	s.sprintf(L"Main Start : %04d-%02d-%02d [%02d:%02d:%02d]", cst.wYear, cst.wMonth, cst.wDay, cst.wHour, cst.wMinute, cst.wSecond);
	lblStartTime->Caption = s;
	Set_LogFolder(OHTUTILITY::Check_LogFolder()); //AOHC-300 D드라이브 상태 표시
}
//---------------------------------------------------------------------------
...
void TOHTMainForm::CreateClass()
{
	...
	pLogFirmware = getLogUnit("D:\\log\\FirmwareVersion.log", MAX_BYTE_SIZE_OF_MAIN_LOG);

	//by puting 2016 1126 : default param load
	m_pParamSet =  new DEFAULT_PARAM_SET();
	m_pLineParamSet = new DEFAULT_LINE_PARAM_SET(); // AOHC-285
	m_pAutoPatchParamSet = new DEFAULT_AUTOPATCH_PARAM_SET();  //AOHC-154
	memset(m_pParamSet, 0x00, sizeof(DEFAULT_PARAM_SET));
	memset(m_pLineParamSet, 0x00, sizeof(DEFAULT_LINE_PARAM_SET));
	memset(m_pAutoPatchParamSet, 0x00, sizeof(DEFAULT_AUTOPATCH_PARAM_SET));  //AOHC-154
	LoadParameter("DataFiles\\OHTPARAM.ini",m_pParamSet);
	LoadLineParameter("DataFiles\\LineParam.ini",m_pLineParamSet);
	LoadAutoPatchParameter("DataFiles\\AutoPatchControl.ini", m_pAutoPatchParamSet);  //AOHC-154
	LoadHighSpeedParameter("DataFiles\\HighSpeed.ini");
#if((OHT_NAME == OHT_NAME_STD_V82)||(OHT_NAME == OHT_NAME_STD_V86)||(OHT_NAME == OHT_NAME_P4_MASK))
	if(m_pParamSet->OhtDetectSensorType == OHTDETECT_HOKUYO)
	{
	...
	try
	{
		...
		if(m_bErrOHTTypeMatching ==false)
		{
			if(((m_dFinalHoistParam_Pulse == HOIST_PARAM_FOUP_V80) || (m_dFinalHoistParam_Pulse == HOIST_PARAM_FOUP_V80_V81_P1L_1F)) && (m_pLineParamSet->dLineHoistDistance_Foup < m_dFinalHoistParam_User))
			{
				m_bErrHoistParamMatching = false;
			}

		}
		else
		{
			m_bErrOHTTypeMatching = (strcmp(m_pParamSet->OHTType, OHTTYPE_FOUP80IM) != 0);
			if(m_bErrOHTTypeMatching ==false)
			{
				if(((m_dFinalHoistParam_Pulse == HOIST_PARAM_FOUP_V80IM) || (m_dFinalHoistParam_Pulse == HOIST_PARAM_FOUP_V80_V81_P1L_1F))  && (m_pLineParamSet->dLineHoistDistance_Foup < m_dFinalHoistParam_User))
				{
					m_bErrHoistParamMatching = false;
				}
			}
		...
		if(m_bErrOHTTypeMatching ==false)
		{
			if((m_dFinalHoistParam_Pulse == HOIST_PARAM_EUV_V80)  && (m_pLineParamSet->dLineHoistDistance_Euv < m_dFinalHoistParam_User))
			{
				m_bErrHoistParamMatching = false;
			}
		}
		...
		if(m_bErrOHTTypeMatching ==false)
		{
			if((m_dFinalHoistParam_Pulse == HOIST_PARAM_FOUP_V81S)  && (m_pLineParamSet->dLineHoistDistance_Foup < m_dFinalHoistParam_User))
			{
              	m_bErrHoistParamMatching = false;
			}
		}
#elif(OHT_NAME == OHT_NAME_STD_V85S)
		m_bErrOHTTypeMatching = (strcmp(m_pParamSet->OHTType, OHTTYPE_FOUP85S) != 0);
		if(m_bErrOHTTypeMatching ==false)
		{
			if((m_dFinalHoistParam_Pulse == HOIST_PARAM_FOUP_V85S)  && (m_pLineParamSet->dLineHoistDistance_Foup < m_dFinalHoistParam_User))
			{
				m_bErrHoistParamMatching = false;
			}
		}
#elif(OHT_NAME == OHT_NAME_STD_V82)
		m_bErrOHTTypeMatching = (strcmp(m_pParamSet->OHTType, OHTTYPE_FOUP82) != 0);
		if(m_bErrOHTTypeMatching ==false)
		{
			if((m_dFinalHoistParam_Pulse == HOIST_PARAM_FOUP_V82) && (m_pLineParamSet->dLineHoistDistance_Foup < m_dFinalHoistParam_User) && m_pParamSet->nCarrierType != MAC_OHT)
			{
				m_bErrHoistParamMatching = false;
			}
#if(USE_MAC == 1)
			else if(m_dFinalHoistParam_Pulse == HOIST_PARAM_MAC_V82 && (m_pLineParamSet->dLineHoistDistance_Foup < m_dFinalHoistParam_User))   //MAC 기어비 변경으로 Hoist Param 값 변경
			{
				m_bErrHoistParamMatching = false;
			}
#endif
		...
		{
			//V80IM에 대한부분 정의 확인.
			if(((m_dFinalHoistParam_Pulse == HOIST_PARAM_FOUP_V81) || (m_dFinalHoistParam_Pulse == HOIST_PARAM_FOUP_V80_V81_P1L_1F) || (m_dFinalHoistParam_Pulse==HOIST_PARAM_FOUP_V80_V81_P1L_4F)) && (m_pLineParamSet->dLineHoistDistance_Foup < m_dFinalHoistParam_User))
			{
				m_bErrHoistParamMatching = false;
			}
		}
...
#elif(OHT_NAME == OHT_NAME_STD_V86)
		m_bErrOHTTypeMatching = (strcmp(m_pParamSet->OHTType, OHTTYPE_EUV86) != 0);
		if(m_bErrOHTTypeMatching ==false)
		{
			if((m_dFinalHoistParam_Pulse == HOIST_PARAM_EUV_V86)  && (m_pLineParamSet->dLineHoistDistance_Euv < m_dFinalHoistParam_User))
			{
				m_bErrHoistParamMatching = false;
			}
		}
		...
		if(m_bErrOHTTypeMatching ==false)
		{
			if((m_dFinalHoistParam_Pulse == HOIST_PARAM_EUV_V85)  && (m_pLineParamSet->dLineHoistDistance_Euv < m_dFinalHoistParam_User))
			{
				m_bErrHoistParamMatching = false;
			}
		}
#elif(OHT_NAME == OHT_NAME_P4_MASK)
		m_bErrOHTTypeMatching = (strcmp(m_pParamSet->OHTType, OHTTYPE_MASK) != 0);
		if(m_bErrOHTTypeMatching ==false)
		{
		//sseung 2022-10-04 P4 Mask Test위해 임시 설정
//			if((m_dFinalHoistParam_Pulse == HOIST_PARAM_EUV_V85)  && (m_pLineParamSet->dLineHoistDistance_Euv < m_dFinalHoistParam_User))
//			{
				m_bErrHoistParamMatching = false;
//			}
	...
	else if(m_bErrHoistParamMatching == true)  //AOHC-285
	{
		AnsiString msg = "## Hoist Param이 맞지 않는 파일을 실행하였습니다. ##\n\n";
		msg += "OHT.exe 실행파일이 HoistParam에 맞늦지 확인하시기 바랍니다.\n";
		msg += "만약 OHT.exe 실행파일이 맞다면, OHTPARAM.ini의 [Hoist]의값을 확인하시기 바랍니다.";
		msg += "(pulse/User값 확인)\n";
		msg += "\n확인 후 확인버튼을 누르면 프로그램이 종료됩니다.\n";

		wchar_t wmsg[1024];
		msg.WideChar(wmsg, sizeof(wmsg));
		Application->MessageBox(wmsg, L"Hoist Param Error");
		Close();
		return;
	}
#if((OHT_NAME == OHT_NAME_STD_V82)||(OHT_NAME == OHT_NAME_STD_V86) ||(OHT_NAME == OHT_NAME_P4_MASK))
#else
	else if(m_pLineParamSet->bUseLowVibration != m_checkLowvibration)   //AOHC-285
	{
		m_bErrDrivingVibrationMatching = true;

		AnsiString msg = "## 대차패턴과 저진동파라미터와 맞지 않는 파일을 실행하였습니다. ##\n\n";
		msg += "대차 패턴 센서 정보와 OHTDetectAreaInfo.ini에 맞늦지 확인하시기 바랍니다.\n";
		msg += "만약 대차 패턴 센서 정보가 맞다면, OHTDetectAreaInfo.ini을 맞게 수정 바랍니다.";
		msg += "(지진동 파라미터및 대차패턴 확인)\n";
		msg += "\n확인 후 확인버튼을 누르면 프로그램이 종료됩니다.\n";
...
#if((OHT_NAME == OHT_NAME_STD_V82) && (USE_MAC == 1))
	if(m_pParamSet->nCarrierType != MAC_OHT)
	{
		AnsiString msg = "## MAC Type 설정 오류 ##\n\n";
		msg += "MAC 재하를 사용하는 호기입니다.\n";
		msg += "OHTParam.INI의 [OHTINFO] CarrierType = 1\n위와 같이 설정하시기 바랍니다.\n";
		msg += "만약 MAC Type이 아니라면 USE_MAC을 확인 해주세요.\n";
		msg += "\n확인을 누르면 프로그램이 종료됩니다.\n";

		m_bErrMACType = true;
		wchar_t wmsg[1024];
		msg.WideChar(wmsg, sizeof(wmsg));
		Application->MessageBox(wmsg, L"MAC Type Error");
		Close();
		return;
	}
#elif((OHT_NAME == OHT_NAME_STD_V82) && (USE_MAC == 0))
	if(m_pParamSet->nCarrierType == MAC_OHT)
	{
		AnsiString msg = "## MAC Type 설정 오류 ##\n\n";
		msg += "OHTParam.INI의 [OHTINFO] CarrierType = 1로 설정된것 같습니다.\n";
		msg += "FOUP 호기라면 [OHTINFO] CarrierType = 0 \n위와 같이 설정하시기 바랍니다.\n.";
		msg += "만약 MAC 호기라면 USE_MAC을 확인 해주세요.\n";
		msg += "\n확인을 누르면 프로그램이 종료됩니다.\n";
...
//	m_CmdList = new LList;
	...
	m_UDPComm = new UdpCom(m_pLineParamSet->nPortNumAdd); //AOHC-285
...
//	m_MapMakerFile_STD = new STD_MapMakerFile;
	...
	m_NetworkDll = new NetworkSinalLib();	// 이준IPC무선랜 Reset기능 추가. mgchoi. AMOHTV80F-1376
	...
	m_CarrierID = new CarrierID();

	if(m_pParamSet->bUse4ChBlackBox == true)
	{
		m_BlackBox = new BlackBox();
    }

	if(m_pParamSet->bUseVMS)
	{
    	m_VMS = new VMS();
	}
...
//	m_MonitorControlThread->beginThread();
  //	TEST_ADD_LOG("CreateClass 01 m_socketThread = %d", m_socketThread);
	...
	if(m_pParamSet->bUseRSA==true)
	{
		if(m_HWSet != NULL)
			m_pRSAControl = m_HWSet->m_pRSAMonitoring;
	}
	else
	{
		if(m_HWSet != NULL)
			m_pServoControl = m_HWSet->m_pServoMonitoring;
	}
#endif


	gm_OHTMainStatus->StatusCommon.MSCmdID.Info.Mode = MODE_MANUAL;
/////////////대차 Power Off 상태로 대차 IO 정상 여부 확인///////////////////
#if((OHT_NAME == OHT_NAME_STD_V82)||(OHT_NAME == OHT_NAME_STD_V86)||(OHT_NAME == OHT_NAME_P4_MASK) )
	if(m_pParamSet->OhtDetectSensorType == OHTDETECT_SOSLAB)
	{
		if(CheckUBGInputStatus() != 0)
		{
			AnsiString msg = "##UBG Power Off 시 UBG IO 전체 On되지 못했습니다.##\n\n";
			msg += "UBG IO 센서 점검이 필요합니다.\n";
			msg += "\n확인을 누르고 대차 센서 점검해주세요.\n";
			msg += "\n대차 센서 미점검 시 오토 전환 불가합니다.\n";

			wchar_t wmsg[1024];
			msg.WideChar(wmsg, sizeof(wmsg));
			Application->MessageBox(wmsg, L"UBG IO Status Error");
		}

		Sleep(1000*10);  // UBG 전원 off,on 시 시리얼 정상화 될때까지 시간 필요

		if(CheckUBGIOFinalStatus()!= 0)    // UBG 전원 on 이후 UBG IO 정상인지 확인
		{
			m_CheckUBGIOFinalResult = ERR_DETECT_SENSOR_FAIL;
		}
	}
#endif
/////////////////////////////////////////////////////////////////////////
	//AVS dll Check
	if(m_pParamSet->bUseAVS)
	{
		ADD_EXCEPT_LOG("AVS Paramameter OK");
		ADD_EXCEPT_LOG("AVS Class NEW");
		m_AvsControl = new AvsControl();
		//Dll Load 성공
		if(!m_AvsControl->bDllLoadFail)
		{
			ADD_EXCEPT_LOG("AVS Dll Load OK");
			ADD_EXCEPT_LOG("AVS Thread Class NEW");
			ADD_EXCEPT_LOG("AVS Begin Thread");
			m_AvsControlThread = new AvsControlThread();
			m_AvsControlThread->setPriority(THREAD_PRIORITY_NORMAL);
			m_AvsControlThread->beginThread();
		}
		//Dll Load Fail
		else
		{
			ADD_EXCEPT_LOG("AVS Dll Load FAIL");
			ADD_EXCEPT_LOG("AVS Class Delete");
			m_pParamSet->bUseAVS = false;
	...
	{
		ADD_EXCEPT_LOG("No AVS Paramameter");
	}

	if(m_pParamSet->bUse4ChBlackBox == true)
	{
		m_BlackBoxThread = new BlackBoxThread();
		m_BlackBoxThread->setPriority(THREAD_PRIORITY_NORMAL);
		m_BlackBoxThread->beginThread();
    }

	if(m_pParamSet->bUseVMS)
	{
		m_VMSThread = new VMSThread();
		m_VMSThread->setPriority(THREAD_PRIORITY_NORMAL);
		m_VMSThread->beginThread();
	}

	//BCR Check
	if(m_pParamSet->bUseLeuze)
	{
		m_HWSet->m_pTransBCR->SetUseLeuze(true);
		m_HWSet->m_pDrivingBCR->SetUseLeuze(true);
	}

	if(m_pParamSet->bUse_CID_FIBER_MONITORING)
	{
		m_DrivingControl->SetUseCIDMonitoring(true, (int)m_pParamSet->nCID_SERIAL_CMD_DELAY_TURN);
		m_HWSet->m_pCID->SetUseCIDMonitoring(true);
	}

	if(m_pParamSet->bUseVMS)
	{
		gm_OHTMainStatus->strVMSBTId_1 = m_pParamSet->strBlueTooth1_DeviceID;
		gm_OHTMainStatus->strVMSBTId_2 = m_pParamSet->strBlueTooth2_DeviceID;
    }
...
int TOHTMainForm::CheckUBGIOFinalStatus()
{
	if(m_HWCommon->m_Input.OHTDETECT_Fail == false && m_HWCommon->m_Input.OHTDETECT_Long == false && m_HWCommon->m_Input.OHTDETECT_Middle == false &&m_HWCommon->m_Input.OHTDETECT_Short==false
	&& m_HWCommon->m_Output.OHTDETECT_Area1_Sel == true && m_HWCommon->m_Output.OHTDETECT_Area2_Sel == true && m_HWCommon->m_Output.OHTDETECT_Area3_Sel == true && m_HWCommon->m_Output.OHTDETECT_Area4_Sel == true)
	{
		return 0;
	}
	else
...
int TOHTMainForm::CheckUBGInputStatus()
{
	int nError = NO_ERR;
	int nCheckCnt =0;
	//대차 Power Off
	nError = m_pOHTDetect-> Disable();
	while(nCheckCnt < 10)
	{
		if(m_HWCommon->m_Input.OHTDETECT_Fail == true && m_HWCommon->m_Input.OHTDETECT_Long == true && m_HWCommon->m_Input.OHTDETECT_Middle == true &&m_HWCommon->m_Input.OHTDETECT_Short==true)
		{
			//센서 정상 체크 후 대차 Power On
			m_pOHTDetect-> Enable();
			SetUBGCheckComplete(true);
			nError = NO_ERR;
			ADD_EXCEPT_LOG("[정상]m_HWCommon->m_Input.OHTDETECT_Fail : %d, m_HWCommon->m_Input.OHTDETECT_Long : %d, m_HWCommon->m_Input.OHTDETECT_Middle : %d, m_HWCommon->m_Input.OHTDETECT_Short : %d",m_HWCommon->m_Input.OHTDETECT_Fail, m_HWCommon->m_Input.OHTDETECT_Long, m_HWCommon->m_Input.OHTDETECT_Middle, m_HWCommon->m_Input.OHTDETECT_Short);
		...
		else
		{
			nError = -1;
			nCheckCnt++;
			SetUBGCheckComplete(false);
			ADD_EXCEPT_LOG("nCheckCnt : %d",nCheckCnt);
			ADD_EXCEPT_LOG("m_HWCommon->m_Input.OHTDETECT_Fail : %d, m_HWCommon->m_Input.OHTDETECT_Long : %d, m_HWCommon->m_Input.OHTDETECT_Middle : %d, m_HWCommon->m_Input.OHTDETECT_Short : %d",m_HWCommon->m_Input.OHTDETECT_Fail, m_HWCommon->m_Input.OHTDETECT_Long, m_HWCommon->m_Input.OHTDETECT_Middle, m_HWCommon->m_Input.OHTDETECT_Short);
		}
		Sleep(20);    //amc thread 16msec 만큼 Sleep
	}

	return nError;
}
void TOHTMainForm::SetUBGCheckComplete(bool set)
{
	m_bUBGIOCheckComplete = set;
}
bool TOHTMainForm::GetUBGCheckComplete()
{
   return m_bUBGIOCheckComplete;
}
//---------------------------------------------------------------------------
...
//	delete m_CmdList;
	...
	//Case 1 . Main 실행할 때, AVS dll Load Fail이 났을 경우, 클래스 Delete
	if(m_AvsControl != NULL)
	{
		if(m_AvsControl->bDllLoadFail)
		{
			delete m_AvsControl;
	...
	//Case 2 . AVS Control Class 가 정상적으로 생성된 경우
	// 쓰레드 먼저 종료 후, AVS Class Delete
	if(m_AvsControlThread != NULL)
	{
		m_AvsControlThread->qiutRealThreadProc();
		while(1)
		{
			if(m_AvsControlThread->isOnThreading() == false)
			{
				break;
			}
			Sleep(1);
		}
		delete m_AvsControlThread;
		m_AvsControlThread = NULL;
	}
	if(m_AvsControl != NULL)
	{
	...
	if(m_pParamSet->bUse4ChBlackBox == true)
	{
		if(m_BlackBoxThread != NULL)
		{
			m_BlackBoxThread->qiutRealThreadProc();
			while(1)
			{
				if(m_BlackBoxThread->isOnThreading() == false)
				{
					break;
				}
				Sleep(1);
			}
			delete m_BlackBoxThread;
			m_BlackBoxThread = NULL;
		}
	...
	if(m_pParamSet->bUseVMS)
	{
		if(m_VMSThread != NULL)
		{
			m_VMSThread->qiutRealThreadProc();
			while(1)
			{
				if(m_VMSThread->isOnThreading() == false)
				{
					break;
				}
				Sleep(1);
	...
	delete m_pParamSet;
	delete m_pAutoPatchParamSet;
    delete m_pLineParamSet;
...
{
	if(m_pParamSet->bUseVisionTeaching == false)
	{
...
int TOHTMainForm::GetReadyToOperation()
{
	...
	if(nError == NO_ERR)
	{
#if((OHT_NAME == OHT_NAME_STD_V85) || (OHT_NAME == OHT_NAME_STD_V85S) ||(OHT_NAME == OHT_NAME_STD_V82)||(OHT_NAME == OHT_NAME_STD_V86))
...
#elif(OHT_NAME == OHT_NAME_P4_MASK)
		if(m_HWSet->m_pInnerFoup->IsFoupDetect() == false)
		{
			if(m_HWSet->m_pHoistAxis->IsHoistEUVHome() == false)
			{
				nError = ERR_HOIST_NOTORIGIN;
			}
		}
...
// 나중에 꼭 풀어줘야 한다
	...
	if(nError == NO_ERR)
	{
		...
		(gm_ExecuteInfo->ExecutePositionInfo.StationType == MARK_TYPE_QR_LEFT_TOP) ||
		(gm_ExecuteInfo->ExecutePositionInfo.StationType == MARK_TYPE_QR_LEFT_BOTTOM) ||
		(gm_ExecuteInfo->ExecutePositionInfo.StationType == MARK_TYPE_QR_LEFT_BOTTOM_S) ||
		(gm_ExecuteInfo->ExecutePositionInfo.StationType == MARK_TYPE_QR_RIGHT_TOP) ||
		(gm_ExecuteInfo->ExecutePositionInfo.StationType == MARK_TYPE_QR_RIGHT_BOTTOM) ||
		(gm_ExecuteInfo->ExecutePositionInfo.StationType == MARK_TYPE_QR_RIGHT_BOTTOM_S) ||
		(gm_ExecuteInfo->ExecutePositionInfo.StationType == MARK_TYPE_QR_PSTB_LEFT) ||
		(gm_ExecuteInfo->ExecutePositionInfo.StationType == MARK_TYPE_QR_PSTB_RIGHT) ||
...
void __fastcall TOHTMainForm::chkAxisClick(TObject *Sender)
{
	...
	case AXIS_MASK_FUNC_SPEED:
		if(bChecked==true)
		{
			pMaskOfAxis->Speed.Value    = nValue;
			pMaskOfAxis->Speed.bUse     = true;
		}
		else
		{
			pMaskOfAxis->Speed.bUse     = false;
		}
		break;
...
//AOHC-285
DEFAULT_LINE_PARAM_SET* TOHTMainForm::GetLineParam()
{
	if(this->m_pLineParamSet == NULL)
		return NULL;
	else
		return this->m_pLineParamSet;
}

CarrierID* TOHTMainForm::GetCarrierID()
{
	if(this->m_CarrierID == NULL)
		return NULL;
    else
        return this->m_CarrierID;
}

DEFAULT_AUTOPATCH_PARAM_SET* TOHTMainForm::GetAutoPatchParam()  //AOHC-154
{
	if(this->m_pAutoPatchParamSet == NULL)
		return NULL;
	else
		return this->m_pAutoPatchParamSet;
...
// 이준IPC무선랜 Reset기능 추가. mgchoi. AMOHTV80F-1376
NetworkSinalLib* TOHTMainForm::GetNetworkDll()
{
	if(this->m_NetworkDll == NULL)
		return NULL;
	else
		return this->m_NetworkDll;
}
//---------------------------------------------------------------------------
...
void __fastcall TOHTMainForm::FormCloseQuery(TObject *Sender, bool &CanClose)
{
	...
	{
		CanClose = true;
		MAIN_EXIT_LOG("======== OHT MAIN PROGRAM TERMINATING  -  IOT Gateway Error =======");
        bCritical = true;
		return;
	}else if(m_bErrOHTTypeMatching)
	{
		CanClose = true;
		MAIN_EXIT_LOG("======== OHT MAIN PROGRAM TERMINATING  -  OHT Type Error =======");
        bCritical = true;
		return;
	}
	else if(m_bErrHoistParamMatching)
	{
		CanClose = true;
		MAIN_EXIT_LOG("======== OHT MAIN PROGRAM TERMINATING  -  Hoist Param Error =======");
        bCritical = true;
		return;
	}
	else if(m_bErrDrivingVibrationMatching)
	{
		CanClose = true;
		MAIN_EXIT_LOG("======== OHT MAIN PROGRAM TERMINATING  -  Low Vibration Error =======");
		bCritical = true;
		return;
	}
	else if(m_bErrMACType)
	{
               CanClose = true;
	        MAIN_EXIT_LOG("======== OHT MAIN PROGRAM TERMINATING  -  MAC Type  Error =======");
		bCritical = true;
		return;
        }

	MAIN_EXIT_LOG("FormCloseQuery Enter");
	...
	if(gm_OHTMainStatus->StatusCommon.MSCmdID.Info.Mode == MODE_MANUAL)
	{
		...
		MAIN_EXIT_LOG("Try Exit - manual mode");
		...
		{
			MAIN_EXIT_LOG("Close Cancled");
			CanClose = false;
		}
		else
		{
            MAIN_EXIT_LOG("======== OHT MAIN PROGRAM TERMINATING  -  MANUAL MODE =======");
			CanClose = true;
	...
	else
	{
		...
		MAIN_EXIT_LOG("FormCloseQuery_Interlock");
...
bool TOHTMainForm::LoadParameter(char* szFilePath, DEFAULT_PARAM_SET* pParamSet)
{
	...
	if(pIniFile != NULL)
	{
		...
		pIniFile->ReadInteger("OHTInfo", "CarrierType", 0, &nTmpValue); //V8.2 FOUP, MAC, NRDK 구분
		pParamSet->nCarrierType = nTmpValue;
#endif
		...
		pIniFile->ReadInteger("OHTInfo", "IsIOTHub", 1, &nTmpValue); //add P1L VS P1_2L & P1_3L 구분 파라미터 #JRespect 2018.01.09
		...
		pIniFile->ReadString("OHTInfo","YSCode", pParamSet->VHLName, strTempValue);   //2022.12.20 sseung 버전 관리 시스템용 구분자 YS-Code 파라미터 추가
		memmove(pParamSet->YSCode, strTempValue, sizeof(pParamSet->YSCode)-1); //YS-Code 파람이 없는경우 VHLName으로 입력
		memset(strTempValue, 0, sizeof(strTempValue));
...
#elif(OHT_NAME == OHT_NAME_STD_V82)
//		pIniFile->ReadInteger("OHTInfo", "MacType", 0, &nTmpValue);   //2023.11.10 sseung Mac VHL 여부 확인
//		pParamSet->bMacType = nTmpValue;
#endif
		...
		pIniFile->ReadIntegerMinMaxCheck("WIRELESS","WlanCheckCount", 1, 1, 5, &nTmpValue);
		pParamSet->nWlanCheckCount = nTmpValue;
		...
		pIniFile->ReadString("DiagAgentInfo","IPAddress", "192.167.0.25", strTempValue);
		pParamSet->strDiagIpAddr.sprintf("%s", strTempValue);


		pIniFile->ReadInteger("DiagAgentInfo", "Port", 9621, &nTmpValue);
		...
		pIniFile->ReadDouble("HOIST", "dPosition1_pulse", 33000.0, &dTmpValue);   	//STB
		pParamSet->HoistDefault.PositionValue[1].dPulse = dTmpValue;

		pIniFile->ReadDouble("HOIST", "dPosition1_User", 110.0, &dTmpValue);
		pParamSet->HoistDefault.PositionValue[1].dUserUnit = dTmpValue;

		pIniFile->ReadDouble("HOIST", "dPosition2_pulse", 220000.0, &dTmpValue);
		pParamSet->HoistDefault.PositionValue[2].dPulse = dTmpValue;

		pIniFile->ReadDouble("HOIST", "dPosition2_User", 650.0, &dTmpValue);      	//STK
		pParamSet->HoistDefault.PositionValue[2].dUserUnit = dTmpValue;

		pIniFile->ReadDouble("HOIST", "dPosition3_pulse", 500000.0, &dTmpValue);    //Centor
		pParamSet->HoistDefault.PositionValue[3].dPulse = dTmpValue;

		pIniFile->ReadDouble("HOIST", "dPosition3_User", 1470.0, &dTmpValue);
		pParamSet->HoistDefault.PositionValue[3].dUserUnit = dTmpValue;

		pIniFile->ReadDouble("HOIST", "dPosition4_pulse", 800000.0, &dTmpValue);    //Centor ~ EQ Start 사이값
		pParamSet->HoistDefault.PositionValue[4].dPulse = dTmpValue;

		pIniFile->ReadDouble("HOIST", "dPosition4_User", 2400.0, &dTmpValue);
		pParamSet->HoistDefault.PositionValue[4].dUserUnit = dTmpValue;

		pIniFile->ReadDouble("HOIST", "dPosition5_pulse", 1000000.0, &dTmpValue);  //EQ Start
		pParamSet->HoistDefault.PositionValue[5].dPulse = dTmpValue;

		pIniFile->ReadDouble("HOIST", "dPosition5_User", 2900.0, &dTmpValue);
		pParamSet->HoistDefault.PositionValue[5].dUserUnit = dTmpValue;

		pIniFile->ReadDouble("HOIST", "dPosition6_pulse", 1150000.0, &dTmpValue);  //EQ End
		pParamSet->HoistDefault.PositionValue[6].dPulse = dTmpValue;
		m_dFinalHoistParam_Pulse =  dTmpValue;  //AOHC-285

		pIniFile->ReadDouble("HOIST", "dPosition6_User", 3200.0, &dTmpValue);
		pParamSet->HoistDefault.PositionValue[6].dUserUnit = dTmpValue;
		m_dFinalHoistParam_User = dTmpValue;    //AOHC-285
...
//		pParamSet->ShiftDefault.dDeviceDifference = 0.000000;
		...
		pIniFile->ReadBoolVerification("AVS", "bUseAVS", false, &bTmpValue);
		pParamSet->bUseAVS = bTmpValue;

		pIniFile->ReadInteger("REGULATOR", "RegulatorCheck", 0, &nTmpValue);
		pParamSet->bRegulatorCheck = nTmpValue;

		pIniFile->ReadInteger("BLACKBOX", "Useblackbox", 0, &nTmpValue);   //AOHC-343
		pParamSet->bUseblackbox = nTmpValue;

		pIniFile->ReadInteger("AUTOTEACHING", "UseVisionTeaching", 0, &nTmpValue);   //AOHC-343
		pParamSet->bUseVisionTeaching = nTmpValue;

		pIniFile->ReadBoolVerification("BCR", "bUseLeuze", false, &bTmpValue);
		pParamSet->bUseLeuze = bTmpValue;

		pIniFile->ReadBoolVerification("UBG", "bUseUBGMonitoring", false, &bTmpValue);
		pParamSet->bUseUBGMonitoring = bTmpValue;

		pIniFile->ReadBoolVerification("CID", "CID_FIBER_MONITORING", false, &bTmpValue);
		pParamSet->bUse_CID_FIBER_MONITORING = bTmpValue;

		if(bTmpValue)
		{
			pIniFile->ReadIntegerMinMaxVerification("CID", "CID_SERIAL_CMD_DELAY_TURN", 10, 7, 100, &nTmpValue);
			pParamSet->nCID_SERIAL_CMD_DELAY_TURN = nTmpValue;
		}

		#if((OHT_NAME == OHT_NAME_STD_V82)||(OHT_NAME == OHT_NAME_STD_V86)||(OHT_NAME == OHT_NAME_P4_MASK)) //timy check!! 네이밍
		pIniFile->ReadInteger("OHTDETECT", "Type", 0, &nTmpValue);
		pParamSet->OhtDetectSensorType = nTmpValue;
		#endif

		pIniFile->ReadInteger("IPC", "Type", 0, &nTmpValue);
		pParamSet->IPCType = nTmpValue;

#if((OHT_NAME == OHT_NAME_STD_V80) ||(OHT_NAME == OHT_NAME_STD_V81) || (OHT_NAME == OHT_NAME_STD_V81S)  ||(OHT_NAME == OHT_NAME_STD_V85)|| (OHT_NAME == OHT_NAME_STD_V80_RETICLE))
		pIniFile->ReadBoolVerification("SERVO", "UseRSA", false, &bTmpValue);
		pParamSet->bUseRSA = bTmpValue;

		pIniFile->ReadBoolVerification("IPC", "SOM6869", false, &bTmpValue);
		pParamSet->bSOM6869 = bTmpValue;
#else
		pParamSet->bUseRSA = false;
		pParamSet->bSOM6869 = false;

#endif

		// VMS
		pIniFile->ReadBoolVerification("VMS", "bUseVMS", false, &bTmpValue);
		pParamSet->bUseVMS = bTmpValue;

		pIniFile->ReadInteger("VMS", "BlueTooth1_StationID", 0, &nTmpValue);
		pParamSet->nBlueTooth1_StationID = nTmpValue;

		pIniFile->ReadString("VMS", "BlueTooth1_DeviceID", "000000000000", strTempValue);
		pParamSet->strBlueTooth1_DeviceID.sprintf("%s", strTempValue);

		pIniFile->ReadInteger("VMS", "BlueTooth2_StationID", 0, &nTmpValue);
		pParamSet->nBlueTooth2_StationID = nTmpValue;

		pIniFile->ReadString("VMS", "BlueTooth2_DeviceID", "000000000000", strTempValue);
		pParamSet->strBlueTooth2_DeviceID.sprintf("%s", strTempValue);

		pIniFile->ReadInteger("BLACKBOX", "Use4ChBlackBox", 0, &nTmpValue);   //AOHC-343
		pParamSet->bUse4ChBlackBox = nTmpValue;

		pIniFile->ReadString("BLACKBOX","IpAddress", "199.10.100.21", strTempValue);
		memmove(&(pParamSet->BlackBoxIP), strTempValue, strlen(strTempValue)); //Return Value
		memset(strTempValue, 0, sizeof(strTempValue));
...
//		strcpy(pParamSet->LineType, "00000");
		...
		pParamSet->bUseAVS = false;
		pParamSet->bUseblackbox = false;
		pParamSet->bUseVisionTeaching = false;

		pParamSet->bUse4ChBlackBox = false;
		strcpy(pParamSet->BlackBoxIP, "199.10.100.21");
		...
		m_dFinalHoistParam_Pulse = 780000.0; //AOHC-285
		m_dFinalHoistParam_User = 3579.0;    //AOHC-285
#ifdef OHT_HIGH_RAIL_HOIST_PRAM
		...
		m_dFinalHoistParam_Pulse = 1290000.0; //AOHC-285
		m_dFinalHoistParam_User = 5556.0;    //AOHC-285
#endif
		...
		pParamSet->bRegulatorCheck = 0;
#if((OHT_NAME == OHT_NAME_STD_V82)||(OHT_NAME == OHT_NAME_STD_V86) || (OHT_NAME == OHT_NAME_P4_MASK))
		pParamSet->OhtDetectSensorType = 0;
#endif
		pParamSet->IPCType = 0;
		pParamSet->bUseRSA = false;
		pParamSet->bSOM6869 = false;
		...
		// VMS
		pParamSet->bUseVMS = false;
		pParamSet->nBlueTooth1_StationID = 0;
		pParamSet->nBlueTooth2_StationID = 0;
	}
...
//AOHC-285
bool TOHTMainForm::LoadLineParameter(char* szFilePath, DEFAULT_LINE_PARAM_SET* pParamSet)
{
	bool bSuccess = false;
	int nTmpValue = 0;
	double dTmpValue = 0.0;
	...
	if(pIniFile != NULL)
	{
		...
		pIniFile->ReadInteger("LineInfo", "UseLowVibration", 0, &nTmpValue);
		pParamSet->bUseLowVibration = nTmpValue;

		pIniFile->ReadInteger("LineInfo", "UseTransProfile", 0, &nTmpValue);
		pParamSet->bUseTransProfile = nTmpValue;

#if((OHT_NAME == OHT_NAME_STD_V85)||(OHT_NAME == OHT_NAME_STD_V86))
		pParamSet->bUseTransProfile = 1;
#endif
		pIniFile->ReadInteger("LineInfo", "UseCarrierID", 0, &nTmpValue);
		pParamSet->bUseCarrierID = nTmpValue;

		pIniFile->ReadInteger("LineInfo", "UseOptionalNode", 0, &nTmpValue);
		pParamSet->bUseOptionalNode = nTmpValue;

		pIniFile->ReadDouble("LineInfo", "LineHoistDistance_Foup", 5500.0, &dTmpValue);
		pParamSet->dLineHoistDistance_Foup = dTmpValue;

		pIniFile->ReadDouble("LineInfo", "LineHoistDistance_Euv", 5500.0, &dTmpValue);
		pParamSet->dLineHoistDistance_Euv = dTmpValue;

		pIniFile->ReadInteger("LineInfo", "PortNumAdd", 0, &nTmpValue);
		pParamSet->nPortNumAdd = nTmpValue;

		pIniFile->ReadInteger("LineInfo", "UseFoupCoverOepnwaitingInterlock", 0, &nTmpValue);   //AOHC-343
		pParamSet->bUseFoupCoverOepnwaitingInterlock = nTmpValue;

//		pIniFile->ReadInteger("LineInfo", "UseE84TimeoutErrorCode", 0, &nTmpValue);   //AOHC-343
		pParamSet->bUseE84TimeoutErrorCode = true;  // V8.X  V1L 사용, 평택 미사용 → 전라인(화성, V1L) 사용으로 동일적용

#if((OHT_NAME == OHT_NAME_STD_V85) || (OHT_NAME == OHT_NAME_STD_V80_RETICLE)||(OHT_NAME == OHT_NAME_STD_V86))
		pParamSet->bUseFoupCoverOepnwaitingInterlock = false;
#endif
		delete pIniFile;
		bSuccess = true;
	...
	else
	{
		...
		//평택 P1 기준으로 설정
		pParamSet->bUseLowVibration = 0;
#if((OHT_NAME == OHT_NAME_STD_V85)||(OHT_NAME == OHT_NAME_STD_V86))
		pParamSet->bUseTransProfile = 1;
#else
		pParamSet->bUseTransProfile = 0;
#endif
		pParamSet->bUseCarrierID = 0;
		pParamSet->bUseOptionalNode = 0;
		pParamSet->dLineHoistDistance_Foup = 3100.0;
		pParamSet->dLineHoistDistance_Euv = 3100.0;
		pParamSet->nPortNumAdd = 0;
		pParamSet->bUseFoupCoverOepnwaitingInterlock = false;
		pParamSet->bUseE84TimeoutErrorCode = true;
...
//AOHC-154
bool TOHTMainForm::LoadAutoPatchParameter(char* szFilePath, DEFAULT_AUTOPATCH_PARAM_SET* pParamSet)
{
	bool bSuccess = false;
	int nTmpValue = 0;
	double dTmpValue = 0.0;
	...
	// AutoPatch 시 Flag on으로 업데이트 -> AutoPatch 완료 후 Auto 전환 완료 시 Flag Off 업데이트
	...
	if(pIniFile != NULL)
	{
		pIniFile->ReadInteger("AutoPatch", "ErrorControl", 0, &nTmpValue);
		pParamSet->bAutoPatchControl = nTmpValue;
...
int TOHTMainForm::ChangeAutoPatchParam(int UseIndex, bool YesOrNo)
{
	...
	{
		 case 0 :
		   m_pAutoPatchParamSet->bAutoPatchControl  = YesOrNo ;
		   break;
	...
	if(Error ==NO_ERR )
	{
		saveAutoPatchParameter("DataFiles\\AutoPatchControl.ini",m_pAutoPatchParamSet );
	}
...
bool TOHTMainForm::saveAutoPatchParameter(char* szFilePath, DEFAULT_AUTOPATCH_PARAM_SET* pParamSet)
{
	 bool bResult = false;
    ...
    if(m_lWriteIniFile != NULL)
	{
		...
		m_lWriteIniFile->WriteInteger("AutoPatch","ErrorControl", pParamSet->bAutoPatchControl);
...
bool TOHTMainForm::saveAutoPatchParameterRecovery(char* szFilePath, DEFAULT_AUTOPATCH_PARAM_SET* pParamSet)
{
	 bool bResult = false;
	...
	{
		m_lWriteIniFile->DeleteIniFile();
		m_lWriteIniFile->WriteInteger("AutoPatch","ErrorControl", 0);
...
bool TOHTMainForm::HoistParamSaveParameter(PKT_TP2OHT_HOIST_TEACH_COMMAND PKT)
{
	...
	if(pIniFile != NULL)
	{
		...
		m_dFinalHoistParam_Pulse = m_pParamSet->HoistDefault.PositionValue[9].dPulse; //AOHC-285
		m_dFinalHoistParam_User = m_pParamSet->HoistDefault.PositionValue[9].dUserUnit;    //AOHC-285
		...
		m_dFinalHoistParam_Pulse = m_pParamSet->HoistDefault.PositionValue[6].dPulse; //AOHC-285
		m_dFinalHoistParam_User = m_pParamSet->HoistDefault.PositionValue[6].dUserUnit;    //AOHC-285
#endif
...
{
	MAIN_EXIT_LOG("======== OHT MAIN PROGRAM TERMINATING  -  FORCE CLOSE =======");
...
bool TOHTMainForm::LoadPatternData(char* szFilePath)
{
	...
	if(!pIniFile->GetIniFileExist())
	{
		ADD_UBG_CHK_LOG("INI File is not exist.");
		return false;
    }

	if(pIniFile == NULL)
	{
		ADD_UBG_CHK_LOG("INI File Load Fail");
		return false;
	}
	...
	if(nSize == 0)
	{
		...
		ADD_UBG_CHK_LOG("Old INI Version. Default 0~6 Pattern setting.");
		//delete pIniFile;
		//return false;
	...
	for(int a=0; a<nSize ; a++)
	{
		...
		for(int b=0; b<COUNTOF_PATTERN_ASCII ; b++)
		{
			...
			//AOHC-285
			if((nPtnIdx ==0) && (b == 47))    //0: 직선, 47 패턴의 마지막 차이점
			{
				if(nTmpVal ==60)    //저진동 직선패턴 마지막 차이점으로 확인(저진동 60, 저진동아님, 108)
					m_checkLowvibration = true;
				else
					m_checkLowvibration = false;
			}
...
void TOHTMainForm::AddUBGPattern(int nCmdFRIdx1234, char *szRecv)
{
	...
	{
		ADD_UBG_CHK_LOG("[ERROR] Add Pattern Index Range Over.");
		return;
    }
	...
	ADD_UBG_CHK_LOG("%s", szLogMsg);   // logging
...
bool TOHTMainForm::DoCompUBGPattern(int nPatternNo0to15)
{
	...
	{
		ADD_UBG_CHK_LOG("[ERROR] Comp Pattern Index Range Over.");
		return false;
	}
	if(m_bUBGINILoaded == false)
	{
		ADD_UBG_CHK_LOG("[ERROR] Comp Failed. INI is not loaded.");
		return false;
    }
...
bool TOHTMainForm::WriteUBGRecv_INI()
{
	...
	if(fp == NULL)
	{
		ADD_UBG_CHK_LOG("[ERROR] Can not save [OHTDetectAreaInfo_UBG_RECV.ini]");
		return false;
	}

	fprintf(fp, "[Setting_Recv]");
	fprintf(fp, "\n");
	...
	if(m_bUBGINILoaded)
	{
		...
		{
			int nPtnIdx = m_nArrCheckPattern[i];
			fprintf(fp, "[Pattern_Recv%d]", nPtnIdx+1);
			fprintf(fp, "\n");
	...
	fclose(fp);
	ADD_UBG_CHK_LOG("[SUCCESS] Writting OK. OHTDetectAreaInfo_UBG_RECV.ini");
...
void __fastcall TOHTMainForm::btnReadMonitorRegClick(TObject *Sender)
{
	...
	if(m_pParamSet->bUseRSA==true)
	{
		DWORD wRSAErrorCode = 0;

		m_pRSAControl->ReadErrorCode(axis, &wRSAErrorCode);
		m_pRSAControl->DisplayRSAErrorCode(wRSAErrorCode);
	}
	else
	{
...
void __fastcall TOHTMainForm::servoClick(TObject *Sender)
{
	...
	if(m_pParamSet->bUseRSA==true)
	{
		if(gm_ExecuteInfo->ExecuteMode != EXECUTE_MODE_AUTO)
		{
			...
			{
				m_pRSAControl->SetVelocityCommandOffset(1);
				m_pRSAControl->SetVelocityCommandOffset(2);
			}
			else
			{
				memoMain->Lines->Add("[RSA]Slope가 아닌 구간에 Mark해주세요");
			}
		}
		else
		{
			memoMain->Lines->Add("[RSA]Manual 전환해주세요");
		}
	}
	else
	{
		...
		&& (gm_ExecuteInfo->ExecutePositionInfo.Marked ==true ))
		{
			...
			if(!m_ControlTask->GetNodeTypeSlop())
			{
				m_pServoControl->SetFn009(1);
				m_pServoControl->SetFn009(2);
			}
			else
			{
...
#if((OHT_NAME == OHT_NAME_STD_V80) ||(OHT_NAME == OHT_NAME_STD_V81) || (OHT_NAME == OHT_NAME_STD_V81S)  ||(OHT_NAME == OHT_NAME_STD_V85)|| (OHT_NAME == OHT_NAME_STD_V80_RETICLE))
	if(m_pParamSet->bUseRSA==true)
		m_pRSAControl->SetInnerLimitTorque_Front();
	else
...
void __fastcall TOHTMainForm::btn_limit_rearClick(TObject *Sender)
{
	...
	if(m_pParamSet->bUseRSA==true)
		m_pRSAControl->SetInnerLimitTorque_Rear();
	else
...
void __fastcall TOHTMainForm::btnUBGDataClick(TObject *Sender)
{
	WriteUBGRecv_INI();    //AOHC-286
}

//---------------------------------------------------------------------------
BlackBox* TOHTMainForm::GetBlackBox()
{
	if(this->m_BlackBox == NULL)
        return NULL;
    else
...
//2022.12.30 sseung 버전관리 시스템 FW Version 수집 함수
void TOHTMainForm::GetFirmwareVersion()
{
	int nError = 0;
	int nFWnum = 0;  //버전 수집된 FW 개수
	int nSWnum = 0;  //버전 수집된 SW 개수

	FIRMWARE_VERSION_LOG("[Version Info Update] GetFirmwareVersion In");

	m_DrivingControl->GetCID_O_Version();

	if(Version_Info.Main_ver != "") nSWnum++;
	if(Version_Info.Assistant_ver != "") nSWnum++;
	if(Version_Info.FM_ver != "") nSWnum++;
	if(Version_Info.AMC_ver != "") nSWnum++;
	if(Version_Info.AMC_Dll_ver != "") nSWnum++;

	FIRMWARE_VERSION_LOG("AS Ver=%s, Date=%s", Version_Info.Assistant_ver, Version_Info.Assistant_Date);
	FIRMWARE_VERSION_LOG("FM Ver=%s, Date=%s", Version_Info.FM_ver, Version_Info.FM_Date);

	if(Version_Info.CID_O_ver == "")
	{
		nError = m_DrivingControl->GetCID_O_Version();    //CID에 Monitoring Cmd 재전송

		for(int i = 0; i<100; i++)
		{
			if(nError != 0) break;

			if(Version_Info.CID_O_ver != "") break;

			Sleep(20);
		}
		FIRMWARE_VERSION_LOG("CID_O Ver = %s", Version_Info.CID_O_ver);
	}
#if((OHT_NAME == OHT_NAME_STD_V82)||(OHT_NAME == OHT_NAME_STD_V86)||(OHT_NAME == OHT_NAME_P4_MASK))
	nError = m_HWSet->m_pLeftEQPIO->Get_EQPIO_Ver();

	if(nError == NO_ERR)
	{
		FIRMWARE_VERSION_LOG("EQPIO FW Ver = %s", Version_Info.EQPIO_ver);
	}

	if(Version_Info.Regulator_ver == "")
	{
		for(int i = 0; i<100; i++)
		{
			if(Version_Info.Regulator_ver != "") break;
			Sleep(50);       //regulator ver check시에 최대 2500ms까지 시간이 걸림
		}
	}
#endif
	if(Version_Info.CID_O_ver != "") nFWnum++;
#if((OHT_NAME == OHT_NAME_STD_V82)||(OHT_NAME == OHT_NAME_STD_V86)||(OHT_NAME == OHT_NAME_P4_MASK))
	if(Version_Info.EQPIO_ver != "") nFWnum++;
	if(Version_Info.LCD_ver != "") nFWnum++;
	if(Version_Info.Regulator_ver != "") nFWnum++;
	if(Version_Info.RSA_Driving_Rear_ver != "") nFWnum++;
	if(Version_Info.RSA_Driving_Front_ver != "") nFWnum++;
	if(Version_Info.RSA_Hoist_ver != "") nFWnum++;
	if(Version_Info.RSA_Slide_ver != "") nFWnum++;
	if(Version_Info.Fine_Slave_Rear_ver != "") nFWnum++;
	if(Version_Info.Fine_Slave_Front_ver != "") nFWnum++;

	FIRMWARE_VERSION_LOG("CID_O Ver = %s", Version_Info.CID_O_ver);
	FIRMWARE_VERSION_LOG("EQPIO Ver = %s", Version_Info.EQPIO_ver);
	FIRMWARE_VERSION_LOG("LCD Ver = %s", Version_Info.LCD_ver);
	FIRMWARE_VERSION_LOG("Regulator Ver = %s", Version_Info.Regulator_ver);
	FIRMWARE_VERSION_LOG("RSA Driving Rear Ver = %s", Version_Info.RSA_Driving_Rear_ver);
	FIRMWARE_VERSION_LOG("Regulator Ver = %s", Version_Info.RSA_Driving_Front_ver);
	FIRMWARE_VERSION_LOG("RSA Hoist Ver = %s", Version_Info.RSA_Hoist_ver);
	FIRMWARE_VERSION_LOG("RSA Slide Ver = %s", Version_Info.RSA_Slide_ver);
	FIRMWARE_VERSION_LOG("Fine Slave Rear Ver = %s", Version_Info.Fine_Slave_Rear_ver);
	FIRMWARE_VERSION_LOG("Fine Slave Front Ver = %s", Version_Info.Fine_Slave_Front_ver);
#endif
	//폴더가 없으면 만들고 0 출력, 이미 존재하거나 경로 문제면 -1 출력
	nError = mkdir("DataFiles\\VERSION_INFO");
	if(nError == 0) FIRMWARE_VERSION_LOG("VERSION_INFO 폴더 생성");

	//폴더가 새로 생성된 경우 파일 탐색 및 불러오기 미실행
	if(nError != 0) FindFileName("DataFiles\\VERSION_INFO\\*.txt");
	WriteVersionFile(nSWnum, nFWnum);
}

/*
 2022.12.30 sseung 버전관리 시스템 DB 파일 중복 확인 함수
 .FILEDB_UPDATE 이름이 포함된 파일이 한개인 경우 정상 Read 진행
 .FILEDB_UPDATE 이름이 포함된 파일이 없는 경우 return
 .FILEDB_UPDATE 이름이 포함된 파일이 한개 이상인 경우 전부 삭제 후 return
*/
void TOHTMainForm::FindFileName(char* path)
{
	FIRMWARE_VERSION_LOG("[Version Info Update] FindFileName IN");
	AnsiString tmpPath = "";
	_finddata_t fd;
	long fileExist;
	int result = 1;
	int nUpdateFileNum = 0;

	fileExist = _findfirst(path, &fd);

	if(fileExist == -1)
	{
		FIRMWARE_VERSION_LOG("파일 미존재");
		_findclose(fileExist);
		return;
	}

	while (result != -1)
	{
		FIRMWARE_VERSION_LOG("파일명 : %s", fd.name);
		if(strstr(fd.name, "_UPDATE") != NULL)
		{
			nUpdateFileNum++;

			if(nUpdateFileNum == 2) DeleteFile(tmpPath);  //파일이 여러개면 새로운 주소 생성 전 처음 찾은 파일 삭제

			tmpPath.sprintf("DataFiles\\VERSION_INFO\\%s", fd.name);  //파일 주소 생성

			//FILEDB_UPDATE 파일이 여러개인 경우 모두 삭제
			if(nUpdateFileNum > 1)
			{
				DeleteFile(tmpPath);
				FIRMWARE_VERSION_LOG("%s 파일 삭제", fd.name);
			}
		}

		result = _findnext(fileExist,&fd);
	}

	_findclose(fileExist);

	if(nUpdateFileNum > 1) return; //FILEDB_UPDATE 파일이 2개 이상이면 다 삭제하고 새로 만들기 때문에 파일 읽어오지 않음

	LoadVersionFile(tmpPath.c_str());
}

 void TOHTMainForm::LoadVersionFile(char* szFileName)
 {
	FIRMWARE_VERSION_LOG("[Version Info Update] LoadVersionFile IN");
	char* pFullPathResult = NULL;
	char m_szTmpbuffer[256];
...
//	//1. 초기화
	...
	FILE* fp;
	fp = fopen(szFileName, "r");    //읽기 버전으로 파일 열기

	if(fp == NULL)
	{
		FIRMWARE_VERSION_LOG("No File to Read");
		//fclose(fp);
		return;
	}
	else FIRMWARE_VERSION_LOG("Get File Success");
	...
	while(nline<20) //임시 값
	{
		...
		{
			nline++;
			char* cStr = fgets(m_szTmpbuffer, sizeof(m_szTmpbuffer), fp);   //한줄씩 읽어오기
			int nLnelen = strlen(m_szTmpbuffer);

			if(cStr == NULL)
			{
				FIRMWARE_VERSION_LOG("No More Line");
				break;
			}

			if(m_szTmpbuffer[nLnelen-1] == '\n') m_szTmpbuffer[nLnelen-1] = '\0';   //줄 끝 개행문자 제거
			FIRMWARE_VERSION_LOG("Get line = %s", m_szTmpbuffer);


			char* cut = strtok(m_szTmpbuffer, "$");
			cut = strtok(NULL, "$");  //2번째 항목 넣기
			char strName[20] = {0,};
			strcpy(strName, cut);
			FIRMWARE_VERSION_LOG("프로그램 명=%s, cut=%s", strName, cut);

			char* cVer;

			for(int i = 0; i < 7; i++)   //i==2:version, i==6:update date
			{
				cut = strtok(NULL, "$");

				FIRMWARE_VERSION_LOG("i=%d, strName=%s, 내용=%s", i, strName, cut);
				if(strcmp(strName, "OHT Main") == 0)
				{
					if(Version_Info.Main_ver == "") break;

					if(i==2)
					{
						cVer = Version_Info.Main_ver.c_str();

						//현재 버전과 update 파일의 버전이 다른 경우
						if(strcmp(cut, cVer) != 0) break; //버전 업데이트 되어 날짜까지 읽울 필요X
						else Version_Info.Main_ver.sprintf(cut);
					}
					else if(i==6) Version_Info.Main_Date.sprintf(cut);
				}
				else if(strcmp(strName, "OHT Assistant") == 0)
				{
					if(Version_Info.Assistant_ver == "") break;
					if(i==2)
					{
						cVer = Version_Info.Assistant_ver.c_str();
						if(strcmp(cut, cVer) != 0) break;
						else Version_Info.Assistant_ver.sprintf(cut);
					}
					else if(i==6) Version_Info.Assistant_Date.sprintf(cut);
				}
				else if(strcmp(strName, "OHT FileManager") == 0)
				{
					if(Version_Info.FM_ver == "") break;
					if(i==2)
					{
						cVer = Version_Info.FM_ver.c_str();
						if(strcmp(cut, cVer) != 0) break;
						else Version_Info.FM_ver.sprintf(cut);
					}
					else if(i==6) Version_Info.FM_Date.sprintf(cut);
				}
				else if(strcmp(strName, "AMC") == 0)
				{
					if(Version_Info.AMC_ver == "") break;
					if(i==2)
					{
						cVer = Version_Info.AMC_ver.c_str();
						if(strcmp(cut, cVer) != 0) break;
						else Version_Info.AMC_ver.sprintf(cut);
					}
					else if(i==6) Version_Info.AMC_Date.sprintf(cut);
				}
				else if(strcmp(strName, "AMC DLL") == 0)
				{
					if(Version_Info.AMC_Dll_ver == "") break;
					if(i==2)
					{
						cVer = Version_Info.AMC_Dll_ver.c_str();
						if(strcmp(cut, cVer) != 0) break;
						else Version_Info.AMC_Dll_ver.sprintf(cut);
					}
					else if(i==6) Version_Info.AMC_Dll_Date.sprintf(cut);
				}
				else if(strcmp(strName, "CID-O Controller") == 0)
				{
					if(Version_Info.CID_O_ver == "") break;
					if(i==2)
					{
						cVer = Version_Info.CID_O_ver.c_str();
						FIRMWARE_VERSION_LOG("CID 현재:%s, 읽어온 버전: %s", cVer, cut);
						//현재 버전과 update 파일의 버전이 다른 경우
						if(strcmp(cut, cVer) != 0) break;
						else Version_Info.CID_O_ver.sprintf(cut);
					}
					else if(i==6) Version_Info.CID_O_DATE.sprintf(cut);
				}
#if((OHT_NAME == OHT_NAME_STD_V82)||(OHT_NAME == OHT_NAME_STD_V86)||(OHT_NAME == OHT_NAME_P4_MASK))
				else if(strcmp(strName, "EQ-PIO Sensor") == 0)
				{
					if(Version_Info.EQPIO_ver == "") break;
					if(i==2)
					{
						cVer = Version_Info.EQPIO_ver.c_str();
						if(strcmp(cut, cVer) != 0) break;
						else Version_Info.EQPIO_ver.sprintf(cut);
					}
					else if(i==6) Version_Info.EQPIO_Date.sprintf(cut);
				}
				else if(strcmp(strName, "LCD Module") == 0)
				{
					if(Version_Info.LCD_ver == "") break;
					if(i==2)
					{
						cVer = Version_Info.LCD_ver.c_str();
						if(strcmp(cut, cVer) != 0) break;
						else Version_Info.LCD_ver.sprintf(cut);
					}
					else if(i==6) Version_Info.LCD_Date.sprintf(cut);
				}
				else if(strcmp(strName, "Regulator") == 0)
				{
					if(Version_Info.Regulator_ver == "") break;
					if(i==2)
					{
						cVer = Version_Info.Regulator_ver.c_str();
						if(strcmp(cut, cVer) != 0) break;
						else Version_Info.Regulator_ver.sprintf(cut);
					}
					else if(i==6) Version_Info.Regulator_Date.sprintf(cut);
				}
				else if(strcmp(strName, "RSA Driving Rear") == 0)
				{
					if(Version_Info.RSA_Driving_Rear_ver == "") break;
					if(i==2)
					{
						cVer = Version_Info.RSA_Driving_Rear_ver.c_str();
						if(strcmp(cut, cVer) != 0) break;
						else Version_Info.RSA_Driving_Rear_ver.sprintf(cut);
					}
					else if(i==6) Version_Info.RSA_Driving_Rear_Date.sprintf(cut);
				}
				else if(strcmp(strName, "RSA Driving Front") == 0)
				{
					if(Version_Info.RSA_Driving_Front_ver == "") break;
					if(i==2)
					{
						cVer = Version_Info.RSA_Driving_Front_ver.c_str();
						if(strcmp(cut, cVer) != 0) break;
						else Version_Info.RSA_Driving_Front_ver.sprintf(cut);
					}
					else if(i==6) Version_Info.RSA_Driving_Front_Date.sprintf(cut);
				}
				else if(strcmp(strName, "RSA Hoist") == 0)
				{
					if(Version_Info.RSA_Hoist_ver == "") break;
					if(i==2)
					{
						cVer = Version_Info.RSA_Hoist_ver.c_str();
						if(strcmp(cut, cVer) != 0) break;
						else Version_Info.RSA_Hoist_ver.sprintf(cut);
					}
					else if(i==6) Version_Info.RSA_Hoist_Date.sprintf(cut);
				}
				else if(strcmp(strName, "RSA Slide") == 0)
				{
					if(Version_Info.RSA_Slide_ver == "") break;
					if(i==2)
					{
						cVer = Version_Info.RSA_Slide_ver.c_str();
						if(strcmp(cut, cVer) != 0) break;
						else Version_Info.RSA_Slide_ver.sprintf(cut);
					}
					else if(i==6) Version_Info.RSA_Slide_Date.sprintf(cut);
				}
				else if(strcmp(strName, "Fine Slave Rear") == 0)
				{
					if(Version_Info.Fine_Slave_Rear_ver == "") break;
					if(i==2)
					{
						cVer = Version_Info.Fine_Slave_Rear_ver.c_str();
						if(strcmp(cut, cVer) != 0) break;
						else Version_Info.Fine_Slave_Rear_ver.sprintf(cut);
					}
					else if(i==6) Version_Info.Fine_Slave_Rear_Date.sprintf(cut);
				}
				else if(strcmp(strName, "Fine Slave Front") == 0)
				{
					if(Version_Info.Fine_Slave_Front_ver == "") break;
					if(i==2)
					{
						cVer = Version_Info.Fine_Slave_Front_ver.c_str();
						if(strcmp(cut, cVer) != 0) break;
						else Version_Info.Fine_Slave_Front_ver.sprintf(cut);
					}
					else if(i==6) Version_Info.Fine_Slave_Front_Date.sprintf(cut);
				}
#endif
				else
				{
					FIRMWARE_VERSION_LOG("해당 안되는 항목 : %s", strName);
					break;
				}
			}
			memset(m_szTmpbuffer, 0, 256);
		...
		{
			FIRMWARE_VERSION_LOG("문제 생김");
			break;
		}
	}
	FIRMWARE_VERSION_LOG("정보 갱신 완료");
	fclose(fp);
	DeleteFile(szFileName);  //생성하는 파일 이름이 달라질 수 있어서 기존 파일은 읽어온 뒤 삭제
 }

 void TOHTMainForm::WriteVersionFile(int nSW, int nFW)
{
	FIRMWARE_VERSION_LOG("[Version Info Update] WriteVersionFile In");
	FILE* fp;
	AnsiString szTmpFilePath = "";

	if(strlen(m_pParamSet->YSCode) != 6)  //크기가 맞지 않는 값이 들어온 경우
	{
		strcpy(m_pParamSet->YSCode, m_pParamSet->VHLName);
	}
	else if(strcmp(m_pParamSet->VHLName, m_pParamSet->YSCode) != 0) //YSCode 번호가 정상 입력된 경우
	{
		char ctmp[20] = "YS-AMOA0";
		strcat(ctmp, m_pParamSet->YSCode);
		strcpy(m_pParamSet->YSCode, ctmp);
	}

	FIRMWARE_VERSION_LOG("YS코드:%s", m_pParamSet->YSCode);
	szTmpFilePath.sprintf("DataFiles\\VERSION_INFO\\FILEDB_UPDATE_%s.txt", m_pParamSet->YSCode);

	char* cTmpFilePath= szTmpFilePath.c_str();

	fp = fopen(cTmpFilePath, "w");

	if(fp == NULL)
	{
		FIRMWARE_VERSION_LOG("파일 생성 실패");
//		fclose(fp);
		return;
	}

#if(OHT_NAME == OHT_NAME_STD_V80 || OHT_NAME == OHT_NAME_STD_V81S)
	fprintf(fp, "HEADER$FOHT$%s$%s$V8.0$%d$%d$0$%s\n", m_pParamSet->YSCode, m_pParamSet->VHLName, nSW, nFW, strVersion_Date);
#elif(OHT_NAME == OHT_NAME_STD_V81)
	if(m_pParamSet->bSOM6869)
		fprintf(fp, "HEADER$FOHT$%s$%s$V8.1A$%d$%d$0$%s\n", m_pParamSet->YSCode, m_pParamSet->VHLName, nSW, nFW, strVersion_Date);
	else
	fprintf(fp, "HEADER$FOHT$%s$%s$V8.1$%d$%d$0$%s\n", m_pParamSet->YSCode, m_pParamSet->VHLName, nSW, nFW, strVersion_Date);
#elif((OHT_NAME == OHT_NAME_STD_V80_RETICLE) && (OHT_RETICLE_EXTEND_MODEL == RETICLE_NOMAL) || (OHT_NAME == OHT_NAME_STD_V80_RETICLE) && (OHT_RETICLE_EXTEND_MODEL == RETICLE_EUV_1))
	fprintf(fp, "HEADER$ROHT$%s$%s$V8.0$%d$%d$0$%s\n", m_pParamSet->YSCode, m_pParamSet->VHLName, nSW, nFW, strVersion_Date);
#elif(OHT_NAME == OHT_NAME_STD_V85)
	fprintf(fp, "HEADER$ROHT$%s$%s$V8.5$%d$%d$0$%s\n", m_pParamSet->YSCode, m_pParamSet->VHLName, nSW, nFW, strVersion_Date);
#elif(OHT_NAME == OHT_NAME_STD_V86)
	if(m_pParamSet->OhtDetectSensorType == OHTDETECT_SOSLAB)
		fprintf(fp, "HEADER$ROHT$%s$%s$V8.6$%d$%d$0$%s\n", m_pParamSet->YSCode, m_pParamSet->VHLName, nSW, nFW, strVersion_Date);
	else
		fprintf(fp, "HEADER$ROHT$%s$%s$V8.6A$%d$%d$0$%s\n", m_pParamSet->YSCode, m_pParamSet->VHLName, nSW, nFW, strVersion_Date);
#else
	if(m_pParamSet->OhtDetectSensorType == OHTDETECT_SOSLAB)
#if(USE_MAC == 0)
		fprintf(fp, "HEADER$FOHT$%s$%s$V8.2$%d$%d$0$%s\n", m_pParamSet->YSCode, m_pParamSet->VHLName, nSW, nFW, strVersion_Date);
#else
		fprintf(fp, "HEADER$MOHT$%s$%s$V8.2$%d$%d$0$%s\n", m_pParamSet->YSCode, m_pParamSet->VHLName, nSW, nFW, strVersion_Date);
#endif
	else
		fprintf(fp, "HEADER$FOHT$%s$%s$V8.2A$%d$%d$0$%s\n", m_pParamSet->YSCode, m_pParamSet->VHLName, nSW, nFW, strVersion_Date);
#endif

	if(Version_Info.Main_ver != "")
		fprintf(fp, "S/W$OHT Main$-$OHT Main$%s$-$SEMES$1$%s\n", Version_Info.Main_ver, Version_Info.Main_Date);
	if(Version_Info.Assistant_ver != "")
		fprintf(fp, "S/W$OHT Assistant$-$OHT Assistant$%s$-$SEMES$1$%s\n", Version_Info.Assistant_ver, Version_Info.Assistant_Date);
	if(Version_Info.FM_ver != "")
		fprintf(fp, "S/W$OHT FileManager$-$OHT FimeManager$%s$-$SEMES$1$%s\n", Version_Info.FM_ver, Version_Info.FM_Date);
	if(Version_Info.AMC_ver != "")
		fprintf(fp, "S/W$AMC$-$AMC$%s$-$SEMES$1$%s\n", Version_Info.AMC_ver, Version_Info.AMC_Date);
	if(Version_Info.AMC_Dll_ver != "")
		fprintf(fp, "S/W$AMC DLL$-$AMC DLL$%s$-$SEMES$1$%s\n", Version_Info.AMC_Dll_ver, Version_Info.AMC_Dll_Date);

	if(Version_Info.CID_O_ver != "")
		fprintf(fp, "F/W$CID-O Controller$-$CID-O Controller$%s$-$-$1$%s\n", Version_Info.CID_O_ver, Version_Info.CID_O_DATE);
#if((OHT_NAME == OHT_NAME_STD_V82)||(OHT_NAME == OHT_NAME_STD_V86)||(OHT_NAME == OHT_NAME_P4_MASK))
	if(Version_Info.EQPIO_ver != "")
		fprintf(fp, "F/W$EQ-PIO Sensor$-$EQ-PIO$%s$-$CANTOPS$1$%s\n", Version_Info.EQPIO_ver, Version_Info.EQPIO_Date);
	if(Version_Info.LCD_ver != "")
		fprintf(fp, "F/W$LCD Module$-$INDICATOR-LCD MODULE$%s$-$COMIJOA$1$%s\n", Version_Info.LCD_ver, Version_Info.LCD_Date);
	if(Version_Info.Regulator_ver != "")
		fprintf(fp, "F/W$Regulator$-$Regulator$%s$-$-$1$%s\n", Version_Info.Regulator_ver, Version_Info.Regulator_Date);

	if (Version_Info.RSA_Driving_Rear_ver != "")
		fprintf(fp, "F/W$RSA Driving Rear$-$RSA Driving Rear$%s$-$-$1$%s\n", Version_Info.RSA_Driving_Rear_ver, Version_Info.RSA_Driving_Rear_Date);
	if (Version_Info.RSA_Driving_Front_ver != "")
		fprintf(fp, "F/W$RSA Driving Front$-$RSA Driving Front$%s$-$-$1$%s\n", Version_Info.RSA_Driving_Front_ver, Version_Info.RSA_Driving_Front_Date);
	if (Version_Info.RSA_Hoist_ver != "")
		fprintf(fp, "F/W$RSA Hoist$-$RSA Hoist$%s$-$-$1$%s\n", Version_Info.RSA_Hoist_ver, Version_Info.RSA_Hoist_Date);
	if (Version_Info.RSA_Slide_ver != "")
		fprintf(fp, "F/W$RSA Slide$-$RSA Slide$%s$-$-$1$%s\n", Version_Info.RSA_Slide_ver, Version_Info.RSA_Slide_Date);
	if (Version_Info.Fine_Slave_Rear_ver != "")
		fprintf(fp, "F/W$Fine Slave Rear$-$Fine Slave Rear$%s$-$-$1$%s\n", Version_Info.Fine_Slave_Rear_ver, Version_Info.Fine_Slave_Rear_Date);
	if (Version_Info.Fine_Slave_Front_ver != "")
		fprintf(fp, "F/W$Fine Slave Front$-$Fine Slave Front$%s$-$-$1$%s\n", Version_Info.Fine_Slave_Front_ver, Version_Info.Fine_Slave_Front_Date);
#else
	if(Version_Info.EQPIO_ver != "")
		fprintf(fp, "F/W$EQ PIO$-$EQ-PIO$%s$-$-$1$%s\n", Version_Info.EQPIO_ver, Version_Info.EQPIO_Date);
#endif
	fclose(fp);

	FIRMWARE_VERSION_LOG("WriteVersionFile 완료----");
}
//---------------------------------------------------------------------------

void __fastcall TOHTMainForm::Button1Click(TObject *Sender)
{
...
void __fastcall TOHTMainForm::btnBlackBoxTimeClick(TObject *Sender)
{
	m_BlackBox->m_nBlackBoxCommand = BLACKBOX_TIMESET;
}
//---------------------------------------------------------------------------

void __fastcall TOHTMainForm::btnEventSaveClick(TObject *Sender)
{
	m_BlackBox->m_nBlackBoxCommand = BLACKBOX_EVENT;
	m_BlackBox->m_nBlackBoxEventType = 1;
}
//---------------------------------------------------------------------------

void __fastcall TOHTMainForm::btnEventStreamingClick(TObject *Sender)
{
	m_BlackBox->m_nBlackBoxCommand = BLACKBOX_EVENT;
	m_BlackBox->m_nBlackBoxEventType = 2;
}
//---------------------------------------------------------------------------
