...
#define VHLDETECT_DECEL  		0.0032     //3.2m/s^2

#elif((OHT_NAME == OHT_NAME_STD_V85S)||(OHT_NAME == OHT_NAME_STD_V82)||(OHT_NAME == OHT_NAME_STD_V86))
//#define DRIVING_GEAR_RATIO      76.4894412
//#define AMCDECEL_VHLDETECT		275362     //3.6m/s^2
#define DRIVING_GEAR_RATIO	    75.7245469       // 주행 기어비 오차에 따른 추가 보정 timy
#define AMCDECEL_VHLDETECT		3400 * DRIVING_GEAR_RATIO * 0.99   //3.4m/s^2
#define VHLDETECT_DECEL  		0.0035     //3.5m/s^2
#else
...
#define AUTORECOVERY_NO_ACTION    0
#define AUTORECOVERY_E231X_CASE1  1
#define AUTORECOVERY_E231X_CASE2  2
#define AUTORECOVERY_E250  3
/**
...
//	int m_CID_Offet_Target_Right;
	...
	//V8.2
	DWORD dwRearSensorChatteringDelayV82;
...
//	int m_OverRunCount_E250; // ERR_MARK_RSTOPSEN_OFF OverRun 횟수 카운트 변수
	...
	bool bCheckDrivParam; //AOHC-358 파라미터 적용 오류 시, 에러 발생

	bool bOverrunReverse;
	int m_nMTLNextNode;
	int m_nMTLCheckCurNode;
	int m_nMTLCheckCurNode2;
	int m_nPreAutoDoorNode;
	bool m_bE250RearOnCheck;
	int nCheckAutoRecoveryTry;
	bool m_bE250AutoRecoveryFlag;
	bool m_bE231xAutoRecovery;
	bool m_bE231xAutoRecovery_RearCheck;
	double m_dE231xAutoRecoveryDist;
	bool m_bE231xAutoRecoveryCase2;
	bool m_bE231xAutoRecoveryCase2_DistCheck;
	int m_nAutoRecoveyCount;
	double m_dE231xFront2RearDist;
	double m_dChekRearDist;
	double m_dE231xFront2CurPositionDist;
	double m_dE250RearOffPosition;
	double m_dE250RearOnPosition;
	int m_nE231xAutoRecoveryCase2Count;
	
	int nMinDecode;
	int nMaxDecode;
	double dAvgDecode;
	int nQRSetupVersion;
	BYTE MarkRetryErrorCase;
	bool m_nShutterRecovery_Cnt;
	DWORD m_nShutterRecovery_Time;
	bool m_nLeftSteerRecovery_Cnt;
	DWORD m_nLFrontSteerRecovery_Time;
	DWORD m_nLRearSteerRecovery_Time;
	bool m_nRightSteerRecovery_Cnt;
	DWORD m_nRFrontSteerRecovery_Time;
	DWORD m_nRRearSteerRecovery_Time;

	bool m_UBGFail_1;
	bool m_DetectFail_Cnt;
	int nUbgFailStart_Time;
...
private:
	...
	PeriodicCounter *m_pUBG_Fail3_HourCounter; // AMOHTV80F-1756 UBG 신호 헌팅으로 인한 추돌 사전 감지 및 Error 처리 용도 (시간 단위)
	PeriodicCounter *m_pUBG_Fail3_WeekCounter; // AMOHTV80F-1756 UBG 신호 헌팅으로 인한 추돌 사전 감지 및 Error 처리 용도 (주 단위)
...
private:
	...
	List<EXECUTE_RESPONSE_MAPMAKING_INFO*>	m_ListOfTmpStationData;	///< 다음 노드를 만나기 전까지 Station 정보를 임시 저장하는 List
	List<EXECUTE_RESPONSE_MAPMAKING_INFO*>	m_ListOfMapData;		///< MapMake 결과 (Node + Station) 를 입력하는 List : Execute Thread 에서 함수를 호출하여 정보를 얻어감: Manual Reset 시 정보 삭제
...
public:	//2	 외부에서 호출 되는 함수: 함수명의 시작을 대문자로 표시
	...
	bool m_bMarkOffsetCheck;

	UINT m_nCurrStation;

	void ClearMapMakingInfo(); ///< Map Make 작업 List 초기화
...
//	UINT checMTLTakeOut_PIO8			(DRIVING_HW_INFO_SET HwInfo);  /// 빼냄 확인         //미사용
//	UINT checMTLTakeIn_PIO8			(DRIVING_HW_INFO_SET HwInfo);  /// 투입 확인             //미사용
	UINT checkJobChange          (DRIVING_INFO_SET *pInfoSet);

	bool CanIgnoreVHLDetect(); // AMOHTV80F-1606 근접 제어 무시 설정 가능 여부 판단

//	void CID_LR_ON(bool LightGuideDirectionIsLeft);               //미사용
	...
	bool checkCommandTarget4CID(CID_CTRL_INFO* Info, bool TargetCID);  // AOHC-353
...
//	void CID_ID_Buffer_Clear(unsigned char index);                   //미사용
	void CID_Monitor_Status(CID_CTRL_INFO* Info,double dSpeedOfNow);
	void CID_R_Status_Chk();

//	void CID_N_BRANCH_SELECT_OFF();                        //미사용
	...
	int markOnQRStationDiffType2	(bool bIsLeft, 	DRIVING_HW_INFO_SET HwInfo, double dRealDistSumToTarget, double dSpeedLimit, int nTargetID
		, DRIVING_MARK_STATE* pMarkState, CTRL_INFO_OF_SYNC_AXIS_MOVE* pCtrlInfo, CTRL_INFO_OF_ID_READER_CMD_TYPE *pIdReaderCmdType, int* pnID, bool bOverrunIngn, double dSTBOffset = 0, int physicalID = 0);
...
//	void checkStartCmd(EXECUTE_DRIVING_INFO *pTmpDrivingInfo);	///< 마크 상태가 Station 인데, 다음 명령이 Next Node 로의 명령이 아닌 경우 처리                 //미사용
//	EXECUTE_ACK checkStartCmdStandard(EXECUTE_DRIVING_INFO *pTmpDrivingInfo);	///< 마크 상태가 Station 인데, 다음 명령이 Next Node 로의 명령이 아닌 경우 처리              //미사용
	...
	int AutoRecovery_Steer(int nErrorNum, DRIVING_INFO_SET *pInfoSet);
...
//	bool Lifter_Prenode_Check(UINT iNodeNum);              //미사용
	...
	bool checkUBG_Fail3_Error(); ///< AMOHTV80F-1756 UBG 신호 헌팅으로 인한 추돌 사전 감지 및 Error 처리 판단
...
//	int SendLowSerial(char* cTemp);                       //미사용

	int VHL_OPC_RESET_RESPONSE();
//	void VHL_OPC_RESET_RESPONSE_SIMUL(OPC_INIT_PKT RECV_PKT);          //미사용
	int VHL_OPC_PASS_REQUEST (char* OHTNAME, unsigned char Priority, unsigned char Turn, unsigned char LightGuideID);
//	int  GetCID_ELEVETORMODE_VALUE( );                    //미사용

//	bool Check_Elevetor_LG1_START_AREA_Tag();             //미사용
//	bool Check_Elevetor_LG1_WAIT_AREA_Tag();              //미사용
//	bool Check_Elevetor_LG2_INSIDE_WAIT_AREA_Tag();       //미사용
//	bool MotionOnOff(bool bOnOff);                        //미사용
	...
	int CID_Find_Stop_Reset(int callNum, DRIVING_HW_INFO_SET HwInfo, int* StopNode, int* ResetNode, bool *TargetCID);
//	int CID_Find_Stop_Reset_Change(int callNum, DRIVING_HW_INFO_SET HwInfo, int* StopNode, int* ResetNode, int Num);          //미사용

//	int ModeChg_CALL();                //미사용
//	int ModeChg_UP();                  //미사용
//	int ModeChg_DOWN();                //미사용

	void ADD_CID_LOG(AnsiString strLog);

//public:
//	void SetOhtName(AnsiString strName);       //미사용
    ...
    int ExeCmdEQPIOForDualStageStbLoad();
	int ExeCmdEQPIOForDualStageStbUnload();
	...
	UINT m_nOptDistChange;

	void Add_MCC_Nack_Log(BYTE &chNackToAckType);

	void SetUseCIDMonitoring(bool bUse, int nDelayCount);

	int GetCID_O_Version();
	UINT Check_Node;
	int Check_AutoRecovery_PreCheck;
};
