...
typedef enum PREVIOS_DIAG_PARAM_NUM_
{
	...
	PREVIOS_DIAG_UBGABLEVEL,                               //직선구간에서 감지-> 미감지(2단계 이상), 미감지->감지하는 경우는 일반적인 케이스이며 감지->미감지에 의한 부분만 확인 되면 미감지->감지하는 경우도 추정가능함.  AOHC-298
	PREVIOS_DIAG_RSSI,										//RSSI , Mac 주소 미기록 Count
	PREVIOS_DIAG_ABNORMAL_SP_CURVE, 						//AOHC-304 곡선 진입(0~500mm) 사이에서 속도가 높을 경우 찍음. 동일 노드는 지날때는 한번만 찍음. 속도 5m/s일때 현재 20msec 주기일경우 100mm 단위로 찍힐수있음.
	// 반드시 마지막 인덱스
	COUNTOF_PREVIOUS
...
private:
	...
	DIAG_STAT_DATA_MAP* m_DiagOverhaulDataMap;		//구동이력 관리용 자료 구조
...
//	void ClearDiagStatusData();                                   //미사용

	void MakeStatDataMap();
	bool bCheckDiagParam;	//AOHC-358 파라미터 적용 오류 시, 에러 발생
public:
...
//	int UpdateStatReferenceValue(BYTE bID, int nRefValue);              //미사용
	int ClearStatDiagData(BYTE bID);
//	bool CheckStatDataToSend(BYTE bID);                                 //미사용
