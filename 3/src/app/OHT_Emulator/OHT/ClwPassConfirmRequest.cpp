...
#pragma resource "*.dfm"
TfrmClwPassConfirmRequest *frmClwPassConfirmRequest;
//---------------------------------------------------------------------------
__fastcall TfrmClwPassConfirmRequest::TfrmClwPassConfirmRequest(TComponent* Owner)
	: TForm(Owner)
{
...
//---------------------------------------------------------------------------
void __fastcall TfrmClwPassConfirmRequest::btnGetOrderIDClick(TObject *Sender)
{
	int iOrderID = GetOrderID();
	edtOrderID->Text = IntToStr(iOrderID);
}
//---------------------------------------------------------------------------
void __fastcall TfrmClwPassConfirmRequest::FormClose(TObject *Sender, TCloseAction &Action)

{
	Action = caFree;
}
//---------------------------------------------------------------------------
void __fastcall TfrmClwPassConfirmRequest::btnCloseClick(TObject *Sender)
{
	Close();
}
//---------------------------------------------------------------------------
void __fastcall TfrmClwPassConfirmRequest::btnSendClick(TObject *Sender)
{
	char buf[MAX_PACKET_SIZE+1];
	int iPacketSize = sizeof(PKT_CMD_PASSPERMITOPEN_OHT2OCS_ID_243);
	int iReturn, iTemp;

	// 패킷을 정의한 후 값을 써 넣는다.
	PKT_CMD_PASSPERMITOPEN_OHT2OCS_ID_243	SEND_PACKET;
	...
	// ---- Tag -------------------------------------------
	SEND_PACKET.TAG.Value = TagID::CMD_PASSPERMITOPEN_OHT2OCS;	// 통과 허가 개방 요구
	...
	// ------ 패킷 전송 ---------------------------------
	Conv_PKT_CMD_PASSPERMITOPEN_OHT2OCS_ID_243(SEND_PACKET);
