...
#pragma resource "*.dfm"
TfrmOcsVersionRequest *frmOcsVersionRequest;
//---------------------------------------------------------------------------
__fastcall TfrmOcsVersionRequest::TfrmOcsVersionRequest(TComponent* Owner)
	: TForm(Owner)
{
}
//---------------------------------------------------------------------------
void __fastcall TfrmOcsVersionRequest::FormClose(TObject *Sender, TCloseAction &Action)
{
		Action = caFree;
}
//---------------------------------------------------------------------------
void __fastcall TfrmOcsVersionRequest::btnCloseClick(TObject *Sender)
{
	Close();
}
//---------------------------------------------------------------------------
void __fastcall TfrmOcsVersionRequest::btnSendClick(TObject *Sender)
{
	char buf[MAX_PACKET_SIZE+1];
	int iPacketSize = sizeof(PKT_OHT_VERSION_REQUEST);
	int iReturn, iTemp;

	// 패킷을 정의한 후 값을 써 넣는다.
	PKT_OHT_VERSION_REQUEST	SEND_PACKET;
	...
	// ---- Tag
	SEND_PACKET.TAG.Value = TagID::CMD_VERSIONREQ_OCS_OHT;
	...
	*/
	...
	Conv_PKT_OHT_VERSION_REQUEST(SEND_PACKET);
