...
#pragma resource "*.dfm"
TfrmOcsFloorMoveOrder *frmOcsFloorMoveOrder;
//---------------------------------------------------------------------------
__fastcall TfrmOcsFloorMoveOrder::TfrmOcsFloorMoveOrder(TComponent* Owner)
	: TForm(Owner)
{
...
//---------------------------------------------------------------------------
void __fastcall TfrmOcsFloorMoveOrder::FormClose(TObject *Sender, TCloseAction &Action)

{
	Action = caFree;
}
//---------------------------------------------------------------------------
void __fastcall TfrmOcsFloorMoveOrder::btnGetOrderIDClick(TObject *Sender)
{
	int iOrderID = GetOrderID();
	edtOrderID->Text = IntToStr(iOrderID);
}
//---------------------------------------------------------------------------
void __fastcall TfrmOcsFloorMoveOrder::btnSendClick(TObject *Sender)
{
	char buf[MAX_PACKET_SIZE+1];
	int iPacketSize = sizeof(PKT_FLOOR_MOVE_ORDER);
	int iReturn, iTemp;

	// 패킷을 정의한 후 값을 써 넣는다.
	PKT_FLOOR_MOVE_ORDER	SEND_PACKET;
	...
	// ---- Tag
	SEND_PACKET.TAG.Value = TagID::CMD_FLOORMOVE_OCS_OHT;
	...
	*/
	...
	Conv_PKT_FLOOR_MOVE_ORDER(SEND_PACKET);
...
void __fastcall TfrmOcsFloorMoveOrder::btnCloseClick(TObject *Sender)
{
	Close();
}
//---------------------------------------------------------------------------
