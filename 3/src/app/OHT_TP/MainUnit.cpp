...
__fastcall TMainForm::TMainForm(TComponent* Owner) : TForm(Owner)
{
	...
	pnlPause_SetReset->Color = clBtnFace;
...
{
	if(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_FOUP || gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_MAC)
	{
...
void TMainForm::SettingStatusBar()
{
    ...
    panAMCVer->Caption = "";
    // -------------------------------------------
	panOHTType = new TPanel(StatusBarVersionInfo);
	panOHTType->Parent = StatusBarVersionInfo;
	panOHTType->ParentBackground = false;
	panOHTType->Visible = true;
	panOHTType->Top = 2;
	panOHTType->Left = panAMCVer->Left + panAMCVer->Width ;
	panOHTType->Height = StatusBarVersionInfo->Height - 2;
	panOHTType->Width = StatusBarVersionInfo->Panels->Items[1]->Width;
	panOHTType->BevelInner = bvNone;
	panOHTType->BevelOuter = bvLowered;
	panOHTType->Font->Color = COLOR_BLACK;
	panOHTType->Caption = "";
}
// ---------------------------------------------------------------------------
...
//	MainForm->panUser->Caption = "개발자";
	...
	//2022.05.11 sseung 무조건 운영자로 로그인되도록 변경(필요할때만 개발자로 로그인)
...
int TMainForm::CommDisconnect()
{
	...
	MaskOnlineCMDForm->Close();     //ssung 2022-11-02 P4 Mask OHT
...
//    gm_OHTStatus->nIsRFPIO6Digit = -1;
	...
	OBS_UBG_Form->Close();
    DistanceSensorViewrForm->Close();
	frmVibrationMeasurement->Close();
	frm_RouteRequest->Close();
	frm_AutoBayTest->Close();
	frm_MapViewLoad->Close();
	// shkim add
...
void __fastcall TMainForm::timerStatusRequestTimer(TObject *Sender)
{
	...
	if(iErrorCount > 30)	// 0.2초 x 30 =  6초 동안 Status가 안오면 끊어졌다고 본다
	{
		// 벨로시티 그래프 멈추도록 함
		VelGraphForm->Motor1Timer->Enabled = false;
		VelGraphForm->Motor24Timer->Enabled = false;
...
void TMainForm::CommOK()
{
	...
	{
		MaskOnlineCMDForm->panVer->Color = COLOR_YELLOW;
	}
...
void TMainForm::CommFail()
{
	...
	{
		MaskOnlineCMDForm->panVer->Color = COLOR_RED;
	}
...
void __fastcall TMainForm::timerUpdateDisplayTimer(TObject *Sender)
{
	...
	// Log 드라이브 유무 표시
	if(gm_OHTStatus->m_OHTStatus.IsWarningState.IswarningLogThreadCycleTime == true)
	{
		panLogDirve->Color = COLOR_MAROON;
	}
	else if(gm_OHTStatus->m_OHTStatus.isLogDrive)
	{
	...
	//AOHC-285
	if(gm_OHTStatus->m_OHTStatus.IsWarningState.IsWarningState == LINE_WARNNING)
	{
			pnalWarning->Color = COLOR_RED;
    }
	else
	{
		pnalWarning->Color = COLOR_LIME;
	}
...
void __fastcall TMainForm::AVSViewerClick(TObject *Sender)
{
	frmVibrationMeasurement->Show();
}
//---------------------------------------------------------------------------

void __fastcall TMainForm::ErrorIgnoalFoupcoveroepn1Click(TObject *Sender)
{
	TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
	...
	if(gm_OHTStatus->m_OHTStatus.Data.MSCmdID.Info.Mode != MODE_MANUAL)
	{
		ShowMessage("이 기능은 메뉴얼  모드에서만 사용 가능합니다");
		return;
	}
	// Pause, Resume 변환
	...
	SEND_PACKET.TAG.Value = TagID::CMD_TP_ERRORIGNOL_ONOFF;
	SEND_PACKET.BODY.iVar1 = 0x01;	// sound on   //0x01 On, 0x02 Off
	SEND_PACKET.BODY.iVar2 = 0x01; // FoupCover   //0x01 :Foupcover
...
void __fastcall TMainForm::ErrorResumeFoupcoveroepn1Click(TObject *Sender)
{
	TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
	...
	SEND_PACKET.TAG.Value = TagID::CMD_TP_ERRORIGNOL_ONOFF;
	SEND_PACKET.BODY.iVar1 = 0x02;	// sound on   //0x01 On, 0x02 Off
	SEND_PACKET.BODY.iVar2 = 0x01; // FoupCover   //0x01 :Foupcover
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::mniAutoMovingTestClick(TObject *Sender)
{
	frm_AutoBayTest->Show();
}

void __fastcall TMainForm::mnWINShutdownClick(TObject *Sender)
{
	if(gm_OHTStatus->m_OHTStatus.Data.MSCmdID.Info.Mode != MODE_MANUAL)
	{
		ShowMessage("이 기능은 Manual 모드에서만 사용 가능합니다");
		return;
	}
	...
	if((gm_OHTStatus->m_OHTStatus.Data.MSCmdID.Info.Status == STS_INIT) ||
	 ...
	 (gm_OHTStatus->m_OHTStatus.Data.MSCmdID.Info.Status == STS_UNLOADED) ||
	 (gm_OHTStatus->m_OHTStatus.Data.MSCmdID.Info.Status == STS_LOADED) ||
	 ...
	 (gm_OHTStatus->m_OHTStatus.Data.MSCmdID.Info.Status == STS_ERROR))
	  {
		 // 이 기능은 동작 중이 아닐때 사용가능.
		 // 대기(Init, Arrived, Unloaded, Loaded, Complete), Error에서만 동작 가능
	  }
	else
	 {
		ShowMessage("이 기능은 동작 중이 아닐떄만 사용가능합니다");
		return;
	 }

	int iPacketSize = sizeof(PKT_OHT2TP_WINDOW_OS_SHUTDOWN_INFO);
	int iReturn;

	PKT_OHT2TP_WINDOW_OS_SHUTDOWN_INFO 	SEND_PACKET;
	memset(&SEND_PACKET, 0, iPacketSize);	// 패킷 메모리 클리어

	SEND_PACKET.HEADER.MachineType = MACHINETYPE_TP;
	memcpy(SEND_PACKET.HEADER.MachineID, "PC_TP", MACHINE_ID_LENGTH);
	SEND_PACKET.TAG.Value = TagID::CMD_WINDOW_OS_SHUTDOWN;

	SEND_PACKET.BODY.Type    = 0;
	SEND_PACKET.BODY.TypeNum = 0;
	SEND_PACKET.BODY.nData1 = 0;
	SEND_PACKET.BODY.nData2 = 0;
	SEND_PACKET.BODY.nData3 = 0;
	SEND_PACKET.BODY.nData4 = 0;
