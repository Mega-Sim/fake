...
#pragma resource "*.dfm"
TMaskOnlineCMDForm *MaskOnlineCMDForm;
TCHKOHT CmdMaskOHTCheck;
...
// ---------------------------------------------------------------------------
__fastcall TMaskOnlineCMDForm::TMaskOnlineCMDForm(TComponent *Owner) : TForm(Owner)
{
...
void __fastcall TMaskOnlineCMDForm::WMEXITSIZEMOVE(TMessage &msg)
{
    int nTmpLeft, nTmpTop, nTmpRight;

    nTmpLeft  = MaskOnlineCMDForm->Left;
    nTmpTop   = MaskOnlineCMDForm->Top;
    nTmpRight = nTmpLeft + MaskOnlineCMDForm->Width;
...
// ---------------------------------------------------------------------------
TTextAttributes *__fastcall TMaskOnlineCMDForm::CurrText(void)
{
...
// ---------------------------------------------------------------------------
/**
 @brief   MaskOnlineCMDForm의 Show 발생시 호출되는 이벤트 함수
 @author  puting
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::FormShow(TObject *Sender)
{
...
// ---------------------------------------------------------------------------
void __fastcall TMaskOnlineCMDForm::FormClose(TObject *Sender,
    TCloseAction &Action)
{
	timerAllDisable();
}
...
// ---------------------------------------------------------------------------
int TMaskOnlineCMDForm::Check_Invalid_Command(String strTmpCMD)
{
    ...
    if((strTmpCMD.Pos("[") != 0) || (strTmpCMD.Pos("]") != 0))
    {
        ...
        {
			if(CmdMaskOHTCheck.Is_Executable
...
void __fastcall TMaskOnlineCMDForm::mniMemoClearClick(TObject *Sender)
{
    PMACMemo->Clear();
}

// ---------------------------------------------------------------------------
void TMaskOnlineCMDForm::PMACDataDisplay(TColor TmpFontColor, String strDisplayData)
{
...
// ---------------------------------------------------------------------------
/**
 @brief   Hand Home버튼 클릭시 발생하는 이벤트 함수
 @author  puting
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnHandHomeClick(TObject *Sender)
{
	if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
		if((gm_OHTStatus->IOInput->uInput.Input_MASK.HAND_UnloadPosition == OFF)
		&& (gm_OHTStatus->IOInput->uInput.Input_MASK.HAND_LoadPosition == OFF))
		{
			Warnning_Display(COLOR_YELLOW, "Load Position[OFF] & Unload Position[OFF]...Hand Can't Home");
			return;
		}

		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
		if(gm_OHTStatus->IOInput->uInput.Input_MASK.HAND_Work == ON && gm_OHTStatus->IOInput->uInputCommon.HAND_Right_Grip == OFF &&
			gm_OHTStatus->IOInput->uInputCommon.HAND_Left_Grip == OFF)
		{
			if(MessageDlg("Gripper Sensor가 OFF 상태입니다.\nHand Home 동작 하시겠습니까?", mtWarning, TMsgDlgButtons() << mbOK << mbCancel, 0) == IDOK)
			{
				m_CMDControl->MakeTPCmd2OHT(TPCMD_HAND_CLOSE);
				MainForm->DataDisplay(COLOR_BLACK, "Hand Home");
			}
		}
		else
		{
...
// ---------------------------------------------------------------------------
/**
 @brief   Hand Work버튼 클릭시 발생하는 이벤트 함수
 @author  puting
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnHandWorkClick(TObject *Sender)
{
	if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
		if((gm_OHTStatus->IOInput->uInput.Input_MASK.HAND_UnloadPosition == OFF)
		&& (gm_OHTStatus->IOInput->uInput.Input_MASK.HAND_LoadPosition == OFF))
		{
			Warnning_Display(COLOR_YELLOW, "Load Position[OFF] & Unload Position[OFF]...Hand Can't Work");
			return;
		}
			...
			gm_OHTStatus->IOInput->uInputCommon.HAND_Left_Grip == OFF)
		{
			...
			MainForm->DataDisplay(COLOR_BLACK, "Hand Work");
		}
 ...
 */
void __fastcall TMaskOnlineCMDForm::btnGoClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
...
/**
 ...
 @param   EStop
 */
void __fastcall TMaskOnlineCMDForm::btnStopClick(TObject *Sender)
{
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnTransMarkForwardClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        String strTmpCMD;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnTransMarkBackwardClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        String strTmpCMD;
...
void __fastcall TMaskOnlineCMDForm::memoJogKeyControlExit(TObject *Sender)
{
    memoJogKeyControl->Color = clMenuHighlight;
    timerDrivingJog->Enabled = false;
	chbBackWard->Checked = false;
	chkEmergencyJog->Checked = false;
}
// ---------------------------------------------------------------------------

void __fastcall TMaskOnlineCMDForm::memoJogKeyControlClick(TObject *Sender)
{
...
@date    2013.02.19
*/
void __fastcall TMaskOnlineCMDForm::memoJogKeyControlKeyDown(TObject *Sender,
	WORD &Key, TShiftState Shift)
{
	if(((chbBackWard->Checked == true) && (Key == VK_LEFT)) || (Key == VK_RIGHT))
	{
		// 조그 명령 Timer로 반복 실시
		CurJogKey = Key;
...
//		}
		...
		if(gm_OHTStatus->m_OHTStatus.IsWarningState.EmergencyNodeMTL ==ON)
		{
			if(chkEmergencyJog->Checked == true)
			{
             	timerDrivingJog->Enabled = true;
			}
			else
			{
				CurJogKey = 0x0000;
				timerDrivingJog->Enabled = false;
				Warnning_Display(COLOR_RED, "진입 불가 구간 입니다. 현장/육안 확인및 진입불가 구간 이동 설정 확인하여 진행해주시기 바랍니다.");
            }
		}
		else
		{
	...
	else if((chbBackWard->Checked == false) && (Key == VK_LEFT))
	{
		CurJogKey = 0x0000;
		timerDrivingJog->Enabled = false;
		Warnning_Display(COLOR_RED, "Backward Not Check...");
	}
	else
	{
		CurJogKey = 0x0000;
...
@date    2013.02.19
*/
void __fastcall TMaskOnlineCMDForm::memoJogKeyControlKeyUp(TObject *Sender,
    WORD &Key, TShiftState Shift)
{
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
    ...
    if((Key == VK_RIGHT) || ((chbBackWard->Checked == true) && (Key == VK_LEFT)))
    {
		MainForm->DataDisplay(COLOR_BLACK, "Driving Stop1");
...
*/
void __fastcall TMaskOnlineCMDForm::timerDrivingJogTimer(TObject *Sender)
{
	bool bError = NO_ERR;
	...
	if(memoJogKeyControl->Focused() == false || bJogKeyPressed == false)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;

		MainForm->DataDisplay(COLOR_BLACK, "Driving Stop2");
	...
	if((gm_OHTStatus->m_OHTStatus.IsWarningState.EmergencyNodeMTL ==ON) && (chkEmergencyJog->Checked == false))
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;

		MainForm->DataDisplay(COLOR_BLACK, "Driving Stop3");
		m_CMDControl->MakeTPCmd2OHT(TPCMD_JOG_DRIVING_STOP, &SndTPCmd);
		CurJogKey = 0x0000;
    ...
    if( (gm_OHTStatus->nProgram_Mode == MODE_WORKER ||
			gm_OHTStatus->nProgram_Mode == MODE_OPERATOR) &&
			(SndTPCmd.CmdTP_Jog_DRV.Velocity > 700) ) // 2025.04.14 전진 속도 limit 700mm/s (손민규님 요청)
		{
			Warnning_Display(COLOR_RED, "ERROR : Speed must be 700 or less....");
			timerDrivingJog->Enabled = false;
			chbBackWard->Checked = false;
			return;
		}
	...
	if(CmdMaskOHTCheck.Is_Executable(EXCLUDE_DRIVING_RUNNING) == NO_ERR)
	{
...
//		else if(fabs(gm_OHTStatus->Motor_Shift->dPosition_mm) > SHIFT_ORIGIN_LIMIT)
		if(fabs(gm_OHTStatus->Motor_Shift->dPosition_mm) > SHIFT_ORIGIN_LIMIT)
		{
...
//	}
	...
	default:
		MainForm->DataDisplay(COLOR_BLACK, "Driving Stop4");
		m_CMDControl->MakeTPCmd2OHT(TPCMD_JOG_DRIVING_STOP, &SndTPCmd);
		CurJogKey = 0x0000;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnHandOriginClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnHandResetClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnHandFreeClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TMaskOnlineCMDForm::btnSteeringLeftClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
// ---------------------------------------------------------------------------
void __fastcall TMaskOnlineCMDForm::btnSteeringRightClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
@date    2012.12.14
*/
void __fastcall TMaskOnlineCMDForm::btnShutterOpenClick(TObject *Sender)
{
...
@date    2012.12.14
*/
void __fastcall TMaskOnlineCMDForm::btnShutterCloseClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
@date    2012.12.14
*/
void __fastcall TMaskOnlineCMDForm::btnShutterOriginClick(TObject *Sender)
{
...
@date    2012.12.14
*/
void __fastcall TMaskOnlineCMDForm::btnShutterFreeClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
// ---------------------------------------------------------------------------
int TMaskOnlineCMDForm::Set_UBGColor(int nOutput)
{
    int nError = 0;
...
/**
 @brief   MaskOnlineCMDForm UI를 주기적으로 갱신하는 Timer 이벤트 함수
 @author  zzang9un
 @date    2013.03.29
 */
void __fastcall TMaskOnlineCMDForm::timerUpdateDisplayTimer(TObject *Sender)
{
    String strVelData;
	...
	//Fan Fail Input On일 때, 화면 Blink 시킴
	bool bOnFanFailPnl = (gm_OHTStatus->IOInput->uInputCommon.FAN_Fail == ON) ? true:false;
	...
	if(bOnFanFailPnl)
	{
		...
		static DWORD dwBlinkTick = 0;
		if(bOnFanFailPnl != bPrevStat)
		{
			bPrevStat = bOnFanFailPnl;
			dwBlinkTick = GetTickCount();
		}
		...
		pnlFanFailAlarm->Visible = bToggle;
	}
	else
	{
		pnlFanFailAlarm->Visible = false;
    }
    ...
    if(gm_OHTStatus->nIsUBGSenUse == 1)
    {
        ...
        if(gm_OHTStatus->nIsPauseSet == 1)
        {
			if(MainForm->btnPause_SetReset->Caption != "Pause:Reset")
            {
				...
				MainForm->pnlPause_SetReset->Color = clRed;
        ...
        else
        {
			if(MainForm->btnPause_SetReset->Caption != "Pause:Set")
            {
				...
				MainForm->pnlPause_SetReset->Color = clBtnFace;
...
//	RcvData.VolatageMonitor[0]&0xFFFF, (RcvData.VolatageMonitor[0]>>16)&0xFFFF, RcvData.VolatageMonitor[1]&0xFFFF, (RcvData.VolatageMonitor[1]>>16)&0xFFFF
    ...
    // ** Hand ***
	// input
	HandWorkLt->Brush->Color    = (gm_OHTStatus->IOInput->uInput.Input_MASK.HAND_Work == ON) ? COLOR_LIME : COLOR_WHITE;
	HandInPosLt->Brush->Color   = (gm_OHTStatus->IOInput->uInput.Input_MASK.HAND_Inposition == ON) ? COLOR_LIME : COLOR_WHITE;
	HandHomeLt->Brush->Color   = (gm_OHTStatus->IOInput->uInput.Input_MASK.HAND_Home == ON) ? COLOR_LIME : COLOR_WHITE;
...
//	panFoupDetect->Color = (gm_OHTStatus->IOInput->uInputCommon.FOUP_Detect == ON) ? COLOR_LIME : COLOR_WHITE;
    ...
    if(panUTB->Visible == false)
        panUTB->Visible = true;
	panUTB->Color = (gm_OHTStatus->IOInput->uInput.Input_MASK.UTB_Detect == ON) ? COLOR_LIME : COLOR_WHITE;

//	panHoist_Belt->Color = (gm_OHTStatus->IOInput->uInputCommon.HOIST_Belt_Detect1 == ON) ? COLOR_LIME : COLOR_WHITE;

	//2022-10-31 Sseung P4 Mask IO 추가
//	panFoupDetect->Color = (gm_OHTStatus->IOInput->uInputCommon.FOUP_Detect == ON) ? COLOR_LIME : COLOR_WHITE;
	panHLoadPosition->Color = (gm_OHTStatus->IOInput->uInput.Input_MASK.HAND_LoadPosition == ON) ? COLOR_LIME : COLOR_WHITE;
	panHUnloadPosition->Color = (gm_OHTStatus->IOInput->uInput.Input_MASK.HAND_UnloadPosition == ON) ? COLOR_LIME : COLOR_WHITE;
//	panHoist_Lower_Home->Color = (gm_OHTStatus->IOInput->uInputCommon. == ON) ? COLOR_LIME : COLOR_WHITE;         //IO가 Main에 없음
	panRSP150Home->Color = (gm_OHTStatus->IOInput->uInput.Input_MASK.RSP150_Home == ON) ? COLOR_LIME : COLOR_WHITE;
	panRSP200Home->Color = (gm_OHTStatus->IOInput->uInput.Input_MASK.RSP200_Home == ON) ? COLOR_LIME : COLOR_WHITE;
	panEUVPodHome->Color = (gm_OHTStatus->IOInput->uInput.Input_MASK.EUV_Pod_Home == ON) ? COLOR_LIME : COLOR_WHITE;
	panAntidropPodDetect1->Color = (gm_OHTStatus->IOInput->uInput.Input_MASK.ANTIDROP_POD_Placement1 == ON) ? COLOR_LIME : COLOR_WHITE;
	panAntidropPodDetect2->Color = (gm_OHTStatus->IOInput->uInput.Input_MASK.ANTIDROP_POD_Placement2 == ON) ? COLOR_LIME : COLOR_WHITE;
	panAntidropPodDetect3->Color = (gm_OHTStatus->IOInput->uInput.Input_MASK.ANTIDROP_POD_Placement3 == ON) ? COLOR_LIME : COLOR_WHITE;
	panAntidropPodDetect4->Color = (gm_OHTStatus->IOInput->uInput.Input_MASK.ANTIDROP_POD_Placement4 == ON) ? COLOR_LIME : COLOR_WHITE;

	panEUVPodDetect->Color = (gm_OHTStatus->IOInput->uInput.Input_MASK.EUVPod_Detect == ON) ? COLOR_LIME : COLOR_WHITE;
	panRSP150Detect->Color = (gm_OHTStatus->IOInput->uInput.Input_MASK.RSP150_Detect == ON) ? COLOR_LIME : COLOR_WHITE;
	panRSP200Detect->Color = (gm_OHTStatus->IOInput->uInput.Input_MASK.RSP200_Detect == ON) ? COLOR_LIME : COLOR_WHITE;
	panPODDetect->Color = (gm_OHTStatus->IOInput->uInput.Input_MASK.Pod_Detect == ON) ? COLOR_LIME : COLOR_WHITE;

	panFFUFailHoist->Color = (gm_OHTStatus->IOInput->uInput.Input_MASK.FFU_Fail_Hoist == ON) ? COLOR_LIME : COLOR_WHITE;
	panFFUFailFrame->Color = (gm_OHTStatus->IOInput->uInput.Input_MASK.FFU_Fail_Frame == ON) ? COLOR_LIME : COLOR_WHITE;
    ...
    */
	...
	if(gm_OHTStatus->m_OHTStatus.IsErrorIgnolState == 0x01)
	{
	   MainForm->ErrorIgnoalFoupcoveroepn1->Enabled = false;
	   MainForm->ErrorResumeFoupcoveroepn1->Enabled = true;
	}
	else if(gm_OHTStatus->m_OHTStatus.IsErrorIgnolState == 0x00)
	{
		MainForm->ErrorIgnoalFoupcoveroepn1->Enabled = true;
		MainForm->ErrorResumeFoupcoveroepn1->Enabled = false;
	}else
	{
		MainForm->ErrorIgnoalFoupcoveroepn1->Enabled = false;
		MainForm->ErrorResumeFoupcoveroepn1->Enabled = false;
...
@date    2012.12.14
*/
void __fastcall TMaskOnlineCMDForm::btnStopTagBCRClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
@date    2012.12.14
*/
void __fastcall TMaskOnlineCMDForm::btnMotor2HomeClick(TObject *Sender)
{
	if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
		CmdMaskOHTCheck.Axis_Hoist_HOME();
	}
}
// ---------------------------------------------------------------------------
...
i226 = (M262/3072) * 16
*/
void __fastcall TMaskOnlineCMDForm::mniHoistOriginSetClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TMaskOnlineCMDForm::mniShiftOriginSetClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
@date    2012.12.14
*/
void __fastcall TMaskOnlineCMDForm::edtSpeedChange(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnMoveStopClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable(EXCLUDE_HOIST_RUNNING EXCLUDE_SHIFT_RUNNING EXCLUDE_ROTATE_RUNNING) == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TMaskOnlineCMDForm::FormCreate(TObject *Sender)
{
    int j                         = 0;
    MaskOnlineCMDForm->Top            = MainForm->Top;
    MaskOnlineCMDForm->Left           = MainForm->Left + MainForm->Width + StatusForm->Width;
...
// ---------------------------------------------------------------------------
void __fastcall TMaskOnlineCMDForm::pgcMainControlDrawTab(
...
// ---------------------------------------------------------------------------
void TMaskOnlineCMDForm::Warnning_Display(TColor tWColor, String strData)
{
...
void __fastcall TMaskOnlineCMDForm::PWGridDrawCell(TObject *Sender, int ACol,
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnHaONClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnHaOFFClick(TObject *Sender)
{
	if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        String strTmpMessage;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnHaRstClick(TObject *Sender)
{
	if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        String strTmpMessage;
...
/**
 ...
 @param   Sender : TObject
 */
void __fastcall TMaskOnlineCMDForm::timerTransLongRunTimer(TObject *Sender)
{
...
void __fastcall TMaskOnlineCMDForm::chkLongRunClick(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnShutterAlarmResetClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @param   EStop
 */
void __fastcall TMaskOnlineCMDForm::btnMotor34HomeClick(TObject *Sender)
{
	if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
		CmdMaskOHTCheck.Axis_Shift_Ratate_HOME();
	}
}
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnLoadMouseDown(TObject *Sender,
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnUnloadMouseDown(TObject *Sender,
    TMouseButton Button, TShiftState Shift, int X, int Y)
{
    TPoint P, Q;
    if(Button == mbLeft)
...
void __fastcall TMaskOnlineCMDForm::pmDrivingFServoOnOffPopup(TObject *Sender)
{
...
void __fastcall TMaskOnlineCMDForm::pmDrivingRServoOnOffPopup(TObject *Sender)
{
...
void __fastcall TMaskOnlineCMDForm::pmHoistServoOnOffPopup(TObject *Sender)
{
...
void __fastcall TMaskOnlineCMDForm::pmShiftServoOnOffPopup(TObject *Sender)
{
...
void __fastcall TMaskOnlineCMDForm::mniDrvFServoOnClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TMaskOnlineCMDForm::mniDrvFServoOffClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TMaskOnlineCMDForm::mniDrivingRServoOnClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TMaskOnlineCMDForm::mniDrivingRServoOffClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TMaskOnlineCMDForm::mniHoistServoOnClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TMaskOnlineCMDForm::mniHoistServoOffClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TMaskOnlineCMDForm::mniShiftServoOnClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TMaskOnlineCMDForm::mniShiftServoOffClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnFSteerONClick(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnFSteerOFFClick(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnRSteerONClick(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnRSteerOFFClick(TObject *Sender)
{
...
void __fastcall TMaskOnlineCMDForm::chkDistClick(TObject *Sender)
{
...
// ---------------------------------------------------------------------------
int TMaskOnlineCMDForm::DistChang_mm_2(void)
{
    int nError = 0;
...
// ---------------------------------------------------------------------------
int TMaskOnlineCMDForm::DistChang_cts_2(void)
{
    int nError = 0;
    float dDist_mm, dDist_M, dDist_cts;
    String strdd;
...
// ---------------------------------------------------------------------------
void __fastcall TMaskOnlineCMDForm::edtDistmmChange(TObject *Sender)
{
	DistChang_mm_2();
}
// ---------------------------------------------------------------------------

void __fastcall TMaskOnlineCMDForm::edtDistctsChange(TObject *Sender)
{
    DistChang_cts_2();
}
// ---------------------------------------------------------------------------

void __fastcall TMaskOnlineCMDForm::btnMoveClick(TObject *Sender)
{
...
//	}
	...
	if(gm_OHTStatus->Motor_Driving_F->dPosition_mm > Distance)
	{
     	Warnning_Display(COLOR_RED, "ERROR : Current Position > Target Position");
		return;
	}
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnScanUTBClick(TObject *Sender)
{
    String strTmpMessage;
    ...
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        ...
        else
        {
            ...
            if(Application->MessageBox(strTmpMessage.w_str(), L"Confirm", MB_OKCANCEL) == IDOK)
            {
                ...
                if(MaskOnlineCMDForm->chkLongRun->Checked == true)
...
void __fastcall TMaskOnlineCMDForm::btnTractionLeftClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TMaskOnlineCMDForm::btnTractionRightClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TMaskOnlineCMDForm::chkTractionModeClick(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnMCControlOnClick(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnMCControlOffClick(TObject *Sender)
{
    ///추가 puting 2012.11.28
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnDrivingBCRClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnDrivingMarkBackwardClick(
    TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TMaskOnlineCMDForm::btnDrivingMarkForwardClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        String strTmpCMD;
...
@return  변환된 값
*/
BYTE TMaskOnlineCMDForm::GetTransNodeType(int index)
{
    BYTE rtnVal = 0x00;
...
@param   Sender : TObject
*/
void __fastcall TMaskOnlineCMDForm::chkGoStationClick(TObject *Sender)
{
    if(chkGoStation->Checked)
    {
...
void __fastcall TMaskOnlineCMDForm::mniLoadEQClick(TObject *Sender)
{
    ...
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
        ...
        {
            Warnning_Display(COLOR_RED, "ERROR : OHT is not Marked");
            return;
        }
...
void __fastcall TMaskOnlineCMDForm::mniLoadEQ_XClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
...
void __fastcall TMaskOnlineCMDForm::mniLoadSTOCKERClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
...
void __fastcall TMaskOnlineCMDForm::mniLoadUTBClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
...
void __fastcall TMaskOnlineCMDForm::mniLoadLOADERClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
...
void __fastcall TMaskOnlineCMDForm::mniUnloadEQClick(TObject *Sender)
{
	// by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
	// shkim  임시 주의!!!!!
	if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
...
void __fastcall TMaskOnlineCMDForm::mniUnloadEQ_XClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
...
void __fastcall TMaskOnlineCMDForm::mniUnloadSTOCKERClick(TObject *Sender)
{
	// by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
	if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
...
void __fastcall TMaskOnlineCMDForm::mniUnloadLOADERClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
...
void __fastcall TMaskOnlineCMDForm::mniUnloadUTBClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
...
void __fastcall TMaskOnlineCMDForm::panTeachingSaveClick(TObject *Sender)
{
    TeachingForm->Show();
...
/**
@brief   MaskOnlineCMDForm의 Timer를 Enable하는 함수
@author  zzang9un
@date    2013.11.20
*/
void TMaskOnlineCMDForm::timerAllDisable()
{
    timerUpdateDisplay->Enabled = false;
    timerDrivingJog->Enabled = false;
    timerTransLongRun->Enabled = false;
...
void __fastcall TMaskOnlineCMDForm::btnMoveTeachingPointMouseDown(TObject *Sender, TMouseButton Button,
...
void __fastcall TMaskOnlineCMDForm::mniMoveTPEQClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TMaskOnlineCMDForm::mniMoveTPEQ_XClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TMaskOnlineCMDForm::mniMoveTPSTOCKERClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TMaskOnlineCMDForm::mniMoveTPLOADERClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TMaskOnlineCMDForm::mniMoveTPUTBClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TMaskOnlineCMDForm::btnHoistHomeAutoSetClick(TObject *Sender)
{
   if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void TMaskOnlineCMDForm::Display_MapMove_Result(PKT_OHT2TP_MOVE_BY_MAP_RESULT PKT)
{
...
void TMaskOnlineCMDForm::Display_MapMake_Result(PKT_OHT2TP_LOG_DATA PKT)
{
...
void TMaskOnlineCMDForm::Display_Com(PKT_OHT2OCS PKT)
{
...
void __fastcall TMaskOnlineCMDForm::btnForceOpenClick(TObject *Sender)
{
/*
	TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
int TMaskOnlineCMDForm::TransLoad(void)
{
	char buf[MAX_PACKET_SIZE+1];
	int iPacketSize;
	int iReturn, iTemp;
...
int TMaskOnlineCMDForm::TransUnLoad(void)
{
	char buf[MAX_PACKET_SIZE+1];
	int iPacketSize;
	int iReturn, iTemp;
...
void __fastcall TMaskOnlineCMDForm::btnMapBaseMoveClick(TObject *Sender)
{
	char buf[MAX_PACKET_SIZE+1];
	int iPacketSize;
	int iReturn;
...
void TMaskOnlineCMDForm::PowerOnOff(int iAxis, int iOnOff)
{
...
void __fastcall TMaskOnlineCMDForm::btnMoveAllHomeClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TMaskOnlineCMDForm::btnAutoChangeClick(TObject *Sender)
{
    if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TMaskOnlineCMDForm::btnOCSMapBaseMoveClick(TObject *Sender)
{
	char buf[MAX_PACKET_SIZE+1];
	int iPacketSize;
	unsigned char cHigh, cMid, cLow;
...
void __fastcall TMaskOnlineCMDForm::chbBackWardClick(TObject *Sender)
{
	if(chbBackWard->Checked == true)
	{
		if(MessageDlg("Possible Backward Moving?", mtWarning, TMsgDlgButtons() << mbOK << mbCancel, 0) == IDOK)
		{
			chbBackWard->Checked = true;
		}
		else
		{
			chbBackWard->Checked = false;
...
//---------------------------------------------------------------------------
void __fastcall TMaskOnlineCMDForm::chkEmergencyJogClick(TObject *Sender)
{
	if(chkEmergencyJog->Checked == true)
	{
		if(MessageDlg("진입불가(ByPass불가) 구간으로 정말로 이동가능하도록 하시겠습니까?", mtWarning, TMsgDlgButtons() << mbOK << mbCancel, 0) == IDOK)
		{
			chkEmergencyJog->Checked = true;
		}
		else
		{
			chkEmergencyJog->Checked = false;
...
void __fastcall TMaskOnlineCMDForm::btnHandILHomeClick(TObject *Sender)
{
	if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
		...
		if(gm_OHTStatus->IOInput->uInputCommon.HAND_Left_Grip == OFF && gm_OHTStatus->IOInput->uInputCommon.HAND_Right_Grip == OFF)
		{
			m_CMDControl->MakeTPCmd2OHT(TPCMD_HAND_HOME_IL);
			MainForm->DataDisplay(COLOR_BLACK, "Hand Interlock Home");
		}
		else if(MessageDlg("FOUP is detected. Really Home?", mtWarning, TMsgDlgButtons() << mbOK << mbCancel, 0) == IDOK)
		{
			m_CMDControl->MakeTPCmd2OHT(TPCMD_HAND_HOME_IL);
			MainForm->DataDisplay(COLOR_BLACK, "Hand Interlock Home");
		}
...
void __fastcall TMaskOnlineCMDForm::btnHandILWorkClick(TObject *Sender)
{
	if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;

		m_CMDControl->MakeTPCmd2OHT(TPCMD_HAND_WORK_IL);
		MainForm->DataDisplay(COLOR_BLACK, "Hand Interlock Work");

		delete m_CMDControl;
	}
}
//---------------------------------------------------------------------------
void __fastcall TMaskOnlineCMDForm::btnDownUnloadPositionClick(TObject *Sender)
{
	if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
		...
		//if(gm_OHTStatus->IOInput->uInput.Input_MASK.HAND_LoadPosition == ON)
		//{
			///재하 포트에 있고 Hand 뻗은채로 Down 못하도록 인터락 추가
			if(gm_OHTStatus->IOInput->uInput.Input_MASK.HAND_Work == ON)
			{
				bYesNo = MessageDlg("Hand Work 상태입니다.\n작업을 진행 하시겠습니까?", mtWarning, TMsgDlgButtons() << mbOK << mbCancel, 0) == IDOK;
			}
			///재하 들고 Hand Home일때 Down 못하도록 인터락 추가
			else if(gm_OHTStatus->IOInput->uInput.Input_MASK.HAND_Home == ON && (gm_OHTStatus->IOInput->uInputCommon.HAND_Right_Grip == ON ||
						gm_OHTStatus->IOInput->uInputCommon.HAND_Left_Grip == ON))
			{
				bYesNo = MessageDlg("재하가 있습니다.\n작업을 진행 하시겠습니까?", mtWarning, TMsgDlgButtons() << mbOK << mbCancel, 0) == IDOK;
			}
		//}

		if(bYesNo)
		{
			m_CMDControl->MakeTPCmd2OHT(TPCMD_HOIST_DOWN_UNLOADPOSITION);
			MainForm->DataDisplay(COLOR_BLACK, "Hoist Down Unload Position");
		}
...
void __fastcall TMaskOnlineCMDForm::btnDownLoadPositionClick(TObject *Sender)
{
	if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;

		m_CMDControl->MakeTPCmd2OHT(TPCMD_HOIST_DOWN_LOADPOSITION);
		MainForm->DataDisplay(COLOR_BLACK, "Hoist Down Load Position");
...
void __fastcall TMaskOnlineCMDForm::btnUpLoadPositionClick(TObject *Sender)
{
	if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;

		m_CMDControl->MakeTPCmd2OHT(TPCMD_HOIST_UP_LOADPOSITION);
		MainForm->DataDisplay(COLOR_BLACK, "Hoist Up Load Position");
...
void __fastcall TMaskOnlineCMDForm::btnDownAntidropNoneClick(TObject *Sender)
{
	if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
		...
		{
			m_CMDControl->MakeTPCmd2OHT(TPCMD_HOIST_DOWN_ANTIDROP);
			MainForm->DataDisplay(COLOR_BLACK, "Hoist Down AntiDrop(None)");
		}
...
void __fastcall TMaskOnlineCMDForm::btnDownAntidropEUVClick(TObject *Sender)
{
	if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;

		if(gm_OHTStatus->IOInput->uInput.Input_MASK.EUVPod_Detect == OFF)
		{
			Warnning_Display(COLOR_YELLOW, "Carrier Not EUV...Hoist can't Down...");
		}
		else
		{
			m_CMDControl->MakeTPCmd2OHT(TPCMD_HOIST_DOWN_ANTIDROP_EUV);
			MainForm->DataDisplay(COLOR_BLACK, "Hoist Down AntiDrop(EUV)");
		}
...
void __fastcall TMaskOnlineCMDForm::btnDownAntidropRSP150Click(TObject *Sender)
{
	if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;

		if(gm_OHTStatus->IOInput->uInput.Input_MASK.RSP150_Detect == OFF)
		{
			Warnning_Display(COLOR_YELLOW, "Carrier Not RSP150...Hoist can't Down...");
		}
		else
		{
			m_CMDControl->MakeTPCmd2OHT(TPCMD_HOIST_DOWN_ANTIDROP_RSP150);
			MainForm->DataDisplay(COLOR_BLACK, "Hoist Down AntiDrop(RSP150)");
		}
...
void __fastcall TMaskOnlineCMDForm::btnDownAntidropRSP200Click(TObject *Sender)
{
	if(CmdMaskOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;

		if(gm_OHTStatus->IOInput->uInput.Input_MASK.RSP200_Detect == OFF)
		{
			Warnning_Display(COLOR_YELLOW, "Carrier Not RSP200...Hoist can't Down...");
		}
		else
		{
			m_CMDControl->MakeTPCmd2OHT(TPCMD_HOIST_DOWN_ANTIDROP_RSP200);
			MainForm->DataDisplay(COLOR_BLACK, "Hoist Down AntiDrop(RSP200)");
		}
