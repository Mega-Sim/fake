...
int TCHKOHT::Axis_Hoist_HOME()
{
	...
	bLookDown = gm_OHTStatus->IOInput->uInputCommon.LOOKDOWN_Detect;
...
//	{
		if((gm_OHTStatus->Motor_Hoist->dPosition_mm > OSCILLATION_IGNORE_DIST ) &&(bOscillation == false))
		{
		   ShowMessage(L"For Moving Hoist Home, Oscillation Detecting!! ");
		   return 0;
...
//	}
	...
	if((gm_OHTStatus->Motor_Hoist->dPosition_mm > LOOKDOWN_DETECT_RANGE ) &&(bLookDown == true))
	{
	   ShowMessage(L"For Moving Hoist Home, LookDown Detecting!! ");
	   return 0;
			...
			if(gm_OHTStatus->Motor_Hoist->dPosition_mm > HOIST_HOME_CHECKDIST_FOR_HANDPUSHOFF*2)
			{
				...
				if(Application->MessageBox(L"15mm 이동했습니다.\n확인을 누르면 Home으로 이동됩니다.", L"Confirm", MB_OKCANCEL) == IDOK )
				{
					if(Is_MoveAxis_Hoist()==NO_ERR)
					{
						...
						if((gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Ver == OHT_VERSION_85) || (gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Ver == OHT_VERSION_82)|| (gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Ver == OHT_VERSION_86))
						{
...
{
	bool bShiftHome = gm_OHTStatus->IOInput->uInputCommon.SHIFT_Home;

	if((fabs(gm_OHTStatus->Motor_Shift->dPosition_mm)> 5)&& (bShiftHome ==true))
	{
	   ShowMessage(L"For Moving Shift Home, Shift Home Detecting!! ");
	   return 0;
	}
...
int TCHKOHT::Axis_Shift_Ratate_HOME(void)
{
...
int TCHKOHT::Is_MoveAxis_Hoist(void)
{
   ...
   if(nError) return nError;
	   ...
	   if(gm_OHTStatus->CheckOnOff->bUse_ShutterControl==ON)
	   {
		  ...
		  {
//			 nError=1;
//			 Warnning_Display(COLOR_RED, "Shutter is Open. Hoist can't Move !!");
		  }
	   }
#endif
