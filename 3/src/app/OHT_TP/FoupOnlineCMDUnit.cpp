  ...
  //기존 :panVer->Caption = SocketForm->m_strOHTNum;
   //20170710 호기명  추가
	...
	if(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_MAC)
		FoupOnlineCMDForm->Caption = "MacOnlineCMDForm";
	else
		FoupOnlineCMDForm->Caption = "FoupOnlineCMDForm";
...
//		panMC_Status->Color   = (gm_OHTStatus->IOInput->uInputCommon.MC_Status == ON) ? COLOR_LIME : COLOR_WHITE;
	...
	if(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_MAC)  //2023.11.13 sseung MAC Type 조건 추가
	{
		panUTB->Visible = false;
		panFoupDetect->Visible = false;

		if(panMAC_Detect->Visible == false)
			panMAC_Detect->Visible = true;
		panMAC_Detect->Color = (gm_OHTStatus->IOInput->uInputCommon.FOUP_Detect == ON) ? COLOR_LIME : COLOR_WHITE;
	}
	else
	{
    ...
    */
	...
	//20.07.21 JHJ BlackBox 추가                        FoupOnlineCMDForm->grpBlackBox->Visible = true;
	if(gm_OHTStatus->m_OHTStatus.IsWarningState.IsblackboxState ==ON)
	{
		if(FoupOnlineCMDForm->grpBlackBox->Visible ==false) FoupOnlineCMDForm->grpBlackBox->Visible = true;

		if(gm_OHTStatus->IOInput->uInputCommon.BlackBox_Error == ON)
		{
			shpBlackBox->Brush->Color = COLOR_RED;
			lblBlackBox->Font->Color = clWhite;
			lblBlackBox->Caption = "BlackBox ERROR";
		}
		else
		{
			shpBlackBox->Brush->Color = COLOR_LIME;
			lblBlackBox->Font->Color = clBlack;
			lblBlackBox->Caption = "BlackBox OK...";
		}
	}else
		if(FoupOnlineCMDForm->grpBlackBox->Visible ==true) FoupOnlineCMDForm->grpBlackBox->Visible = false;

	pnlSTBSensor->Color = (gm_OHTStatus->m_OHTStatus.IsWarningState.IsAutoTeachingVision == ON) ? COLOR_LIME : COLOR_WHITE;

	if(gm_OHTStatus->m_OHTStatus.IsWarningState.IsAutoTeachingVision != ON)
	{
		btnLSTBQRRRead->Enabled = false;
		btnRSTBQRRRead->Enabled = false;
	}
	else
	{
		btnLSTBQRRRead->Enabled = true;
		btnRSTBQRRRead->Enabled = true;
	}

	panCharging->Color = (gm_OHTStatus->m_OHTStatus.cHID_ChargeState == 'C') ? COLOR_LIME : COLOR_WHITE;
}
// ---------------------------------------------------------------------------
...
void __fastcall TFoupOnlineCMDForm::btnMotor34HomeClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		CmdFoupOHTCheck.Axis_Shift_Ratate_HOME();
	}
}
    ...
    TMouseButton Button, TShiftState Shift, int X, int Y)
{
    ...
    if(Button == mbLeft)
    {
		...
		if(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_MAC)  //2023.12.13 sseung MAC일 때 EQ, STB_R만 있는 팝업 사용
			pmLoadMAC->Popup(Q.x, Q.y);
		else
...
void __fastcall TFoupOnlineCMDForm::chbBackWardClick(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TFoupOnlineCMDForm::chkEmergencyJogClick(TObject *Sender)
{
	if(chkEmergencyJog->Checked == true)
	{
...
//---------------------------------------------------------------------------
void __fastcall TFoupOnlineCMDForm::btnLSTBQRRReadClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;

		m_CMDControl->MakeTPCmd2OHT(TPCMD_LSTB_TEACHING_MEASURE);
		MainForm->DataDisplay(COLOR_BLACK, "LSTB Teaching Measure");
...
void __fastcall TFoupOnlineCMDForm::btnRSTBQRRReadClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;

		m_CMDControl->MakeTPCmd2OHT(TPCMD_RSTB_TEACHING_MEASURE);
		MainForm->DataDisplay(COLOR_BLACK, "RSTB Teaching Measure");

		delete m_CMDControl;
	}
}
//---------------------------------------------------------------------------
void __fastcall TFoupOnlineCMDForm::mniLoadSTB_L_TClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
		TransForm->SetTrans(CMD_LOAD, PORT_TYPE_STB_L_T);
        TransForm->ShowModal();
    }
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::mniLoadSTB_L_BClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
		TransForm->SetTrans(CMD_LOAD, PORT_TYPE_STB_L_B);
		TransForm->ShowModal();
	}
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::mniLoadSTB_R_TClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
		TransForm->SetTrans(CMD_LOAD, PORT_TYPE_STB_R_T);
        TransForm->ShowModal();
    }
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::mniLoadSTB_R_BClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
		TransForm->SetTrans(CMD_LOAD, PORT_TYPE_STB_R_B);
		TransForm->ShowModal();
	}
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::mniUnloadSTB_L_TClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
		TransForm->SetTrans(CMD_UNLOAD, PORT_TYPE_STB_L_T);
		TransForm->ShowModal();
	}
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::mniUnloadSTB_L_BClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
		TransForm->SetTrans(CMD_UNLOAD, PORT_TYPE_STB_L_B);
		TransForm->ShowModal();
	}
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::mniUnloadSTB_R_TClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
		TransForm->SetTrans(CMD_UNLOAD, PORT_TYPE_STB_R_T);
		TransForm->ShowModal();
	}
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::mniUnloadSTB_R_BClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
		TransForm->SetTrans(CMD_UNLOAD, PORT_TYPE_STB_R_B);
		TransForm->ShowModal();
	}
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::mniMoveTPSTB_L_TClick(TObject *Sender)
{
    if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
		TransForm->SetTrans(CMD_MOVETPOINT, PORT_TYPE_STB_L_T);
        TransForm->ShowModal();
	}
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::mniMoveTPSTB_L_BClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
		TransForm->SetTrans(CMD_MOVETPOINT, PORT_TYPE_STB_L_B);
		TransForm->ShowModal();
	}
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::mniMoveTPSTB_R_TClick(TObject *Sender)
{
    if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
		TransForm->SetTrans(CMD_MOVETPOINT, PORT_TYPE_STB_R_T);
        TransForm->ShowModal();
	}
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::mniMoveTPSTB_R_BClick(TObject *Sender)
{
    if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
        TransForm->SetTrans(CMD_MOVETPOINT, PORT_TYPE_STB_R_B);
        TransForm->ShowModal();
	}
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::mniLoadSTB_L_B_SClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
		TransForm->SetTrans(CMD_LOAD, PORT_TYPE_STB_L_B_S);
		TransForm->ShowModal();
	}
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::mniLoadSTB_R_B_SClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
		TransForm->SetTrans(CMD_LOAD, PORT_TYPE_STB_R_B_S);
		TransForm->ShowModal();
	}
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::mniUnloadSTB_L_B_SClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
		TransForm->SetTrans(CMD_UNLOAD, PORT_TYPE_STB_L_B_S);
		TransForm->ShowModal();
	}
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::mniUnloadSTB_R_B_SClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
		TransForm->SetTrans(CMD_UNLOAD, PORT_TYPE_STB_R_B_S);
		TransForm->ShowModal();
	}
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::mniMoveTPSTB_L_B_SClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
		TransForm->SetTrans(CMD_MOVETPOINT, PORT_TYPE_STB_L_B_S);
		TransForm->ShowModal();
	}
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::mniMoveTPSTB_R_B_SClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
		TransForm->SetTrans(CMD_MOVETPOINT, PORT_TYPE_STB_R_B_S);
		TransForm->ShowModal();
	}
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::btnDualStageClick(TObject *Sender)
{
	char buf[MAX_PACKET_SIZE+1];
	int iPacketSize;
	int iReturn;
	...
	SEND_PACKET.BODY.nCycle = rgDualStageSTB->ItemIndex + 2;

	SEND_PACKET.BODY.PortType = PORT_TYPE_PSTB;
...
//---------------------------------------------------------------------------
//2023.12.13 sseung MAC 타입일 때 Popup Menu 동작
void __fastcall TFoupOnlineCMDForm::mniLoadMAC_EQClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
//---------------------------------------------------------------------------
void __fastcall TFoupOnlineCMDForm::mniLoadMAC_STB_RClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TFoupOnlineCMDForm::mniUnloadMAC_EQClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TFoupOnlineCMDForm::mniUnloadMAC_STB_RClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TFoupOnlineCMDForm::PSTBL1Click(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
		TransForm->SetTrans(CMD_LOAD, PORT_TYPE_PSTB);
		TransForm->ShowModal();
	}
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::PSTBL2Click(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
		TransForm->SetTrans(CMD_UNLOAD, PORT_TYPE_PSTB);
		TransForm->ShowModal();
	}
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::mniUnloadMAC_EQ_XClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TFoupOnlineCMDForm::mniUnloadMAC_STOCKERClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TFoupOnlineCMDForm::mniUnloadMAC_LOADERClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TFoupOnlineCMDForm::mniUnloadMAC_UTBClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TFoupOnlineCMDForm::mniUnloadMAC_STB_R_B_SEMESClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TFoupOnlineCMDForm::mniLoadMAC_EQ_XClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TFoupOnlineCMDForm::mniLoadMAC_STOCKERClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TFoupOnlineCMDForm::mniLoadMAC_LOADERClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TFoupOnlineCMDForm::mniLoadMAC_UTBClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TFoupOnlineCMDForm::mniLoadMAC_STB_R_B_SEMESClick(TObject *Sender)
{
	if(CmdFoupOHTCheck.Is_Executable() == NO_ERR)
	{
		// TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TFoupOnlineCMDForm::btnGetBTInfoClick(TObject *Sender)
{
	TCMDCONTROL *m_CMDControl = new TCMDCONTROL;

	m_CMDControl->MakeTPCmd2OHT(TPCMD_GET_BLUETOOTH_INFO);
	MainForm->DataDisplay(COLOR_BLACK, "Get Bluetooth Information");

	delete m_CMDControl;
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::btnSetWaitClick(TObject *Sender)
{
	TCMDCONTROL *m_CMDControl = new TCMDCONTROL;

	m_CMDControl->MakeTPCmd2OHT(TPCMD_BLUETOOTH_SET_WAIT);
	MainForm->DataDisplay(COLOR_BLACK, "Bluetooth Set Wait");

	delete m_CMDControl;
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::btnSWResetClick(TObject *Sender)
{
	TCMDCONTROL *m_CMDControl = new TCMDCONTROL;

	m_CMDControl->MakeTPCmd2OHT(TPCMD_BLUETOOTH_SW_RESET);
	MainForm->DataDisplay(COLOR_BLACK, "Bluetooth SW Reset");

	delete m_CMDControl;
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::btnHWResetClick(TObject *Sender)
{
	TCMDCONTROL *m_CMDControl = new TCMDCONTROL;

	m_CMDControl->MakeTPCmd2OHT(TPCMD_BLUETOOTH_HW_RESET);
	MainForm->DataDisplay(COLOR_BLACK, "Bluetooth HW Reset");

	delete m_CMDControl;
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::btnVMSConnectClick(TObject *Sender)
{
	TCMDCONTROL *m_CMDControl = new TCMDCONTROL;

	m_CMDControl->MakeTPCmd2OHT(TPCMD_VMS_CONNECT);
	MainForm->DataDisplay(COLOR_BLACK, "VMS Connect");

	delete m_CMDControl;
}
//---------------------------------------------------------------------------

void __fastcall TFoupOnlineCMDForm::btnVMSDisconnectClick(TObject *Sender)
{
	TCMDCONTROL *m_CMDControl = new TCMDCONTROL;

	m_CMDControl->MakeTPCmd2OHT(TPCMD_VMS_DISCONNECT);
	MainForm->DataDisplay(COLOR_BLACK, "VMS Disconnect");
