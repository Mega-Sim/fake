...
__fastcall TTeachingForm::TTeachingForm(TComponent* Owner) : TForm(Owner)
{
	...
	cbbPre_SpecailEQLevel->Enabled = false;
	cbbPre_PIOType->Enabled = false;
	...
	cbbPre_MaterialType->Enabled = false;
...
void __fastcall TTeachingForm::FormShow(TObject *Sender)
{
	...
	{
		btnCheckPodEuv->Visible = true;
	}
	else
	{
		btnCheckPodEuv->Visible = false;
	}

	// 개발자 모드에서만 pulseMove 가능 lsg 2025.06.12
	if(gm_OHTStatus->nProgram_Mode == MODE_SETUP)
		grphoistpulse->Visible = true;
	else
		grphoistpulse->Visible = false;
...
void __fastcall TTeachingForm::timerUpdateDisplayTimer(TObject *Sender)
{
	...
	lbHoistCurCts->Caption = gm_OHTStatus->Motor_Hoist->dPosition_cts;
...
//    edtRotate->Text = gm_OHTStatus->Motor_Rotate->dPosition_mm;
	...
	{
		btnVisionAutoTeaching->Enabled = false;
		cbb1->Enabled = false;
	}
	else
	{
		btnVisionAutoTeaching->Enabled = true;
		cbb1->Enabled = true;
	}
	if(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_MASK)
	{
		preMaterialType->Visible = false;
		cbbPre_MaterialType->Visible = false;
		MaterialType->Visible = false;
		cbbMaterialType->Visible =false;
		btnCheckPodEuv->Visible = false;
	}
	else
	{
		preMaterialType->Visible = true;
		cbbPre_MaterialType->Visible = true;
		MaterialType->Visible = true;
		cbbMaterialType->Visible =true;
		btnCheckPodEuv->Visible = true;
	}
...
{
	// 2025.04.14 lsg Hand Alarm, Gripper 이상 시 PopUp
	if( ((gm_OHTStatus->IOInput->uInputCommon.HAND_Left_Grip == gm_OHTStatus->IOInput->uInputCommon.HAND_Right_Grip) && (gm_OHTStatus->IOInput->uInputCommon.HAND_Alarm) == false) ||
		bHandCheck == true )
	{
		if(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_EUV)
		{
	...
	// 2025. 04. 14 lsg Hand Gripper IO not equal or Hand Alarm On 
	else if(MessageDlg("Check Hand!! Check Foup!! Really Move?",
		...
		bHandCheck == true )
	{
		if(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_EUV)
		{
			if((TechOHTCheck.Is_Executable() == NO_ERR) && (TechOHTCheck.Is_MoveAxis_Hoist() == NO_ERR))
			{
				...
				tmrJogMove->Enabled = true;
				MainForm->DataDisplay(COLOR_BLACK, "HOIST_JOG_DOWN");
			}
		}
		else
		{
...
void __fastcall TTeachingForm::btnHoistMoveUpDistClick(TObject *Sender)
{
	...
	// 2025.04.14 lsg Hand Check
	if( (gm_OHTStatus->IOInput->uInputCommon.HAND_Left_Grip != gm_OHTStatus->IOInput->uInputCommon.HAND_Right_Grip) ||
		(gm_OHTStatus->IOInput->uInputCommon.HAND_Alarm) == true )
	{
		if(MessageDlg("Check Hand!! Check Foup!! Really Move?",
				mtWarning, TMsgDlgButtons() << mbOK << mbCancel, 0) != IDOK)
		{
			bHandCheck = false;
		}
	...
	if((TechOHTCheck.Is_Executable() == NO_ERR) && (TechOHTCheck.Is_MoveAxis_Hoist() == NO_ERR) && bHandCheck == true)
	{
...
void __fastcall TTeachingForm::btnShiftMoveLeftDistClick(TObject *Sender)
{
	...
	if((gm_OHTStatus->IOInput->uInputCommon.HOIST_Home == false)
	&&(fabs(StrToFloat(gm_OHTStatus->Motor_Hoist->dPosition_mm)) > HOIST_ORIGIN_LIMIT))
	{
		bYesNo = MessageDlg("Hoist가 Home이 아닙니다. Slide를 움직이시겠습니까?", mtWarning, TMsgDlgButtons() << mbOK << mbCancel, 0) == IDOK;
	}
...
void __fastcall TTeachingForm::btnSetHoistNegLimitClick(TObject *Sender)
{
	...
	nLimit = edtHoistNegLimit->Text.ToInt();
   //	TPCmd.TPCmd.Data1 = StrToIntDef(edtHoistNegLimit->Text, 0);
	TPCmd.CmdTP_Jog_TR.Distance  = edtHoistNegLimit->Text.ToInt();
...
void __fastcall TTeachingForm::btnSetHoistPosLimitClick(TObject *Sender)
{
	...
	nLimit = edtHoistPosLimit->Text.ToInt();
  //	TPCmd.TPCmd.Data1 = StrToIntDef(edtHoistPosLimit->Text, 0);
	TPCmd.CmdTP_Jog_TR.Distance  = edtHoistPosLimit->Text.ToInt();
...
void TTeachingForm::SetDefaultTeachingData()
{
	...
	cbbPre_SpecailEQLevel->ItemIndex = -1;
...
void __fastcall TTeachingForm::btnTeachingSaveClick(TObject *Sender)
{
	...
	if(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_EUV)
	{
		if(gm_OHTStatus->m_OHTStatus.bCheckCompletePodEuv == false)
		{
			ERROR_MSGBOX("Check Material Type!!!");
			return;
		}
	}
	...
	*/
	...
	if(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_EUV)
	{
		SEND_PACKET.BODY.Hoist = m_dHoistPos;
		SEND_PACKET.BODY.Shift = m_dShiftPos;
		SEND_PACKET.BODY.Rotate = m_dRotatePos;
	}
	...
	*/
	...
	SEND_PACKET.BODY.PIOCS = edtPIOCS->Text.ToInt();

	///PIO Type과 PIO ID/CS간 정합성 기능 추가
	SEND_PACKET.BODY.PIOChannel = edtPIOChannel->Text.ToInt();

	if(cbbPIOType->ItemIndex == EQ_PIO_IR)
	{
		SEND_PACKET.BODY.PIOID = edtPIOID->Text.ToInt();

		if((SEND_PACKET.BODY.PIOID !=0) || (SEND_PACKET.BODY.PIOChannel != 0))
		{
			ERROR_MSGBOX("IR Type에 PIO ID/CH이 있습니다.");
			return;
		}
	}else if(cbbPIOType->ItemIndex == EQ_PIO_HYBRID_SN)
	{
		int CheckChar = edtPIOID->Text.Length();
		char szTmp[1024] ={0,};

		for(int i =0; i< CheckChar; i++)
		{
			sprintf(szTmp, "%s", edtPIOID->Text.SubString(i+1,1));
			if(!((szTmp[0] >= 0x30 && szTmp[0] <= 0x39) ||
				(szTmp[0] >= 0x41 && szTmp[0] <= 0x46) ||
				(szTmp[0] >= 0x61 && szTmp[0] <= 0x66)))
				{
					ERROR_MSGBOX("PIO ID가 16진수(0~F) 범위 외 입니다.");
					return;
				}
		}

		AnsiString strRecv_Data = edtPIOID->Text;

		SEND_PACKET.BODY.PIOID = (int)strtol(strRecv_Data.c_str(), NULL, 16); // hex 처리

		if((SEND_PACKET.BODY.PIOID ==0) || (SEND_PACKET.BODY.PIOChannel == 0))
		{
			ERROR_MSGBOX("Hybrid Type에 PIO ID/CH이 없습니다 .");
			return;
		}
	}
	else
	...
	if(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_EUV)
	SEND_PACKET.BODY.MaterialType = cbbMaterialType->ItemIndex;
	else
	SEND_PACKET.BODY.MaterialType = 0;
	...
	SEND_PACKET.BODY.EtcData.Info.SpecialEQLevel = cbbSpecailEQLevel->ItemIndex;   //AOHC-294
...
void TTeachingForm::DisplayTeachingData(PKT_OHT2TP_TEACHING_INFO *pTData)
{
    ...
    else
    {
        ...
        cbbPre_PortType->ItemIndex = pTData->BODY.PortType - 1;

        // by zzang9un 2013.09.09 : 프로토콜 v2.9에 의해 단위 수정함(1/10000m)
        edtPre_Hoist->Text = pTData->BODY.Hoist / 10.0;
        edtPre_Shift->Text = pTData->BODY.Shift / 10.0;
        edtPre_Rotate->Text = pTData->BODY.Rotate;

        cbbPre_PIODirection->ItemIndex = pTData->BODY.EtcData.Info.PIODirection - 1;
		edtPre_PIOTimeLevel->Text = pTData->BODY.EtcData.Info.PIOTimeLevel;
		edtPre_LookdownLevel->Text = pTData->BODY.EtcData.Info.LookDownLevel;
		chkPre_RFReader->Checked = pTData->BODY.EtcData.Info.RFReader;
		chkPre_TransSound->Checked = pTData->BODY.EtcData.Info.TransSound;
		chkPre_Oscillation->Checked = pTData->BODY.EtcData.Info.Oscillation;
		chkPre_HandDetectEQ->Checked = pTData->BODY.EtcData.Info.HandDetectEQ;
		cbbPre_SpecailEQLevel->ItemIndex = pTData->BODY.EtcData.Info.SpecialEQLevel;

		cbbPre_PIOType->ItemIndex = pTData->BODY.PIOType-1;

		//= (int)strtol(strRecv_Data.c_str(), NULL, 16); // hex 처리

		AnsiString s;
		s.sprintf("%x",pTData->BODY.PIOID);

		if(cbbPre_PIOType->ItemIndex == EQ_PIO_HYBRID_SN)
			edtPre_PIOID->Text = s;
		else
			edtPre_PIOID->Text = pTData->BODY.PIOID;

		edtPre_PIOChannel->Text = pTData->BODY.PIOChannel;
		edtPre_PIOCS->Text = pTData->BODY.PIOCS;
		cbbPre_MaterialType->ItemIndex =pTData->BODY.MaterialType;
    }
}
//---------------------------------------------------------------------------
/**
...
//		S.sprintf(L"ID :%d, Port :%d", pTData->BODY.Reserved1, pTData->BODY.PortType);
//		memoAT->Lines->Add(S);
...
void __fastcall TTeachingForm::btnVisionAutoTeachingClick(TObject *Sender)
{
	// Step 2. Port Type 체크
	if(cbb1->ItemIndex < 0)
	{
	...
	SEND_PACKET.BODY.PortType = cbb1->ItemIndex + 1;
//	SEND_PACKET.BODY.Reserved1 = StrToInt(edtStationIDAT->Text);
...
void __fastcall TTeachingForm::btnPULSE_MOVEClick(TObject *Sender)
{
	...
	else
	{
		...
		{
			ShowMessage(L"ERROR : ERR_SHUTTER_NOT_OPEN...");
			return;
		}

		// MainForm->timerStatusRequest->Enabled = false;

		double dblMoveSpeed = hoistpulSpeed->Text.ToInt();
		double dblHoistPulse = hoistpulsevalue->Text.ToDouble();

		if(gm_OHTStatus->Motor_Hoist->dPosition_mm < HoistNegativeLimit4PulseMoving)
		{
			ERROR_MSGBOX("Hoist Negative Limit 값 초과,Origin 확인 필요");
			return;
		}
		// 여기서 기능 실행
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
		...
		SEND_PACKET.BODY.PositionValue[0].dPulse = dblHoistPulse;
...
void __fastcall TTeachingForm::btnCheckPodEuvClick(TObject *Sender)
{
	//POD_EUV 구분을 위하여 Hoist 이동
	if(Application->MessageBox(L"재하 구분을 위하여 Hoist Home 이동시키겠습니까?\n다시 한번 확인해보시기 바랍니다.", L"Confirm", MB_OKCANCEL) != IDOK)
		return;
	if(gm_OHTStatus->m_OHTStatus.Data.MSCmdID.Info.Mode == MODE_AUTO)
	{
		ERROR_MSGBOX("OHT가 Manual 상태가 아닙니다.");
		return;
	}

	if(gm_OHTStatus->IOInput->uInputCommon.HAND_Close == false || gm_OHTStatus->IOInput->uInputCommon.HAND_Left_Grip == false || gm_OHTStatus->IOInput->uInputCommon.HAND_Right_Grip == false)
	{
		ERROR_MSGBOX("Hand Close 상태가 아닙니다.");
		return;
	}
	...
	m_dHoistPos = gm_OHTStatus->Motor_Hoist->dPosition_mm*10;
	m_dShiftPos = gm_OHTStatus->Motor_Shift->dPosition_mm*10;
	m_dRotatePos = gm_OHTStatus->Motor_Rotate->dPosition_mm;///10;
	...
	// 이적재 명령 구조체
	iPacketSize = sizeof(PKT_TP2OHT_CHECK_MATERIAL);
	PKT_TP2OHT_CHECK_MATERIAL	SEND_PACKET;
	...
	SEND_PACKET.TAG.Value = TagID::CMD_TP_CHECK_MATERIAL;
	SEND_PACKET.BODY.MaterialType = cbbMaterialType->ItemIndex;//0 : FOUP, 1 : RSP-150, 2 : RSP-200, 3 : EUV-POD
