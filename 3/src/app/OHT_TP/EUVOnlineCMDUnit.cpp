...
#pragma resource "*.dfm"
TEUVOnlineCMDForm *EUVOnlineCMDForm;
...
// ---------------------------------------------------------------------------
__fastcall TEUVOnlineCMDForm::TEUVOnlineCMDForm(TComponent *Owner) : TForm(Owner)
{
...
void __fastcall TEUVOnlineCMDForm::WMEXITSIZEMOVE(TMessage &msg)
{
    int nTmpLeft, nTmpTop, nTmpRight;

    nTmpLeft  = EUVOnlineCMDForm->Left;
    nTmpTop   = EUVOnlineCMDForm->Top;
    nTmpRight = nTmpLeft + EUVOnlineCMDForm->Width;
...
// ---------------------------------------------------------------------------
TTextAttributes *__fastcall TEUVOnlineCMDForm::CurrText(void)
{
...
// ---------------------------------------------------------------------------
/**
 @brief   EUVOnlineCMDForm의 Show 발생시 호출되는 이벤트 함수
 @author  puting
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::FormShow(TObject *Sender)
{
...
//		panVer->Font->Size = 48;
  ...
  //기존 :panVer->Caption = SocketForm->m_strOHTNum;
   //20170710 호기명  추가
	...
	//2022.05.11 sseung 운영용 모드일때만 Map 기반이동 비활성화
	if(gm_OHTStatus->nProgram_Mode == MODE_WORKER)
	{
		btnMapBaseMove->Enabled = false;
	}
	else
	{
		btnMapBaseMove->Enabled = true;
	}
...
// ---------------------------------------------------------------------------
void __fastcall TEUVOnlineCMDForm::FormClose(TObject *Sender,
    TCloseAction &Action)
{
    timerAllDisable();
}
...
// ---------------------------------------------------------------------------
int TEUVOnlineCMDForm::Check_Invalid_Command(String strTmpCMD)
{
    ...
    if((strTmpCMD.Pos("[") != 0) || (strTmpCMD.Pos("]") != 0))
    {
        ...
        {
            if(CmdEUVOHTCheck.Is_Executable
...
void __fastcall TEUVOnlineCMDForm::mniMemoClearClick(TObject *Sender)
{
    PMACMemo->Clear();
}

// ---------------------------------------------------------------------------
void TEUVOnlineCMDForm::PMACDataDisplay(TColor TmpFontColor, String strDisplayData)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnHandCloseClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnHandOpenClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        if(gm_OHTStatus->nProgram_Mode == MODE_SETUP || gm_OHTStatus->nProgram_Mode == MODE_WORKER)// 개발자, 운영자 동일
        {
            TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
 ...
 */
void __fastcall TEUVOnlineCMDForm::btnGoClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
...
/**
 ...
 @param   EStop
 */
void __fastcall TEUVOnlineCMDForm::btnStopClick(TObject *Sender)
{
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnTransMarkForwardClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        String strTmpCMD;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnTransMarkBackwardClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        String strTmpCMD;
...
void __fastcall TEUVOnlineCMDForm::memoJogKeyControlExit(TObject *Sender)
{
...
void __fastcall TEUVOnlineCMDForm::memoJogKeyControlClick(TObject *Sender)
{
...
@date    2013.02.19
*/
void __fastcall TEUVOnlineCMDForm::memoJogKeyControlKeyDown(TObject *Sender,
...
@date    2013.02.19
*/
void __fastcall TEUVOnlineCMDForm::memoJogKeyControlKeyUp(TObject *Sender,
    WORD &Key, TShiftState Shift)
{
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
*/
void __fastcall TEUVOnlineCMDForm::timerDrivingJogTimer(TObject *Sender)
{
    bool bError = NO_ERR;
	...
	if(memoJogKeyControl->Focused())
    {
		...
		if(CmdEUVOHTCheck.Is_Executable(EXCLUDE_DRIVING_RUNNING) == NO_ERR)
        {
         ...
         //8.6 IO 재확인 후 수정 필요
		if(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Ver == OHT_VERSION_86)
		{
			if((gm_OHTStatus->IOInput->uInputCommon.FOUP_Detect == true) ||
			   (gm_OHTStatus->IOInput->uInput.Input_V86_EUV.POD_EUV_1 == true))
			{
				...
				if((gm_OHTStatus->IOInput->uInputCommon.FOUP_Detect == true) &&
				   (gm_OHTStatus->IOInput->uInput.Input_V86_EUV.POD_EUV_1 == false))
//					(gm_OHTStatus->IOInput->uInputCommon.POD_EUV_2 == false)))
				{
				...
				if((gm_OHTStatus->IOInput->uInput.Input_V86_EUV.POD_EUV_1 == true))
//				   (gm_OHTStatus->IOInput->uInputCommon.POD_EUV_2 == true))
			   {
				   if (gm_OHTStatus->IOInput->uInputCommon.SHUTTER_F_Middle_Close == false ||
					   gm_OHTStatus->IOInput->uInputCommon.SHUTTER_R_Middle_Close == false)
					{
						Warnning_Display(COLOR_RED, "ERROR : ERR_SHUTTER_NOT_MIDDLE_CLOSE...");
						timerDrivingJog->Enabled = false;
						chbBackWard->Checked = false;
						return;
					}
		...
		{
			if((gm_OHTStatus->IOInput->uInputCommon.FOUP_Detect == true) ||
			   (gm_OHTStatus->IOInput->uInputCommon.POD_EUV_1 == true))
//				(gm_OHTStatus->IOInput->uInputCommon.POD_EUV_2 == true)))
			{
				if (gm_OHTStatus->IOInput->uInputCommon.HAND_Open == true ||
					gm_OHTStatus->IOInput->uInputCommon.HAND_Close == false ||
				...
				if((gm_OHTStatus->IOInput->uInputCommon.FOUP_Detect == true) &&
				   (gm_OHTStatus->IOInput->uInputCommon.POD_EUV_1 == false))
//					(gm_OHTStatus->IOInput->uInputCommon.POD_EUV_2 == false)))
				{
					if (gm_OHTStatus->IOInput->uInputCommon.SHUTTER_F_Close == false ||
						gm_OHTStatus->IOInput->uInputCommon.SHUTTER_R_Close == false)
				...
				if((gm_OHTStatus->IOInput->uInputCommon.POD_EUV_1 == true))
//				   (gm_OHTStatus->IOInput->uInputCommon.POD_EUV_2 == true))
			   {
				   if (gm_OHTStatus->IOInput->uInputCommon.SHUTTER_F_Middle_Close == false ||
					   gm_OHTStatus->IOInput->uInputCommon.SHUTTER_R_Middle_Close == false)
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnHandOriginClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnHandResetClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnHandFreeClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TEUVOnlineCMDForm::btnSteeringLeftClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
// ---------------------------------------------------------------------------
void __fastcall TEUVOnlineCMDForm::btnSteeringRightClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
@date    2012.12.14
*/
void __fastcall TEUVOnlineCMDForm::btnShutterOpenClick(TObject *Sender)
{
...
@date    2012.12.14
*/
void __fastcall TEUVOnlineCMDForm::btnShutterCloseClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
@date    2012.12.14
*/
void __fastcall TEUVOnlineCMDForm::btnShutterOriginClick(TObject *Sender)
{
...
@date    2012.12.14
*/
void __fastcall TEUVOnlineCMDForm::btnShutterFreeClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
// ---------------------------------------------------------------------------
int TEUVOnlineCMDForm::Set_UBGColor(int nOutput)
{
    int nError = 0;
...
/**
 @brief   EUVOnlineCMDForm UI를 주기적으로 갱신하는 Timer 이벤트 함수
 @author  zzang9un
 @date    2013.03.29
 */
void __fastcall TEUVOnlineCMDForm::timerUpdateDisplayTimer(TObject *Sender)
{
	String strVelData;
    ...
    if(pgcPosVel->ActivePageIndex == 0)
    {
        ...
        panPosition_Rotate->Caption    = gm_OHTStatus->Motor_Rotate->dPosition_mm/10;
...
//	RcvData.VolatageMonitor[0]&0xFFFF, (RcvData.VolatageMonitor[0]>>16)&0xFFFF, RcvData.VolatageMonitor[1]&0xFFFF, (RcvData.VolatageMonitor[1]>>16)&0xFFFF
	...
	shpShutterMiddleClose_Front->Brush->Color = (gm_OHTStatus->IOInput->uInputCommon.SHUTTER_F_Middle_Close == ON) ? COLOR_LIME : COLOR_WHITE;
	shpShutterMiddleClose_Rear->Brush->Color = (gm_OHTStatus->IOInput->uInputCommon.SHUTTER_R_Middle_Close == ON) ? COLOR_LIME : COLOR_WHITE;
	...
	//UTB Sensor 감지시 색 변환
	if(gm_OHTStatus->m_OHTStatus.MarkInfo.Info.StopTag)
    {
	...
	panMC_Status->Color   = (gm_OHTStatus->IOOutput->uOutputCommon.SAFETY_Reset == ON) ? COLOR_LIME : COLOR_WHITE;
	...
	if(panEUV_Detect1->Visible == false)
		panEUV_Detect1->Visible = true;
	if(panEUV_Detect2->Visible == false)
		panEUV_Detect2->Visible = true;
	panFoupDetect->Color = (gm_OHTStatus->IOInput->uInputCommon.FOUP_Detect == ON) ? COLOR_LIME : COLOR_WHITE;
	if(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Ver == OHT_VERSION_86)
		panEUV_Detect1->Color = (gm_OHTStatus->IOInput->uInput.Input_V86_EUV.POD_EUV_1 == ON) ? COLOR_LIME : COLOR_WHITE;
	else
	{
		panEUV_Detect1->Color = (gm_OHTStatus->IOInput->uInputCommon.POD_EUV_1 == ON) ? COLOR_LIME : COLOR_WHITE;
		panEUV_Detect2->Color = (gm_OHTStatus->IOInput->uInputCommon.POD_EUV_2 == ON) ? COLOR_LIME : COLOR_WHITE;
	}
...
@date    2012.12.14
*/
void __fastcall TEUVOnlineCMDForm::btnStopTagBCRClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
@date    2012.12.14
*/
void __fastcall TEUVOnlineCMDForm::btnMotor2HomeClick(TObject *Sender)
{
	if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
	{
		CmdEUVOHTCheck.Axis_Hoist_HOME();
	}
}
// ---------------------------------------------------------------------------
...
i226 = (M262/3072) * 16
*/
void __fastcall TEUVOnlineCMDForm::mniHoistOriginSetClick(TObject *Sender)
{
	if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TEUVOnlineCMDForm::mniShiftOriginSetClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
@date    2012.12.14
*/
void __fastcall TEUVOnlineCMDForm::edtSpeedChange(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnMoveStopClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable(EXCLUDE_HOIST_RUNNING EXCLUDE_SHIFT_RUNNING EXCLUDE_ROTATE_RUNNING) == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TEUVOnlineCMDForm::FormCreate(TObject *Sender)
{
    int j                         = 0;
    EUVOnlineCMDForm->Top            = MainForm->Top;
    EUVOnlineCMDForm->Left           = MainForm->Left + MainForm->Width + StatusForm->Width;
...
// ---------------------------------------------------------------------------
void __fastcall TEUVOnlineCMDForm::pgcMainControlDrawTab(
...
// ---------------------------------------------------------------------------
void TEUVOnlineCMDForm::Warnning_Display(TColor tWColor, String strData)
{
...
void __fastcall TEUVOnlineCMDForm::PWGridDrawCell(TObject *Sender, int ACol,
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnHaONClick(TObject *Sender)
{
	if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnHaOFFClick(TObject *Sender)
{
	if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        String strTmpMessage;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnHaRstClick(TObject *Sender)
{
	if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        String strTmpMessage;
...
/**
 ...
 @param   Sender : TObject
 */
void __fastcall TEUVOnlineCMDForm::timerTransLongRunTimer(TObject *Sender)
{
...
void __fastcall TEUVOnlineCMDForm::chkLongRunClick(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnShutterAlarmResetClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @param   EStop
 */
void __fastcall TEUVOnlineCMDForm::btnMotor34HomeClick(TObject *Sender)
{
	if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnLoadMouseDown(TObject *Sender,
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnUnloadMouseDown(TObject *Sender,
    TMouseButton Button, TShiftState Shift, int X, int Y)
{
    TPoint P, Q;
    if(Button == mbLeft)
...
void __fastcall TEUVOnlineCMDForm::pmDrivingFServoOnOffPopup(TObject *Sender)
{
...
void __fastcall TEUVOnlineCMDForm::pmDrivingRServoOnOffPopup(TObject *Sender)
{
...
void __fastcall TEUVOnlineCMDForm::pmHoistServoOnOffPopup(TObject *Sender)
{
...
void __fastcall TEUVOnlineCMDForm::pmShiftServoOnOffPopup(TObject *Sender)
{
...
void __fastcall TEUVOnlineCMDForm::pmRotatePopup(TObject *Sender)
{
...
void __fastcall TEUVOnlineCMDForm::mniDrvFServoOnClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TEUVOnlineCMDForm::mniDrvFServoOffClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TEUVOnlineCMDForm::mniDrivingRServoOnClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TEUVOnlineCMDForm::mniDrivingRServoOffClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TEUVOnlineCMDForm::mniHoistServoOnClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TEUVOnlineCMDForm::mniHoistServoOffClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TEUVOnlineCMDForm::mniShiftServoOnClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TEUVOnlineCMDForm::mniShiftServoOffClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TEUVOnlineCMDForm::mniRotateServoOnClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TEUVOnlineCMDForm::mniRotateServoOffClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TEUVOnlineCMDForm::mniRotateOriginSetClick(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnRotate_OrignClick(TObject *Sender)
{
    bool bExec = false;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnRotate_EnableClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnRotate_DisableClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnRotate_StopClick(TObject *Sender)
{
	if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnFSteerONClick(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnFSteerOFFClick(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnRSteerONClick(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnRSteerOFFClick(TObject *Sender)
{
...
int TEUVOnlineCMDForm::DistChang_mm_2(void)
{
    int nError = 0;
...
// ---------------------------------------------------------------------------
int TEUVOnlineCMDForm::DistChang_cts_2(void)
{
    int nError = 0;
    float dDist_mm, dDist_M, dDist_cts;
    String strdd;
...
// ---------------------------------------------------------------------------
void __fastcall TEUVOnlineCMDForm::edtDistmmChange(TObject *Sender)
{
    DistChang_mm_2();
}
// ---------------------------------------------------------------------------

void __fastcall TEUVOnlineCMDForm::edtDistctsChange(TObject *Sender)
{
    DistChang_cts_2();
}
// ---------------------------------------------------------------------------

void __fastcall TEUVOnlineCMDForm::btnMoveClick(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnScanUTBClick(TObject *Sender)
{
    String strTmpMessage;
...
void __fastcall TEUVOnlineCMDForm::btnTractionLeftClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TEUVOnlineCMDForm::btnTractionRightClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TEUVOnlineCMDForm::chkTractionModeClick(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnRotateOriginClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnRotateResetClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnRotate0Click(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnRotate90Click(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnRotate180Click(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnRotate270Click(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnRotateFreeClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnMCControlOnClick(TObject *Sender)
{
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnMCControlOffClick(TObject *Sender)
{
    ///추가 puting 2012.11.28
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnSTB_R_WrokOnClick(TObject *Sender)
{
    ///추가 puting 2012.11.28
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnSTB_R_WorkOffClick(TObject *Sender)
{
    ///추가 puting 2012.11.28
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnSTB_L_WrokOnClick(TObject *Sender)
{
    ///추가 puting 2012.11.28
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnSTB_L_WrokOffClick(TObject *Sender)
{
    ///추가 puting 2012.11.28
    TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnDrivingBCRClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnDrivingMarkBackwardClick(
    TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
...
/**
 ...
 @date    2012.12.14
 */
void __fastcall TEUVOnlineCMDForm::btnDrivingMarkForwardClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        String strTmpCMD;
...
@return  변환된 값
*/
BYTE TEUVOnlineCMDForm::GetTransNodeType(int index)
{
    BYTE rtnVal = 0x00;
...
@param   Sender : TObject
*/
void __fastcall TEUVOnlineCMDForm::chkGoStationClick(TObject *Sender)
{
/*
    if(chkGoStation->Checked)
    {
...
void __fastcall TEUVOnlineCMDForm::mniLoadEQClick(TObject *Sender)
{
...
void __fastcall TEUVOnlineCMDForm::mniLoadEQ_XClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TEUVOnlineCMDForm::mniLoadSTOCKERClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TEUVOnlineCMDForm::mniLoadSTB_LClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TEUVOnlineCMDForm::mniLoadSTB_RClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TEUVOnlineCMDForm::mniLoadUTBClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TEUVOnlineCMDForm::mniLoadLOADERClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TEUVOnlineCMDForm::mniUnloadEQClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
	// shkim  임시 주의!!!!!
	if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
...
void __fastcall TEUVOnlineCMDForm::mniUnloadEQ_XClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TEUVOnlineCMDForm::mniUnloadSTOCKERClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TEUVOnlineCMDForm::mniUnloadLOADERClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TEUVOnlineCMDForm::mniUnloadSTB_LClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TEUVOnlineCMDForm::mniUnloadSTB_RClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
...
void __fastcall TEUVOnlineCMDForm::mniUnloadUTBClick(TObject *Sender)
{
    // by zzang9un 2012.12.02 : 새로 변경한 TransForm을 이용하도록 수정
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
...
/**
 ...
 @date    2013.03.26
 */
void __fastcall TEUVOnlineCMDForm::btnQRTagLeftMarkClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        if((gm_OHTStatus->Motor_Driving_F->nError == 1) ||
...
/**
 ...
 @date    2013.03.26
 */
void __fastcall TEUVOnlineCMDForm::btnQRTagRightMarkClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        if((gm_OHTStatus->Motor_Driving_F->nError == 1) ||
...
// ---------------------------------------------------------------------------
void __fastcall TEUVOnlineCMDForm::panTeachingSaveClick(TObject *Sender)
{
    TeachingForm->Show();
...
/**
@brief   EUVOnlineCMDForm의 Timer를 Enable하는 함수
@author  zzang9un
@date    2013.11.20
*/
void TEUVOnlineCMDForm::timerAllDisable()
{
    timerUpdateDisplay->Enabled = false;
	timerDrivingJog->Enabled = false;
    timerTransLongRun->Enabled = false;
...
void __fastcall TEUVOnlineCMDForm::btnMoveTeachingPointMouseDown(TObject *Sender, TMouseButton Button,
...
void __fastcall TEUVOnlineCMDForm::mniMoveTPEQClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TEUVOnlineCMDForm::mniMoveTPEQ_XClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TEUVOnlineCMDForm::mniMoveTPSTOCKERClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TEUVOnlineCMDForm::mniMoveTPLOADERClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TEUVOnlineCMDForm::mniMoveTPSTB_LClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TEUVOnlineCMDForm::mniMoveTPSTB_RClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TEUVOnlineCMDForm::mniMoveTPUTBClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        // TransForm을 Show하기 전 기본 이적재 정보를 Setting한다.
...
void __fastcall TEUVOnlineCMDForm::btnHoistHomeAutoSetClick(TObject *Sender)
{
	if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
	...
	/*
	if((CmdEUVOHTCheck.Is_Executable() == NO_ERR) && (CmdEUVOHTCheck.Is_MoveAxis_Hoist() == NO_ERR))
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
		// 이적재 명령 구조체
...
void __fastcall TEUVOnlineCMDForm::btnCidLeftSelClick(TObject *Sender)
{
	if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
//---------------------------------------------------------------------------
void __fastcall TEUVOnlineCMDForm::btnCidRightSelClick(TObject *Sender)
{
	if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
//---------------------------------------------------------------------------
void __fastcall TEUVOnlineCMDForm::btnCidOccupReqClick(TObject *Sender)
{
	if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
//---------------------------------------------------------------------------
void __fastcall TEUVOnlineCMDForm::btnCidPassComptClick(TObject *Sender)
{
	if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
//---------------------------------------------------------------------------
void __fastcall TEUVOnlineCMDForm::btnCidManualClick(TObject *Sender)
{
	if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TEUVOnlineCMDForm::btnCidOperationClick(TObject *Sender)
{
	if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
//---------------------------------------------------------------------------
void __fastcall TEUVOnlineCMDForm::btnCidRev1Click(TObject *Sender)
{
	if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
//---------------------------------------------------------------------------
void __fastcall TEUVOnlineCMDForm::btnCidRev2Click(TObject *Sender)
{
	if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
	{
		TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void TEUVOnlineCMDForm::Display_CID_INFO(PKT_OHT2TP_CID_INFO PKT)
{
	char cDisplay[4];
	AnsiString strTmp;
...
void TEUVOnlineCMDForm::Display_MapMove_Result(PKT_OHT2TP_MOVE_BY_MAP_RESULT PKT)
{
...
void TEUVOnlineCMDForm::Display_MapMake_Result(PKT_OHT2TP_LOG_DATA PKT)
{
...
void TEUVOnlineCMDForm::Display_Com(PKT_OHT2OCS PKT)
{
...
void __fastcall TEUVOnlineCMDForm::btnForceOpenClick(TObject *Sender)
{
/*
	TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
int TEUVOnlineCMDForm::TransLoad(void)
{
	char buf[MAX_PACKET_SIZE+1];
	int iPacketSize;
	int iReturn, iTemp;
...
int TEUVOnlineCMDForm::TransUnLoad(void)
{
	char buf[MAX_PACKET_SIZE+1];
	int iPacketSize;
	int iReturn, iTemp;
...
//---------------------------------------------------------------------------
// 이 함수는 필요하다면 TEUVOnlineCMDForm::Display_Com(PKT_OHT2OCS PKT)
// 을 참고하여 보강한다

void __fastcall TEUVOnlineCMDForm::tmrTransRunTimer(TObject *Sender)
{
#if 0
	int iReturn;
...
void __fastcall TEUVOnlineCMDForm::btnRotate_0Click(TObject *Sender)
{
    bool bHoistHome = gm_OHTStatus->IOInput->uInputCommon.HOIST_Home;
...
void __fastcall TEUVOnlineCMDForm::btnRotate_90Click(TObject *Sender)
{
    bool bHoistHome = gm_OHTStatus->IOInput->uInputCommon.HOIST_Home;
...
void __fastcall TEUVOnlineCMDForm::btnRotate_180Click(TObject *Sender)
{
    bool bHoistHome = gm_OHTStatus->IOInput->uInputCommon.HOIST_Home;
...
void __fastcall TEUVOnlineCMDForm::tmrCidInfoReadTimer(TObject *Sender)
{
...
void __fastcall TEUVOnlineCMDForm::btnCidMonitoringClick(TObject *Sender)
{
	if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
	{
...
void __fastcall TEUVOnlineCMDForm::btnMapBaseMoveClick(TObject *Sender)
{
	char buf[MAX_PACKET_SIZE+1];
	int iPacketSize;
	int iReturn;
	...
	(gm_OHTStatus->m_OHTStatus.MarkInfo.Info.DrivingTag == 0))
	{
		Warnning_Display(COLOR_RED, "ERROR : OHT Has Adjust Map... Please Mark Node...");
		return;
    }
		if(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Ver == OHT_VERSION_86)
		{
...
//            (gm_OHTStatus->IOInput->uInputCommon.POD_EUV_2 == false))
		...
		else if(gm_OHTStatus->IOInput->uInput.Input_V86_EUV.POD_EUV_1 == true)
...
//                (gm_OHTStatus->IOInput->uInputCommon.POD_EUV_2 == true))
	...
	//            (gm_OHTStatus->IOInput->uInputCommon.POD_EUV_2 == false))
			...
			else if(gm_OHTStatus->IOInput->uInputCommon.POD_EUV_1 == true)
...
void __fastcall TEUVOnlineCMDForm::btnCidMonitoring_RightClick(TObject *Sender)
{
	if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
	{
...
void TEUVOnlineCMDForm::PowerOnOff(int iAxis, int iOnOff)
{
...
void __fastcall TEUVOnlineCMDForm::btnMoveAllHomeClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TEUVOnlineCMDForm::btnAutoChangeClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
...
void __fastcall TEUVOnlineCMDForm::btnShutterMiddleCloseClick(TObject *Sender)
{
    if(CmdEUVOHTCheck.Is_Executable() == NO_ERR)
    {
        TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
		...
		if(fabs(StrToFloat(gm_OHTStatus->Motor_Hoist->dPosition_mm)) > HOIST_ORIGIN_LIMIT)
        {
            Warnning_Display(COLOR_RED, "Hoist Not Origin...MID Shutter can't Close...");
        }
        else if(fabs(StrToFloat(gm_OHTStatus->Motor_Shift->dPosition_mm)) > SHIFT_ORIGIN_LIMIT)
        {
            Warnning_Display(COLOR_RED, "Shift Not Origin...MID Shutter can't Close...");
        }
        else if((fabs(StrToFloat(gm_OHTStatus->Motor_Rotate->dPosition_mm)) != 0) &&
				(fabs(StrToFloat(gm_OHTStatus->Motor_Rotate->dPosition_mm)) != 1800))
        {
        ...
        {
            m_CMDControl->MakeTPCmd2OHT(TPCMD_SHUTTER_MID_CLOSE);
            MainForm->DataDisplay(COLOR_BLACK, "Shutter MID Close");
		}
...
void __fastcall TEUVOnlineCMDForm::btnRotateMoveCmdClick(TObject *Sender)
{
	bool bHoistHome = gm_OHTStatus->IOInput->uInputCommon.HOIST_Home;
...
void __fastcall TEUVOnlineCMDForm::btnOCSMapBaseMoveClick(TObject *Sender)
{
  	char buf[MAX_PACKET_SIZE+1];
	int iPacketSize;
	unsigned char cHigh, cMid, cLow;
...
void __fastcall TEUVOnlineCMDForm::chkDistClick(TObject *Sender)
{
...
void __fastcall TEUVOnlineCMDForm::chbBackWardClick(TObject *Sender)
{
...
void __fastcall TEUVOnlineCMDForm::chkEmergencyJogClick(TObject *Sender)
{
	if(chkEmergencyJog->Checked == true)
	{
