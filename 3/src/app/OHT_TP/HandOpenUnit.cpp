...
int THandOpenForm::Check_Is_HandOpen(void)
{
...
#if (OHT_NAME == OHT_NAME_MASK)
   bHandClose = gm_OHTStatus->IOInput->uInput.Input_MASK.HAND_Home;
   bHandOpen = gm_OHTStatus->IOInput->uInput.Input_MASK.HAND_Work;
#else
   ...
   if(bHandAlarm==ON)
   {
		...
		{
			MaskOnlineCMDForm->Warnning_Display(COLOR_RED, "[ERROR] Hand Alarm");
		}
   ...
   else if((bHandClose==OFF)&&(bHandOpen==OFF))
   {
	  nError=1;
		...
		{
			MaskOnlineCMDForm->Warnning_Display(COLOR_RED, "[ERROR] Hand Work/Home Limit Sensor==OFF");
		}
   ...
   else if(gm_OHTStatus->m_OHTStatus.Data.CurNodeID==0)
   {
	  nError=1;
		...
		{
			MaskOnlineCMDForm->Warnning_Display(COLOR_RED, "[ERROR] OHT CurrentNode==0");
		}
