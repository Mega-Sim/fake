...
//        chkEQPIODisable->Checked = false;
	...
	case PORT_TYPE_N2STB_L:
	case PORT_TYPE_N2STB_R:
	case PORT_TYPE_STB_L_T:
	case PORT_TYPE_STB_L_B:
	case PORT_TYPE_STB_R_T:
	case PORT_TYPE_STB_R_B:
...
void __fastcall TTransForm::btnDoTransClick(TObject *Sender)
{
	...
	else if(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_MASK)
    {
		...
		{
			MaskOnlineCMDForm->timerTransLongRun->Enabled = true;
			MaskOnlineCMDForm->timerTransLongRun->Interval = MaskOnlineCMDForm->edtLongRunInterval->Text.ToInt();
		}
		else
			MaskOnlineCMDForm->timerTransLongRun->Enabled = false;
	}
...
{
    pTransCmd->CurNodeID = gm_OHTStatus->m_OHTStatus.Data.CurNodeID;
    pTransCmd->CurStationID = gm_OHTStatus->m_OHTStatus.Data.CurStationID;
    pTransCmd->PortType = m_PortType;

    // by zzang9un 2013.02.22 : EQ PIO Direction 추가
    pTransCmd->PIODirection = cbbPIODirection->ItemIndex + 1;

    // by zzang9un 2013.09.02 : Hoist, Shift Level을 계산하여 전달
	pTransCmd->HoistSpeedLevel = (cbbHoistSpeedLevel->ItemIndex + 1) * 10;
	pTransCmd->ShiftSpeedLevel = (cbbShiftSpeedLevel->ItemIndex + 1) * 10;

    // by zzang9un 2013.08.26 : EQPIO Disable 여부를 추가
    pTransCmd->TransOption.Info.AutoRecovery = false;
    pTransCmd->TransOption.Info.HandDetectEQ = chkHandDetectEQ->Checked;
