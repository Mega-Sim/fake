...
//			m_CMDControl->MakeTPCmd2OHT(TPCMD_HAND_WORK);
//#else
...
//#endif
	...
	else if(bHandOpen == ON)
	{
		...
		if(nCCount >= 5)
		{
			...
			TCMDCONTROL *m_CMDControl = new TCMDCONTROL;
//#if (OHT_NAME == OHT_NAME_MASK)
//			m_CMDControl->MakeTPCmd2OHT(TPCMD_HAND_HOME);
//#else
...
void __fastcall TLongRunForm::tmrRotateTimer(TObject *Sender)
{
	...
	// 속도는 edit에 있는 값을 사용
	if((gm_OHTStatus->IOInput->uInputCommon.HOIST_Home == ON) && (fabs(StrToFloat(gm_OHTStatus->Motor_Hoist->dPosition_mm)) < HOIST_ORIGIN_LIMIT))
	{
