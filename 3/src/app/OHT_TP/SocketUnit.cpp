...
//    MainForm->panDataStatus->Visible = true;
	...
	MainForm->mnWINShutdown->Enabled 	  = true;
...
//    MainForm->panDataStatus->Visible = false;
	...
	MainForm->mnWINShutdown->Enabled 	  = false;
    gm_OHTStatus->bReceiveStatus          = true;
	...
	{
		MaskOnlineCMDForm->timerAllDisable();
	}
...
void TSocketForm::UdpSocket_Status_Read(PKT_OHT2TP_RESPONSE Pkt, int iMachineType)
{
	static int ReadCount = 0;
	 ...
	 memcpy(MaskOnlineCMDForm->m_VHLNAME, Pkt.HEADER.MachineID, sizeof(Pkt.HEADER.MachineID));

	if((gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type != OHT_TYPE_FOUP) && (gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type != OHT_TYPE_FOUP_S) &&
	   (gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type != OHT_TYPE_EUV) && (gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type != OHT_TYPE_SERVICE) &&
	   (gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type != OHT_TYPE_MAC))
	{
		if(tmrOhtTypeCheck->Enabled == false)
			tmrOhtTypeCheck->Enabled = true;
	}
	...
	else if(Pkt.TAG.Value == TagID::CMD_INITREQ_TP_RESPONSE)
	{
...

		{
				...
				gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_MAC)
			{
				...
				MainForm->ErrorIgnoalFoupcoveroepn1->Visible= true;
				MainForm->ErrorResumeFoupcoveroepn1->Visible= true;
				...
				|| ((gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Ver == OHT_VERSION_85)&&(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_FOUP_S))
				|| (gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Ver == OHT_VERSION_82))
				{
					FoupOnlineCMDForm->panMAC_Detect->Visible = false;

					if(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Ver == OHT_VERSION_81) // OHT_VERSION_82 필요한지 확인 후 추가
					   FoupOnlineCMDForm->grpBlackBox->Visible = true;
					else
                       FoupOnlineCMDForm->grpBlackBox->Visible = false;
				}
			...
			else if(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_EUV)
			{
				...
				MainForm->ErrorIgnoalFoupcoveroepn1->Visible= false;
				MainForm->ErrorResumeFoupcoveroepn1->Visible= false;
				// 2016-11-28, Rotate
				// Add by Labeler
			...
			{
				MaskOnlineCMDForm->chkLongRun->Checked = false;
			...
			else if(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_MAC)
			{
				FoupOnlineCMDForm->panMAC_Detect->Visible = true;
				FoupOnlineCMDForm->panFoupDetect->Visible = false;
				FoupOnlineCMDForm->panUTB->Visible = false;
...
void TSocketForm::UdpSocket_MapMove_Result(PKT_OHT2TP_MOVE_BY_MAP_RESULT PKT)
{
	...
	{
		if(MaskOnlineCMDForm != NULL)
			MaskOnlineCMDForm->Display_MapMove_Result(PKT);
	}
}
			...
			case TAG_CMDDATA_INIT_INFO_TP :
            {
				if(Header.uTag.Info.Receiver == TAG_RECEIVER_TP)
                {
					...
					if((gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_FOUP) || (gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_FOUP_S) || gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_MAC)
					{
						...
						else if((gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Ver == OHT_VERSION_70)
						|| (gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Ver == OHT_VERSION_71)
						|| (gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Ver == OHT_VERSION_80)
						|| (gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Ver == OHT_VERSION_81)
						|| ((gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Ver == OHT_VERSION_85)&&(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_FOUP_S))
					...
					else if(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_EUV)
					{
						...
						if((gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Ver == OHT_VERSION_70) ||
							...
							(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Ver == OHT_VERSION_86) ||
							(gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Ver == OHT_VERSION_71))
						{
...
void TSocketForm::UdpSocket_AVS_Vibration_Data_Read(PKT_OHT2TP_VIBRATION_INFO PKT)
{
	...
	{
		frmVibrationMeasurement->DisplayVibrationData(PKT.BODY,false);
	}
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
void TSocketForm::UdpSocket_AutoDriving_Route_Response(PKT_OHT2TP_AUTO_DRIVING_TEST_ROUTE_RESPONSE PKT)
{
	bool bSuccess = PKT.BODY.Success;
	if(frm_AutoBayTest != NULL)
	{
		frm_AutoBayTest->setRouteCheckSignalTrue();
        frm_AutoBayTest->setRouteCheckResult(bSuccess);
    }
}

void TSocketForm::UdpSocket_AutoDriving_Drive_Response(PKT_OHT2TP_AUTO_DRIVING_TEST_MOVE_RESPONSE PKT)
{
	bool bSuccess = PKT.BODY.Success;
	if(frm_AutoBayTest != NULL)
	{
		frm_AutoBayTest->setDriveCheckSignalTrue();
		frm_AutoBayTest->setDriveCheckResult(bSuccess);
    }
}



void TSocketForm::UdpSocket_MapViewStatusRead(int nStatus)
{
	frm_MapViewLoad->setMapViewStatus(nStatus);
}

void TSocketForm::UdpSocket_MapViewMapVersionRead(char* buf)
{
	frm_MapViewLoad->setMapVersion(buf);
}

void TSocketForm::UdpSocket_MapViewUpdateResultRead(int nStatus, int nSuccess)
{
	frm_MapViewLoad->setUIUpdateResult(nStatus, nSuccess);
}
void __fastcall TSocketForm::tmrOhtTypeCheckTimer(TObject *Sender)
{
	tmrOhtTypeCheck->Enabled = false;

	if((gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_FOUP) || (gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_FOUP_S) ||
	   (gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_EUV) || (gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_SERVICE) ||
	   (gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_MASK)||
	   (gm_OHTStatus->m_OHTInfo.OHTInfo.OHT_Type == OHT_TYPE_MAC))
		return;
	...
	TOHTLOGFILE *m_LogFile = new TOHTLOGFILE;
	String strLog;
	strLog.sprintf(L"OHT_INIT_INFO_REQ SEND AGAIN");
	m_LogFile->WriteLogFile("TEST", strLog);
	delete m_LogFile;
