...
public:
    int  LogMsg(const char* fmt, ...) {
        va_list ap;
        va_start(ap, fmt);
        vLogMsg(LogLevel::TRACE, fmt, ap);
        va_end(ap);
        return 0;
    }
    int  LogMsgAdd(const char* fmt, ...) {
        va_list ap;
        va_start(ap, fmt);
        vLogMsg(LogLevel::TRACE, fmt, ap);
        va_end(ap);
    ...
    int  LogError(const char* fmt, ...) {
        va_list ap;
        va_start(ap, fmt);
        vLogMsg(LogLevel::ERROR, fmt, ap);
        va_end(ap);
        return 0;
    }
    int  LogErrorAdd(const char* fmt, ...) {
        va_list ap;
        va_start(ap, fmt);
        vLogMsg(LogLevel::ERROR, fmt, ap);
        va_end(ap);
...
private:
    int  vLogMsg(LogLevel lvl, const char* fmt, va_list ap) {
        const char* filename = NULL;
        int lineno = 0;
        LogMessage* log = theLogger.get()->getLogObject();

        // text setting
        log->vset(lvl, filename, lineno, fmt, ap);
