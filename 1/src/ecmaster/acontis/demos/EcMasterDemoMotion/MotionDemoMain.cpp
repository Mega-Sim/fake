...
TLR_RESULT motionDemo(TLR_VOID FAR* pvInit)
#elif (defined STARTERWARE_NOOS)
extern "C" int EcMasterDemoMotion(int nArgc, char* ppArgv[])
#elif (defined RTAI)
extern "C" void EcMasterDemoMotionMain(long int t)
...
#endif
{
...
#endif
    EC_T_INT                nVerbose            = -1; /* command line overwrite config file setting */
#if (defined UNDER_CE) && (_WIN32_WCE >= 0x600)
    BOOL                    bRes                = FALSE;
    ...
    if( pszCfgBuffer != EC_NULL )
    {
        ...
        {
            OsDbgMsg( "ERROR: Can't open demo configuration buffer %s\n",  pszCfgFilePath);
            nRetVal = APP_ERROR;
            goto Exit;
        }
    }
    ...
    if(dwNumLinkLayer == 0)  
    {
    EC_T_CHAR szLinkLayerParameters[256];
        ...
        {
           dwRes = CreateLinkParmsFromCmdLine(&ptcWord, (EC_T_CHAR**)&szLinkLayerParameters, &tcStorage, &bGetNextWord, &apLinkParms[dwNumLinkLayer]);
           if (EC_E_NOERROR != dwRes)
           {
               nRetVal = SYNTAX_ERROR;
               goto Exit;
...
Exit:
    ...
    OsDbgMsg("EcMasterDemoMotion stop.\n");
#if !(defined VXWORKS)
