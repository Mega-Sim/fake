...
class ThreadedTCPRequestHandler(SocketServer.BaseRequestHandler):
    def handle(self):
        ...
        print (cur_thread, 'Begin session from', self.client_address)

        while True:
            try:
                log = self.request.recv(1024)                
                if not log  :
                    print (cur_thread, 'End session!', self.client_address)
                    break
                msg += log
            except:
...
if __name__ == "__main__":
    ...
    uds_addr = "/tmp/sock_log"

    #server = ThreadedTCPServer((HOST, PORT), ThreadedTCPRequestHandler)
    if os.path.exists(uds_addr): 
		os.remove(uds_addr)
    
    server = ThreadedTCPServer(uds_addr, ThreadedTCPRequestHandler)
